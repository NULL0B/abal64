...
#if !defined(SPARC) && !defined(LINUX) && !defined(AVIION) && !defined(AIX_VERS)
TYPF bd_insertc();
#else
...
TYPF bd_creat()
{
  ...
  /* position et taille */
	...
	if ((rt_arg.t[Crt_i_pli] & TYPE_BCD) 
	||  (rt_arg.t[Crt_i_pco] & TYPE_BCD) 
	||  (rt_arg.t[Crt_i_nli] & TYPE_BCD) 
	||  (rt_arg.t[Crt_i_nco] & TYPE_BCD))
		return(Bd_err_typ);

	plig = GetAbalWord( rt_arg.v[Crt_i_pli] );
	pcol = GetAbalWord( rt_arg.v[Crt_i_pco] );
	nlig = GetAbalWord( rt_arg.v[Crt_i_nli] );
	ncol = GetAbalWord( rt_arg.v[Crt_i_nco] );

	/* style */
	/* ----- */

	if (rt_arg.t[Crt_i_sty] == TYPE_DIESE)
		style = GetAbalByte( rt_arg.v[Crt_i_sty] );
	else if (rt_arg.t[Crt_i_sty] == TYPE_POURCENT)
		style = GetAbalWord( rt_arg.v[Crt_i_sty] );
	else if (rt_arg.t[Crt_i_sty] == TYPE_DEUXPOINT)
		style = GetAbalLong( rt_arg.v[Crt_i_sty] );
	else	return(Bd_err_sty);

	/* controle et recuperation du titre */
	/* --------------------------------- */
	dim = 0;	/* valeur par defaut */
	...
	if(rt_arg.c == NB_ARG_MIN_CREAT) /* pas de titre */
		...
		if (rt_arg.c == NB_ARG_MIN_CREAT+2) { /* dimension*/
			if (rt_arg.t[Crt_i_dim] == TYPE_POURCENT)
				dim = GetAbalWord( rt_arg.v[Crt_i_dim] );
			else if (rt_arg.t[Crt_i_dim] == TYPE_DEUXPOINT)
				dim = GetAbalLong( rt_arg.v[Crt_i_dim] );
			else	return(Bd_err_typ);
			if (dim >2)
				return(Bd_err_sty);
...
TYPF bd_insert()
{
  ...
  /* on recupere le nø de la boite */
	...
	if (rt_arg.t[Ins_i_nol] == TYPE_POURCENT)
		nlog = GetAbalWord( rt_arg.v[Ins_i_nol] );
	else if(rt_arg.t[Ins_i_nol] == TYPE_DEUXPOINT)
		nlog = GetAbalLong( rt_arg.v[Ins_i_nol] );
	else	return(Bd_err_typ);
  ...
  /* type element */
	...
	if (rt_arg.t[Ins_i_typ] == TYPE_POURCENT)
		typ_e = GetAbalWord( rt_arg.v[Ins_i_typ] );
	else if (rt_arg.t[Ins_i_typ] == TYPE_DEUXPOINT)
		typ_e = GetAbalLong( rt_arg.v[Ins_i_typ] );
	else	return(Bd_err_typ);

  /* position element */

	if (rt_arg.t[Ins_i_pli] == TYPE_POURCENT)
		plig = GetAbalWord( rt_arg.v[Ins_i_pli] );
	else if (rt_arg.t[Ins_i_pli] == TYPE_DEUXPOINT)
		plig = GetAbalLong( rt_arg.v[Ins_i_pli] );
	else	return(Bd_err_typ);

	if (rt_arg.t[Ins_i_pco] == TYPE_POURCENT)
		pcol = GetAbalWord( rt_arg.v[Ins_i_pco] );
	else if (rt_arg.t[Ins_i_pco] == TYPE_DEUXPOINT)
		pcol = GetAbalLong( rt_arg.v[Ins_i_pco] );
	else 	return(Bd_err_typ);
	...
	switch(typ_e & ~E_INTERACTIF & ~E_INTERACTIF2) {
		...
		case MESSAGE:
			...
			if (rt_arg.t[Mes_i_lib] != TYPE_CHAINE)
				return(Bd_err_typ);
			templen = rt_arg.l[Mes_i_lib];
			templen +=1;
			if (!(lib = allouer(templen)))
				return( werr_mem );
		...
		case BOUTON:
			...
			if(rt_arg.t[Bt_i_title] != TYPE_CHAINE)
				return(Bd_err_typ);
			if(!(lib = allouer((rt_arg.l[Bt_i_title]) + 1)))
			 	 return(werr_mem);
			...
			if (rt_arg.t[Bt_i_code] == TYPE_POURCENT)
				val_word = GetAbalWord( rt_arg.v[Bt_i_code] );
			else if (rt_arg.t[Bt_i_code] == TYPE_DEUXPOINT)
				val_word = GetAbalLong( rt_arg.v[Bt_i_code] );
			else	return(Bd_err_typ);
		...
		case R_BOUTON:
			...
			if(rt_arg.t[RB_i_nb] == TYPE_DIESE) {
 				nb = GetAbalByte( rt_arg.v[RB_i_nb] );
				}
			else if(rt_arg.t[RB_i_nb] == TYPE_POURCENT) {
				nb = GetAbalWord( rt_arg.v[RB_i_nb] );
				}
			else if(rt_arg.t[RB_i_nb] == TYPE_DEUXPOINT) {
				nb = GetAbalLong( rt_arg.v[RB_i_nb] );
				}
			else	 return(Bd_err_typ);

			/* puis la taille */
			if (rt_arg.t[RB_i_size] == TYPE_DIESE) {
				size = GetAbalByte( rt_arg.v[RB_i_size] );
				}
			else if (rt_arg.t[RB_i_size] == TYPE_POURCENT) {
				size = GetAbalWord( rt_arg.v[RB_i_size] );
				}
			else if (rt_arg.t[RB_i_size] == TYPE_DEUXPOINT) {
				size = GetAbalLong( rt_arg.v[RB_i_size] );
				}
			else	return(Bd_err_typ);
			...
			if (size == 0) {
				...
				/* test du nombre d'argument */
				 if ((rt_arg.c < NB_ARG_R_BOUT+1) 
				 ||  (rt_arg.c > NB_ARG_R_BOUT+MAX_BOUT_R_C-1))
	   				return(Bd_err_arg);
	 			...
	 			for(i = 0; i < nb; i++) {
	  				if (rt_arg.t[RB_i_str+i] != TYPE_CHAINE)
						...
						if (!(temp = f_allouer((rt_arg.l[RB_i_str+i]) + 1)))
							return(werr_mem);
				...
				if (rt_arg.c != NB_ARG_R_BOUT)
					return(Bd_err_arg);
				if ((rt_arg.t[RB_i_str] != TYPE_CHAINE) 
				||  ((rt_arg.l[RB_i_str]) < (nb*size)))
					return(Bd_err_arg);
				 for(	i = 0, k = 0; 
					(i < nb) && (k < rt_arg.l[RB_i_str]); 
					i++, k += size) {
					if(!(temp = f_allouer(size + 1)))
						return(werr_mem);
	    				for(j = 0; j < size; j++)
		...
		case C_BOUTON:
			...
			if (rt_arg.t[CB_i_typ] == TYPE_DIESE) {
				type = GetAbalByte( rt_arg.v[CB_i_typ] );
				}
			else if (rt_arg.t[CB_i_typ] == TYPE_POURCENT) {
				type = GetAbalWord( rt_arg.v[CB_i_typ] );
				}
			else if (rt_arg.t[CB_i_typ] == TYPE_DEUXPOINT) {
				type = GetAbalLong( rt_arg.v[CB_i_typ] );
				}
			else	return(Bd_err_typ);
			...
			if (rt_arg.t[CB_i_nb] == TYPE_DIESE) {
	    			nb = GetAbalByte( rt_arg.v[CB_i_nb] );
				}
			else if (rt_arg.t[CB_i_nb] == TYPE_POURCENT) {
				nb = GetAbalWord( rt_arg.v[CB_i_nb] );
				}
			else if (rt_arg.t[CB_i_nb] == TYPE_DEUXPOINT) {
				nb = GetAbalLong( rt_arg.v[CB_i_nb] );
				}
			else	return(Bd_err_typ);
			...
			if (rt_arg.t[CB_i_size] == TYPE_DIESE) {
				size = GetAbalByte( rt_arg.v[CB_i_size] );
				}
			else if (rt_arg.t[CB_i_size] == TYPE_POURCENT) {
				size = GetAbalWord( rt_arg.v[CB_i_size] );
				}
			else if (rt_arg.t[CB_i_size] == TYPE_DEUXPOINT) {
				size = GetAbalLong( rt_arg.v[CB_i_size] );
				}
			else	return(Bd_err_typ);
			...
			if(size == 0) {
				...
				|| (rt_arg.c > NB_ARG_C_BOUT+MAX_BOUT_R_C-1))
					return(Bd_err_arg);
				for(i = 0; i < nb; i++) {
					if(rt_arg.t[CB_i_str+i] != TYPE_CHAINE)
						return(Bd_err_typ);
					else	{
						if(!(temp = f_allouer((rt_arg.l[CB_i_str+i]) + 1)))
							return(werr_mem);
				...
				if(rt_arg.c != NB_ARG_C_BOUT)
					return(Bd_err_arg);
				if((rt_arg.t[CB_i_str] != TYPE_CHAINE) 
				|| ((rt_arg.l[CB_i_str]) < (nb*size)))
					return(Bd_err_arg);
				for (	i = 0, k = 0; 
					(i < nb) && (k < rt_arg.l[CB_i_str]); 
					i++, k += size) {
	    				if (!(temp = f_allouer(size + 1)))
						return(werr_mem);
					for (j = 0; j < size; j++)
		...
		case LISTE:
			...
			if(rt_arg.t[Li_i_nol] == TYPE_POURCENT)
				val_word = GetAbalWord( rt_arg.v[Li_i_nol] );
			else if(rt_arg.t[Li_i_nol] == TYPE_DEUXPOINT)
				val_word = GetAbalLong( rt_arg.v[Li_i_nol] );
			else	return(Bd_err_typ);
		...
		case SAISIE:
			...
			if (rt_arg.t[Sai_i_nco] == TYPE_POURCENT)
				val_word = GetAbalWord( rt_arg.v[Sai_i_nco] );
			else if (rt_arg.t[Sai_i_nco] == TYPE_DEUXPOINT)
				val_word = GetAbalLong( rt_arg.v[Sai_i_nco] );
			else	return(Bd_err_typ);

			/* recup nb caracteres */
			if(rt_arg.t[Sai_i_nca] == TYPE_POURCENT)
				val_word1 = GetAbalWord( rt_arg.v[Sai_i_nca] );
			else if(rt_arg.t[Sai_i_nca] == TYPE_DEUXPOINT)
				val_word1 = GetAbalLong( rt_arg.v[Sai_i_nca] );
			else	return(Bd_err_typ);
		...
		case AIDE:
			...
			if (rt_arg.t[Aid_i_lib] != TYPE_CHAINE)
				return(Bd_err_typ);
			if (!(lib = allouer((rt_arg.l[Aid_i_lib]) + 1)))
				return(werr_mem);
		...
		case FUNFREE:
			...
			if (rt_arg.t[Fun_i_nol] == TYPE_POURCENT)
				val_word = GetAbalWord( rt_arg.v[ Fun_i_nol ] );
			else if (rt_arg.t[Fun_i_nol] == TYPE_DEUXPOINT)
				val_word = GetAbalLong( rt_arg.v[ Fun_i_nol ] );
			else	return(Bd_err_typ);
...
TYPF bd_use()
{
  ...
  /* on recupere le nø de la boite */
	...
	if (rt_arg.t[Dia_i_nol] == TYPE_POURCENT)
		nlog = GetAbalWord( rt_arg.v[ Dia_i_nol ] );
	else if (rt_arg.t[Dia_i_nol] == TYPE_DEUXPOINT)
		nlog = GetAbalLong( rt_arg.v[ Dia_i_nol ] );
	else	return(Bd_err_typ);

  /* on recupere le nø de l'element actif */

	if(rt_arg.t[Dia_i_elem] == TYPE_POURCENT)
		elem = GetAbalWord( rt_arg.v[ Dia_i_elem ] );
	else if(rt_arg.t[Dia_i_elem] == TYPE_DEUXPOINT)
		elem = GetAbalLong( rt_arg.v[ Dia_i_elem ] );
	else	return(Bd_err_typ);
  ...
  /* l'adresse de retour */
  	...
  	exit_value = rt_arg.v[Dial_i_exit];
  	...
  	if(rt_arg.c == NB_ARG_DIAL+1) {		/** arg optionnel prov_exit */
		if ( (rt_arg.t[Dial_i_prov] != (TYPE_POURCENT | MODIFIABLE))
	     	...
	     	&&   (rt_arg.t[Dial_i_prov] != (TYPE_DEUXPOINT| MODIFIABLE)) )
			return(Bd_err_typ);
  		}
	...
	ret = bd_usec(nlog ,elem, &lexit, prov_exit);

	PutAbalWord( exit_value, lexit );
  
	if (rt_arg.c == NB_ARG_DIAL+1) {	/* arg optionnel prov_exit */
	     	if (rt_arg.t[Dial_i_prov] == (TYPE_POURCENT | MODIFIABLE)) {
			PutAbalWord( rt_arg.v[Dial_i_prov], prov );
			}
	     	else if (rt_arg.t[Dial_i_prov] == (TYPE_DEUXPOINT | MODIFIABLE)) {
			PutAbalLong( rt_arg.v[Dial_i_prov], prov );
			}
		else	*(BPTR)rt_arg.v[Dial_i_prov] = (BYTE)prov;
		}
...
TYPF bd_usec(nol,elem,eptr,prov_exit)
WORD nol;
WORD elem;
WPTR eptr;
WPTR prov_exit;
...
#ifdef SPARC
	     sortie = ANNULER;	
	     memcpy(eptr,&sortie,sizeof(WORD));
#else
	     *eptr = ANNULER;
#endif
	     Wkup_out();
...
TYPF bd_close()
{
  ...
  /* on recupere le nø de la boite */
	...
	if(rt_arg.t[Close_i_nol] == TYPE_POURCENT)
		nlog = GetAbalWord( rt_arg.v[Close_i_nol] );
	else if(rt_arg.t[Close_i_nol] == TYPE_DEUXPOINT)
		nlog = GetAbalLong( rt_arg.v[Close_i_nol] );
    	else	return(Bd_err_typ);
...
TYPF bd_sup()
{
  ...
  /* on recupere le nø de la boite */
	...
	if (rt_arg.t[Kill_i_nol] == TYPE_POURCENT)
		nlog = GetAbalWord( rt_arg.v[Kill_i_nol] );
	else if (rt_arg.t[Kill_i_nol] == TYPE_DEUXPOINT)
		nlog = GetAbalLong( rt_arg.v[Kill_i_nol] );
	else	return(Bd_err_typ);
...
TYPF bd_putdata()
{
  ...
  /* on recupere le nø de la boite */
  	...
  	if (rt_arg.t[Put_i_nol] == TYPE_POURCENT)
		nlog = GetAbalWord( rt_arg.v[Put_i_nol] );
  	else if (rt_arg.t[Put_i_nol] == TYPE_DEUXPOINT)
		nlog = GetAbalLong( rt_arg.v[Put_i_nol] );
    	else	return(Bd_err_typ);
...
TYPF bd_getdata()
{
  ...
  /* on recupere le nø de la boite */
  	...
  	if (rt_arg.t[Get_i_nol] == TYPE_POURCENT)
		nlog = GetAbalWord( rt_arg.v[Get_i_nol] );
  	else if (rt_arg.t[Get_i_nol] == TYPE_DEUXPOINT)
		nlog = GetAbalLong( rt_arg.v[Get_i_nol] );
    	else	return(Bd_err_typ);
