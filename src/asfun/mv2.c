 ...
 *
 *	$Header: /home/Produits/Abal3/asfun/mv2.c,v 1.1.1.1 2003/11/04 23:40:49 root Exp $
 *
 */
...
extern BPTR _rgb_colour(WORD);
extern BPTR _get_media(WORD);
...
static VOID _mv2_foreground();
static VOID _mv2_background();
...
APP_MV2 *desc;
{
	...
	foreground= desc->adr_fore;
	background= desc->adr_back;
	media = desc->adr_media;
	attribut = desc->adr_atb;
	increment = desc->inc_buf - desc->mvnbcol;

	// standard attribute colour
	_mv2_attribut(trace = *attribut);

	// overload with RGB colour
	if ((fg = *foreground) != 0)
		_mv2_foreground(fg);
	if ((bg = *background) != 0)
		_mv2_background(bg);
	...
	for (ligne = desc->mvlig - 1; ligne < desc->mvlig - 1 + desc->mvnblig; ligne++) 
	{
		...
		for (; colonne < desc->mvcol - 1 + desc->mvnbcol; colonne++) 
		{
			...
			// detect a foreground change
			if (*foreground != fg)
				_mv2_foreground(fg=*foreground);
			foreground++;

			// detect a background change
			if (*background != bg)
				_mv2_background(bg=*background);
			background++;
			...
			if ( *media != 0 )
			{
				...
				_co_out(0x001B);
				for ( 	mptr=_get_media(*media & 0x00FF);
					*mptr != 0;
					mptr++ )
					_co_out( *mptr & 0x00FF );
			}
...
static VOID _mv2_background(mot)
WORD mot;
{
	BPTR sptr;
	if (( mot ) &&  ((sptr = _rgb_colour(mot-1)) != (BPTR) 0))
	{
		_co(ESCAPE);
		_co('S');
		while ( *sptr )
...
static VOID _mv2_foreground(mot)
WORD mot;
{
	BPTR sptr;
	if (( mot ) &&  ((sptr = _rgb_colour(mot-1)) != (BPTR) 0))
