 ...
 *
 *	$Header: /home/Produits/Abal3/asfun/mv2rf.c,v 1.1.1.1 2003/11/04 23:40:49 root Exp $
 *
 */
...
for ( l = plig; l < plig + nlig ; l++)
  {
   ...
   while ( c < pcol+ncol)
    {
     f = *(mapecran + (max_colonne*(l-1)) + (c-1) );
     cpt = 0;
     t = f;
...
/* calcul nbre colonnes meme fenetre */
     while (( t == f ) && ( c < pcol+ncol))
      {
       cpt++;
       t = *(mapecran + (max_colonne*(l-1)) + (++c -1) );
      }
...
/* recopie texte */
	    for ( lf = 0; lf < cpt; lf++)
		*mv2_text++ = *(ftext + ((w->nb_col+(2*bd))*lf) + cf + lf );
