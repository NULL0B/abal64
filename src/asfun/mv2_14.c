 ...
 *	   llb 08.09.92 reset souris
 *	$Header: /home/Produits/Abal3/asfun/mv2_14.c,v 1.1.1.1 2003/11/04 23:40:49 root Exp $
 *
 */
...
extern WORD 	mousson;	/* etat souris on/off*/

extern BOOL flag_couleur;
extern WORD max_ligne;
extern WORD max_colonne;
#ifdef SPARC
extern BYTE nombre_ligne;
#else
extern BYTE nombre_ligne;
#endif
...
/* IJM 05/10/2001 for ASFUN LINUX and USE IT	*/

#define	_co_start	x_bstart
#define	_co_flush	x_bflush
#define	_co_pos(l,c)	x_pos(l,c)
#define	_co_cursor(a)	x_atb((25+a))
#define	_co_conv(a)	x_putch(a)
#define	_co_out(a)	x_putch(a)
#define	_co_attribut(a)	x_atb(a)
#define	_co_couleur_texte(f) x_foregr(f)
#define	_co_couleur_fond(b)  x_backgr(b)
...
APP_MV2 *desc;
{
	...
	act_mouse = FALSE;
	if (mousson) {
		StateSouris(&mcol,&mlig);	/* position courante souris*/
		mlig=(mlig/8)+1;
		mcol=(mcol/8)+1;
		if (    (mlig >= desc->mvlig) && ( mlig < (desc->mvlig+desc->mvnblig))
		     && (mcol >= desc->mvcol) && ( mcol < (desc->mvcol+desc->mvnbcol))) {
			mouse_off();		/* off*/
			act_mouse= TRUE;
			} 
		}
	...
	for (lligne = (desc->mvlig - 1); lligne < ((desc->mvlig - 1) + desc->mvnblig); lligne++) {
		lcolonne = (desc->mvcol - 1);
#ifdef MSDOS
		courant->ligne = lligne;
		courant->colonne = lcolonne;
#endif /* MSDOS */
		_co_pos(lligne, lcolonne);
		for (; lcolonne < ((desc->mvcol - 1) + desc->mvnbcol); lcolonne++) {
			if (*attribut != trace)
				_mv2_attribut((trace = *attribut));
			attribut++;
#ifdef MSDOS
...
#else
#ifdef R40
/* conversion caractere paragraphe  */
 
			if (*caractere == (BYTE) 0xD2)
				*caractere = '\017';
			if (*caractere == (BYTE) 0xEF)
				*caractere = '\322';
/* fin conversion 15.09.94 */
#endif


/* this bug is everywhere !!!
			if ((lligne != max_ligne) || (lcolonne != max_colonne)) {
 */
				_co_conv(*(caractere++));
...
#ifndef PRL
	if (act_mouse) 
		mouse_on();	/* permet de reactualiser l'attribut courant*/ 
#endif /*DOSMOUS*/
}
...
APP_MV2 *desc;
{
   ...
   AREG x;
	...
	GETWORD(nombre,desc->adr_text);
/*	nombre = *(WORD FAR *)desc->adr_text;*/
	GETWORD(direction,desc->adr_text+1);
...
if (( pcol > (max_colonne + 1))
||  ( plig > (max_ligne   + 1)))
	return;
if (( pcol + ncol) > (max_colonne+1))
	ncol = ((max_colonne+1) - pcol);
if (( plig + nlig) > (max_ligne+1))
	nlig = ((max_ligne+1) - plig);
