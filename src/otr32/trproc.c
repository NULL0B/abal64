...
WORD		mode;
{
	...
	if ( is_mnemonic( "IN" ) ) {
		...
		else	pptr->module = resolve_module_number( TokenBuffer );
		}
...
WORD	translate_extern_segment()
{
	...
	if ( is_mnemonic("IN" ) ) {
		if ( is_mnemonic("ANY") )
			...
			modunum = resolve_module_number( mptr );
			}
		}
...
WORD	translate_extern_field()
{
	...
	if ( is_equals() ) {
		...
		if ( is_mnemonic( "IN" ) ) {
			...
			else	module = resolve_module_number( TokenBuffer );
			}
