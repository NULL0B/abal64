		...
		&&   ( dptr->structname != (BPTR) 0)) {
			...
			&& (!(member->access & _MEMBER_LOCAL))) {  
				...
				if (( size = structure_occupation( class )) != 0) {
					...
					||  ((variable->type = allocate_for_type( (_TYPE_POINTER | _TYPE_DEFINED | _TYPE_STRING | _TYPE_PARAMETER), size , 1, 1, class )) == (TCTYPPTR) 0)) {
...
WORD		mode;
{
	...
	if (number == 1) {
/*		echo_transmit_common( method->parameters );	*/
				...
				/* ---------------------------- */
{
	...
	if ( allocation == 1 ) {
		...
		if ( mode & 2 ) {
			...
			/* ---------------------------------- */
			if ((status = add_destruction_method(resolve_method_table( object), object, (TCMEMBERPTR) 0, (TCMBMETHPTR) 0, (TCTEXTPTR) 0, mode )) != 0)
				return( status );

			/* REMOVE OBC (object container) */
			/* ----------------------------- */
			if ((class->size)
			&&  ((status = add_destruction_method( resolve_object_container( object ), object, (TCMEMBERPTR) 0, (TCMBMETHPTR) 0,(TCTEXTPTR) 0, mode )) != 0))
				return( status );
			}
...
TCMEMBERPTR	member;
{
	...
	TCSTRUCTPTR	class;
/* --------------------------------------------------------------------------------------------------------------------------------- */
/* IJM : TRACE CLASS MODULE if (( member ) && (member->name)) sysprintf("base_class_component(%s) for %s \n",member->name,TokenBuffer); */
/* --------------------------------------------------------------------------------------------------------------------------------- */
...
/*	sysprintf(" level = %d, master = %x , %s(%x)  : %s \n",level,master,object->name,object->identity,member->name); */
...
WORD		field;
{
	...
	if (!( field )) {
		...
		rptr->contents = (VPTR) set_shadow_type( rptr->contents, (object->type->type & 3)  );
...
/*		sysprintf("common_interface_method : %u \n",number);	*/
...
/*	sysprintf("overlay_interface_methods()\n");	*/
