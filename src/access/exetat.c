...
static int	interro(key_t msgqueue,char * nfic);
...
int 	reporter( msgqueue, tname )
key_t	msgqueue;
char *	tname;
{
	char	cmd[256];

	if ( interro(msgqueue,tname) == SUCCESS ) {
		strcpy(cmd,"cat ");
		strcat(cmd,COREFILE);
		system(cmd);
		printf("\n");
...
static int	interro(msgqueue,tname)
key_t	msgqueue;
char *	tname;
{
	int	lockqueue;
	...
	if ( (lockqueue = openqueue(msgqueue)) != FAILURE ) {
		...
		if ( get_mode_fast() )
			MsgZone.AccessZone.lock_func  = COEXASTATUSF;
		else	MsgZone.AccessZone.lock_func  = COEXASTATUS;
	
		if ( tname ) {
			if (!( strcmp(tname,"tty") ))
			       if (!( tname = ttyname(STDOUT_FILENO)))
					return(FAILURE);
			strcpy(MsgZone.AccessZone.lock_name,tname);
			}
		else	strcpy(MsgZone.AccessZone.lock_name,"\0");
...
/*
 *	R E S I Z E R (msgqueue,pidpurge)
 *	---------------------------------
 *	Resize the message Queue
 *
 */

int 	resizer(msgqueue,memsize)
key_t	msgqueue;
int	memsize;
{
	int	lockqueue;
	...
	if ( (lockqueue = openqueue(msgqueue)) != FAILURE ) {
		...
		/* Establish Lock Request Structure Infos */
		/* -------------------------------------- */
		MsgZone.AccessZone.lock_inode = memsize;
		MsgZone.AccessZone.lock_dnode = 0;
		MsgZone.AccessZone.lock_owner = 0;
		MsgZone.AccessZone.lock_mode  = 0;
		MsgZone.AccessZone.lock_func  = MSGQSIZE;
		MsgZone.msg_ds  = LOCKMSGID;
		MsgZone.msg_id  = (long) (getpid() + ECART);
		MsgZone.msg_err = SUCCESS;
...
int	qid;
{
	if (( qid = msgget((key_t) 13, 0)) != -1 ) {
		if ( msgctl(qid,IPC_RMID,  (struct msqid_ds *) "  ") != 0 ) {
			return( FAILURE );
			} 
		else	{
			return(SUCCESS);
