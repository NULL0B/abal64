...
static		unsigned short int	transparence;
		...
		switch ( es ) {
			...
			case	gif_control_extension		:
				...
				if ( queue[0] & 1 ) {				
					...
					if ( colourmaped )
	                			istransparent = isprite;
					else	istransparent = 1;
					}
...
static	void	transcodePixel(int pixie, int pixel)
{
...
#ifdef OLDHQCOLORMESS
	if ((!( colourmaped )) && (!( pixie )) && ( istransparent )) {
		put_logical_pixel( (rowpointer+(pixel*pixelsize)),0,0,0);
		return;
		}
#endif
	gifputpixel(rowpointer,pixel,pixie);
	return;
}
#ifdef OLDHQCOLORMESS
	if (( istransparent ) &&  ( pixie == transparence )) {
...
{
	int	ps=set_pixel_size(1);
	int	pt=set_pixel_format(1);
	struct standard_image * iptr=(struct standard_image *) 0;
	if (!( initialise_gif(filename) ))
		iptr = parse_gif(iptr);
	terminate_gif();
