...
int	vga_approximation(int  r, int  g, int  b )
{
	...
	// ---------------------
	for ( pptr=thispalette, nt=255+255+255, ni=0, i = 0; i < 256; i++ ) {
...
int	vga_approximation_two(int  r, int  g, int  b )
{
	int	nr=255,ng=255,nb=255,ni=0;
	int	lr=255,lg=255,lb=255,li=0;
	int	nl;
	int	dr,dg,db,dt;
	int	pr,pg,pb;
	int	nt=255+255+255;
	...
	l = ((r*_RED_WEIGHT)+(g*_GREEN_WEIGHT)+(b*_BLUE_WEIGHT));
	nl = ((256*_RED_WEIGHT)+(256*_GREEN_WEIGHT)+(256*_BLUE_WEIGHT));
	ld = nl;
	...
	for ( qptr=pptr=thispalette, nt=255+255+255, ni=0, i = 0; i < 256; i++ ) {
		...
		/* calculate luminence difference */
		/* ------------------------------ */
		pl = ((pr*_RED_WEIGHT)+(pg*_GREEN_WEIGHT)+(pb*_BLUE_WEIGHT));

		/* exact luminosity */
		/* ---------------- */
		if ( pl == l ) { return( i ); }

		/* calculate colour vector difference */
		/* ---------------------------------- */
		if (pr > r) dr = pr-r; else dr = r-pr;
		if (pg > g) dg = pg-g; else dg = g-pg;
		if (pb > b) db = pb-b; else db = b-pb;

		/* calculate luminosity differential */
		/* ---------------------------------*/
		if (pl > l ) {
			if ((pl - l) < ld)
				ld = (pl -l);
		...
		/* Calculate Total Colour Differentiel */
		/* ----------------------------------- */
		dt = dr+dg+db;
...
void	get_rgb_values(int entry, int * rvalue, int * gvalue, int * bvalue )
{
	if ( entry < 256 ) {
		...
		*bvalue = ( *(thispalette+(entry*4)+2) & 0x00FF );
		}
	else	{
		*rvalue = 0;
		*gvalue = 0;
