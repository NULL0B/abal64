...
#ifdef	ABAL64
static	char * default_library_path=(char *) "/home/abal64/lib";
#else
#ifdef	ABALDEBIAN
static	char * default_library_path=(char *) "/usr/lib/abal3";
#else
...
#ifdef	VERSION666
#ifdef ABAL64
	if (( rptr = allocate( (strlen(sptr) + strlen("64.so") + strlen(default_library_path) + 16) )) !=( char *) 0)
		sprintf(rptr,"%s/%s64.so",default_library_path,sptr);
#else
