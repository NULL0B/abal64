...
static int wmenu(BINAIRE l,BINAIRE c,BINAIRE nombre,BINAIRE i,int opt[],int hlp[]);
static int waffmenu(BINAIRE l,BINAIRE c,BINAIRE nombre,BINAIRE i,int opt[],int hlp[]);
static int vselect();
...
*/
	...
	if (argtmp[0] == EOSTR) {
		...
		if ( reptmp[0]) {
			Xstrcat(param," ");
			Xstrcat(param,DELIMOPT);
			Xstrcat(param,AbalLinkInclusion); /* a2 : r, a3 : i */
...
/* #BG	}*/
	...
	if ( AbalToolVersion != 3 ) {
		...
		fprintf(fc,"\t$(ALD) -i$(POBJ) -d$(PBIN)$(@B).at ");
		}
