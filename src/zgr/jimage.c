...
#if defined(BDA_WIN32) // Mot clé de l'API Windows
   result=LoadImageM(filename,1, &pImage);
#else
 ...
 */
// ****************************************************************************
SWORD   zwin_bbmp(WORD bout_type,char FAR * bout_lib,WORD entry,WORD larg)
{
  struct standard_image * iptr;
...
// --------------------------------------------------------------
rptr = allocate( (grcx+1) * pixelsize );
if (!rptr)
            ...
            if (IsMirror)
               {
					if (pixelformat)
						...
						{
						for (ipix=0 ; ipix<pixelsize ; ipix++)
							rptr[0+ipix] = rptrOri[tabx[grcx-1]+ipix];
						for( ix=0 ; ix<grcx ; ix++)
							{
							for (ipix=0 ; ipix<pixelsize ; ipix++)
								rptr[ (ix+1) * pixelsize + ipix ] = rptrOri[ tabx[grcx-1-ix] * pixelsize + ipix ];
							}
						}
               }
            else
               {
	               ...
	               rptr[0] = rptrOri[tabx[0]];
		            ...
		            for( ix=0 ; ix<grcx ; ix++)
							{
							for (ipix=0 ; ipix<pixelsize ; ipix++)
								rptr[ (ix+1) * pixelsize + ipix ] = rptrOri[ tabx[ix] * pixelsize + ipix ];
							}
						}
               }
            }
