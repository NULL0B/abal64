...
// * VarGlob:
// ****************************************************************************
VOID GuiWinTitleInternal(WORD State, WORD x,WORD y,WORD larg,char FAR * title,WORD lg,WORD appel,WORD IsActive)
{   
PTI_COMP_WINDOW_DESCR pWin = &g_pLook->Window;
WORD        ColFG;
WORD        ColBG;
...
// * VarGlob:
// ****************************************************************************
WORD zFrame_Internal( SWORD style,WORD atcol,WORD atlin,WORD nbcol,WORD nblin, WORD coul,WORD fond,WORD lg_titre,char FAR * titre)
{
  switch (style) 
    {
    case CE_FRAME_LIST :
...
MI_TRACE_BEGIN();
  ...
  if(!scroll_holder){
    MsgBox( "List Eroor", ZMB_OK|ZMB_ICONEXCLAMATION );
