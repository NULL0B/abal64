...
#define SIGNATURE22	"CICO22"        /* new signature                */
 ...
 * variables diverses
 */
static int making22=0;		/* file type control			*/
...
static char *mes[] = {
	...
	"Copyright (c) Amenesik / Sologic s.a. 1988, 1989",
	"terminal:",
	"cree",
	"creation impossible",
	"ecriture impossible",
...
main()
{
	...
	if ( making22 )
		printf("\t new version 2.2 with extended console functions \n\n");
	...
	if ( making22 ) {
		if (write(fd, SIGNATURE22, 6) == -1)
			fin(ERR_ECRITURE);
		}
	else if (write(fd, SIGNATURE, 6) == -1)
		fin(ERR_ECRITURE);
