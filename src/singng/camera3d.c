...
PRIVATE	struct camera3d * drop_camera( struct camera3d * cptr )
{
	...
	if ( cptr ) {
		...
		while ((vptr = cptr->topview) != (struct view3d *) 0) {
			cptr->topview = vptr->next;
			liberate( vptr );
			}
		if ( cptr->name )
...
PRIVATE	struct camera3d * allocate_camera()
{
	...
	if ((cptr = (struct camera3d *) allocate( sizeof( struct camera3d ) )) == (struct camera3d *) 0)
		...
		cptr->topview = cptr->curview = cptr->endview = (struct view3d *) 0;
		if (!( cptr->background= allocate( sizeof(struct background2d) )))
			(void) set_background(cptr);
		else	{
			cptr->background->iname = (char *) 0;
			cptr->background->image = (struct standard_image *) 0;
			cptr->background->style	=0;
			cptr->background->upperfill	=0;
			cptr->background->lowerfill	=0;
			cptr->background->leftfill	=0;
			cptr->background->rightfill	=0;
			cptr->background->fillcolumn=0;
			cptr->background->fillrow	=0;
			cptr->background->imagecolumn=0;
			cptr->background->imagerow	=0;
			cptr->background->colour	=0;
			}
...
PUBLIC	void	insert_cube(struct camera3d * cptr,struct object3d * optr, struct object3d * nptr)
{
	if ((!( cptr ))
	||  (!( optr )) 
	||  (!( nptr )))
		...
		if (!( nptr->previous = optr->previous ))
			cptr->first = nptr;
...
PUBLIC	void	append_cube(struct camera3d * cptr,struct object3d * optr, struct object3d * nptr)
{
	if ((!( cptr ))
	||  (!( optr )) 
	||  (!( nptr )))
...
PUBLIC	void	disconnect_cubes(struct camera3d * cptr )
{
	struct object3d * optr;
	while ((optr = cptr->foot) != (struct object3d *) 0) {
		cptr->foot = optr->before;
		optr->before = optr->after = (struct object3d *) 0;
		}
	cptr->head = cptr->foot = (struct object3d *) 0;
	return;
...
PUBLIC	void	disconnect_cube(struct camera3d * cptr,struct object3d * optr)
{
	if ((!( cptr ))
	||  (!( optr )))
		return;
