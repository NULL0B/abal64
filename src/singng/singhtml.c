...
#define	_DEFAULT_COPYRIGHT	"Copyright &copy; 2002,2007 Amenesik / Sologic "
...
private	void	server_interface_record(char * nptr, int type );
...
private	void	html_rgb_colour( FILE * h, int colourid );
private	void	html_text_style(FILE * h, struct form_item * iptr,int fg, int bg, int mode  );
private	void	close_server_description();
...
private	int		html_macro(char * sptr)
{
	int	i;
	for ( i=0; i < MaxMacMan; i++ )
		if ( upper_compare( sptr, MacMan[i].macro ) != 0 )
...
private	void	html_open_align(FILE * handle,int v )
{
...
private	void	html_close_align(FILE * handle,int v )
{
	fprintf(handle,"</div>");
	return;
}
...
private	void	html_image_widget(FILE * handle, struct form_item * iptr,int mode)
{
	...
	if (!( sptr = abal_payload(iptr)))
		...
		server_interface_record(sptr,_WSI_IMAGE);
		html_open_tag(handle,"img");
		if ( mode & 3 ) { 
			html_id(handle,iptr->Contents.name); 
			}
...
private	void	html_screen_style( FILE * h )
{
	...
	server_interface_record( "sing.js", _WSI_SCRIPT );
	...
	server_interface_record( "sing.css", _WSI_STYLE );
	return;
}
...
private	void	html_rgb_colour( FILE * h, int colourid )
{
	...
	fprintf(h,"#%02.2X%02.2X%02.2X;",(rvalue&0x00FF),(gvalue&0x00FF),(bvalue&0x00FF));
	return;
}
...
private	void	html_form_script( FILE * h, struct form_control * fptr, int xjs )
{
	...
	if ( xjs ) {
		...
		server_interface_record( jsnom, _WSI_SCRIPT );
...
private	void	html_content_type(FILE * handle)
{
...
private	void	html_generator(FILE * handle)
{
	/* META GENERATOR */
	/* -------------- */
	html_open_tag(handle,"meta");
...
private	void	html_doc_type( FILE * handle, int mode )
{
	char *	xptr;
	if ( mode ) {
...
private	int	open_server_description(struct form_control * fptr)
{
	if ( WSI.lock )
		return;
	...
	server_interface_record( WSI.root, _WSI_ROOT );

	if ( WSI.style )
		server_interface_record( WSI.style, _WSI_STYLE );
...
public	void	html_form_body( FILE * handle, int mode )
{
	...
	else if (!( open_server_description(Context.Form)))
		return;
...
public	int	export_html_form( FILE * handle, int mode, int automatic, int restrict, int parametric, int points )
{
	int	autotrad=1;
	FILE	*	h;
	struct	form_control *	fptr;
