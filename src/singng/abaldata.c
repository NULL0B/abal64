...
public  int	abal_file_instance(FILE * h, struct form_item * iptr, int t )
{
	...
	if ( t == _MCFILE_FRAME ) { xlen += (fields*32); xlen += ( 1024 + 64 ); }
	...
	switch ((t = (iptr->Contents.align & 0x000F)) ) {
		...
		case	_VISUAL_FRAME	:
			...
			||  ( iptr->Contents.datatype == _WIDGET_POINTER )) {
				...
				instance_widget_overlay(h,iptr,_OVL_METHODS+overlay_interface_count(iptr));
				}
			break;
...
public	void	abal_ngf_collect(FILE * h, struct form_item* iptr, int margin )
{
	...
	switch ( t ) {
		...
		case	_DBFILE_FRAME	:
			...
			/* ------------------------- */
			abal_new_collect(h,iptr,margin);
			indent(h,margin++);
	...
	if ( abal_ngf_method(iptr) == _MCFILE_FRAME ) {
		...
		switch ( rubtype ) {
			...
			case	_ABAL_WORD 	: 
				...
				break;
			case	_ABAL_LONG 	: 
				fprintf(h,"_extNewNum4(h_%s,%c%s%c,1",iptr->Contents.name,0x0022,rubname,0x0022);;
				break;
			}
		fprintf(h,",e_%s)",iptr->Contents.name);
		linefeed(h);
...
public	int	generate_data_widget(struct file_control * optr, struct form_control * fptr, char * nptr, struct form_item * iptr)
{
	char * yptr;
	...
	char * tptr, int pageitems )
{
	...
	return( generate_data_widget(optr, fptr, tptr, &DFC.template ));
}
	...
	char * tptr, int pageitems )
{
	...
	DFC.template.Contents.h = (dptr->indexfields*20)+10;
	...
	char * tptr, int pageitems )
{
	...
	if (( DFC.maxitems > pageitems ) &&  ( pageitems != 0 )) {
		DFC.template.Contents.h 	= (pageitems*20)+10;
		...
		DFC.template.Contents.h 	= (dptr->datafields*20)+10;
		return( generate_data_widget(optr, fptr, (char *) 0, &DFC.template ) );
		}
}
	...
	char * labels, char * format, int pageitems )
{
	...
	if (!( format ))
		...
		return(118);
	else if ((status = generate_data_widget(optr, fptr, "FileList", &DFC.formlate )) != 0)
		return( status );
	else if (!( fptr->last->Contents.format = allocate_string( format ) ))
		return( 27 );
	else if (!( fptr->last->Contents.payload = allocate_string( labels ) ))
		return( 27 );
	...
	char * nptr, char * kptr, int fmlen)
{
	...
	DFC.editlate.Contents.h 	= 16;
	DFC.editlate.Contents.align 	= 0;
	DFC.editlate.Contents.font	= 1;

	if ((status = generate_data_widget(optr, fptr, optr->filename, &DFC.editlate )) != 0)
		return( status );
	...
	char * tptr, int pageitems )
{
	...
	if ((status = generate_data_widget(optr, fptr, optr->filename, &DFC.filelate )) != 0)
		return( status );
	...
	char * tptr, int pageitems, int listcolumns, int maxcol )
{
	...
	/* forms method event control */
	fptr->last->previous->Contents.events->xo_get_focus 	= _FUNCTION_NOT_FOCUS;
	fptr->last->previous->Contents.events->xo_show  	= _EVENT_IS_FUNCTION;
	fptr->last->previous->Contents.events->xo_event 	= _FUNCTION_AND_FOCUS;

	/* forms show method */
	if (( status = generate_widget_action( fptr->last->previous,dptr, 0x029,optr )) != 0)
		return( status );
	/* forms event method */
	else if (( status = generate_widget_action( fptr->last->previous,dptr, 0x04A,optr )) != 0)
		return( status );
	...
	if (( status = generate_widget_action( fptr->last,dptr, 0x01B,optr )) != 0)
		return( status );
	/* scrollbar event method */
	else if (( status = generate_widget_action( fptr->last,dptr, 0x04C,optr )) != 0)
		return( status );
	/* scrollbar show method */
	else if (( status = generate_widget_action( fptr->last,dptr, 0x02D,optr )) != 0)
		return( status );
	...
	char * tptr, int pageitems )
{
	...
	if ( pageitems ){
		DFC.template.Contents.h 	= 80+((dptr->indexfields+pageitems)*20)+100;
		}
	else	{
		DFC.template.Contents.h 	= 80+((dptr->indexfields+dptr->datafields)*20)+40;
		}
	...
	DFC.template.Contents.font 	= 2;
	DFC.template.Contents.h 	= 16;
	DFC.template.Contents.align = 0x0502;	/* Left : Bold / Shadow */
	DFC.template.Contents.colour= 0x0010;	/* Black on Transparent */
	...
	DFC.editlate.Contents.h 	= 16;
	DFC.editlate.Contents.align = 0;	/* Left : Bold / Shadow */
	DFC.editlate.Contents.colour= 0x0010;	/* Black on Transparent */
	DFC.editlate.Contents.datatype = _WIDGET_STRING;

	for (first=0,f=0; f < dptr->indexfields; f++ ) {
		if (!(nptr = dptr->name[f+first] ))
			continue;
		else if ((status = generate_data_widget((struct file_control *) 0, fptr, nptr, &DFC.template )) != 0)
			return( status );
		else	DFC.template.Contents.y += 20;
		DFC.editlate.Contents.w = data_pixel_width(dptr,f+first,10);
		if ((status = generate_data_widget((struct file_control *) 0, fptr, nptr, &DFC.editlate )) != 0)
			return( status );
		else if (( status = generate_widget_action( fptr->last,dptr, 0x058,optr )) != 0)
			return( status );
		else	DFC.editlate.Contents.y += 20;


		}

	if (!( pageitems )) {
		DFC.template.Contents.y += 10;
		DFC.editlate.Contents.y += 10;
		}
	...
	for (curitem=0,first=dptr->indexfields,f=0; f < dptr->datafields; f++ ) {
		...
		if ((status = generate_data_widget(optr, fptr, nptr, &DFC.template )) != 0)
			return( status );
		else	DFC.template.Contents.y += 20;
	...
	DFC.template.Contents.x 	= 10;
	if ( pageitems )
		DFC.template.Contents.y     = 120+((dptr->indexfields+pageitems)*20);
	else	DFC.template.Contents.y     = 60+((dptr->indexfields+dptr->datafields)*20);
	...
	if ((status = generate_data_widget(optr, fptr, "Accept", &DFC.template )) != 0)
		return( status );
	else if (( status = generate_widget_action( fptr->last,dptr, 1,optr )) != 0)
		return( status );
	else	DFC.template.Contents.x 	+= DFC.template.Contents.w;
	if ((status = generate_data_widget(optr, fptr, "Up", &DFC.template )) != 0)
		return( status );
	else if (( status = generate_widget_action( fptr->last,dptr, 2,optr )) != 0)
		return( status );
	else	DFC.template.Contents.x 	+= DFC.template.Contents.w;
	if ((status = generate_data_widget(optr, fptr, "Load", &DFC.template )) != 0)
		return( status );
	else if (( status = generate_widget_action( fptr->last,dptr, 3,optr )) != 0)
		return( status );
	else	DFC.template.Contents.x 	+= DFC.template.Contents.w;
	if ((status = generate_data_widget(optr, fptr, "Delete", &DFC.template )) != 0)
		return( status );
	else if (( status = generate_widget_action( fptr->last,dptr, 4,optr )) != 0)
		return( status );
	else	DFC.template.Contents.x 	+= DFC.template.Contents.w;
	if ((status = generate_data_widget(optr, fptr, "Save", &DFC.template )) != 0)
		return( status );
	else if (( status = generate_widget_action( fptr->last,dptr, 5,optr )) != 0)
		return( status );
	else	DFC.template.Contents.x 	+= DFC.template.Contents.w;
	if ((status = generate_data_widget(optr, fptr, "Down", &DFC.template )) != 0)
		return( status );
	else if (( status = generate_widget_action( fptr->last,dptr, 6,optr )) != 0)
		return( status );
	else	DFC.template.Contents.x 	+= DFC.template.Contents.w;
	if ((status = generate_data_widget(optr, fptr, "Cancel", &DFC.template )) != 0)
		return( status );
