...
#define pt__Auto2744   ""
#define xx__Auto2744   ""
#define en_Auto2745   "Widget References"
#define fr_Auto2745   "R‚f‚rences au Widget"
#define it_Auto2745   "Widget References"
#define es_Auto2745   "Widget References"
#define de_Auto2745   "Widget References"
#define nl_Auto2745   "Widget References"
#define pt_Auto2745   "Widget References"
#define xx_Auto2745   "Widget References"
#define en_Auto2746   "Name"
#define fr_Auto2746   "Nom"
#define it_Auto2746   "Nom"
#define es_Auto2746   "Name"
#define de_Auto2746   "Name"
#define nl_Auto2746   "Name"
#define pt_Auto2746   "Name"
#define xx_Auto2746   "Ainm"
#define en_Target   ""
...
_Context->indsort_RefList=NULL;	/* pas de pointeur de table intermédiaire */		_Context->max_RefBar=0;
	...
	_Context->x_Auto2744=10;
	_Context->y_Auto2744=10;
	_Context->w_Auto2744=370;
	_Context->h_Auto2744=620;
	...
	_Context->pushbuffer=visual_buffer(_Context->x_Auto2744,_Context->y_Auto2744,370+10,620+10);
	return(0);
}
...
public 	int	widget_reference_locator_hide(struct widget_reference_locator_context * _Context)
{
	...
	&& (_Context->ctrlbuffer != 0)) {
		(void)visual_buffer_put(_Context->pushbuffer,_Context->x_Auto2744,_Context->y_Auto2744);
		_Context->ctrlbuffer=0;
		}
...
private int on_RefList_show(struct widget_reference_locator_context * _Context) {
	...
	get_widget_reference(i+_Context->value_RefBar,(_Context->row_RefList+0),40);
		...
		visual_table(_Context->x_Auto2744+10,_Context->y_Auto2744+80,330,530,vfh[1],0,28,_Context->msg_RefList[_Context->language],_Context->buffer_RefList,0x4401,"($)");
;
	return(-1);
}
...
public	int	widget_reference_locator_show(struct widget_reference_locator_context * _Context)
{
	...
	if ((_Context->pushbuffer != (void *) 0)
	&& (!(_Context->ctrlbuffer))) {
		(void)visual_buffer_get(_Context->pushbuffer,_Context->x_Auto2744,_Context->y_Auto2744);
		_Context->ctrlbuffer=1;
		}
	visual_freeze();
	if (((_Context->status = visual_styled_element(_Context->x_Auto2744,_Context->y_Auto2744,370,620,GetWidgetStyle(0),GetWindowVersion(),strlen(GetWindowVersion()))) != 0)
) {
		visual_window(_Context->x_Auto2744,_Context->y_Auto2744,370,620,vfh[2],GetWindowVersion(),strlen(GetWindowVersion()),0x407);
		}
	visual_text(_Context->x_Auto2744+10,_Context->y_Auto2744+30,322,20,vfh[3],16,0,_Context->msg_Auto2745[_Context->language],strlen(_Context->msg_Auto2745[_Context->language]),1282);
	visual_text(_Context->x_Auto2744+10,_Context->y_Auto2744+60,42,16,vfh[2],16,0,_Context->msg_Auto2746[_Context->language],strlen(_Context->msg_Auto2746[_Context->language]),258);
	visual_frame(_Context->x_Auto2744+48,_Context->y_Auto2744+60,288+2,16+2,5);
	visual_text(_Context->x_Auto2744+48+1,_Context->y_Auto2744+60+1,288,16,vfh[1],0,28,_Context->buffer_Target,255,0);
	(void) on_RefList_show(_Context);
	visual_scrollbar(_Context->x_Auto2744+340,_Context->y_Auto2744+80,20,530,vfh[1],0,28,_Context->value_RefBar,_Context->limit_RefBar,_Context->max_RefBar,259);
	visual_thaw(_Context->x_Auto2744,_Context->y_Auto2744,370,620);
...
private int Auto2744action(struct widget_reference_locator_context * _Context) {
	...
	do {
		...
		_Context->x_Auto2744 += (visual_event(7)-x);
		x=visual_event(7);
		if (_Context->x_Auto2744 < 0) { _Context->x_Auto2744=0; }
		_Context->y_Auto2744 += (visual_event(6)-y);
		y=visual_event(6);
		if (_Context->y_Auto2744 < 0) { _Context->y_Auto2744=0; }
...
private int on_RefBar_action(struct widget_reference_locator_context * _Context) {
	...
	if ( visual_event(6) < (_Context->y_Auto2744+80+20) ) {
	...
	if ( visual_event(6) > (_Context->y_Auto2744+80+530-20) ) {
		do { while( visual_getch() != 256); } while ( visual_event(1)  != _MIMO_UP );
		_Context->value_RefBar += 1;
	...
	do {
		...
		_Context->value_RefBar = (((visual_event(6) - (_Context->y_Auto2744+80+20)) * _Context->max_RefBar) / (530 - (2 * 20)));
		if (_Context->value_RefBar < 0) 
			_Context->value_RefBar = 0;
		else if (_Context->value_RefBar > (_Context->max_RefBar-_Context->limit_RefBar)) 
			_Context->value_RefBar = (_Context->max_RefBar-_Context->limit_RefBar);
...
struct widget_reference_locator_context * _Context){
	...
	switch (mt) {
	case _MIMO_UP :
	if ((mx >= (_Context->x_Auto2744+349) )  
	&&  (my >= (_Context->y_Auto2744+4) )  
	&&  (mx <= (_Context->x_Auto2744+366) )  
	&&  (my <= (_Context->y_Auto2744+20) )) {
		return(1);	/* Auto2744 */

		}
	if ((mx >= (_Context->x_Auto2744+333) )  
	&&  (my >= (_Context->y_Auto2744+4) )  
	&&  (mx <= (_Context->x_Auto2744+350) )  
	&&  (my <= (_Context->y_Auto2744+20) )) {
	...
	if ((mx >= (_Context->x_Auto2744+317) )  
	&&  (my >= (_Context->y_Auto2744+4) )  
	&&  (mx <= (_Context->x_Auto2744+334) )  
	&&  (my <= (_Context->y_Auto2744+20) )) {
	...
	case _MIMO_DOWN :
		if ((mx >= (_Context->x_Auto2744+4) )  
		&&  (my >= (_Context->y_Auto2744+4) )  
		&&  (mx <= (_Context->x_Auto2744+302) )  
		&&  (my <= (_Context->y_Auto2744+20) )) {
			return( Auto2744action(_Context) );
			}
		}
	if (( mx >= (_Context->x_Auto2744+48) ) 
	&&  ( my >= (_Context->y_Auto2744+60) ) 
	&&  ( mx <= (_Context->x_Auto2744+48+288) ) 
	&&  ( my <= (_Context->y_Auto2744+60+16))) {
		return(2); /* Target */
		}
	if (( mx >= (_Context->x_Auto2744+10) ) 
	&&  ( my >= (_Context->y_Auto2744+80) ) 
	&&  ( mx <= (_Context->x_Auto2744+10+330) ) 
	&&  ( my <= (_Context->y_Auto2744+80+530))) {
		if ((_Context->value_RefList = ((visual_event(6) - (_Context->y_Auto2744+80)) / 16)) < 1)
		...
		_Context->column_RefList = (visual_event(7) - (_Context->x_Auto2744+10));
	...
	if (( mx >= (_Context->x_Auto2744+340) ) 
	&&  ( my >= (_Context->y_Auto2744+80) ) 
	&&  ( mx <= (_Context->x_Auto2744+340+20) ) 
	&&  ( my <= (_Context->y_Auto2744+80+530))) {
		return(4); /* RefBar */
		}
