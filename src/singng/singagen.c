...
private	void	web_agent_oncreate( FILE * h, struct form_control * fptr )
{
	...
	web_agent_method(h,"OnCreate",2);		linefeed(h);
		...
		indent(h,1); fprintf(h,"if ( i > 1 ) ");			 		linefeed(h);
			...
			indent(h,3); fprintf(h,"i = Index(LineBuffer,%c=%c)",__QUOTE,__QUOTE); 	linefeed(h);
			indent(h,3); fprintf(h,"if ( i = 0 ) :: Break :: Endif");		linefeed(h);
			indent(h,3); fprintf(h,"NameToken = Left(LineBuffer,(i-1))");		linefeed(h);
			indent(h,3); fprintf(h,"LineBuffer=Includ(1,Space(i))"); 		linefeed(h);
			indent(h,3); fprintf(h,"LineBuffer=Shl(LineBuffer)"); 			linefeed(h);
			indent(h,3); fprintf(h,"i = Index(LineBuffer,%c&%c)",__QUOTE,__QUOTE); 	linefeed(h);
			indent(h,3); fprintf(h,"Select i");					linefeed(h);
			indent(h,3); fprintf(h,"Case   0");					linefeed(h);
				indent(h,4); fprintf(h,"ValueToken = LineBuffer");		linefeed(h);
				indent(h,4); fprintf(h,"LineBuffer = %c %c",__QUOTE,__QUOTE);	linefeed(h);
			indent(h,3); fprintf(h,"Case   1");					linefeed(h);
				indent(h,4); fprintf(h,"ValueToken = %c %c",__QUOTE,__QUOTE);	linefeed(h);
				indent(h,4); fprintf(h,"LineBuffer=Includ(1,Space(i))"); 	linefeed(h);
				indent(h,4); fprintf(h,"LineBuffer=Shl(LineBuffer)"); 		linefeed(h);
			indent(h,3); fprintf(h,"Default ");					linefeed(h);
				indent(h,4); fprintf(h,"ValueToken = Left(LineBuffer,(i-1))");		linefeed(h);
				indent(h,4); fprintf(h,"LineBuffer=Includ(1,Space(i))"); 	linefeed(h);
				indent(h,4); fprintf(h,"LineBuffer=Shl(LineBuffer)"); 		linefeed(h);
...
private	void	web_agent_request( FILE * h, struct form_control * fptr )
{
	...
	indent(h,1); fprintf(h,"if ( i > 1 ) ");		 		linefeed(h);
		...
		if ( fptr->pages ) {
			...
			indent(h,4); fprintf(h,"While(Index(LineBuffer,Chr$(/2F)) <> 0) :: LineBuffer=Includ(Index(LineBuffer,Chr$(/2F)),Chr$(/5C),1) :: Wend"); linefeed(h);
			indent(h,3); fprintf(h,"Endif"); linefeed(h);
	...
	if (!( fptr->pages )) {
		...
		indent(h,2); fprintf(h,"if ( ClientRedirect = 0 )");			 linefeed(h);
		indent(h,3); fprintf(h,"if ( Conf(10) = 1) "); linefeed(h);
		indent(h,4); fprintf(h,"While(Index(url,Chr$(/2F)) <> 0) :: url=Includ(Index(url,Chr$(/2F)),Chr$(/5C),1) :: Wend"); linefeed(h);
		indent(h,3); fprintf(h,"Endif"); linefeed(h);
		indent(h,3); fprintf(h,"Select PageNumber");			 linefeed(h);
		for (	i=1; i <= fptr->pages; i++ ) {
			indent(h,3); fprintf(h,"Case %u :: ResponseFile = Print(($,%cpage%u_%c,$),Parse(url,7),Parse(url,8+16))",i,__QUOTE,i,__QUOTE);	 linefeed(h);
			}
		indent(h,3); fprintf(h,"EndSel");			 linefeed(h);
		indent(h,2); fprintf(h,"EndIf");			 linefeed(h);
		}
...
{
	abal_onlose_methods( h, __abal_method_lose_focus, 0 );
