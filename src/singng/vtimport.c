...
private struct text_import_context {
	int	keycode;
	int	focus_item;
	int	focus_items;
	int	window_col;
...
private	int	text_import_create(
	char * pFileName)
{

	if (!(_Context = allocate( sizeof( struct text_import_context ) ) ))
		return(27);
	_Context->keycode=0;
...
private	int	text_import_hide()
{

	return(0);
}

private	int	text_import_remove(
	char * pFileName)
{
...
private	int	text_import_show()
{
	...
	visual_text(115,260,560,16,2,38,0,"Name of Text File to Import",27,258);
	...
	visual_text(329,324,46,44,8,27,24,"abcdefghijklmnopqrstuvwxyzabcdefghijklmnop",42,0);
...
retry:
		...
		case	0x2 :
			...
			case	0x100 :
				...
				if ((retcode=text_import_event()) == -1)
					continue;
...
public	int	text_import(
	char * pFileName)
{
	int	status=0;
	status = text_import_create(
	 pFileName);
	if ( status != 0) return(status);
	status = text_import_show();
		enter_modal();
	status = text_import_focus();
		leave_modal();
	status = text_import_hide();
	status = text_import_remove(
	 pFileName);
