...
private int	cpp_module_description( FILE * h,char * cname)
{
	...
	/*if (!( headname = allocate( strlen(cname)+3 ) )) {
		allocate_failure();
		return(0);
		}
 
	sprintf(headname,"%spp",cname);
	if (!( headname = enforce_extension( headname, H_EXTENSION ) )) {
		allocate_failure();
		return(0);
		}*/
	else	{
...
private char *allocate_cpp_module_description(char * cname)
{
	...
	return(headername( fptr->prodname, OBJ_EXTENSION));
...
if ((iptr->Contents.style & _FRAME_TYPE )==_RADIO_FRAME)
	...
	sprintf(t,"%s%s",iptr->Contents.name,DECORE_CLASS);
return(t);
}
...
if (nclass)
	...
	sprintf(buffer,"fonction %s",fonction);
...
for (i=0;i<(80-strlen(buffer))/2;i++) fprintf(h," ");
fprintf(h,"%s",buffer);
for (i=0;i<80-((80-strlen(buffer))/2)-strlen(buffer);i++) fprintf(h," ");
...
{
indent(h,margin);
//fprintf(h,"%s(%s,%s,%s_%s,this);",_ATTACHE,cpp_item_name(iptr),methode,iptr->Contents.name,fonction);
...
{
indent(h,margin);
//fprintf(h,"%s(%s,%s,%s->%s,%s);",_ATTACHE,cpp_item_name(iptr),methode,cpp_item_name(iptr),fonction,cpp_item_name(iptr));
...
public  int	cpp_member_parameters( FILE * h ,int iscall)
{
	...
	if ((iscall==2) && (nb)) fprintf(h,"%s",suffix);
	return(nb);
}
...
private	void	sort_cpp_form( FILE * h,char * cname )
{
	...
	for (	iptr=Context.Form->first; 
		...
		if ((iptr->Contents.style & _FRAME_TYPE) == _FORM_FRAME)
			...
			fprintf(h,"/* ---------------------------- */");
			linefeed(h);
			fprintf(h,"/* select the linesort to fill  */");
			linefeed(h);
			fprintf(h,"/* ---------------------------- */");
			linefeed(h);
			...
			linefeed(h);
			calculate_edit_size( &iptr->Contents );
			calculate_form_lines( &iptr->Contents, &linelength, &nblines );
			...
			fprintf(h,"/* ------------------------------------------ */");
			linefeed(h);
			fprintf(h,"/* sort the intermediate table into tab_sort  */");
			linefeed(h);
			fprintf(h,"/* ------------------------------------------ */");
			linefeed(h);
			...
			fprintf(h,"/* --------------------------------------- */");
			linefeed(h);
			fprintf(h,"/* compare the elements a et b of libsort  */");
			linefeed(h);
			fprintf(h,"/* --------------------------------------- */");
			linefeed(h);
			...
			if (( fields = calculate_form_fields(iptr,fieldwidth,_MAX_FORM_FIELDS,linelength) ))
				...
				fprintf(h,"i=memicmp(&this.libsort[%d*a],&this.libsort[%d*b],%d);	/* comparaison zone 1 */",
			...
			fprintf(h,"/* ------------------------ */");
			linefeed(h);
			fprintf(h,"/* select the line to fill  */");
			linefeed(h);
			fprintf(h,"/* ------------------------ */");
			linefeed(h);
			...
			fprintf(h,"/* ------------------------------------- */");
			linefeed(h);
			fprintf(h,"/* select the adresse of column to fill  */");
			linefeed(h);
			fprintf(h,"/* ------------------------------------- */");
			linefeed(h);
...
private	void	create_cpp_form(FILE * h, char * cname,	struct form_item * *value_wptr)
{
	...
	for (	iptr=Context.Form->first; 
		...
		if (((iptr->Contents.style & _FRAME_TYPE ) == _DATA_FRAME) &&
			...
			(iptr->Contents.align & _A_BOLD) ||
			(!method_is_valid(c_payload(iptr))))) continue;
...
private	void	construct_cpp_form(FILE * h, char * cname)
{
	...
	struct	window_control * pwptr=(struct window_control *) 0;
		...
		if ((iptr->Contents.style & _FRAME_TYPE ) == _LINE_FRAME)
			...
			fprintf(h,",%u,%u",iptr->Contents.h,iptr->Contents.w);
		...
		if (nprevious==NULL)
			...
			fprintf(h,",%s",cpp_item_name(nprevious));
		nprevious=iptr;
		fprintf(h,",1");	/* indexer */

		switch ((iptr->Contents.style & _FRAME_TYPE )) {

			case	_FILL_FRAME	:	/* align */
				fprintf(h,",%u",(iptr->Contents.align&_VFILL_MASK));
				break;

			case	_LINE_FRAME 	: /* 8=visual_line, thickness   */
				fprintf(h,",%u",iptr->Contents.size );
				break;

			case	_OUTSET_FRAME	: /* 1=visual_frame, just style,text,align  */
			case	_INSET_FRAME	: /* 2=visual_frame, just style  */
			case	_RIDGE_FRAME	: /* 3=visual_frame, just style  */
			case	_GROOVE_FRAME	: /* 4=visual_frame, just style  */
				fprintf(h,",%u",iptr->Contents.style);
				if (is_payload_unquoted(iptr))
					fprintf(h,",(char*)0");
				else
					cpp_payload_parameter(h,iptr);
				align_parameter(h,iptr);
				break;

			case	_IMAGE_FRAME	: /* 7=visual_image, payload/align */
				if (is_payload_unquoted(iptr))
					fprintf(h,",(char*)0");
				else
					cpp_payload_parameter(h,iptr);
			...
			case	_TAB_FRAME	: /* 11=visual_tabpage, sptr,a,adjust*/
			...
			case	_BUTTON_FRAME	: /* 9=visual_button */
				if (is_payload_unquoted(iptr))
					fprintf(h,",(char*)0");
				else
					cpp_payload_parameter(h,iptr);
			...
			case	_SELECT_FRAME	: /* 14=visual_select, sptr,v*/
				if (is_payload_unquoted(iptr))
					fprintf(h,",(char*)0");
				else
					cpp_payload_parameter(h,iptr);
			...
			case	_RADIO_FRAME	: /* 13=visual_radio, sptr,a,v */
				if ( iptr->Contents.radio == 1 )
					items = add_interface_item(items, iptr, _SYMBOLIC_WORD );
				if (is_payload_unquoted(iptr))
					fprintf(h,",(char*)0");
			...
			case	_GRAPH_FRAME	: /* 19=visual_graph, sptr,a,v */
				if (is_payload_unquoted(iptr))
					fprintf(h,",(char*)0");
				else
					cpp_payload_parameter(h,iptr);
			...
			case	_SWITCH_FRAME	: /* 18=visual_switch, sptr,a,v */
			case	_CHECK_FRAME	: /* 12=visual_check, sptr,a,v */
				items = add_interface_item(items, iptr, _SYMBOLIC_WORD );
				if (is_payload_unquoted(iptr))
					fprintf(h,",(char*)0");
				else
			...
			case	_WINDOW_FRAME	: /* 10=visual_window, sptr,a,multilangue */
				wptr = iptr;
				if (is_payload_unquoted(iptr))
					fprintf(h,",(char*)0");
				else
			...
			case _PROGRESS_FRAME:	/* 17=visual_progress value,limit */
				fprintf(h,",1,1");
				items = add_interface_item(items, iptr, _SYMBOLIC_WORD );
				items = add_interface_item(items, iptr, _SYMBOLIC_WORD );
				break;

			case _SCROLL_FRAME:		/* 16=visual_scroll value,limit,max,align */
				fprintf(h,",1,1,1");
				align_parameter(h,iptr);
				items = add_interface_item(items, iptr, _SYMBOLIC_WORD );
				items = add_interface_item(items, iptr, _SYMBOLIC_WORD );
			...
			case	_EDIT_FRAME	: /* 5=visual_edit, sptr,length,tlig,nlig */
			...
			case _FORM_FRAME:	/* 15=visual_form, nlig,tlig,nbfields,tfields */
				if (is_payload_unquoted(iptr))
					fprintf(h,",(char*)0");
				else
					cpp_payload_parameter(h,iptr);
				if (!( is_valid_format( iptr->Contents.format ) )) 
					...
					string_parameter(h,iptr->Contents.format);
				align_parameter(h,iptr);
		...
		if ( iptr->Contents.events->xo_hide & _EVENT_IS_POP )
			{
			indent(h,1);
			fprintf(h,"%s->SetPopUp();",cpp_item_name(iptr));	/* gestion popup */
			linefeed(h);
			};
		...
		if (method_is_valid( iptr->Contents.events->on_create )
			&&  ( iptr->Contents.events->xo_create & _EVENT_IS_FUNCTION ))
				...
				attache_method_base(h,iptr,_METHOD_ONCREATE,_ON_CREATE,1);
		...
		if (!( iptr->Contents.events->xo_show & _EVENT_IS_COND ))
			...
			if ( method_is_valid( iptr->Contents.events->on_show ) &&
				(iptr->Contents.events->xo_show & _EVENT_IS_FUNCTION))
					...
					attache_method_base(h,iptr,_METHOD_ONSHOW,_ON_SHOW,1);
			};
		...
		if (( iptr->Contents.events->xo_event & _EVENT_IS_FOCUS ))
			...
			&&  ( iptr->Contents.events->xo_event & _EVENT_IS_FUNCTION ))
				...
				attache_method_base(h,iptr,_METHOD_ONEVENT,_ON_EVENT,1);
			};
		...
		if (( iptr->Contents.events->xo_get_focus & _EVENT_IS_FOCUS ))
			...
			if ( method_is_valid( iptr->Contents.events->on_get_focus )
				...
				attache_method_base(h,iptr,_METHOD_ONFOCUS,_ON_GETFOCUS,1);
			};
		...
		if (method_is_valid( iptr->Contents.events->on_hide )
			&&  ( iptr->Contents.events->xo_hide & _EVENT_IS_FUNCTION ))
				...
				attache_method_base(h,iptr,_METHOD_ONHIDE,_ON_HIDE,1);

		if (method_is_valid( iptr->Contents.events->on_remove )
			&&  ( iptr->Contents.events->xo_remove & _EVENT_IS_FUNCTION ))
				attache_method(h,iptr,_METHOD_ONREMOVE,cname,__method_remove,1);
			else
				attache_method_base(h,iptr,_METHOD_ONREMOVE,_ON_REMOVE,1);

		if (method_is_valid( iptr->Contents.events->on_lose_focus )
			&&  ( iptr->Contents.events->xo_lose_focus & _EVENT_IS_FUNCTION ))
				attache_method(h,iptr,_METHOD_ONLOSEFOCUS,cname,__method_losefocus,1);
			else
				attache_method_base(h,iptr,_METHOD_ONLOSEFOCUS,_ON_LOSEFOCUS,1);
		...
		if 	((Context.Form->onlinehelp & 7) && (detect_widget_help(iptr)))
			...
			if ((iptr->Contents.style & _FRAME_TYPE ) != _RADIO_FRAME)
				...
				attache_method_radio(h,iptr,_METHOD_ONHELP,cname,"help",1);
			};
#endif
	...
	if (first)
		...
		indent(h,1);
		fprintf(h,"/* initialise container */");
		linefeed(h);
		indent(h,1);
...
private	void	hide_cpp_form(FILE * h, char * cname)
{
	...
	for (	wptr=(struct form_item*) 0, 
		...
		if (((iptr->Contents.style & _FRAME_TYPE ) == _DATA_FRAME) &&
			...
			(iptr->Contents.datatype == _WIDGET_POINTER) ||
			(!method_is_valid(c_payload(iptr))) ||
			((iptr->Contents.align & _A_BOLD)))) continue;
...
// GGBI a voir car apelle fonction Hide 
//			if (( iptr->Contents.style & _FRAME_TYPE) == _DATA_FRAME) {
...
private	void	remove_cpp_form(FILE * h, char * cname)
{
	...
	for (	wptr=(struct form_item*) 0, 
		...
		if ( method_is_valid( iptr->Contents.events->on_remove ) ) {
			/*if ( iptr->Contents.events->xo_hide & _EVENT_IS_POP ) {
				drop_cpp_widget_saver(h,iptr,1);
				}*/
...
private	char *	cpp_member_column(FILE * h, struct form_item * iptr )
{
	...
	if (!(c = c_get_punctuation() ))
		...
		if (!( tptr = c_get_token(0) ))
			...
			return( (char *) 0 );
		else 	{ fprintf(h,"%scolumn(%s)",lwidget,tptr);
				return((char *) 0);
				}
		}
	else 	{
...
private	char *	cpp_member_line(FILE * h, struct form_item * iptr )
{
	...
	if (!(c = c_get_punctuation() ))
		...
		if (!( tptr = c_get_token(0) ))
			...
			return( (char *) 0 );
		else 	{ fprintf(h,"%sline(%s)",lwidget,tptr);
				return((char *) 0);
				}
		}
	else 	{
...
private	char *	cpp_member_linesort(FILE * h, struct form_item * iptr )
{
	...
	if (!(c = c_get_punctuation() ))
		...
		if (!( tptr = c_get_token(0) ))
			...
			return( (char *) 0 );
		else 	{ fprintf(h,"%slinesort(%s);",lwidget,tptr);
				return((char *) 0);
				}
		}
	else 	{
...
private	char *	cpp_macro_columnsize( FILE * h, struct form_item * iptr, char * tptr )
{
	...
	if (!( tptr )) {
		...
		if ((iptr->Contents.style & _FRAME_TYPE) == _FORM_FRAME )
			...
			fprintf(h,"%stlig",lwidget,field-1);
		return((char *) 0);
		}
}
...
if (method_is_valid(tptr)>9)
{
	...
	if (upper_compare(tptr,"pere"))
		...
		if (string_compare( tptr, "_Context" ))
			...
			{
			if (!( iptr = locate_widget( tptr ) ) && (!string_compare( tptr, LineParser.thisname )))
				{
				if ((underscore=strchr(tptr,'_')) != (char *)0)
					{
					if (iptr=locate_widget(underscore+1))
						{
						if ((c=parser_getch())=='(')
							{
							parser_ungetch();
				...
				parser_ungetch();
/* GGBI, soit on reconnait un widget et on le nomme soit on renvoie le libellé */
				if (iptr)
		...
		if (!iptr)
			...
			{
/*		if (!LineParser.thisname[0])
			{
			if ((iptr->Contents.style & _FRAME_TYPE ) == _RADIO_FRAME)
				sprintf(lwidget,"((%s *)pere)->%s%s%u->",cname,iptr->Contents.name,DECORE_CLASS,iptr->Contents.radio);
			else
				sprintf(lwidget,"((%s *)pere)->%s%s->",cname,iptr->Contents.name,DECORE_CLASS);
			}
		else
			{*/
...
//			if ((iptr->Contents.style&_FRAME_TYPE) != _TAB_FRAME) {
		...
		case	VISUAL_MEMBER_LOSEFOCUS	:
			...
			fprintf(h,"_%s()",tptr);
/* si on veut surcharger les classes remplacer par
			fprintf(h,"%s->%s()",lwidget,tptr);*/
			return((char *) 0);

		case -1:
			/* fonction non répertoriée -> si DATA OK */
			if ((iptr) && ((iptr->Contents.style & _FRAME_TYPE) == _DATA_FRAME)
	...
	while (1) {
		if ((c = c_get_punctuation())) {
			...
			fprintf(h,"%c",c);
				...
				/* cas du struct.qqchose ou struct->qqchose, si ce qqchose porte le même nom */
...
private	void	export_cpp_formhead( FILE * h, char * cname )
{
	...
	if ((pwptr = connect_production_window()) != (struct window_control *) 0)
		...
		if (wptr != (struct form_item*) 0)
			...
			fprintf(h,"int px=0,int py=0,int ph=0,int pw=0");
		};
...
dir_cl=allocate_string(SingConf->cpp.program);
fn_parser(SingConf->cpp.program,dir_cl,0x0007);

cde=allocate(strlen(SingConf->cpp.program)+strlen(SingConf->cpp.path)+strlen(SingConf->cpp.options)+strlen(dir_cname)*4+strlen(cname)+70+method_is_valid(fptr->OptionsCompile)+method_is_valid(SingConf->cpp.define));
#ifdef WIN32
...
include=allocate_string(SingConf->cpp.path);
ptr_include=include;
...
/*                          */
/* ------------------------ */
void compile_dll_cpp(char *cname)
{
int coder,with_path=1;
char sv_dir[257];
char *dir_cl,*dir_cname;
