...
int	databaseLinkInfo(struct visual_database_relation * rptr)
{
	...
	if (!( sptr = rptr->source ))
		...
		if ( accept_database_relation(&rptr->name,&rptr->jointure,&lnature,&rnature,&smptr->name,&sptr->name,&tmptr->name,&tptr->name) != 27 ) {
...
int	databaseLinkMenu(struct visual_database_relation * rptr)
{
	switch ( database_link_menu() ) {
		case	_LINK_INFO	:
			...
			break;
		case	_LINK_DELETE	:
			databaseLinkDelete( rptr );
			break;
		case	_LINK_MOVE	:
			databaseLinkMove(rptr);
			break;
		}
	return;
}
...
void	databaseRelationSave(struct visual_database_relation * rptr )
{
	...
	if (!( rptr ))
		...
		rptr->relation = create_database_link( rptr->name, rptr->jointure , snptr, tnptr, rptr->nature);
		liberate( tnptr );
		liberate( snptr );
		return;
		}
