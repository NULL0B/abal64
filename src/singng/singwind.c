...
public	void	draw_window_grid(int topx,int topy,struct form_item * iptr )
{
	int	v;
	int	n;
	int	gw=Context.gridwidth;
	int	gh=Context.gridheight;
	if (!( iptr ))
		return;
	for (	v=0, n = iptr->Contents.h;
		v < n;
		v += gh ) {
		filzone(topx+iptr->Contents.x,
			topy+iptr->Contents.y+v,
			iptr->Contents.w,1,19);
		}
	for (	v=0, n=iptr->Contents.w;
		v < n;
		v += gw ) {
		filzone(topx+iptr->Contents.x+v,
			topy+iptr->Contents.y,
			1,iptr->Contents.h,19);
		}
	return;
}
