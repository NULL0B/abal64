...
private	void	build_national_image( struct form_control * fptr, char * nomfic, char * pathroot, char * natroot, int nature )
{
	...
	if ( pathroot )
		if (!( nptr = path_prefix( nptr, pathroot )))
			return;
#ifdef	ABAL32A
	save_form_image( nptr, fptr->image, -1, fptr->weboptions );
#else
	save_form_image( nptr, fptr->image, -1, 0);
#endif
	liberate( nptr );
	return;
}
...
public	void	generate_form_images( struct form_control * fptr, char * nomfic )
{
	...
	if (!( fptr->pages )) {
		if (!( fptr->formflags & _FORM_MULTILINGUAL )) {
			...
			build_national_image( fptr, nomfic, Document.pathroot, (char *) 0, form_imagetype( fptr ) );
			store_payloads(fptr);
			}
		else	{
			n = fptr->natlang;
			for ( i=0; i < MAXNATLANG; i++ ) {
				...
				build_national_image( fptr, nomfic, Document.pathroot, PathPrefix[fptr->natlang], form_imagetype( fptr ) );
				store_payloads(fptr);
				}
			fptr->natlang = n;
			}
		...
		for (i=1; i <= fptr->pages; i++ ) {
			...
			if (!( fptr->formflags & _FORM_MULTILINGUAL )) {
				(void) visual_national_language(fptr->natlang);
				reload_payloads(fptr);
				build_national_image( fptr, pnptr, Document.pathroot, (char *) 0, form_imagetype( fptr ) );
				store_payloads(fptr);
				}
			else	{
				n = fptr->natlang;
				for ( x=0; x < MAXNATLANG; x++ ) {
					...
					(void) visual_national_language(fptr->natlang);
					reload_payloads(fptr);
					build_national_image( fptr, pnptr, Document.pathroot, PathPrefix[fptr->natlang], form_imagetype( fptr ) );
					store_payloads(fptr);
					}
				fptr->natlang = n;
				}
...
private	int	export_Camera(struct camera3d * cptr, int automatic)
{
	...
	if (!( automatic & _INHIBIT_PROPERTIES )) {
		...
		else if ((prodlang = (aptr->language-_GENERATE_BAL)) < 0 ) {
			whoops++;
			prodlang = aptr->language;
			}		
