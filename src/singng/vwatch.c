...
#include <stdio.h>
#include "visual.h"
#define en_Auto2771   "Debug Memory Inspection"
#define fr_Auto2771   "Modification de la M‚moire"
#define it_Auto2771   "Debug Memory Inspection"
#define es_Auto2771   "Debug Memory Inspection"
#define de_Auto2771   "Debug Memory Inspection"
#define nl_Auto2771   "Debug Memory Inspection"
#define pt_Auto2771   "Debug Memory Inspection"
#define xx_Auto2771   "Debug Memory Inspection"
#define en_Auto2772   ""
#define en_Adresse   ""
...
_Context->indsort_DumpZone=NULL;	/* pas de pointeur de table intermédiaire */		_Context->max_DragBar=0;
	...
	_Context->x_Auto2771=70;
	_Context->y_Auto2771=45;
	_Context->w_Auto2771=660;
	_Context->h_Auto2771=510;
	...
	_Context->pushbuffer=visual_buffer(_Context->x_Auto2771,_Context->y_Auto2771,660+10,510+10);
	return(0);
}
...
public 	int	accept_watch_inspection_hide(struct accept_watch_inspection_context * _Context)
{
	...
	&& (_Context->ctrlbuffer != 0)) {
		(void)visual_buffer_put(_Context->pushbuffer,_Context->x_Auto2771,_Context->y_Auto2771);
		_Context->ctrlbuffer=0;
		}
...
private int on_DumpZone_show(struct accept_watch_inspection_context * _Context) {
	...
	*(vptr++)=muffer[1];
	...
	visual_freeze();
		visual_table(_Context->x_Auto2771+20,_Context->y_Auto2771+60,596,420,vfh[1],27,28,_Context->msg_DumpZone[_Context->language],_Context->buffer_DumpZone,0x4406,"(W8,W48,$)");
;
	visual_thaw(_Context->x_Auto2771+20,_Context->y_Auto2771+60,596,420);
	return(-1);
}
...
public	int	accept_watch_inspection_show(struct accept_watch_inspection_context * _Context)
{
	...
	if ((_Context->pushbuffer != (void *) 0)
	&& (!(_Context->ctrlbuffer))) {
		(void)visual_buffer_get(_Context->pushbuffer,_Context->x_Auto2771,_Context->y_Auto2771);
		_Context->ctrlbuffer=1;
		}
	visual_freeze();
	visual_window(_Context->x_Auto2771,_Context->y_Auto2771,660,510,vfh[2],_Context->msg_Auto2771[_Context->language],strlen(_Context->msg_Auto2771[_Context->language]),0x407);
	visual_frame(_Context->x_Auto2771+10,_Context->y_Auto2771+30,640,470,3);
	visual_frame(_Context->x_Auto2771+20,_Context->y_Auto2771+40,72+2,16+2,5);
	visual_text(_Context->x_Auto2771+20+1,_Context->y_Auto2771+40+1,72,16,vfh[1],27,28,_Context->buffer_Adresse,9,0);
	visual_frame(_Context->x_Auto2771+100,_Context->y_Auto2771+40,540+2,16+2,5);
	visual_text(_Context->x_Auto2771+100+1,_Context->y_Auto2771+40+1,540,16,vfh[1],27,28,_Context->buffer_VariableName,67,0);
	(void) on_DumpZone_show(_Context);
	visual_scrollbar(_Context->x_Auto2771+620,_Context->y_Auto2771+60,20,420,vfh[1],27,28,_Context->value_DragBar,_Context->limit_DragBar,_Context->max_DragBar,259);
	visual_thaw(_Context->x_Auto2771,_Context->y_Auto2771,660,510);

	return(0);
}

private int Auto2771action(struct accept_watch_inspection_context * _Context) {
	...
	do {
		...
		_Context->x_Auto2771 += (visual_event(7)-x);
		x=visual_event(7);
		if (_Context->x_Auto2771 < 0) { _Context->x_Auto2771=0; }
		_Context->y_Auto2771 += (visual_event(6)-y);
		y=visual_event(6);
		if (_Context->y_Auto2771 < 0) { _Context->y_Auto2771=0; }
...
private int on_DragBar_action(struct accept_watch_inspection_context * _Context) {
	...
	if ( visual_event(6) < (_Context->y_Auto2771+60+20) ) {
	...
	if ( visual_event(6) > (_Context->y_Auto2771+60+420-20) ) {
		do { while( visual_getch() != 256); } while ( visual_event(1)  != _MIMO_UP );
		_Context->value_DragBar += 1;
	...
	do {
		...
		_Context->value_DragBar = (((visual_event(6) - (_Context->y_Auto2771+60+20)) * _Context->max_DragBar) / (420 - (2 * 20)));
		if (_Context->value_DragBar < 0) 
			_Context->value_DragBar = 0;
		else if (_Context->value_DragBar > (_Context->max_DragBar-_Context->limit_DragBar)) 
			_Context->value_DragBar = (_Context->max_DragBar-_Context->limit_DragBar);
...
struct accept_watch_inspection_context * _Context){
	...
	switch (mt) {
	case _MIMO_UP :
	if ((mx >= (_Context->x_Auto2771+639) )  
	&&  (my >= (_Context->y_Auto2771+4) )  
	&&  (mx <= (_Context->x_Auto2771+656) )  
	&&  (my <= (_Context->y_Auto2771+20) )) {
		return(1);	/* Auto2771 */

		}
	if ((mx >= (_Context->x_Auto2771+623) )  
	&&  (my >= (_Context->y_Auto2771+4) )  
	&&  (mx <= (_Context->x_Auto2771+640) )  
	&&  (my <= (_Context->y_Auto2771+20) )) {
	...
	if ((mx >= (_Context->x_Auto2771+607) )  
	&&  (my >= (_Context->y_Auto2771+4) )  
	&&  (mx <= (_Context->x_Auto2771+624) )  
	&&  (my <= (_Context->y_Auto2771+20) )) {
	...
	case _MIMO_DOWN :
		if ((mx >= (_Context->x_Auto2771+4) )  
		&&  (my >= (_Context->y_Auto2771+4) )  
		&&  (mx <= (_Context->x_Auto2771+592) )  
		&&  (my <= (_Context->y_Auto2771+20) )) {
			return( Auto2771action(_Context) );
			}
		}
	if (( mx >= (_Context->x_Auto2771+20) ) 
	&&  ( my >= (_Context->y_Auto2771+40) ) 
	&&  ( mx <= (_Context->x_Auto2771+20+72) ) 
	&&  ( my <= (_Context->y_Auto2771+40+16))) {
		return(2); /* Adresse */
		}
	if (( mx >= (_Context->x_Auto2771+100) ) 
	&&  ( my >= (_Context->y_Auto2771+40) ) 
	&&  ( mx <= (_Context->x_Auto2771+100+540) ) 
	&&  ( my <= (_Context->y_Auto2771+40+16))) {
		return(3); /* VariableName */
		}
	if (( mx >= (_Context->x_Auto2771+20) ) 
	&&  ( my >= (_Context->y_Auto2771+60) ) 
	&&  ( mx <= (_Context->x_Auto2771+20+596) ) 
	&&  ( my <= (_Context->y_Auto2771+60+420))) {
		if ((_Context->value_DumpZone = ((visual_event(6) - (_Context->y_Auto2771+60)) / 16)) < 1)
		...
		_Context->column_DumpZone = (visual_event(7) - (_Context->x_Auto2771+20));
	...
	if (( mx >= (_Context->x_Auto2771+620) ) 
	&&  ( my >= (_Context->y_Auto2771+60) ) 
	&&  ( mx <= (_Context->x_Auto2771+620+20) ) 
	&&  ( my <= (_Context->y_Auto2771+60+420))) {
		return(5); /* DragBar */
		}
...
public	int	accept_watch_inspection_focus(struct accept_watch_inspection_context * _Context)
{
	...
	while(1) {
		switch (_Context->focus_item) {
			case	0x2 :
				/* Adresse */
				_Context->keycode = visual_edit(_Context->x_Auto2771+20+1,_Context->y_Auto2771+40+1,72,16,vfh[1],_Context->buffer_Adresse,9);
				on_DumpZone_show(_Context);
							break;
			case	0x5 :
				/* DragBar */
