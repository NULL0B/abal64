...
#define pt__Auto2726   ""
#define xx__Auto2726   ""
#define en_Auto2727   "File References"
#define fr_Auto2727   "Instances du fichier"
#define it_Auto2727   "File References"
#define es_Auto2727   "File References"
#define de_Auto2727   "File References"
#define nl_Auto2727   "File References"
#define pt_Auto2727   "File References"
#define xx_Auto2727   "File References"
#define en_Auto2728   "Class"
#define fr_Auto2728   "Nom"
#define it_Auto2728   "Nom"
#define es_Auto2728   "Name"
#define de_Auto2728   "Name"
#define nl_Auto2728   "Name"
#define pt_Auto2728   "Name"
#define xx_Auto2728   "Ainm"
#define en_Target   ""
...
_Context->indsort_RefList=NULL;	/* pas de pointeur de table intermédiaire */		_Context->max_RefBar=0;
	...
	_Context->x_Auto2726=10;
	_Context->y_Auto2726=10;
	_Context->w_Auto2726=370;
	_Context->h_Auto2726=620;
	...
	_Context->pushbuffer=visual_buffer(_Context->x_Auto2726,_Context->y_Auto2726,370+10,620+10);
	return(0);
}
...
public 	int	file_reference_locator_hide(struct file_reference_locator_context * _Context)
{
	...
	&& (_Context->ctrlbuffer != 0)) {
		(void)visual_buffer_put(_Context->pushbuffer,_Context->x_Auto2726,_Context->y_Auto2726);
		_Context->ctrlbuffer=0;
		}
...
private int on_RefList_show(struct file_reference_locator_context * _Context) {
	...
	get_file_reference(i+_Context->value_RefBar,(_Context->row_RefList+0),40);
		...
		visual_table(_Context->x_Auto2726+10,_Context->y_Auto2726+80,330,530,vfh[1],0,28,_Context->msg_RefList[_Context->language],_Context->buffer_RefList,0x4401,"($)");
;
	return(-1);
}
...
public	int	file_reference_locator_show(struct file_reference_locator_context * _Context)
{
	...
	if ((_Context->pushbuffer != (void *) 0)
	&& (!(_Context->ctrlbuffer))) {
		(void)visual_buffer_get(_Context->pushbuffer,_Context->x_Auto2726,_Context->y_Auto2726);
		_Context->ctrlbuffer=1;
		}
	visual_freeze();
	if (((_Context->status = visual_styled_element(_Context->x_Auto2726,_Context->y_Auto2726,370,620,GetWidgetStyle(0),GetWindowVersion(),strlen(GetWindowVersion()))) != 0)
) {
		visual_window(_Context->x_Auto2726,_Context->y_Auto2726,370,620,vfh[2],GetWindowVersion(),strlen(GetWindowVersion()),0x407);
		}
	visual_text(_Context->x_Auto2726+10,_Context->y_Auto2726+30,322,20,vfh[3],16,0,_Context->msg_Auto2727[_Context->language],strlen(_Context->msg_Auto2727[_Context->language]),1282);
	visual_text(_Context->x_Auto2726+10,_Context->y_Auto2726+60,42,16,vfh[2],16,0,_Context->msg_Auto2728[_Context->language],strlen(_Context->msg_Auto2728[_Context->language]),258);
	visual_frame(_Context->x_Auto2726+48,_Context->y_Auto2726+60,288+2,16+2,5);
	visual_text(_Context->x_Auto2726+48+1,_Context->y_Auto2726+60+1,288,16,vfh[1],0,28,_Context->buffer_Target,255,0);
	(void) on_RefList_show(_Context);
	visual_scrollbar(_Context->x_Auto2726+340,_Context->y_Auto2726+80,20,530,vfh[1],0,28,_Context->value_RefBar,_Context->limit_RefBar,_Context->max_RefBar,259);
	visual_thaw(_Context->x_Auto2726,_Context->y_Auto2726,370,620);
...
private int Auto2726action(struct file_reference_locator_context * _Context) {
	...
	do {
		...
		_Context->x_Auto2726 += (visual_event(7)-x);
		x=visual_event(7);
		if (_Context->x_Auto2726 < 0) { _Context->x_Auto2726=0; }
		_Context->y_Auto2726 += (visual_event(6)-y);
		y=visual_event(6);
		if (_Context->y_Auto2726 < 0) { _Context->y_Auto2726=0; }
...
private int on_RefList_event(struct file_reference_locator_context * _Context) {
	...
	if(l>0){
		file_reference_locator_hide(_Context);
;
	use_file_reference(_Context->value_RefBar+l);
	draw_console();
		file_reference_locator_show(_Context);
;
	}
...
private int on_RefBar_action(struct file_reference_locator_context * _Context) {
	...
	if ( visual_event(6) < (_Context->y_Auto2726+80+20) ) {
	...
	if ( visual_event(6) > (_Context->y_Auto2726+80+530-20) ) {
		do { while( visual_getch() != 256); } while ( visual_event(1)  != _MIMO_UP );
		_Context->value_RefBar += 1;
	...
	do {
		...
		_Context->value_RefBar = (((visual_event(6) - (_Context->y_Auto2726+80+20)) * _Context->max_RefBar) / (530 - (2 * 20)));
		if (_Context->value_RefBar < 0) 
			_Context->value_RefBar = 0;
		else if (_Context->value_RefBar > (_Context->max_RefBar-_Context->limit_RefBar)) 
			_Context->value_RefBar = (_Context->max_RefBar-_Context->limit_RefBar);
...
struct file_reference_locator_context * _Context){
	...
	switch (mt) {
	case _MIMO_UP :
	if ((mx >= (_Context->x_Auto2726+349) )  
	&&  (my >= (_Context->y_Auto2726+4) )  
	&&  (mx <= (_Context->x_Auto2726+366) )  
	&&  (my <= (_Context->y_Auto2726+20) )) {
		return(1);	/* Auto2726 */

		}
	if ((mx >= (_Context->x_Auto2726+333) )  
	&&  (my >= (_Context->y_Auto2726+4) )  
	&&  (mx <= (_Context->x_Auto2726+350) )  
	&&  (my <= (_Context->y_Auto2726+20) )) {
	...
	if ((mx >= (_Context->x_Auto2726+317) )  
	&&  (my >= (_Context->y_Auto2726+4) )  
	&&  (mx <= (_Context->x_Auto2726+334) )  
	&&  (my <= (_Context->y_Auto2726+20) )) {
	...
	case _MIMO_DOWN :
		if ((mx >= (_Context->x_Auto2726+4) )  
		&&  (my >= (_Context->y_Auto2726+4) )  
		&&  (mx <= (_Context->x_Auto2726+302) )  
		&&  (my <= (_Context->y_Auto2726+20) )) {
			return( Auto2726action(_Context) );
			}
		}
	if (( mx >= (_Context->x_Auto2726+48) ) 
	&&  ( my >= (_Context->y_Auto2726+60) ) 
	&&  ( mx <= (_Context->x_Auto2726+48+288) ) 
	&&  ( my <= (_Context->y_Auto2726+60+16))) {
		return(2); /* Target */
		}
	if (( mx >= (_Context->x_Auto2726+10) ) 
	&&  ( my >= (_Context->y_Auto2726+80) ) 
	&&  ( mx <= (_Context->x_Auto2726+10+330) ) 
	&&  ( my <= (_Context->y_Auto2726+80+530))) {
		if ((_Context->value_RefList = ((visual_event(6) - (_Context->y_Auto2726+80)) / 16)) < 1)
		...
		_Context->column_RefList = (visual_event(7) - (_Context->x_Auto2726+10));
	...
	if (( mx >= (_Context->x_Auto2726+340) ) 
	&&  ( my >= (_Context->y_Auto2726+80) ) 
	&&  ( mx <= (_Context->x_Auto2726+340+20) ) 
	&&  ( my <= (_Context->y_Auto2726+80+530))) {
		return(4); /* RefBar */
		}
