...
#include <stdio.h>
#include "visual.h"
#define en_Auto2761   "Sing : Version 1.0a"
#define fr_Auto2761   "Sing : Version 1.0a"
#define it_Auto2761   "Sing : Version 1.0a"
#define es_Auto2761   "Sing : Version 1.0a"
#define de_Auto2761   "Sing : Version 1.0a"
#define nl_Auto2761   "Sing : Version 1.0a"
#define pt_Auto2761   "Sing : Version 1.0a"
#define xx_Auto2761   "Sing : Version 1.0a"
#define en_Variable   ""
#define en_FirstDim   ""
#define en_SecondDim   ""
#define en_Auto2762   "("
...
_Context->indsort_Liste=NULL;	/* pas de pointeur de table intermédiaire */		_Context->max_Slider=0;
	...
	_Context->x_Auto2761=97;
	_Context->y_Auto2761=274;
	_Context->w_Auto2761=830;
	_Context->h_Auto2761=220;
	...
	_Context->pushbuffer=visual_buffer(_Context->x_Auto2761,_Context->y_Auto2761,830+10,220+10);
	return(0);
}
...
public 	int	accept_variable_inspection_hide(struct accept_variable_inspection_context * _Context)
{
	...
	&& (_Context->ctrlbuffer != 0)) {
		(void)visual_buffer_put(_Context->pushbuffer,_Context->x_Auto2761,_Context->y_Auto2761);
		_Context->ctrlbuffer=0;
		}
...
private int on_Liste_show(struct accept_variable_inspection_context * _Context) {
	...
	break;
		...
		visual_table(_Context->x_Auto2761+10,_Context->y_Auto2761+60,780,150,vfh[1],27,28,_Context->msg_Liste[_Context->language],_Context->buffer_Liste,0x4401,"(W8,W24,W8,$)");
;
	return(-1);
}
...
public	int	accept_variable_inspection_show(struct accept_variable_inspection_context * _Context)
{
	...
	if ((_Context->pushbuffer != (void *) 0)
	&& (!(_Context->ctrlbuffer))) {
		(void)visual_buffer_get(_Context->pushbuffer,_Context->x_Auto2761,_Context->y_Auto2761);
		_Context->ctrlbuffer=1;
		}
	visual_freeze();
	visual_window(_Context->x_Auto2761,_Context->y_Auto2761,830,220,vfh[2],_Context->msg_Auto2761[_Context->language],strlen(_Context->msg_Auto2761[_Context->language]),0x407);
	visual_frame(_Context->x_Auto2761+10,_Context->y_Auto2761+30,616+2,16+2,5);
	visual_text(_Context->x_Auto2761+10+1,_Context->y_Auto2761+30+1,616,16,vfh[1],27,28,_Context->buffer_Variable,77,0);
	visual_frame(_Context->x_Auto2761+650,_Context->y_Auto2761+30,40+2,16+2,5);
	visual_text(_Context->x_Auto2761+650+1,_Context->y_Auto2761+30+1,40,16,vfh[1],27,28,_Context->buffer_FirstDim,5,0);
	visual_frame(_Context->x_Auto2761+710,_Context->y_Auto2761+30,40+2,16+2,5);
	visual_text(_Context->x_Auto2761+710+1,_Context->y_Auto2761+30+1,40,16,vfh[1],27,28,_Context->buffer_SecondDim,5,0);
	visual_text(_Context->x_Auto2761+630,_Context->y_Auto2761+30,20,20,vfh[2],16,0,_Context->msg_Auto2762[_Context->language],strlen(_Context->msg_Auto2762[_Context->language]),1283);
	visual_text(_Context->x_Auto2761+750,_Context->y_Auto2761+30,20,20,vfh[2],16,0,_Context->msg_Auto2763[_Context->language],strlen(_Context->msg_Auto2763[_Context->language]),1283);
	visual_text(_Context->x_Auto2761+690,_Context->y_Auto2761+30,20,20,vfh[2],16,0,_Context->msg_Auto2764[_Context->language],strlen(_Context->msg_Auto2764[_Context->language]),1283);
	visual_frame(_Context->x_Auto2761+770,_Context->y_Auto2761+30,48+2,16+2,5);
	visual_text(_Context->x_Auto2761+770+1,_Context->y_Auto2761+30+1,48,16,vfh[1],0,0,_Context->buffer_Counter,6,0);
	(void) on_Liste_show(_Context);
	visual_scrollbar(_Context->x_Auto2761+800,_Context->y_Auto2761+60,20,150,vfh[1],27,28,_Context->value_Slider,_Context->limit_Slider,_Context->max_Slider,259);
	visual_thaw(_Context->x_Auto2761,_Context->y_Auto2761,830,220);

	return(0);
}

private int Auto2761action(struct accept_variable_inspection_context * _Context) {
	...
	do {
		...
		_Context->x_Auto2761 += (visual_event(7)-x);
		x=visual_event(7);
		if (_Context->x_Auto2761 < 0) { _Context->x_Auto2761=0; }
		_Context->y_Auto2761 += (visual_event(6)-y);
		y=visual_event(6);
		if (_Context->y_Auto2761 < 0) { _Context->y_Auto2761=0; }
...
private int on_Slider_action(struct accept_variable_inspection_context * _Context) {
	...
	if ( visual_event(6) < (_Context->y_Auto2761+60+20) ) {
	...
	if ( visual_event(6) > (_Context->y_Auto2761+60+150-20) ) {
		do { while( visual_getch() != 256); } while ( visual_event(1)  != _MIMO_UP );
		_Context->value_Slider += 1;
	...
	do {
		...
		_Context->value_Slider = (((visual_event(6) - (_Context->y_Auto2761+60+20)) * _Context->max_Slider) / (150 - (2 * 20)));
		if (_Context->value_Slider < 0) 
			_Context->value_Slider = 0;
		else if (_Context->value_Slider > (_Context->max_Slider-_Context->limit_Slider)) 
			_Context->value_Slider = (_Context->max_Slider-_Context->limit_Slider);
...
struct accept_variable_inspection_context * _Context){
	...
	switch (mt) {
	case _MIMO_UP :
	if ((mx >= (_Context->x_Auto2761+809) )  
	&&  (my >= (_Context->y_Auto2761+4) )  
	&&  (mx <= (_Context->x_Auto2761+826) )  
	&&  (my <= (_Context->y_Auto2761+20) )) {
		return(1);	/* Auto2761 */

		}
	if ((mx >= (_Context->x_Auto2761+793) )  
	&&  (my >= (_Context->y_Auto2761+4) )  
	&&  (mx <= (_Context->x_Auto2761+810) )  
	&&  (my <= (_Context->y_Auto2761+20) )) {
	...
	if ((mx >= (_Context->x_Auto2761+777) )  
	&&  (my >= (_Context->y_Auto2761+4) )  
	&&  (mx <= (_Context->x_Auto2761+794) )  
	&&  (my <= (_Context->y_Auto2761+20) )) {
	...
	case _MIMO_DOWN :
		if ((mx >= (_Context->x_Auto2761+4) )  
		&&  (my >= (_Context->y_Auto2761+4) )  
		&&  (mx <= (_Context->x_Auto2761+762) )  
		&&  (my <= (_Context->y_Auto2761+20) )) {
			return( Auto2761action(_Context) );
			}
		}
	if (( mx >= (_Context->x_Auto2761+10) ) 
	&&  ( my >= (_Context->y_Auto2761+30) ) 
	&&  ( mx <= (_Context->x_Auto2761+10+616) ) 
	&&  ( my <= (_Context->y_Auto2761+30+16))) {
		return(2); /* Variable */
		}
	if (( mx >= (_Context->x_Auto2761+650) ) 
	&&  ( my >= (_Context->y_Auto2761+30) ) 
	&&  ( mx <= (_Context->x_Auto2761+650+40) ) 
	&&  ( my <= (_Context->y_Auto2761+30+16))) {
		return(3); /* FirstDim */
		}
	if (( mx >= (_Context->x_Auto2761+710) ) 
	&&  ( my >= (_Context->y_Auto2761+30) ) 
	&&  ( mx <= (_Context->x_Auto2761+710+40) ) 
	&&  ( my <= (_Context->y_Auto2761+30+16))) {
		return(4); /* SecondDim */
		}
	if (( mx >= (_Context->x_Auto2761+770) ) 
	&&  ( my >= (_Context->y_Auto2761+30) ) 
	&&  ( mx <= (_Context->x_Auto2761+770+48) ) 
	&&  ( my <= (_Context->y_Auto2761+30+16))) {
		return(5); /* Counter */
		}
	if (( mx >= (_Context->x_Auto2761+10) ) 
	&&  ( my >= (_Context->y_Auto2761+60) ) 
	&&  ( mx <= (_Context->x_Auto2761+10+780) ) 
	&&  ( my <= (_Context->y_Auto2761+60+150))) {
		if ((_Context->value_Liste = ((visual_event(6) - (_Context->y_Auto2761+60)) / 16)) < 1)
		...
		_Context->column_Liste = (visual_event(7) - (_Context->x_Auto2761+10));
	...
	if (( mx >= (_Context->x_Auto2761+800) ) 
	&&  ( my >= (_Context->y_Auto2761+60) ) 
	&&  ( mx <= (_Context->x_Auto2761+800+20) ) 
	&&  ( my <= (_Context->y_Auto2761+60+150))) {
		return(7); /* Slider */
		}
...
public	int	accept_variable_inspection_focus(struct accept_variable_inspection_context * _Context)
{
	...
	while(1) {
		switch (_Context->focus_item) {
			case	0x2 :
				/* Variable */
				_Context->keycode = visual_edit(_Context->x_Auto2761+10+1,_Context->y_Auto2761+30+1,616,16,vfh[1],_Context->buffer_Variable,77);
			break;
			case	0x3 :
				/* FirstDim */
				_Context->keycode = visual_edit(_Context->x_Auto2761+650+1,_Context->y_Auto2761+30+1,40,16,vfh[1],_Context->buffer_FirstDim,5);
				(void) on_FirstDim_losefocus(_Context);
			break;
			case	0x4 :
				/* SecondDim */
				_Context->keycode = visual_edit(_Context->x_Auto2761+710+1,_Context->y_Auto2761+30+1,40,16,vfh[1],_Context->buffer_SecondDim,5);
