static	int	apply_visual_style(struct element_style * sptr, struct style_object * optr )
{
	...
	if ( sptr->margincolour.entry != 0 ) {
		filzone(border.x,border.y,
			border.w,border.h,
			( colourmapped ? sptr->margincolour.entry : rgbtriad(sptr->margincolour ))
			);
		}
	/* ----------------------------- */
	...
	switch ((qstyle = sptr->bordertype.top)) {
		...
		case	_keyword_solid		:
			visual_filzone(	border.x,border.y,
					border.w,border.h,
					( colourmapped ? sptr->bordercolour.entry : newrgb(sptr->bordercolour )),
					1);
			break;
		...
		case	_keyword_double		:
			visual_filzone(	border.x,border.y,
					...
					( colourmapped ? sptr->bordercolour.entry : newrgb(sptr->bordercolour )),
					1);
			visual_filzone(	border.x+2,border.y+2,
	...
	switch (sptr->bordertype.top) {
		...
		case	_keyword_shade    	:
			if (( frame.w ) || ( frame.h )) {
				...
				visual_filzone(	border.x,border.y,
					border.w,border.h,
					( colourmapped ? sptr->backgroundcolour.entry : newrgb(sptr->backgroundcolour )),
					16);
				}
			break;
		case	_keyword_hole	:
