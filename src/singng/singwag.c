...
private	void	simple_image_division(FILE * h, struct form_item * iptr, char * nptr)
{
	char	*	iname;
	fprintf(h,"<div id=%c%s%c>",	__QUOTE,prefixed_widget_name(iptr),__QUOTE);
	server_interface_record(nptr,_WSI_IMAGE);
...
public	int	export_html_image_form(  FILE * h, int mode, int automatic, int restrict, int parametric, int points )
{
	int	i;
	int	autotrad=0;
	struct	form_control 	* fptr;
	...
	if (!( open_server_description(Context.Form)))
		return(0);
