...
public	void	abal_resize_method( FILE * h,struct form_control *fptr,char * prefix,int overlay)
{
			...
			if (is_xy_modif(iptr))
				...
				switch ( (iptr->Contents.style & _FRAME_TYPE ) ) {
					...
					case	_FORM_FRAME	:
						...
						/* éventuellement redéfinir r_liste */
						break;
					}
				}
		}
	...
	if ( Abal_Config.generate_program ) {
		if ( Abal_Config.generate_overlay )
			...
			fprintf(h,"%s=On%s()",__abal_keyword_status,__abal_method_show);
		}
	else  {
...
private	struct	form_item * locate_forms_window( struct form_control * fptr)
{
...
public	void	abal_overlay_trigger( FILE * h, struct form_item * iptr )
{
	...
	instance_widget_overlay(h,iptr,_OVL_METHODS);
	abal_endloc(h,0);
...
public	void	abal_table_widget(FILE * handle, struct form_item * iptr, int margin )
{
	...
	if (!( is_valid_format( iptr->Contents.format ) )) {
		abal_hex_parameter(handle,iptr->Contents.align|(iptr->Contents.special & 1 ? 0x2000:0));
		string_parameter(handle,"()");
		length_parameter(handle,2);
		}
	else	{
		abal_hex_parameter(handle,(iptr->Contents.align | 0x4000|(iptr->Contents.special & 1 ? 0x2000:0)));
...
public	void	widget_mouse_event(FILE * h, struct form_item * iptr, int margin )
{
	...
	for (	f=0,xfi=0;
		...
		indent(h,margin++);
		if (is_w_modif(iptr)) {
			fprintf(h,"x_internal = (%u*_wref)/%u",xfi,w_base_resize(iptr));
			linefeed(h);
			indent(h,margin);
	...
	for ( 	f = 0, xfi=0;
		...
		if ( iptr->Contents.datatype == _WIDGET_KONSTANT ) {
			...
			&&  ( iptr->Contents.align & _A_BOLD )) {
				...
				fprintf(h,",m%u_%s",(f+1),iptr->Contents.name);
				length_parameter(h,fieldwidth[f]-1);
				fprintf(h,",%u)",(iptr->Contents.align & ~ _A_SHADOW));
				linefeed(h);
				}
...
#ifdef	BEFORE05062006
		if ( *tptr == _UNQUOTED_STRING )
			return(0);
		else
#endif
			return(vformat(iptr->Contents.format, fieldwidth, maxfield, linelength ) );
		}
