...
int ctConsoleMain (ctConsolePtr Console)
	...
	if (GlobalShowConsole)
		...
		waDisplayMessage(Console->CommandLine);
...
#ifndef __WALD__
		/* #BG 07/10/03 Affiche la ligne de commande complète ds ctConsoleMain().*/
...
/* #SP: 27/01/00 ALD 1 & 4 (end) */
			if ( init_term ) {
				/* don't do this unless screen has started */
				/* --------------------------------------- */
				TEST_STOP
				}
			pt_node = SlistGetNext( liste_modules );
...
waDisplayMessage("is_module_solve_variable");
	...
	while ( node ) {
		...
		if (pt_ancrage->par_qui != MOINS_UN && match_module == TRUE) {
			ListGetNameFromNum( pt_res->liste_mod_ext, pt_ancrage->par_qui, NomModule2 );
			Xfn_parser( NomModule2, NomModule, FILE_NAME);
			Xfn_parser( pt_module->code.ent->nom, NomModule2, FILE_NAME);
			if (! Xstrcmp( NomModule, NomModule2 ) ) {
			...
			if (pe->par_qui != MOINS_UN && pe->any != MOINS_UN ) {
				if ( pt_res == pt_module ) {
					...
					if ( pp->num_module == NUM_RES ) {
						...
						ListGetNameFromNum( pt_res->liste_mod_ext, pe->par_qui, NomModule2 );			
						Xfn_parser( NomModule2, NomModule, FILE_NAME);
						Xfn_parser( pt_module->code.ent->nom, NomModule2, FILE_NAME);
						if (! Xstrcmp( NomModule, NomModule2 ) ) {
							/* je suis le bon module */
			...
			if (pe->par_qui != MOINS_UN && pe->any != MOINS_UN) {
				if ( pt_resultat == pt_module ) {
					...
					if ( pp->num_module == NUM_RES ) {
						...
						ListGetNameFromNum( pt_resultat->liste_mod_ext, pe->par_qui, NomModule2 );			
						Xfn_parser( NomModule2, NomModule, FILE_NAME);
						Xfn_parser( pt_module->code.ent->nom, NomModule2, FILE_NAME);
						if (! Xstrcmp( NomModule, NomModule2 ) ) {
							/* je suis le bon module */
