...
#include "visual.h"
#define en__Auto3636   ""
#define en_Auto3637   "Style Class Characteristics"
#define fr_Auto3637   "File Model Characteristics"
#define it_Auto3637   "File Model Characteristics"
#define es_Auto3637   "File Model Characteristics"
#define de_Auto3637   "File Model Characteristics"
#define nl_Auto3637   "File Model Characteristics"
#define pt_Auto3637   "File Model Characteristics"
#define xx_Auto3637   "File Model Characteristics"
#define en__Auto3637   ""
#define fr__Auto3637   ""
#define it__Auto3637   ""
#define es__Auto3637   ""
...
#define pt__Auto3637   ""
#define xx__Auto3637   ""
#define en_Auto3638   "Style Class Characteristics"
#define fr_Auto3638   "File Model Characteristics"
#define it_Auto3638   "File Model Characteristics"
#define es_Auto3638   "File Model Characteristics"
#define de_Auto3638   "File Model Characteristics"
#define nl_Auto3638   "File Model Characteristics"
#define pt_Auto3638   "File Model Characteristics"
#define xx_Auto3638   "File Model Characteristics"
#define en__Auto3638   ""
#define fr__Auto3638   ""
#define it__Auto3638   ""
#define es__Auto3638   ""
...
static int	vfh[16];
private int on_Auto3636_show(struct accept_style_inspection_context * _Context);
	...
	char * pExample)
{
	...
	_Context->x_Auto3636=180;
	_Context->y_Auto3636=130;
	_Context->w_Auto3636=440;
	_Context->h_Auto3636=340;
	_Context->window_col=0;
	_Context->window_row=0;
	if(((_Context->y_Auto3636=visual_event(6))+340)>hardrows())
	_Context->y_Auto3636=(hardrows()-340);
	if(((_Context->x_Auto3636=visual_event(7))+440)>hardcolumns())
	_Context->x_Auto3636=(hardcolumns()-440);
	...
	_Context->pushbuffer=visual_buffer(_Context->x_Auto3636,_Context->y_Auto3636,440+10,340+10);
	return(0);
}
...
public 	int	accept_style_inspection_hide(struct accept_style_inspection_context * _Context)
{
	...
	&& (_Context->ctrlbuffer != 0)) {
		(void)visual_buffer_put(_Context->pushbuffer,_Context->x_Auto3636,_Context->y_Auto3636);
		_Context->ctrlbuffer=0;
		}
...
private int on_Auto3636_show(struct accept_style_inspection_context * _Context) {
		if (((_Context->status = visual_styled_element(_Context->x_Auto3636,_Context->y_Auto3636,440,340,GetWidgetStyle(0),GetWindowVersion(),strlen(GetWindowVersion()))) != 0)
) {
		visual_window(_Context->x_Auto3636,_Context->y_Auto3636,440,340,vfh[1],GetWindowVersion(),strlen(GetWindowVersion()),0x405);
	}

	open_window_editor(_Context->x_Auto3636+10,_Context->y_Auto3636+60,
	416,192,_Context->buffer_Editor);
	return(-1);
}
private int on_Editor_show(struct accept_style_inspection_context * _Context) {
	visual_frame(_Context->x_Auto3636+10,_Context->y_Auto3636+60,416+2,192+2,5);
;
	show_window_editor();
	return(-1);
}
private int on_Example_show(struct accept_style_inspection_context * _Context) {
	...
	visual_styled_element(_Context->x_Auto3636+90,_Context->y_Auto3636+280,256,48,_Context->buffer_Example,_Context->buffer_Example,l);
...
public	int	accept_style_inspection_show(struct accept_style_inspection_context * _Context)
{
	...
	if ((_Context->pushbuffer != (void *) 0)
	&& (!(_Context->ctrlbuffer))) {
		(void)visual_buffer_get(_Context->pushbuffer,_Context->x_Auto3636,_Context->y_Auto3636);
		_Context->ctrlbuffer=1;
		}
	visual_freeze();
	(void) on_Auto3636_show(_Context);
	visual_text(_Context->x_Auto3636+10,_Context->y_Auto3636+30,414,20,vfh[3],16,0,_Context->msg_Auto3637[_Context->language],strlen(_Context->msg_Auto3637[_Context->language]),259);
	visual_text(_Context->x_Auto3636+10,_Context->y_Auto3636+260,414,20,vfh[3],16,0,_Context->msg_Auto3638[_Context->language],strlen(_Context->msg_Auto3638[_Context->language]),259);
	(void) on_Editor_show(_Context);
	_Context->trigger_Accept=visual_trigger_code(_Context->msg_Accept[_Context->language],strlen(_Context->msg_Accept[_Context->language]));
	visual_button(_Context->x_Auto3636+10,_Context->y_Auto3636+280,56,48,vfh[1],27,28,_Context->msg_Accept[_Context->language],strlen(_Context->msg_Accept[_Context->language]),1040);
	(void) on_Example_show(_Context);
	_Context->trigger_Cancel=visual_trigger_code(_Context->msg_Cancel[_Context->language],strlen(_Context->msg_Cancel[_Context->language]));
	visual_button(_Context->x_Auto3636+370,_Context->y_Auto3636+280,56,48,vfh[1],27,28,_Context->msg_Cancel[_Context->language],strlen(_Context->msg_Cancel[_Context->language]),1040);
	visual_thaw(_Context->x_Auto3636,_Context->y_Auto3636,440,340);

	return(0);
}

private int Auto3636action(struct accept_style_inspection_context * _Context) {
	...
	do {
		...
		_Context->x_Auto3636 += (visual_event(7)-x);
		x=visual_event(7);
		if (_Context->x_Auto3636 < 0) { _Context->x_Auto3636=0; }
		_Context->y_Auto3636 += (visual_event(6)-y);
		y=visual_event(6);
		if (_Context->y_Auto3636 < 0) { _Context->y_Auto3636=0; }
...
struct accept_style_inspection_context * _Context){
	...
	switch (mt) {
	case _MIMO_UP :
	if ((mx >= (_Context->x_Auto3636+419) )  
	&&  (my >= (_Context->y_Auto3636+4) )  
	&&  (mx <= (_Context->x_Auto3636+436) )  
	&&  (my <= (_Context->y_Auto3636+20) )) {
		return(1);	/* Auto3636 */

		}
	if ((mx >= (_Context->x_Auto3636+387) )  
	&&  (my >= (_Context->y_Auto3636+4) )  
	&&  (mx <= (_Context->x_Auto3636+404) )  
	&&  (my <= (_Context->y_Auto3636+20) )) {
	...
	case _MIMO_DOWN :
		if ((mx >= (_Context->x_Auto3636+4) )  
		&&  (my >= (_Context->y_Auto3636+4) )  
		&&  (mx <= (_Context->x_Auto3636+372) )  
		&&  (my <= (_Context->y_Auto3636+20) )) {
			return( Auto3636action(_Context) );
			}
		}
	if (( mx >= (_Context->x_Auto3636+10) ) 
	&&  ( my >= (_Context->y_Auto3636+60) ) 
	&&  ( mx <= (_Context->x_Auto3636+10+416) ) 
	&&  ( my <= (_Context->y_Auto3636+60+192))) {
		return(2); /* Editor */
		}
	if (( mx >= (_Context->x_Auto3636+10) ) 
	&&  ( my >= (_Context->y_Auto3636+280) ) 
	&&  ( mx <= (_Context->x_Auto3636+10+56) ) 
	&&  ( my <= (_Context->y_Auto3636+280+48))) {
		return(3); /* Accept */
		}
	if (( mx >= (_Context->x_Auto3636+370) ) 
	&&  ( my >= (_Context->y_Auto3636+280) ) 
	&&  ( mx <= (_Context->x_Auto3636+370+56) ) 
	&&  ( my <= (_Context->y_Auto3636+280+48))) {
		return(4); /* Cancel */
		}
...
public	int	accept_style_inspection_focus(struct accept_style_inspection_context * _Context)
{
	...
	while(1) {
		switch (_Context->focus_item) {
			...
			case	0x3 :
				/* Accept */
				_Context->trigger_Accept=visual_trigger_code(_Context->msg_Accept[_Context->language],strlen(_Context->msg_Accept[_Context->language]));
				visual_button(_Context->x_Auto3636+10,_Context->y_Auto3636+280,56,48,vfh[1],27,28,_Context->msg_Accept[_Context->language],strlen(_Context->msg_Accept[_Context->language]),1042);
	...
	visual_button(_Context->x_Auto3636+10,_Context->y_Auto3636+280,56,48,vfh[1],27,28,_Context->msg_Accept[_Context->language],strlen(_Context->msg_Accept[_Context->language]),1040);
			...
			case	0x4 :
				/* Cancel */
				_Context->trigger_Cancel=visual_trigger_code(_Context->msg_Cancel[_Context->language],strlen(_Context->msg_Cancel[_Context->language]));
				visual_button(_Context->x_Auto3636+370,_Context->y_Auto3636+280,56,48,vfh[1],27,28,_Context->msg_Cancel[_Context->language],strlen(_Context->msg_Cancel[_Context->language]),1042);
				_Context->keycode = visual_getch();
				_Context->trigger_Cancel=visual_trigger_code(_Context->msg_Cancel[_Context->language],strlen(_Context->msg_Cancel[_Context->language]));
	visual_button(_Context->x_Auto3636+370,_Context->y_Auto3636+280,56,48,vfh[1],27,28,_Context->msg_Cancel[_Context->language],strlen(_Context->msg_Cancel[_Context->language]),1040);
				break;
...
retry:
		...
		case	0x100 :
			if (!( visual_event(1)  & _MIMO_ALT )) { 
				if (( visual_event(7) <  _Context->x_Auto3636 )
				|| ( visual_event(6) < _Context->y_Auto3636  )
				|| ( visual_event(7) > (_Context->x_Auto3636+_Context->w_Auto3636))
				|| ( visual_event(6) >  (_Context->y_Auto3636+_Context->h_Auto3636))) {
