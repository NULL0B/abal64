...
/* ------------------------------------------------------------------------- */
/* Sing Version   : 1.1b.0.16  */
/* Production     : C  */
/* Project        : test.xml  */
/* Target         : vtest.c  */
/* Identification : 0.0-1114596565-2712.2711 */
/* ------------------------------------------------------------------------- */
...
public	int	sing_c_production_tester_create(struct sing_c_production_tester_context **cptr)
{
	...
	_Context->x_Auto1912=135;
	_Context->y_Auto1912=70;
	_Context->w_Auto1912=530;
	_Context->h_Auto1912=460;
...
public	int	sing_c_production_tester_show(struct sing_c_production_tester_context * _Context)
{
	...
	if (_Context->page_number == 1 ) {
	visual_switch(365,270,168,32,vfh[2],27,28,"Test Switch",strlen("Test Switch"),_Context->value_TestSwitch| 0x0000);
		}
	if (_Context->page_number == 1 ) {
...
private int on_TestBar_action(struct sing_c_production_tester_context * _Context) {
	...
	switch( visual_event(2) ) {
		case _MIMO_WHEELUP   :
			_Context->value_TestBar -= (_Context->limit_TestBar/2);
			if (_Context->value_TestBar < 0) 
				_Context->value_TestBar = 0;
	status = sing_c_production_tester_show(_Context);
	visual_scrollbar(395,140,20,310,vfh[1],27,28,_Context->value_TestBar,_Context->limit_TestBar,_Context->max_TestBar,259);
	...
	while(1) {
		switch (_Context->focus_item) {
			...
			case	0x6 :
				/* TestSwitch */
				if (_Context->page_number == 1 ) {
				visual_switch(365,270,168,32,vfh[2],27,28,"Test Switch",strlen("Test Switch"),(_Context->value_TestSwitch | 2 | 0x0000 ));
					_Context->keycode = visual_getch();
				visual_switch(365,270,168,32,vfh[2],27,28,"Test Switch",strlen("Test Switch"),(_Context->value_TestSwitch | 0 | 0x0000 ));
				} else {
				_Context->keycode = 5;
					}
				break;
...
public	int	sing_c_production_tester()
{
	int	status=0;
	struct sing_c_production_tester_context * _Context=(struct sing_c_production_tester_context*) 0;
