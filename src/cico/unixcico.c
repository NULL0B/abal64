 ...
 *
 *	$Header: /home/Produits/Abal3/cico/unixcico.c,v 1.3 2007/01/11 09:39:55 jamie Exp $
 *
	...
	Modif 31/08/94 : probleme de desynchro jeu primaire - jeu second
			 sur load.go : finterm()
	Modif 09/05/95 : bi-mode de gestion de la tempo svr4 ou non
	Modif 14/02/97 : purge des ‚mission de SIGALARM aprŠs lecture buffer
			 dans la fonction _read_buf
        Modif 02/09/97 :(S Barthe) ajout ifdef SHAREDLO pour partageabilit‚ imprimante
	Modif 07/12/00 :(S Barthe) avec ifdef SHAREDLO possibilite de mettre un parametre ABALNORESET pour eviter ioctl (rsh)
 */
 ...
 * (bg+ijm 04/10/2001)
 * -------------------
 * Ces pointeurs de fonctions et les declarations "forwards" sont necessaires pour le version LINUX pour la mise en
 * oeuvre des BDA par les moyens nouveaux, c'est a dire les "shared libraries"
 */
  
static WORD	local_x_mi();
static WORD	local_x_ci();
static VOID	local_x_co(int);
static VOID	local_x_lo(int);
static VOID	local_x_mes(BYTE *);
static WORD	local_x_info(int);

static WORD (*relay_x_mi  )() 		=local_x_mi;
static WORD (*relay_x_ci  )()		=local_x_ci;
static VOID (*relay_x_co  )(int)	=local_x_co;
static VOID (*relay_x_lo  )(int)	=local_x_lo;
static VOID (*relay_x_mes )(BYTE *)	=local_x_mes;
static WORD (*relay_x_info)(int)	=local_x_info;
...
/* gestion de la temporisation SCO SVR4 ou 3.2 */
static BOOL svr4_flag = TRUE;
...
static VOID local_x_mes(BYTE * mptr) 
{ 
	while ( *mptr )
		x_putch( *(mptr++) );
	return; 
...
static WORD local_x_info( int item) 
{ 
	int	holder=prndest;
	int	result=0;
	...
	if (proterm.c_cc[VTIME]  == 0)
	 {
		...
		proterm.c_cc[VMIN] = 0;
		ioctl(STDIN, TCSETA, &proterm); 	/* pour programmer */
	}
#endif
		...
		if ( ! svr4_flag ) {
			(VOID) alarm(sv_alarm32);
#ifdef SYS_BSD
			sigaction(SIGALRM,&oact,NULL);
#endif /* SYS_BSD */

		} else {
			sv_alarm.it_interval.tv_sec=0L;		/* purger les r‚‚missions */
			sv_alarm.it_interval.tv_usec=0L;
			sv_alarm.it_value.tv_sec=0L;		/* purger les r‚‚missions */
			sv_alarm.it_value.tv_usec=0L;
			ret_alarm=setitimer(ITIMER_REAL,&sv_alarm,&value);
		}
...
{
	void * vptr;	
/* petit connard ca n'est pas a toi de le faire tu n'est qu'une pauvre
 * fonction qui se doit s'appliquer ! merde!
 */

#ifdef	MERDE_04102001
	...
	if ((indicateur & CO) == CO) {
		vptr = relay_x_co;
		relay_x_co = descripteur->cico.Relay_Co;
		descripteur->cico.function_co = vptr;
		}

	if ((indicateur & CI) == CI) {
		vptr = relay_x_ci;
		relay_x_ci = descripteur->cico.Relay_Ci;
		descripteur->cico.function_ci = vptr;
		}
	if ((indicateur & MES) == MES) {
		vptr = relay_x_mes;
		relay_x_mes = descripteur->cico.Relay_Mes;
		descripteur->cico.function_mes = vptr;
		}
	if ((indicateur & INFO) == INFO) {
		vptr = relay_x_info;
		relay_x_info = descripteur->cico.Relay_Infos;
		descripteur->cico.function_info = vptr;
		}

	if ((indicateur & LO) == LO) {
		vptr = relay_x_lo;
		relay_x_lo = descripteur->cico.Relay_Lo;
		descripteur->cico.function_lo = vptr;
		}
...
*/
		...
		/* Test de la version d'unix SCO */
		if ((cp = getenv("CICOSVR4")) != NULL) {
			if ( strcmp(cp, "NO") == 0)
				svr4_flag = FALSE;
		}
#ifdef ABALCONF
...
WORD type;
{
	...
	/* 31/08/94 */
	/* Toujour retour au jeu primaire avant finterm */
