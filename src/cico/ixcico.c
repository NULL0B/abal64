 ...
 *	redirected by_ dynamic libraries but here we do not care !
 *
 */
#ifndef	_ixcico_c
#define	_ixcico_c

#define	_LTS_ESC_UPLOAD
#define	_LTS_ESC_H
#define	_LTS_ESC_I
#define	_LTS_ESC_N
#define	_LTS_ESC_R
...
#ifdef _MOUSE_TRACE
#define MOUSE_TRACE_FILE	"/tmp/MouseTrace.txt"
#endif
#ifdef _KEYB_TRACE
#define KEYB_TRACE_FILE		"/tmp/KeybTrace.txt"
#endif
...
#define MAX_LOG_COSYSWRITE	10		/* #BG 16/06/20 Eviter de saturer le /var par un exces de logs */

static int	GetchEscQ=-2;
static int	C_FORE=C_GREEN;
static int	C_BACK=C_BLACK;
static int	CoSysWriteLogCount=0;		/* #BG 16/06/20 Eviter de saturer le /var par un exces de logs */

int	utf8_flag = 0;				/* #BG 24/03/11 Gestion UTF-8, non static!*/

//	enhanced colour management 
static 	int	extra=0;
static	char	collector[256];
static	int	rgbetd=-1;
static	char	paraname[256];
static	int	paraxcico=0;
#ifdef	_LTS_ESC_N
static	int	default_ligne=24;
static	int	default_colonne=80;
static	int	dyna_ligne=0;
static	int	dyna_colonne=0;
#else
#define default_ligne 24
#define	default_colonne 80
#endif

static	void	cico_home();
static	void	cico_clear();
static	int	cico_allocate_screen();
static	void	cico_liberate_screen();
...
#ifdef EXABAL
			if ((IsExaSysLog() & SYSLOG_CICO) && (CoSysWriteLogCount < MAX_LOG_COSYSWRITE)) {
				CoSysWriteLogCount++;
				sprintf(SysLogBuffer, "Error: cosyswrite, errno=%d", errno);
...
static	int 	_mimo_detect()
{
	...
	if ( TraceMouse )
	{
		if (MouseTrace == (FILE*)0)
			MouseTrace = fopen(MOUSE_TRACE_FILE,"a");
		fprintf(MouseTrace,"ixcico.c 2: mimo_detect(), CicoMimo.limit=%d\r\n", CicoMimo.limit);	
		fclose(MouseTrace);
		MouseTrace = (FILE*) 0;
	}
#endif
	...
	if ( i > 0 ) 
	{
		...
		//		values[0] &= CicoMimo.limit; /* Limit to useful button information (3 or 7 if moving), cf cico file */
		//		values[0]++;
		if (values[0] == 0x40)			// Evt 0x60 - 0x20
			values[0] = _X_WHEEL_UP;
		else if (values[0] == 0x41)		// Evt 0x61 - 0x20
			values[0] = _X_WHEEL_DOWN;
		else 
		{

			values[0] &= CicoMimo.limit;	// Limit to useful button information (mask 11 if 3 or 111 if 7), cf CICO file and set_mimo_limit()
			values[0]++;			// _X_BUTTON_1/2/3, _X_BUTTON_OFF, _X_MOVING
		}
		...
		if ( TraceMouse )
		{
			...
			fprintf(MouseTrace,"ixcico.c 3b: => Values: %x %x %x\r\n", values[0], values[1] ,values[2]);	
			fclose(MouseTrace);
			MouseTrace = (FILE*) 0;
		}
#endif
		 ...
		 * values[0] = 0x23 - 0x20 + 1 = 4 = _X_BUTTON_OFF
		 */ 
		...
		case	_X_MOVING	:
			...
			if (!( CicoMimo.param = CicoMimo.buttons )) 
			{
				...
				return(0);
			else	CicoMimo.type = _MIMO_WHILE_DOWN;
			break;

		case	_X_WHEEL_DOWN	:	//#BG 15/12/20 Mouse wheel
#ifdef	_MOUSE_TRACE
			if ( TraceMouse )
			{
				if (MouseTrace == (FILE*)0)
					MouseTrace = fopen(MOUSE_TRACE_FILE,"a");
				fprintf(MouseTrace,"Wheel Down\r\n");	
				fclose(MouseTrace);
				MouseTrace = (FILE*) 0;
			}
#endif
			if (CicoMimo.mask & _MIMO_SEND_WHEEL) 
			{
				CicoMimo.param = CicoMimo.buttons = _MIMO_WHEELDOWN;
				if ( CicoMimo.mask & _MIMO_DOWN )
					CicoMimo.type = _MIMO_DOWN;
				else	return(0);
			}
		...
		case	_X_WHEEL_UP	:		//#BG 15/12/20 Mouse wheel
#ifdef	_MOUSE_TRACE
			if ( TraceMouse )
			{
				if (MouseTrace == (FILE*)0)
					MouseTrace = fopen(MOUSE_TRACE_FILE,"a");
				fprintf(MouseTrace,"Wheel Up\r\n");	
				fclose(MouseTrace);
				MouseTrace = (FILE*) 0;
			}
#endif
			if (CicoMimo.mask & _MIMO_SEND_WHEEL) 
			{
				CicoMimo.param = CicoMimo.buttons = _MIMO_WHEELUP;
				if ( CicoMimo.mask & _MIMO_UP )
					CicoMimo.type = _MIMO_UP;
				else	return(0);
			}
	...
	BOOL 	delay;
{
	...
	{
#else
	while ((rbr = _read_buf(in_buf + in_nb, in_nb, utf8_flag )) > 0) 
	{ 	/*(NEWTEMPO)*/
#endif /* vms */
test:
		in_nb++;
...
//	------------------------------
//	   LTS EXTENDED ATTRIBUTES
//	------------------------------

#ifdef	_LTS_ESC_S
//	------------------------------
//	Extended RGB foreground colour
//	------------------------------
//	ESC S {colour};
//	------------------------------
//	colour may be 
...
static	void	cico_esc_S(char * sptr)
{
	int	c;
	if ( rgbetd == -1 )
	{
...
//	ESC H  [ 0 | 1 ]
//	---------------------------------
static	void	cico_esc_upload(int v)
{
	int	c;
	// transmettre to subsequent XCICO screen managers
	if ( paraxcico )
	{
		_co_out(0x001B);
		_co_out('!');
...
//	Display Colour
//	---------------------------------
//	ESC H number red bleu green alpha
//	---------------------------------
static	void	cico_esc_H(char * sptr)
{
	int	c;
	// transmettre to subsequent XCICO screen managers
	if ( paraxcico )
...
//	ESC Y fontname;
//	--------------------------------
static	void	cico_esc_Y(char * sptr)
{
	int	c;
	// transmettre to subsequent XCICO screen managers
	if ( paraxcico )
...
//	Display Video
//	--------------------------------
//	ESC V {NBCOL} , {NBLIN} , {NAME}
//	--------------------------------
static	void	cico_esc_V(char * sptr)
{
	int	c;
	// transmettre to subsequent XCICO screen managers
	if ( paraxcico )
...
//	Display Label
//	---------------------------------------
//	ESC X {NBCOL} , {NBLIN} , {FONT} {NAME}
//	---------------------------------------
static	void	cico_esc_X(char * sptr)
{
	int	c;
	// transmettre to subsequent XCICO screen managers
	if ( paraxcico )
...
//	Display Image
//	--------------------------------
//	ESC I {NBCOL} , {NBLIN} , {NAME}
//	--------------------------------
static	void	cico_esc_I(char * sptr)
{
	int	c;
	// transmettre to subsequent XCICO screen managers
	if ( paraxcico )
...
#ifdef	_LTS_ESC_N
//	------------------------
//	Redimension the Screen
//	------------------------
//	ESC N {NBLIN} , {NBCOL}; 
//	------------------------
static	void	cico_esc_N(char * sptr)
{
	int	newcol=0;
	int	newlin=0;
	int	c;
	...
	// ignore incorrect sequence
	if (!( c )) return;

	// ignore sequence if zero columns
	if (!( newcol = atoi(wptr) ))
		return;

	// ok legal values to do it
	dyna_colonne = newcol;
	dyna_ligne = newlin;

	// allow a sleep to allow debug
	if ((eptr = getenv("ESCNSLEEP")) != (char *) 0)
		if ((c = atoi(eptr)) != 0)
			sleep(c);
	...
	// transform the local screen now
	cico_liberate_screen();
	cico_allocate_screen();
	cico_clear();
	cico_home();	
...
//		colour name
//	------------------------------
static	void	cico_esc_R(char * sptr)
{
	int	c;
	if ( rgbetd == -1 )
	{
...
//	------------------------------
//	END OF LTS EXTENDED ATTRIBUTES
//	------------------------------
...
/* 29/10/92:DLR */
...
#ifdef	_LTS_ESC_UPLOAD
		case '!' :
			courant->etat = ETAT_UPLOAD;
			break;
#endif
#ifdef	_LTS_ESC_H
		case 'H' :
			courant->etat = ETAT_HUE;
			break;
#endif
#ifdef	_LTS_ESC_Y
		case 'Y' :
			courant->etat = ETAT_FONT;
			break;
#endif
#ifdef	_LTS_ESC_X
		case 'X' :
			courant->etat = ETAT_LABEL;
			break;
#endif
#ifdef	_LTS_ESC_V
		case 'V' :
			courant->etat = ETAT_VIDEO;
			break;
#endif
#ifdef	_LTS_ESC_I
		case 'I' :
			courant->etat = ETAT_IMAGE;
			break;
#endif
#ifdef	_LTS_ESC_N
		case 'N' :
			courant->etat = ETAT_DEFINE;
			break;
#endif
#ifdef	_LTS_ESC_R
		case 'R':               /* couleur caractere */
			extra = 0;
			courant->etat = ETAT_TEXTE_ETD;
			break;
#endif
		case 'r':               /* couleur caractere */
	...
	case ETAT_TEXTE_ETD:
		if (flag_couleur == TRUE) 
		{
			if (( collector[extra++] = c ) != ';' )
				break;
			else
			{
				collector[extra] = 0;
				extra = 0;
				cico_esc_R(collector);
			}
		}
		courant->etat = ETAT_STANDARD;
		break;
...
WORD attribut;
{
	...
	case A_NORMAL:
		_co_capacite(EXIT_ATTRIBUTE);
		if ((flag_couleur == TRUE) && (!( paraxcico ))) 
		{
#ifdef ABALCONF
...
WORD couleur;
{
	WORD couleurmask = couleur & 0x000F;
...
//#BG 24/05/19 16 couleurs de fond
	...
	switch ((couleurmask)) {
		...
		case C_WHITE:
			...
			break;
//#BG 24/05/19 16 couleurs de fond - debut
		case C_GREY:
			_co_capacite(BACKGR_8);
			break;
		case C_LIGHT_RED:
			_co_capacite(BACKGR_9);
			break;
		case C_LIGHT_GREEN:
			_co_capacite(BACKGR_A);
			break;
		case C_YELLOW:
			_co_capacite(BACKGR_B);
			break;
		case C_LIGHT_BLUE:
			_co_capacite(BACKGR_C);
			break;
		case C_LIGHT_MAGENTA:
			_co_capacite(BACKGR_D);
			break;
		case C_LIGHT_CYAN:
			_co_capacite(BACKGR_E);
			break;
		case C_INTENSE_WHITE:
			_co_capacite(BACKGR_F);
//#BG 24/05/19 16 couleurs de fond - fin
		}
...
#ifdef EXABAL
			if ((IsExaSysLog() & SYSLOG_CICO) && (CoSysWriteLogCount < MAX_LOG_COSYSWRITE)) {
				sprintf(SysLogBuffer, "Error: _co_out failed: Step 2, c=%X, err_val=%d", c, err_val);
				ExaSysLog(2 /*LOG_ERROR*/, SysLogBuffer);
				}
#endif
		}
...
char * sptr;
{
	...
	if (!( sptr ))
		...
		&&  ( *(sptr+4) == 'h' ))
			CicoMimo.limit = 7;
//#BG 15/12/20 For _MIMO_MOVE		else	CicoMimo.limit = 3;
		else	CicoMimo.limit = 7;
		}
	else	CicoMimo.limit = 3;
#endif
	return;
...
static	VOID	check_lines_and_columns()
{
	...
	{
#ifdef	_LTS_ESC_NN
		if (!( nb_ligne = dyna_ligne ))
		{
#endif
			if (!( eptr = getenv("LINES") ))
				nb_ligne = default_ligne;
	...
	{
#ifdef	_LTS_ESC_N
		if (!( nb_colonne = dyna_colonne ))
		{
#endif
			if (!( eptr = getenv("COLUMNS") ))
				nb_colonne = default_colonne;
...
static	void	cico_liberate_screen()
{
	// check for colour management
	if (flag_couleur == TRUE) 
	{
		// release the background colour map
		if ( courant->image.fond )
			courant->image.fond = liberate( courant->image.fond );

		// release the foreground colour map
		if (courant->image.texte)
			courant->image.texte = liberate( courant->image.texte );
	}

	// release atribute storage buffer
	if ( courant->image.attribut )
		courant->image.attribut = liberate( courant->image.attribut );

	// release character storage buffer
	if ( courant->image.caractere )
		courant->image.caractere = liberate( courant->image.caractere );
}
...
static	int cico_allocate_screen()
{
	...
	// allocate the character storage buffer
	if ((courant->image.caractere = (BYTE *)allocate(longueur+1)) == NULL)
		return(E_ALLOC);

	// allocate the attribute storage buffer
	if ((courant->image.attribut = (BYTE *)allocate(longueur+1)) == NULL)
		return(E_ALLOC);

	// if colour is possible
	if (flag_couleur == TRUE) 
	{
		// allocate the foreground colour map
		if ((courant->image.texte = (BYTE *)allocate(longueur+1)) == NULL)
			return(E_ALLOC);

		// allocate the background colour map
...
#ifdef	_MOUSE_TRACE
		/* ------------------------------- */
		/* collect the mouse trace control */
		/* ------------------------------- */
		if ((cp = getenv("TRACEMOUSE")) != NULL) 
			TraceMouse = atoi(cp);
...
#endif /* vms */
		strcat(name, term);
		strcpy(paraname,term);
		if (!( strncmp( paraname, "xcico", strlen("xcico") ) ))
			paraxcico = 1;
		else	paraxcico = 0;
#ifdef vms
...
#ifdef	_USEIT_FOR_WHATEVER
//#BG 24/05/19 Ne gere pluis les signatures 12 et 14 !
		if (memcmp(entete, ECR_SIGNATURE41, ECR_LONGUEUR_SIGNATURE) != 0) 
		{
			if (memcmp(entete, ECR_SIGNATURE22, ECR_LONGUEUR_SIGNATURE) != 0)
				erreur(E_BADF, name, fh);
			else
				signature_version = 22;
		}
#else
		if (memcmp(entete, ECR_SIGNATURE14, ECR_LONGUEUR_SIGNATURE) > 0)
			erreur(E_BADF, name, fh);
#endif
...
/***************/
		...
		//#BG 24/05/19 16 couleurs de fond
		if (signature_version < 41)
			if (i == SET_NB + ATB_NB + FORE_NB + BACK_NB22 -1)	//32
				for (j = i + 1; j < SET_NB + ATB_NB + FORE_NB + BACK_NB - 1; j++)
					term_str[j] = (BYTE *) NULL;
		...
		{
			for (j = 0; j < BACK_NB - BACK_NB22; j++)
				term_str[SET_NB + ATB_NB + FORE_NB + BACK_NB22 + j] = (BYTE *)NULL;

			for (i = 0; i < lg_off / 2; i++) 
			{
				if (i <= SET_NB + ATB_NB + FORE_NB + BACK_NB22 -1) 
				{
					bp = (BYTE *)(off + i * 2);
					term_str[i] = (BYTE *)(tab_str + *bp + *(bp + 1) * 256);
				}
				...
				{
					bp = (BYTE *)(off + i * 2);
					term_str[i + j] = (BYTE *)(tab_str + *bp + *(bp + 1) * 256);
				}
			}
		}
