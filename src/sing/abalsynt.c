...
/*	------------------------------------------------------		*/
public	void	abal_define( FILE * h, char * nptr, char * vptr )
{
	if (!( h ))
		return;
	else if (!( nptr ))
		return;
	else 	fprintf(h,"#define %s",nptr);

	if ( vptr ) { fprintf(h," %s",vptr); }
	linefeed(h);
	return;
}
...
public	void	abal_use( FILE * handle, char * sptr )
{
	fprintf(handle,"%s %s",keyword_use,sptr);
	linefeed(handle);
	return;
}
...
public	void	abal_variable( FILE * h,
	int	datatype,
	int	length,
	int	first, int second, char * comment )
{
	switch ( datatype ) {
		case	_ABAL_BYTE 	:
			fprintf(h,"#"); break;
	...
	if (method_is_valid(comment))
		abal_comment(h,comment,2); 
	else	linefeed(h);
	return;
}
	...
	int	first, int second )
{
	...
	abal_variable( h, datatype, length, first, second, (char *) 0 );
	return;
}
	...
	int	first, int second, char * comment )
{
	...
	abal_variable( h, datatype, length, first, second, comment );
	return;
}

public	void	abal_const( FILE * h, char *  prefix, char* postfix, int datatype,char * vptr )
{
	fprintf(h,"%s\t%s%s",__abal_keyword_const,prefix,postfix);
	switch ( datatype ) {
		case	_ABAL_BYTE 	:
			if (!(vptr)) vptr = "0";
			fprintf(h,"#=%s",vptr); break;
		case	_ABAL_WORD 	:
			if (!(vptr)) vptr = "0";
			fprintf(h,"%c=%s",0x0025,vptr); break;
		case	_ABAL_LONG 	:
			if (!(vptr)) vptr = "0";
			fprintf(h,":=%s",vptr); break;
		case	_ABAL_BCD  	:
		...
		case	_ABAL_STRING 	:
			if (!(vptr)) 
				...
				vptr = " ";
			fprintf(h,"$=%c%s%c",__QUOTE,vptr,__QUOTE); break;
		case	_ABAL_PROC	:
			break;
		}
	linefeed(h);
...
public	void	abal_arg( FILE * h,
	...
	int	first, int second )
{
	abal_dcl(h,prefix,postfix,datatype,length,first,second,NULL);
...
public	void	abal_dimensions(FILE * h, int first, int second )
{
	if ( first ) {
		fprintf(h,"(%u",first);
		if ( second )
...
public	void	dimensioned_abal_field(FILE * h, char * prefix, char * vname, int first, int second )
{
	...
	if ( prefix ) {
		...
		if ( vname )
			fprintf(h,"%s",vname);
		abal_dimensions( h, first, second );
		}
	linefeed(h);
	return;
}
...
public	void	abal_vfield_member(FILE * h, int id,char * mptr)
{
	if (!( mptr ))
		abal_vfield(h,id);
	else	{
		fprintf(h,"Field=%u,%s",id,mptr);
		linefeed(h);
		}
	return;
}

public	void	abal_vfield_dimensioned_member(FILE * h, int id,char * mptr,int first,int second)
{
	if (!( mptr ))
		abal_vfield(h,id);
	else	{
...
public	void	abal_forward_proc( FILE * h, char * nptr, char * sptr, char * rptr )
{
	fprintf(h,"%s %s %s(%s) %s",__abal_keyword_forward,__abal_keyword_proc,nptr,sptr, rptr);
	linefeed(h);
	return;
}

public	void	abal_standard_proc( FILE * h, char * nptr, char * sptr, char * rptr )
{
	fprintf(h,"%s %s(%s) %s",__abal_keyword_proc,nptr,sptr, rptr);
	linefeed(h);
	return;
}

public	void	abal_function_type( FILE * h, int type )
{
	switch ( type ) {
		case	_ABAL_BYTE	:
			fprintf(h," : # ");
...
public	void	abal_forward_func( FILE * h, char * nptr, char * sptr, int type )
{
	fprintf(h,"%s %s %s(%s) ",__abal_keyword_forward,__abal_keyword_proc,nptr,sptr);
	abal_function_type(h,type);
	linefeed(h);
	return;
}

public	void	abal_standard_function( FILE * h, char * nptr, char * sptr, int type )
{
	fprintf(h,"%s %s(%s) ",__abal_keyword_proc,nptr,sptr);
	abal_function_type(h,type);
	linefeed(h);
	return;
}
...
public  void	abal_pause( FILE * h, char * vptr,int i)
{
	indent(h,i);
	fprintf(h,"%s %c%s%c","Pause",0x0022,vptr,0x0022);
	linefeed(h);
	return;
}
...
public	void	abal_throw( FILE * handle,char * vptr, int indentation )
{
	indent(handle,indentation);
	fprintf(handle,"%s %s",__abal_keyword_error,vptr);
	linefeed(handle);
