...
int	bifs_tab_frame( FILE * h, struct form_item * iptr, int started )
{
	char	work[1024];
	int	fh = guifontsize(iptr->Contents.font);
	int	fw = (fh & 0x00FF);
	...
	{
		bifs_tag(h,"Switch");
		bifs_integer_field(h,"whichChoice",0);
			...
			for ( i=0; i <= fptr->pages; i++ )
			{
					...
					iptr = iptr->next )
				{
					...
					else if ( iptr->Contents.style == _TAB_FRAME )
					{
						...
						started = bifs_tab_frame ( h, iptr,started );
					}
