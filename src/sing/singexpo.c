...
public	void	calculate_edit_lines( struct form_item * iptr, int * nbc, int * nbl );
public	void	calculate_form_lines( struct form_item * iptr, int * nbc, int * nbl );
public	char *	forms_modules(struct form_control * fptr,char * sptr);
public	char *	join_strings( char * sptr, char * tptr);
public	char *	headername( char * sptr, char * xptr );

public	char * set_abal_empty_string();
...
public	int	widget_foreground_value( struct form_item * iptr )
{
	return( iptr->Contents.colour & 0x00FF );
}

private	char	wfgbuffer[256];
public	char * widget_foreground( struct form_item * iptr )
{
	if (!( iptr->Contents.events->xo_create & _EVENT_IS_FOCUS ))
	{
		sprintf(wfgbuffer,"%u",widget_foreground_value(iptr));
	}
	else
	{
		if (( iptr->Contents.style & _FRAME_TYPE) == _RADIO_FRAME)
			sprintf(wfgbuffer,"fg%u_%s",iptr->Contents.radio,iptr->Contents.name);
		else	sprintf(wfgbuffer,"fg_%s",iptr->Contents.name);
	}
	return( wfgbuffer );

}

public	int	widget_background_value( struct form_item * iptr )
{
	sprintf(wfgbuffer,"%u",widget_foreground_value(iptr));
	return( (iptr->Contents.colour & 0xFF00) >> 8) ;
}

private	char	wbgbuffer[256];
public	int	widget_background( struct form_item * iptr )
{
	if (!( iptr->Contents.events->xo_create & _EVENT_IS_FOCUS ))
	{
		sprintf(wbgbuffer,"%u",widget_background_value(iptr));
	}
	else
	{
		if (( iptr->Contents.style & _FRAME_TYPE) == _RADIO_FRAME)
			sprintf(wbgbuffer,"bg%u_%s",iptr->Contents.radio,iptr->Contents.name);
		else	sprintf(wbgbuffer,"bg_%s",iptr->Contents.name);
	}
	return( wbgbuffer );
			...
			char *	expression )
{
	switch ( iptr->Contents.align & 0x0003 ) {
		...
		case	3	:	/* ZONEC */
			fprintf(handle,",%s,(%u-%s)",
				widget_foreground(iptr),
				widget_background_value(iptr),expression);
			break;
...
public	int	set_variable_macros( int newvalue )
{
	int	oldvalue = variable_macros;
	variable_macros = newvalue;
	return( oldvalue );
}
...
public	char *	resolve_widget_payload( struct form_item * iptr )
{
	struct	form_control * fptr;
	if (!( iptr ))
		return( (char *) 0);
...
public	void	calculate_edit_lines( struct form_item * iptr, int * nbc, int * nbl )
{
	...
	if (!( iptr->Contents.font ))
		return;

	if (((fw  = Context.FontTable[iptr->Contents.font].width  ) != 0)
	&&  (( fh  = Context.FontTable[iptr->Contents.font].height) != 0)) {
		if (!(nc = (iptr->Contents.w  / fw)))
			nc = 1;
		if (!(nl = (iptr->Contents.h / fh)))
			nl = 1;
...
public	void	calculate_form_lines( struct form_item * iptr, int * nbc, int * nbl )
{
	...
	if (((fw  = Context.FontTable[iptr->Contents.font].width  ) != 0)
	&&  ((fh  = Context.FontTable[iptr->Contents.font].height ) != 0)) {
		if (!(nc = ((iptr->Contents.w-xa)  / fw)))
			nc = 1;
		if (!(nl = ((iptr->Contents.h-ya) / fh)))
			nl = 1;
		if ((iptr->Contents.style & _FRAME_TYPE) == _FORM_FRAME)
			if ( method_is_valid(resolve_widget_payload(iptr)) ) 
				if ( nl > 0 )
...
private	char	*	language_extension( int language )
{
	switch (language) {
		...
		case	_GENERATE_ANDROID		:
			return((char *)JSON_EXTENSION);
		case	_GENERATE_PHP		:
			return((char *)PHP_EXTENSION);
		case	_GENERATE_REACT		:
			return((char *)REACT_EXTENSION);
		case	_GENERATE_NODE		:
			return((char *)NODE_EXTENSION);
		case	_GENERATE_CPP		:
...
#ifdef	_SING_MPEG
		case	_GENERATE_BIFS		:
			return((char *)BIFS_EXTENSION);
		case	_GENERATE_LASER		:
			return((char *)LASER_EXTENSION);
		case	_GENERATE_XMTA		:
			return((char *)XMTA_EXTENSION);
#endif
		default				:
			return((char *) 0);
		}
		...
		int	automatic )
{
	...
	switch ( language ) {
		...
		case	_GENERATE_NODE		:
			break;
		case	_GENERATE_PHP 		:
			if (!( fptr->weboptions & _WEB_OPTION_AGENT ))
				break;
		case	_GENERATE_HTML		:
			/* ------------------------ */
...
public	void	set_empty_string(char * h)
{
	EmptyString=h;
	return;
}
...
public	char *	set_c_empty_string()
{
	char * h=EmptyString;
...
public 	int	export_Form( struct form_control * fptr, int automatic )
{
	...
	if ( fptr->weboptions & _WEB_OPTION_RESTRICT )
		restriction=1;
	else	restriction=0;
	if ( fptr->weboptions & _WEB_OPTION_IMAGE )
		sessionimage=1;
	else	sessionimage=0;
	...
	if ( fptr->weboptions & 256 )
		iMethod = 1;
	else	iMethod = 0;
	if ( fptr->weboptions & 512 )
		withstyle=1;
	else	withstyle=0;
#endif
	...
	if (!( automatic & _INHIBIT_PROPERTIES )) {
		...
		switch ( fptr->language ) {
			...
			case	_GENERATE_HTML		:
				...
				if ( webopt )
					fptr->weboptions |= _WEB_OPTION_AGENT;
				else	fptr->weboptions &= ~_WEB_OPTION_AGENT;
				if ( restriction )
					fptr->weboptions |= _WEB_OPTION_RESTRICT;
				else	fptr->weboptions &= ~_WEB_OPTION_RESTRICT;
				if ( sessionimage )
					fptr->weboptions |= _WEB_OPTION_IMAGE;
				else	fptr->weboptions &= ~_WEB_OPTION_IMAGE;
				if ( iType )
					fptr->weboptions |= 16;
				...
				if ( iMethod )
					fptr->weboptions |= 256;
				else	fptr->weboptions &= ~256;
				if ( withstyle )
					fptr->weboptions |= 512;
				else	fptr->weboptions &= ~512;
				break;
#endif	
			case	_GENERATE_BAL		:
			case	_GENERATE_ABAL		:
			...
			case	_GENERATE_PHP		:
			case	_GENERATE_NODE		:
				if ( accept_php_production(
					&fptr->prodname, 
					&fptr->hostname,
					&webopt, 	/* generate ABAL WS Service 	*/
					&autotrad,
					&restriction, 	/* window restriction		*/
					&contextual, 	/* contextual menus 		*/
					&withstyle	/* generate with style		*/
					) == 27 ) {
					draw_console();
					return(0);
					}
...
#ifdef	_SING_MPEG
			case	_GENERATE_LASER	:
				if ( accept_laser_production(fptr) != 27 )
					break;
				else	return(0);
		...
		switch ((variable_macros=fptr->language)) {
			...
			case	_GENERATE_HTML		:
				...
				switch ( webmode ) {
					...
					case	0 :
						...
						autotrad = export_html_image_form(h,webopt,automatic,restriction,metrics,points, iMethod );
						if (!( fptr->weboptions & 16 )) {
							set_jpeg_quality( iQuality );
							}
						break;
					case	2 :
						autotrad = export_html_form(h,webopt,automatic,restriction,metrics,points, iMethod);
						break;
					}
				break;
#endif
			...
			case	_GENERATE_ANDROID	:
				...
				switch ( fptr->formflags & _FORM_TYPE_FLAGS ) {
					case	_FORM_TYPE_DIALOG	:
					case	_FORM_TYPE_EVENT 	:
						export_android_form( h, webopt, fptr->hostname, restriction, contextual, autotrad, source_name );
						break;
					case	_FORM_TYPE_QUALITY	:
					case	_FORM_TYPE_PAGE  	:
						ZeroWindow = 1;
						WindowCol = ProductionWindow.x;
						WindowRow = ProductionWindow.y;
						export_android_page( h );
						ZeroWindow = 0;
						break;
					case	_FORM_TYPE_SERVICE	:
						export_android_web_service( fptr );
					}
				break;
#endif
			case	_GENERATE_NODE		:
				...
				case	_FORM_TYPE_DIALOG	:
				case	_FORM_TYPE_EVENT 	:
					export_node_form( h, webopt, fptr->hostname, restriction, contextual, autotrad, source_name );
					break;
				}
				break;

			/* ------------------------- */
			/* FACEBOOK REACT PRODUCTION */
			/* ------------------------- */
			case	_GENERATE_REACT		:
				language_instance_prefix="\0";
				language_constant_prefix="msg";
				...
				case	_FORM_TYPE_DIALOG	:
				case	_FORM_TYPE_EVENT 	:
					export_react_form( h, restriction, contextual, autotrad, source_name );
					break;
				}
				break;

			/* --------------------- */
			/* PHP OBJECT PRODUCTION */
			/* --------------------- */
			case	_GENERATE_PHP		:
				language_instance_prefix="\0";
				language_constant_prefix="msg";
				...
				case	_FORM_TYPE_DIALOG	:
				case	_FORM_TYPE_EVENT 	:
					export_php_form(h,webopt,fptr->hostname,restriction,contextual,autotrad,source_name);
					break;
				case	_FORM_TYPE_QUALITY	:
				case	_FORM_TYPE_PAGE  	:
					ZeroWindow = 1;
			...
			default				:
				switch ( fptr->formflags & _FORM_TYPE_FLAGS ) {
					case	_FORM_TYPE_DIALOG	:
					case	_FORM_TYPE_EVENT 	:
						export_other_form( h, fptr );
						break;
					case	_FORM_TYPE_QUALITY	:
					case	_FORM_TYPE_PAGE  	:
						export_other_page( h, fptr );
						break;
					}
				break;
