			...
			if ( (c+atx+colindent) >= iw )
				...
				*(rptr+c+atx+colindent) = p;
				...
				filzone(dptr->x+(((c+atx)*zf)+border),
					...
					(zf-border),(zf-border),
					p);
				pptr = (rptr + ((c+atx+colindent)*pixelsize));
				if (!( p & _TRUE_COLOUR_PIXEL ))
					put_palette_pixel(pptr,(p & 0x00FF));
				else	put_physical_pixel(pptr,(p & _TRUE_COLOUR));
				}
			}
		}
	show(0,0,0,0);
...
private	void	zoom_pen_frame(struct zoom_control * zptr, int atx,int aty, int nbx, int nby, int hl, int bg, int ll )
{
	if ( bg != transparent )
		zoom_filzone(zptr,atx,aty,nbx,nby,bg);
	zoom_filzone(zptr,atx,aty,nbx,Context.thickness,hl);
	zoom_filzone(zptr,atx,aty,Context.thickness,nby,hl);
	zoom_filzone(zptr,(atx+nbx-Context.thickness),aty,Context.thickness,nby,ll);
	zoom_filzone(zptr,atx,(aty+nby-Context.thickness),nbx,Context.thickness,ll);
	return;
}
...
private	int	zoom_pen_pixel(struct zoom_control * zptr, int x, int y )
{
	int	atx,aty;
	int	p;
	int	button;
	...
	atx = (Context.Item.x/zf);
	aty = (Context.Item.y/zf);
	zoom_filzone(zptr,atx,aty,Context.thickness,Context.thickness,p);
	return(256);
}

private	int	matching_pixel(char * bptr, int offset, int colour )
{
	int	what;
	if ( get_pixel_format() ) {
		if ( *(bptr+offset) != colour )
			return(0);
		else	return(1);
		}
	else	{
		what = get_physical_pixel( bptr, offset );
		if ((what & _TRUE_COLOUR) != (colour & _TRUE_COLOUR))
			return(0);
		else	return(1);
		}
}
	...
	unsigned int	newcolour )
{
	...
	else if (!( matching_pixel(bptr,atx,oldcolour) )) {
		return;
		}
	...
	for (	inx = atx; inx != 0; inx-- )
		if (!( matching_pixel(bptr,inx,oldcolour) ))
			break;
		
	/* step over if not matching */
	/* ------------------------- */
	if (!( matching_pixel(bptr,inx,oldcolour) )) inx++;

	/* scan till end, replacing until not matching */
	/* ------------------------------------------- */
	for ( fnx = inx; fnx < iptr->columns; fnx++ ) {
		if (!( matching_pixel(bptr,fnx,oldcolour) ))
			break;
		else if ( colourmaped )
			*(bptr+fnx) = newcolour;
		else	put_physical_pixel((bptr+(fnx*pixelsize)),(newcolour&_TRUE_COLOUR));
		}
	...
	if ( aty > 0 ) {
		...
		for ( atx = inx; atx < fnx; atx++ )
			if ( matching_pixel(bptr,atx,oldcolour) )
...
private	unsigned int	zoom_put_pixels(struct zoom_control * zptr, unsigned int x, unsigned int y )
{
	...
	if ((rptr = connect_pixel_row(
		...
		if ( colourmaped ) {
			oldcolour = (*(rptr + atx + zptr->colindent) & 0x00FF);
			newcolour = (button == _MIMO_LEFT ? Context.foreground : Context.background );
			}
		else	{
			oldcolour = get_physical_pixel( rptr, (atx+zptr->colindent));
			newcolour = (button == _MIMO_LEFT ? Context.foreground : Context.background );
			if (!( newcolour & _TRUE_COLOUR_PIXEL ))
				newcolour = (get_palette_pixel( newcolour ) | _TRUE_COLOUR_PIXEL );
			/* printf("zoom_put_pixels : old= %06.6lx new= %08.8lx \r\n",oldcolour,newcolour); */
			}
		if ( oldcolour != (newcolour & _TRUE_COLOUR) ) {
			replace_around( 
				zptr->image, 
				(atx+zptr->colindent),
				(aty+zptr->rowindent),
		...
		switch (visual_event(2)) {
			case	_MIMO_LEFT	:
				if ( colourmaped ) {
					...
					Context.foreground = (get_physical_pixel(rptr,(atx + zptr->colindent)) | _TRUE_COLOUR_PIXEL);
					get_logical_pixel(rptr,&Context.fgrgb[0],&Context.fgrgb[1],&Context.fgrgb[2]); 
					}
				break;
			default			:
				if ( colourmaped ) {
					...
					Context.background = (get_physical_pixel(rptr,(atx + zptr->colindent)) | _TRUE_COLOUR_PIXEL);
					get_logical_pixel(rptr,&Context.bgrgb[0],&Context.bgrgb[1],&Context.bgrgb[2]); 
					}
				break;
			}
		draw_ToolBar();
...
public	void	zoom_circle(struct zoom_control * zptr, int atx, int aty, int width, int height, int p, int mode )
{
	...
	for ( 	y=(radius-1); y > 0; y-- ) {
		...
		if ( mode ) {
			...
			zoom_filzone(zptr,atx-x,aty-y,nb,thick,p);
			zoom_filzone(zptr,atx-x,aty+(y-1),nb,thick,p);

			zoom_filzone(zptr,atx+nx,aty-y,nb,thick,p);
			zoom_filzone(zptr,atx+nx,aty+(y-1),nb,thick,p);
...
public	int	zoom_function(struct zoom_control * zptr, int style)
{
	...
	switch (( style & _FRAME_TYPE )) {
		case	_OUTLINE_FRAME	:
			p = (button == _MIMO_LEFT ? Context.foreground : Context.background );
			zoom_pen_frame( zptr, atx,aty,nbx,nby, p, transparent,p );
			break;
		case	_FILL_FRAME	:
			p = (button == _MIMO_LEFT ? Context.foreground : Context.background );
		...
		switch ( t ) {
			...
			case	_MIMO_WHILE_DOWN:
				...
				if ( Context.Item.style == -1 ) {
					...
					else	zoom_pen_pixel(zptr,Context.Item.x,Context.Item.y);
					}
					...
					zoom_pen_pixel(zptr,(Context.Item.x=x),(Context.Item.y=y));
					}
				else	{
			...
			case	_MIMO_CLICK	:
				if ( Context.Item.style == -1 ) {
					...
					if ((Context.Item.style = Context.style) == _PIPE_FRAME) 
						...
						zoom_pen_pixel(zptr,(Context.Item.x),(Context.Item.y));
	...
	for ( r=0; r < dptr->h; r++ ) {
		if (!(bptr = connect_pixel_row(iptr->storage,(r+rowindent) ))) {
			...
			if (!( border )) {
				...
				else	putzone(dptr->x,dptr->y+r,dptr->w,1,(bptr + (colindent*pixelsize)));
				}
			else if ( colourmaped ) {
				for (c=0; c < dptr->w; c++ )
					filzone(dptr->x+((c*zf)+border),
						dptr->y+((r*zf)+border),
						(zf-border),(zf-border),
						(*(bptr+(c+colindent))&0x00FF) );
				}
			else	{
				for (c=0; c < dptr->w; c++ )
					filzone(dptr->x+((c*zf)+border),dptr->y+((r*zf)+border),(zf-border),(zf-border),(get_physical_pixel(bptr,(c+colindent))|_TRUE_COLOUR_PIXEL));
...
private	int	image_reflect_h( struct zoom_control * zptr )
{
	...
	else					return( visual_reflect_h( iptr->storage ) );
...
private	int	image_reflect_v( struct zoom_control * zptr )
{
	...
	else					return( visual_reflect_v( iptr->storage ) );
			...
			*/
			...
			for (r=0; r < nby; r++ ) {
				if (!( sptr = connect_pixel_row( zptr->image->storage, r+aty+rowindent ) ))
					...
					break;
				memcpy( tptr, (sptr+((atx+colindent)*pixelsize)), (nbx*pixelsize) );
				}
			...
			*/
			for (r=0; r < nby; r++ ) {
				if (!( tptr = connect_pixel_row( zptr->image->storage, r+aty+rowindent ) ))
					...
					break;
				memcpy( (tptr+((atx+colindent)*pixelsize)), sptr, (nbx*pixelsize) );
				}
			zoom_data( zptr );
			zoom_cursor_off(zptr,0);
			break;
...
struct	image_transformer	{
	...
	int	g;
	int	b;
	struct standard_image * holder;
	struct zoom_control * zoom;
	} Transformer = { 
		0,0,0,
		0,0,0,0, 
		(struct standard_image *) 0, 
		(struct zoom_control *) 0
		};


private	void	calculate_transformation( int d, int a, int f, int w )
{
	int	rowindent;
	int	colindent;
	int	rv;
	...
	int	nby;
	ps = get_pixel_size();
	if (!( iptr = Transformer.zoom->image ))
		return;
	else if (!( yptr = Transformer.holder ))
		return;
	if ( d ) {
		if ( f & 1 ) {	Transformer.radjust -= a;	}
		if ( f & 2 ) {	Transformer.gadjust -= a;	}
		if ( f & 4 ) {	Transformer.badjust -= a;	}
		}
	else	{
		if ( f & 1 ) {	Transformer.radjust += a;	}
		if ( f & 2 ) {	Transformer.gadjust += a;	}
	...
	if (!(zf = Transformer.zoom->factor)) zf = 1;
	...
	if (!( w )) {
		...
		colindent = Transformer.zoom->colindent;
		rowindent = Transformer.zoom->rowindent;
		if ( Context.Item.x > Context.Item.w ) {
			atx = (Context.Item.w/zf);
			nbx = (((Context.Item.x-Context.Item.w)/zf)+1);
			}
	...
	for (	r=0; r < nby; r++ ) {
		...
		if (!( bptr = connect_pixel_row( iptr->storage, r+aty+rowindent) ))
			break;
		else if (!( sptr = connect_pixel_row( yptr->storage, r+aty+rowindent) ))
			break;

		for (c=0; c < nbx; c++ ) {
			get_logical_pixel((sptr+((c+atx+colindent)*ps)),&rv,&gv,&bv);
			rv += Transformer.radjust;
			if ( rv < 0 ) rv = 0; else if ( rv > 255 ) rv = 255;
			gv += Transformer.gadjust;
			if ( gv < 0 ) gv = 0; else if ( gv > 255 ) gv = 255;
			bv += Transformer.badjust;
			if ( bv < 0 ) bv = 0; else if ( bv > 255 ) bv = 255;
			put_logical_pixel((bptr+((c+atx+colindent)*ps)),rv,gv,bv);
			}
		}
	return;
}
...
public 	int	sing_light_transformation( int v, int w )
{
	...
	if ( Transformer.zoom ) {
		if ( v != Transformer.l ) {
			...
			calculate_transformation(d,a,7,w);
			Transformer.l = v;
			zoom_data( Transformer.zoom );
			}
		}
	return;
}
...
public	int	sing_red_transformation( int v,int w )
{
	...
	if ( Transformer.zoom ) {
		if ( v != Transformer.r ) {
			...
			calculate_transformation(d,a,1,w);
			Transformer.r = v;
			zoom_data( Transformer.zoom );
			}
		}
...
private	void	transform_palette_range( char * newptr,int start, int finish)
{
	int	i;
	int	rv;
	int	gv;
	...
	for (i=start; i < finish; i++ ) {
		rv = ( *(newptr+(i*4)) & 0x00FF);
		rv += Transformer.radjust;
		if ( rv < 0 ) rv = 0; else if ( rv > 255 ) rv = 255;
		*(newptr+(i*4))   = (rv & 0x00FF);
	
		gv = ( *(newptr+(i*4)+1) & 0x00FF);
		gv += Transformer.gadjust;
		if ( gv < 0 ) gv = 0; else if ( gv > 255 ) gv = 255;
		*(newptr+(i*4)+1) = (gv & 0x00FF);

		bv = ( *(newptr+(i*4)+2) & 0x00FF);
		bv += Transformer.badjust;
		if ( bv < 0 ) bv = 0; else if ( bv > 255 ) bv = 255;

		*(newptr+(i*4)+2) = (bv & 0x00FF);
		}
	return;
}

public	int	sing_build_palette(char * pname, int options )
{
	FILE *	h=(FILE *) 0;
	char * 	oldptr;
	char *	newptr;
	if (!( oldptr = get_vga_palette()))
		return(118);
	else if (!( newptr = allocate( 256 * 4 )))
		return(27);
	else	{
		memcpy( newptr, oldptr, 256*4 );
		if ( options & 1 ) {
			transform_palette_range(newptr,0,16);
			}
		if ( options & 2 ) {
			transform_palette_range(newptr,16,24);
			}
		if ( options & 4 ) {
			transform_palette_range(newptr,24,256);
			}
		if ( pname ) {
			if ((h = fopen( pname,"w" )) != (FILE *) 0) {
				fwrite(newptr,256,4,h);
				fclose(h);
				}
			else	return(46);
			}
		else	{
			memcpy( oldptr, newptr, 256*4 );
			put_palette();
			}
		liberate( newptr );
		return(0);
...
public	int	sing_green_transformation( int v, int w )
{
	...
	if ( Transformer.zoom ) {
		if ( v != Transformer.g ) {
			...
			calculate_transformation(d,a,2,w);
			Transformer.g = v;
			zoom_data( Transformer.zoom );
			}
		}
...
public	int	sing_blue_transformation( int v, int w )
{
	...
	if ( Transformer.zoom ) {
		if ( v != Transformer.b ) {
			...
			calculate_transformation(d,a,4,w);
			Transformer.b = v;
			zoom_data( Transformer.zoom );
			}
		}
...
private	int	image_colour_transformation( struct zoom_control * zptr )
{
	void *	vptr;
	char *	sptr;
	char *	tptr;
	int	r;
	int	ps;
	struct	standard_image * hptr;
	struct	standard_image * iptr;
	ps = get_pixel_size();
	if (!( zptr )) 	return(0);
	if ((hptr = zptr->image) != (struct standard_image*) 0) {
		iptr = local_allocate_image( hptr->columns, hptr->rows, 0,0 );
		iptr->format = hptr->format;
		for ( r=0; r < hptr->rows; r++ ) {
			if (!( sptr = connect_pixel_row( hptr->storage, r ) ))
				break;
			else if (!( tptr = connect_pixel_row( iptr->storage, r ) ))
				break;
			else	memcpy( tptr, sptr, iptr->columns * ps);
			}
		zptr->image = iptr;
		}
	Transformer.l = 256;
	...
	Transformer.g = 256;
	Transformer.b = 256;
	Transformer.radjust=0;
	Transformer.gadjust=0;
	Transformer.badjust=0;
	Transformer.zoom = zptr;
	Transformer.holder = hptr;
	if ( accept_colour_transformation() != 27 ) {
		for ( r=0; r < hptr->rows; r++ ) {
			if (!( tptr = connect_pixel_row( hptr->storage, r ) ))
				break;
			else if (!( sptr = connect_pixel_row( iptr->storage, r ) ))
				break;
			else	memcpy( tptr, sptr, iptr->columns * ps);
	...
	zptr->image = hptr;
	local_drop_image( iptr );
	Transformer.zoom = (struct zoom_control * ) 0;
	return(1);
}
	...
	if ( accept_image_dimensions( &lwidth, &lheight, &makefit ) == 27 )
		...
		for (	r=0;
			...
			if (!( tptr = connect_pixel_row( rptr->storage, r ) ))
				break;
			memset( tptr, Context.background, (rptr->columns*pixelsize));
			if (( r < iptr->rows ) 
			&&  (( sptr = connect_pixel_row( iptr->storage, r )) != (char *) 0))
				memcpy( tptr,sptr,(xc*pixelsize));
			}
		}
...
public	int	image_reset( struct standard_image * iptr)
{
	...
	for (	r=0; r < iptr->rows; r++ ) {
		if (!( rptr = connect_pixel_row( iptr->storage, r ) ))
			break;
		else	memset(rptr,Context.background,(iptr->columns*pixelsize));
		}
	return(1);
}
	...
	if (( Window.w != 0) && (Window.h != 0)) {
		...
		while (1) {
			...
			switch (e) {
				...
				case	_ZOOM_COLOUR	:
					if (!( get_pixel_format() )) {
						if ( image_colour_transformation(&Zoom) )
							draw_console();
						}
					else if ( image_zap_colour(&Zoom) )
						draw_console();
					continue;
