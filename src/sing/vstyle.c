...
private	int	style_element_hash( char * nptr );
...
private	void	reset_style(struct element_style * sptr, int iscopy )
{
	...
	sptr->cellposition.option = _keyword_none;
	...
	sptr->textfont=1;
	sptr->textatribut=_keyword_none;;
...
public	struct	element_style *	allocate_for_style()
{
...
public	void	liberate_style( struct element_style * sptr )
{
	if ( sptr ) {
		if ( sptr->name ) {
			liberate( sptr->name );
...
private	int	is_style_plus() 	{	return( check_punctuation( '+' ) );	}
...
int	resolve_style_margin( char * sname, int * t, int * r, int * b, int * l )
{
	int	nb=0;
	struct	element_style * eptr;
	if (!( eptr = resolve_style_class( sname ) ))
		...
		if (eptr->marginsize.right.type  == _PIXEL_DIMENSION ) {
			*r = eptr->marginsize.right.value; nb |=2;
			}
		else	*r = 0;
		if (eptr->marginsize.left.type  == _PIXEL_DIMENSION ) {
			*l = eptr->marginsize.left.value; nb |=8;
			}
		else	*l = 0;
		if (eptr->marginsize.top.type  == _PIXEL_DIMENSION ) {
			*t = eptr->marginsize.top.value; nb |=1;
			}
		else	*t = 0;
		if (eptr->marginsize.bottom.type  == _PIXEL_DIMENSION ) {
			*b = eptr->marginsize.bottom.value; nb |=4;
			}
		else	*b = 0;
		return( nb );
		}
...
int	resolve_style_position( char * sname, int * t, int * r, int * b, int * l)
{
	int	nb=0;
	struct	element_style * eptr;
	if (!( eptr = resolve_style_class( sname ) ))
		...
		if (eptr->cellposition.right.type  == _PIXEL_DIMENSION ) {
			*r = eptr->cellposition.right.value; nb |=2;
			}
		else	*r = -1;
		if (eptr->cellposition.left.type  == _PIXEL_DIMENSION ) {
			*l = eptr->cellposition.left.value; nb |=8;
			}
		else	*l = -1;
		if (eptr->cellposition.top.type  == _PIXEL_DIMENSION ) {
			*t = eptr->cellposition.top.value; nb |= 1;
			}
		else	*t = -1;
		if (eptr->cellposition.bottom.type  == _PIXEL_DIMENSION ) {
			*b = eptr->cellposition.bottom.value; nb |= 4;
			}
		else	*b = -1;
		return(nb);
		}
...
/*	---------------------------------------------------	*/
#ifdef	_vatb_c
private	int	apply_visual_atribut( struct element_style * sptr )
{
...
private	int	visual_atribut( struct element_style * sptr )
{
	int	status=0;
	char *	aptr;
	char *	cptr;
	...
	int	item=0;
	if (!(aptr = sptr->alias))
		return( apply_visual_atribut( sptr ) );
		...
		while ((vptr = selection_item(item++,cptr )) != (char *) 0) {
			items++;
			if (!( strcmp( vptr, "this" ))) {
				if ((status = apply_visual_atribut( sptr )) != 0) {
					liberate( cptr );
					liberate( vptr );
					return(status);
					}
				}
			else if ((status = visual_atribut(sptr)) != 0 ) {
				liberate( cptr );
				liberate( vptr );
				return(78);
				}
...
char *	get_element_alias(struct element_style * eptr )
{
	return( eptr->alias );
}
