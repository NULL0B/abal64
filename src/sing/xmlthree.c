...
public	struct link_control * create_database_link( char * nptr, char * jptr, char * slptr, char * tlptr, int nature,int inhibit)
{
	struct	link_control * lptr;
...
public	int	export_database_record( FILE * h, struct file_control * tptr)
{
	char *	tagname;
	int	i;
	int	v;
	...
	indent(h,1);
	switch ( tptr->nature ) {
		case	_ADFILE_FRAME	: tagname = "ADFILE"; break;
	...
	if (( dptr = tptr->payload ) != (struct data_control *) 0) {
		...
		if ( dptr->indexfields != 0 ) {
		...
		else if (( tptr->nature == _MCFILE_FRAME ) 
		     ||  ( tptr->nature == _SIFILE_FRAME )) {
			/* Generate Index Information */
			/* -------------------------- */
			indent(h,2);
			fprintf(h,"<INDEX");
			...
			fprintf(h," SIZE=%u>",2);
			linefeed(h);
			abal_xml_dcl(h,"\0","FILEINDEX",_ABAL_WORD,2,0,0,0,0);
			indent(h,2);
			fprintf(h,"</INDEX>");
			linefeed(h);
			}
...
private	int	export_database_records( FILE * h )
{
	...
	for ( 	tptr  = FileManager->first;
		...
		else	export_database_record(h,tptr );
	...
	for ( 	lptr  = FileManager->firstlink;
		...
		lptr = lptr->next ) {
		indent(h,1);
		fprintf(h,"<LINK NAME=%s JOINTURE=%s SOURCE=%s TARGET=%s TYPE=%u INHIBIT=%u/>",
...
public	int	print_database_records( FILE * h )
{
	...
	for ( 	tptr  = FileManager->first;
		...
		/* --------------------- */
		switch ( tptr->nature ) {
			case	_ADFILE_FRAME	: tagname = "AD"; break;
...
public 	void	xml_put_Camera( struct camera3d * kptr, char * dtdname, int automatic )
{
	...
	if ((h = open_xml_target( kptr->name, &sname, "THREED", xml_threed_type, xml_threed_style )) != (FILE*) 0) {
		...
		xml_put_word(h,"BUILD",aptr->build,1);
		xml_put_word(h,"VALIDATE",aptr->validate,1);
		xml_put_word(h,"VISIBLE",aptr->visible,1);
#else
		...
		for (	optr=kptr->first;
			...
			switch ( optr->nature ) {
				...
				case	__3D_IMAGE	:
					...
					break;
				case	__3D_MOVIE	:
					xml_put_filename_string(h,"MOVIE",optr->name,2);
					break;
				case	__3D_AUDIO	:
					xml_put_filename_string(h,"AUDIO",optr->name,2);
					break;
				default			:
		...
		switch ( i ) {
			...
			case	DATABASE_VMEMORY:
				...
				if (!( fptr = allocate_file_control((void*)0)) != (struct file_control*) 0)
					...
					fptr->members=0;
					switch ( i ) {
						case	DATABASE_ADFILE	: fptr->nature = _ADFILE_FRAME; break;
	...
	switch ((i = xptr->heap[--xptr->nested])) {
		...
		case	DATABASE_INHIBIT	:
			if ( xptr->heap[(xptr->nested-1)] != DATABASE_LINK ) 
				...
				lptr->inhibit = quoted_atoi( value );
				return(0);
				}
		...
		switch ( i ) {
			...
			case	ELEMENT_TEXT		:
				if ( xptr->heap[(xptr->nested-1)] == ELEMENT_OBJECT ) 
				{
					...
					{
					case	ELEMENT_MOVIE		:
						optr->nature = __3D_MOVIE;
						break;
					case	ELEMENT_AUDIO		:
						optr->nature = __3D_AUDIO;
						break;
					case	ELEMENT_FORM		:
						optr->nature = __3D_FORM;
						break;
					case	ELEMENT_TEXT		:
						optr->nature = __3D_TEXT;
						break;
					}
				}
				break;
			...
			case	ELEMENT_BUILD		:
				xml_get_word( xptr, &aptr->build );
				break;
			case	ELEMENT_VALIDATE	:
				xml_get_word( xptr, &aptr->validate);
				break;
			case	ELEMENT_VISIBLE		:
				xml_get_word( xptr, &aptr->visible);
				break;
			case	ELEMENT_ICONS 		:
#ifdef	ABAL32A
