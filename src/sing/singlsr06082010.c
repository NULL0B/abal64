...
/*	-----------------------------------------------------------------------	*/
void	laser_filzone( FILE * handle, int x, int y, int w, int h, int colour, char * name )
{
	char	work[1024];
...
{
	if ( bg != transparent )
		laser_filzone(h,atx,aty,nbx,nby,bg,name);
	laser_filzone(h,atx,aty,nbx,1,hl,(char *) 0);
	laser_filzone(h,atx,aty,1,nby,hl,(char *) 0);
	laser_filzone(h,(atx+nbx),aty,1,nby,ll,(char *) 0);
	laser_filzone(h,atx,(aty+nby),nbx,1,ll,(char *) 0);
	return;
}
...
/*	-----------------------------------------------------------------------	*/
void	laser_text( FILE * handle, int x, int y, int w, int h, char * mptr, int colour, char * name, int align )
{
	int	fheight=0;
	char	work[1024];
	char	awork[256];
...
{
	laser_text( handle,x,y,w,h,mptr, colour, align, nptr );
	return;
}
...
void	sing_svg_image( FILE * handle, int x, int y, int w, int h, char * iptr,char * nptr, int option )
{
	...
	case	_I_TILE		:
		laser_indent(handle);
		fprintf(handle,"<rect x=%c%u%c y=%c%u%c width=%c%u%c height=%c%u%c fill=%curl(#%s)%c/>\n", 
			0x0022,x,0x0022,
			0x0022,y,0x0022,
			0x0022,w,0x0022,
			0x0022,h,0x0022,
...
void	laser_checkbox( FILE * handle, int x, int y, int w, int h, char * mptr, struct form_item * iptr )
{
	...
	laser_text( handle, x+(2*LC.fontHeight),y,w-(2*LC.fontHeight),h, mptr, standard_black, (char *) 0,_A_LEFT);
	sprintf(work,"VALUE_%s",iptr->Contents.name);
	laser_comment(handle,"table check value");
	laser_open_switch( handle, work,0 );
		laser_filzone(handle,x+3,y+3,LC.fontHeight-6,LC.fontHeight-6,standard_white,(char *) 0);
		laser_filzone(handle,x+3,y+3,LC.fontHeight-6,LC.fontHeight-6,standard_black,(char *) 0);
...
void	laser_switch( FILE * handle, int x, int y, int w, int h, char * mptr, struct form_item * iptr )
{
	...
	laser_open_switch( handle, work,0 );
		...
		laser_group(handle);
			laser_filzone(handle,x+2,y+2,w-4,h-4,bg,(char *) 0);
			laser_text( handle, x+2,y+2,w-4,h-4, mptr, fg, (char *) 0,_A_CENTER);
		laser_close_tag(handle,"g");

		laser_comment(handle,"table switch true value");
		laser_group(handle);	
			laser_filzone(handle,x+2,y+2,w-4,h-4,fg,(char *) 0);
			laser_text( handle, x+2,y+2,w-4,h-4,mptr, bg, (char *) 0,_A_CENTER);
		laser_close_tag(handle,"g");
...
void	laser_button( FILE * handle, int x, int y, int w, int h, char * message, char * name, int align, int style )
{
	...
	else
	{
		if (!( align & 2 )) // has frame
		{
			...
			laser_relief( handle, x,y,w,h, style, name);
			laser_text( handle, x+2,y+8,w-4,h-4, message, standard_text, (char *) 0,_A_CENTER);
		}
		else
		{
			laser_comment(handle,"text button");
			laser_text( handle, x,y,w,h, message, standard_text, (char *) 0,_A_CENTER);
		}
	}
}
...
void	laser_pushbutton( FILE * h, struct form_item * iptr )
{
	...
	case _WIDGET_TASK    :
	case _WIDGET_LOADGO  :
		laser_comment(h,"table button link");
...
void	laser_tab_button( FILE * handle, struct form_item * iptr )
{
	...
	laser_open_proto_use( handle,
			0,0,
			work,iwork);
		laser_click_sensor(handle,iptr);
	laser_close_tag(handle,"use");
...
void	laser_edit( FILE * handle, int x, int y, int w, int h, struct form_item * iptr )
{
	...
	if ( LC.testData )
	{
		laser_comment(handle,"edit text");
		laser_text_value(handle, x+1,y+1,w-2,h-2, iptr->Contents.name, standard_black, _A_LEFT, iptr->Contents.name );
	}
	return;
}
...
void	laser_window( FILE * handle, int x, int y, int w, int h, char * message, char * name, struct form_item * iptr )
{
	...
	laser_filzone( handle, x+2,y+2,w-4,fh, standard_title, (char *) 0);
	laser_comment(handle,"window title message");
	laser_text( handle, x+fw,y+2,w-(fw*2),fh, message, standard_text, (char *) 0,_A_LEFT);
	return;
}
...
/*	--------------------------------------------------------	*/
void	laser_proto_use( FILE * h, int x, int y, char * cname, char * iname )
{
	laser_indent(h);
	fprintf(h,"<use x=%c%u%c y=%c%u%c xlink:href=%c#%s%c id=%c%s%c/>\n",
		0x0022,x,0x0022,
...
/*	-------------------------------------------	*/
void	laser_proto_instance( FILE * h, struct form_control * fptr, char * iname, int xpos, int ypos )
{
	char	work[1024];
	...
	fprintf(h,"<use x=%c%u%c y=%c%u%c xlink:href=%c#%s%s%c id=%c%s%c/>\n",
		0x0022,xpos,0x0022,
		0x0022,ypos,0x0022,
		0x0022,fptr->identifier,_FORM_SUFFIX,0x0022,
		0x0022,iname,0x0022);
	return;
}
...
void	laser_dataframe( FILE * h, struct form_item * iptr )
{
	...
	else
	{
		...
		laser_group(h);
			laser_proto_instance(h,ffptr,iptr->Contents.name,iptr->Contents.x,iptr->Contents.y);
		laser_close_tag(h,"g");
		return;
	}
}
...
//	else	focusline = 0;
	...
	laser_filzone( handle, x,y,w,fh, standard_title,(char *) 0);
	...
	switch ( options & 0x0007 ) {
		...
		case	3 :
			...
			for ( r=(fh*2); (r+fh) < h; r+= (fh*2) )
				laser_filzone(handle, x,y+r,w,fh,zbg,(char *) 0);
			break;
		}
	...
	if ( tptr ) {
		...
		if ( fields != 0) {
			...
			for (lmargin=0,field=0; field < fields; field++ ) {
				...
				libelle[tlen] = 0;
				laser_text(handle,((lmargin+padd)*fw)+x,y,relw,fh+1,libelle,txfg,(char *) 0,_A_LEFT);
				...
				if ( *(wptr++) != '|' )
					...
					/* was X */
					laser_filzone(handle,(((lmargin*fw)+x)-1),y+adjust,1,h-adjust,dath,(char *) 0);
					}
				}
			...
			switch ( options & 0x0007 ) {
				...
				case	5 :
					...
					for (lmargin=0,field=0; field < fields; field++ ) {
						...
						if (!( field & 1 ))
							laser_filzone(handle,lmargin+x,y+fh,tw-1,(h-fh),zbg,(char *) 0);
						lmargin += (tw-xh);			
						}
					break;
				...
				case	7 :
					...
					for ( nbr=0,r=fh; (r+fh) < h; r+= fh, nbr++ ) {
						for (lmargin=0,field=0; field < fields; field++ ) {
							...
							if (!( (field+nbr) & 1 ))
								laser_filzone(handle,lmargin+x,y+r,tw-1,fh,zbg,(char *) 0);
							lmargin += (tw-xh);			
							}
						}
					break;
...
void	LASER_pattern( FILE * handle, struct form_item * iptr )
{
	...
	fprintf(handle,"<defs><pattern id=%c%s%c x=%c%u%c y=%c%u%c width=%c%u%c height=%c%u%c>\n", 
...
void	laser_open_svg(FILE *h)
{
	if (!( LC.useSaf ))
	{
		...
		fprintf(h,"\txmlns:xlink='%s'\n",_XLINK_NAMESPACE);
		fprintf(h,"\txmlns:ev='%s'\n",_EVENT_NAMESPACE);
		fprintf(h,"\twidth='%upx' height='%upx' viewBox='0 0 %u %u' version='1.1'>\n",LC.width,LC.height,LC.width,LC.height);
	}
	else
	{
		fprintf(h,"<svg\n");
		...
		fprintf(h,"\txmlns:ev='%s'\n",_EVENT_NAMESPACE);
		fprintf(h,"\twidth='%upx' height='%upx' viewBox='0 0 %u %u' version='1.1'>\n",LC.width,LC.height,LC.width,LC.height);
//		fprintf(h,"<svg width='%upx' height='%upx' viewBox='0 0 %u %u' version='1.1'>\n",LC.width,LC.height,LC.width,LC.height);
	}	
	return;
}
...
/*	-------------------------------------------	*/
int	laser_proto_body( FILE * h, struct form_control * fptr )
{
	int	i;
	int	started=0;
	struct	form_item * iptr;
	...
	if ( fptr->first )
	{
			...
			iptr = iptr->next )
		{
			if ( iptr->Contents.style != _IMAGE_FRAME )
				continue;
			else if ( iptr->Contents.align != _I_TILE )
				continue;
			else
			{
		...
		if ( fptr->pages )
		{
			...
			if ( started )
			{
				...
				if ( iiptr )
				{
					...
					sprintf(iwork,"%s%s",_FRAME_PREFIX,fptr->identifier);
					laser_proto_use( h, 0,0, work, iwork );
				}
				started=0;
				/* --------------------------------- */
					...
					iptr = iptr->next )
				{
					if ( iptr->Contents.style != _TAB_FRAME )
						continue;
					else	laser_tab_conditional( h, iptr, fptr );
				}
			}
		}
		else
...
/*	-----------------------------------------------------------------------	*/
void	laser_proto_definition( FILE * handle, struct form_control * fptr )
{
	FILE *	h;
	char	work[1024];
	struct	form_item * iptr;
...
int	export_LASER_form( FILE * h, struct form_control * fptr )
{
	...
	laser_open_svg(h);
	laser_proto_definition( h, fptr );
	laser_proto_instance( h, fptr,"tHis",0,0 );
	laser_close_svg(h);

#ifdef	_NO_WAY
	laser_comment(h,"Initial Scene Graph");
	laser_tag(h,"OrderedGroup");
	laser_background2d(h,0,(char *) 0);
	laser_worldinfo(h,"BiFS Text Generated by Sing",fptr->identifier);
	laser_proto_instance(h,fptr,"This",0,0);
	if ( LC.keySensor == 2 )
	{
		laser_key_sensor(h,fptr->identifier);
	}
	laser_close_tag(h,"OrderedGroup");
	laser_comment(h,"Scene Route Descriptions");
	if ( LC.keySensor )
	{
		switch ( LC.keySensor )
		{
		case	2	:
			laser_input_route(h,fptr);
		case	1	:
			laser_comment(h,"Key Sensor Object Descriptor");
			laser_key_sensor_object_descriptor( h );
		}
	}
	else if ( LC.stringSensor )
	{
		laser_comment(h,"String Sensor Object Descriptor");
		laser_string_sensor_object_descriptor( h );
	}
#endif
