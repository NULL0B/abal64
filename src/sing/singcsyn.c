...
public	void	c_comment_line( FILE * handle, char * vptr , int indentation )
{
	indent(handle,indentation);
	fprintf(handle,"/*\t",vptr);
	while ( *vptr ) {
...
public	void	c_comment_space( FILE * handle, char * vptr , int indentation )
{
	indent(handle,indentation);
	fprintf(handle,"/*\t",vptr);
	while ( *vptr ) {
...
public	void	c_expanded_comment( FILE * handle, char * vptr , int indentation )
{
	indent(handle,indentation);
	fprintf(handle,"/*\t",vptr);
	while ( *vptr )
		fprintf(handle,"%c ",(*(vptr++)&0x00FF));
	fprintf(handle,"\t*/",vptr);
	linefeed(handle);
	return;
}

public	void	c_expanded_line( FILE * handle, char * vptr , int indentation )
{
	indent(handle,indentation);
	fprintf(handle,"/*\t",vptr);
	while ( *vptr ) {
