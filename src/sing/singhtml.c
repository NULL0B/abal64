...
#define	_DEFAULT_COPYRIGHT	"Copyright &copy; 2002,2010 Amenesik / Sologic "
...
public	void	html_rgb_alpha( FILE * h, int colourid, int a )
{
	int	rvalue=0;
	int	gvalue=0;
	int	bvalue=0;
	...
	{
		fprintf(h,"rgba(%u,%u,%u,0.5);",rvalue&255,gvalue&255,bvalue&255);
	}
}

public	void	html_rgb_gradient( FILE * h, int colourid, int hv, int xv )
{
	int	rvalue=0;
	int	gvalue=0;
	int	bvalue=0;
	...
	else
	{
		if ( hv ) /* vertical */
		{
			...
			{
				/* dark to the left */
				start[0] = (rvalue > 16? rvalue-16 :1);
				start[1] = (gvalue > 16? gvalue-16 :1);
				start[2] = (bvalue > 16? bvalue-16 :1);
				finish[0] = (rvalue < 0x00F0? rvalue += 16: 255);
				finish[1] = (gvalue < 0x00F0? gvalue += 16: 255);
				finish[2] = (bvalue < 0x00F0? bvalue += 16: 255);
				fprintf(h,"linear-gradient(to right,rgb(%u,%u,%u),rgb(%u,%u,%u));",
					start[0],start[1],start[2], 
					finish[0],finish[1],finish[2]);
			}
			else /* convex */
			{
				start[0] = (rvalue < 0x00F0? rvalue += 16: 255);
				start[1] = (gvalue < 0x00F0? gvalue += 16: 255);
				start[2] = (bvalue < 0x00F0? bvalue += 16: 255);
				finish[0] = (rvalue > 16? rvalue-16 :1);
				finish[1] = (gvalue > 16? gvalue-16 :1);
				finish[2] = (bvalue > 16? bvalue-16 :1);
				fprintf(h,"linear-gradient(to right,rgb(%u,%u,%u),rgb(%u,%u,%u));",
					start[0],start[1],start[2], 
					finish[0],finish[1],finish[2]);
			}
		...
		else /* horizontal */
		{
			if ( xv ) /* concave */
			{
				...
				finish[1] = (gvalue < 0x00F0? gvalue += 16: 255);
				finish[2] = (bvalue < 0x00F0? bvalue += 16: 255);
				fprintf(h,"linear-gradient(to bottom,rgb(%u,%u,%u),rgb(%u,%u,%u));",
					start[0],start[1],start[2], 
					finish[0],finish[1],finish[2]);
			}
			else /* convex */
...
public	void	html_generator(FILE * handle)
{
	...
	html_open_tag(handle,"meta");
		...
		html_field(handle,"content","Amenesik / Sologic Sing Html Version 2.2b");
...
private	int	html_national_form( FILE * handle, int mode, int automatic, int restrikt, int parametric, int points )
{
	struct form_item * iptr;
	int	autotrad=1;
...
public	int	export_html_form( FILE * handle, int mode, int automatic, int restrikt, int parametric, int points, int hMethod )
{
	int	autotrad=1;
	FILE	*	h;
	struct	form_control *	fptr;
