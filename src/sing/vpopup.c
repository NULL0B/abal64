...
#include <stdio.h>
#include "visual.h"
#define en_Auto3622   "Sing : Version 2.1a"
#define fr_Auto3622   "Sing : Version 2.1a"
#define it_Auto3622   "Sing : Version 2.1a"
#define es_Auto3622   "Sing : Version 2.1a"
#define de_Auto3622   "Sing : Version 2.1a"
#define nl_Auto3622   "Sing : Version 2.1a"
#define pt_Auto3622   "Sing : Version 2.1a"
#define xx_Auto3622   "Sing : Version 2.1a"
#define en__Auto3622   ""
#define fr__Auto3622   ""
#define it__Auto3622   ""
#define es__Auto3622   ""
...
_Context->indsort_hList=NULL;	/* pas de pointeur de table intermédiaire */		_Context->max_hBar=0;
	...
	_Context->x_Auto3622=55;
	_Context->y_Auto3622=80;
	_Context->w_Auto3622=690;
	_Context->h_Auto3622=440;
	...
	_Context->pushbuffer=visual_buffer(_Context->x_Auto3622,_Context->y_Auto3622,690+10,440+10);
	return(0);
}
...
public 	int	accept_visual_popup_hide(struct accept_visual_popup_context * _Context)
{
	...
	&& (_Context->ctrlbuffer != 0)) {
		(void)visual_buffer_put(_Context->pushbuffer,_Context->x_Auto3622,_Context->y_Auto3622);
		_Context->ctrlbuffer=0;
		}
...
private int on_hList_show(struct accept_visual_popup_context * _Context) {
	...
	(_Context->row_hList+6),74);
		...
		visual_table(_Context->x_Auto3622+10,_Context->y_Auto3622+30,644,404,vfh[1],27,28,_Context->msg_hList[_Context->language],_Context->buffer_hList,0x4400,"(N4Z,$)");
;
	return(-1);
}
...
public	int	accept_visual_popup_show(struct accept_visual_popup_context * _Context)
{
	...
	if ((_Context->pushbuffer != (void *) 0)
	&& (!(_Context->ctrlbuffer))) {
		(void)visual_buffer_get(_Context->pushbuffer,_Context->x_Auto3622,_Context->y_Auto3622);
		_Context->ctrlbuffer=1;
		}
	visual_freeze();
	visual_window(_Context->x_Auto3622,_Context->y_Auto3622,690,440,vfh[2],_Context->msg_Auto3622[_Context->language],strlen(_Context->msg_Auto3622[_Context->language]),0x407);
	(void) on_hList_show(_Context);
	visual_scrollbar(_Context->x_Auto3622+660,_Context->y_Auto3622+30,20,400,vfh[1],27,28,_Context->value_hBar,_Context->limit_hBar,_Context->max_hBar,3);
	visual_thaw(_Context->x_Auto3622,_Context->y_Auto3622,690,440);

	return(0);
}

private int Auto3622action(struct accept_visual_popup_context * _Context) {
	...
	do {
		...
		_Context->x_Auto3622 += (visual_event(7)-x);
		x=visual_event(7);
		if (_Context->x_Auto3622 < 0) { _Context->x_Auto3622=0; }
		_Context->y_Auto3622 += (visual_event(6)-y);
		y=visual_event(6);
		if (_Context->y_Auto3622 < 0) { _Context->y_Auto3622=0; }
...
private int on_hBar_action(struct accept_visual_popup_context * _Context) {
	...
	if ( visual_event(6) < (_Context->y_Auto3622+30+20) ) {
		do { while( visual_getch() != 256); } while ( visual_event(1)  != _MIMO_UP );
		_Context->value_hBar -= 1;
		if (_Context->value_hBar < 0) 
			_Context->value_hBar = 0;
		(void) on_hBar_event(_Context);
	visual_scrollbar(_Context->x_Auto3622+660,_Context->y_Auto3622+30,20,400,vfh[1],27,28,_Context->value_hBar,_Context->limit_hBar,_Context->max_hBar,259);
		return(-1);
		}
	if ( visual_event(6) > (_Context->y_Auto3622+30+400-20) ) {
		do { while( visual_getch() != 256); } while ( visual_event(1)  != _MIMO_UP );
		_Context->value_hBar += 1;
	...
	do {
		...
		_Context->value_hBar = (((visual_event(6) - (_Context->y_Auto3622+30+20)) * _Context->max_hBar) / (400 - (2 * 20)));
		if (_Context->value_hBar < 0) 
			_Context->value_hBar = 0;
		else if (_Context->value_hBar > (_Context->max_hBar-_Context->limit_hBar)) 
			_Context->value_hBar = (_Context->max_hBar-_Context->limit_hBar);
...
struct accept_visual_popup_context * _Context){
	...
	switch (mt) {
	case _MIMO_UP :
	if ((mx >= (_Context->x_Auto3622+669) )  
	&&  (my >= (_Context->y_Auto3622+4) )  
	&&  (mx <= (_Context->x_Auto3622+686) )  
	&&  (my <= (_Context->y_Auto3622+20) )) {
		return(1);	/* Auto3622 */

		}
	if ((mx >= (_Context->x_Auto3622+653) )  
	&&  (my >= (_Context->y_Auto3622+4) )  
	&&  (mx <= (_Context->x_Auto3622+670) )  
	&&  (my <= (_Context->y_Auto3622+20) )) {
	...
	if ((mx >= (_Context->x_Auto3622+637) )  
	&&  (my >= (_Context->y_Auto3622+4) )  
	&&  (mx <= (_Context->x_Auto3622+654) )  
	&&  (my <= (_Context->y_Auto3622+20) )) {
	...
	case _MIMO_DOWN :
		if ((mx >= (_Context->x_Auto3622+4) )  
		&&  (my >= (_Context->y_Auto3622+4) )  
		&&  (mx <= (_Context->x_Auto3622+622) )  
		&&  (my <= (_Context->y_Auto3622+20) )) {
			return( Auto3622action(_Context) );
			}
		}
	if (( mx >= (_Context->x_Auto3622+660) ) 
	&&  ( my >= (_Context->y_Auto3622+30) ) 
	&&  ( mx <= (_Context->x_Auto3622+660+20) ) 
	&&  ( my <= (_Context->y_Auto3622+30+400))) {
		return(2); /* hBar */
		}
