...
public	int use_php_pdo()
{
	FILE * h;
	if ( PhpPdo == -1 )
	{
		if (!( h = fopen("./usephppdo","r") ))
			PhpPdo = 0;
		else	
		{ 
			PhpPdo = 1; 
...
static	void	php_class_members(FILE * h, struct form_control *  fptr, struct form_item * iptr, char * classname )
{
	...
	else	
	{
		...
		fprintf(h,"\tpublic    $Direction= 0;\n");
		if ( use_php_pdo() )
		{ fprintf(h,"\tpublic    $usepdo=1;\n"); }
		else
		{ fprintf(h,"\tpublic    $usepdo=0;\n"); }
		fprintf(h,"\tpublic    $collection = NULL;\n");
		fprintf(h,"\tpublic    $error = 0;\n");
		fprintf(h,"\tpublic    $offset= 0;\n");
		fprintf(h,"\tpublic    $limit = 0;\n");
		fprintf(h,"\tpublic    $pages = 0;\n");
		fprintf(h,"\tpublic    $page  = 0;\n");
		...
		fprintf(h,"\t}\n");
		fprintf(h,"\n\tpublic function OnUsePDO($v)\n\t{ $this->usepdo = $v; return; }\n");
	}
}
...
static	void	php_insert_table(FILE * h, struct form_control *  fptr, struct form_item * iptr, char * classname )
{
	...
	{
		fprintf(h,"\n\tpublic function PdoOnInsert()\n\t{\n");
		/* ----------------- */
		/* PDO Prepare Query */
		/* ----------------- */
		fprintf(h,"\t$this->sqlbase->prepare_query(%cINSERT INTO %c . $this->tablename . %c (",__QUOTE,__QUOTE,__QUOTE);
		for (i=0; i < fields; i++ ) 
		{
			if ( i ) fprintf(h,", %s",check_sql_name(dptr->name[i]));
			else	 fprintf(h," %s",check_sql_name(dptr->name[i]));
		...
		fprintf(h,")%c);\n",__QUOTE);
		/* ---------------------- */
		/* Inject Query Variables */
		/* ---------------------- */
		for (i=0; i < fields; i++ ) 
		{
			fprintf(h,"\t$this->sqlbase->query_value(':%s',$this->_%s);\n",dptr->name[i],dptr->name[i]);
		}
		/* ---------------------------- */
		/* Execute the Query Expression */
		/* ---------------------------- */
		fprintf(h,"\tif (!( $this->sqlbase->exec_query($this->connection) )) { $this->error = 78; }\n");
		fprintf(h,"\telse { $this->error = 0; }\n");
		fprintf(h,"\t}\n");
		
		fprintf(h,"\n\tpublic function StdOnInsert()\n\t{\n");
		...
		fprintf(h,"\tif ( $this->usepdo != 0 ) { $this->PdoOnInsert(); }\n");
		fprintf(h,"\telse { $this->StdOnInsert(); }\n");
		fprintf(h,"\t}\n");
	}
...
static	void	php_count_table(FILE * h, struct form_control *  fptr, struct form_item * iptr, char * classname )
{
	...
	if (!(dptr = iptr->Contents.extra ))
		...
		fprintf(h,"\t$result = $this->sqlbase->query(%cSELECT COUNT(*) AS response",__QUOTE);
		fprintf(h," FROM %c . $this->tablename . %c ",__QUOTE,__QUOTE);
		fprintf(h,"WHERE %c . $this->question, $this->connection);\n",__QUOTE);
		fprintf(h,"\tif (($row = $this->sqlbase->fetch($result))!= NULL) ");
		fprintf(h,"{ $this->response = $row['response']; }\n");
		fprintf(h,"\t}\n");
		}
...
static	void	php_collect_table(FILE * h, struct form_control *  fptr, struct form_item * iptr, char * classname )
{
	...
	{
		/* --------------------------------------- */
		/* ON PAGE Calculates the Pagination Infos */
		/* --------------------------------------- */
		fprintf(h,"\n\tprivate function OnPage($nb)\n\t{\n");
		fprintf(h,"\tif (!( $this->limit )) { $this->error = 118; }\n\telse {\n");
		fprintf(h,"\t$this->OnCount();\n");
		fprintf(h,"\t$this->pages = ceil(($this->response / $this->limit));\n");
		fprintf(h,"\tif ($nb > 0) { $this->page = ($nb-1); } else { $this->page = 0; }\n");
		fprintf(h,"\t$this->offset = ($this->page * $this->limit);\n");
		fprintf(h,"\t}\n\t}\n");

		/* ---------------------------------------- */
		/* ON WINDOW Collects FROM offset FOR limit */
		/* ---------------------------------------- */
		fprintf(h,"\n\tpublic function OnWindow($wid=0,$windowsize=200)\n\t{\n");
		fprintf(h,"\t$this->response = 0;\n");
		fprintf(h,"\t$this->limit = $windowsize;\n");
		fprintf(h,"\t$this->OnPage($wid);\n");
		fprintf(h,"\t$this->response = 0;\n");
		fprintf(h,"\t$this->collection = array();\n");
		fprintf(h,"\t$result = $this->sqlbase->query(%cSELECT ",__QUOTE);
		for (i=0; i < dptr->indexfields; i++ ) 
		...
		fprintf(h," FROM %c.$this->tablename.%c",__QUOTE,__QUOTE);
		fprintf(h," WHERE %c.$this->question.%c LIMIT %c.$this->offset.%c,%c.$this->limit, $this->connection);\n",
				__QUOTE,__QUOTE,__QUOTE,__QUOTE,__QUOTE);
		...
		fprintf(h,"\t\t$this->distinct = %c%c;\n",__QUOTE,__QUOTE);
		fprintf(h,"\t\t}\n");
		fprintf(h,"\t}\n");

		/* ----------------------------------- */
		/* ON COLLECT collects from 0 to count */
		/* ----------------------------------- */
...
static	void	php_modif_table(FILE * h, struct form_control *  fptr, struct form_item * iptr, char * classname )
{
	...
	{
		fprintf(h,"\n\tpublic function PdoOnModif()\n\t{\n");
		fprintf(h,"\t$this->sqlbase->prepare_query(%cUPDATE %c . $this->tablename . %c SET",__QUOTE,__QUOTE,__QUOTE);
		for (i=dptr->indexfields; i < fields; i++ ) 
		{
			if ( i > dptr->indexfields ) fprintf(h,", %s = :%s",check_sql_name(dptr->name[i]),dptr->name[i]);
			else	 fprintf(h," %s = :%s",check_sql_name(dptr->name[i]),dptr->name[i]);
		}
		fprintf(h,"\n\tWHERE");
		for (i=0; i < dptr->indexfields; i++ ) 
		{
			if ( i ) fprintf(h," AND %s = :%s",check_sql_name(dptr->name[i]),dptr->name[i]);
			else	 fprintf(h," %s = :%s",check_sql_name(dptr->name[i]),dptr->name[i]);
		}
		fprintf(h,"%c);\n",__QUOTE);
		/* ---------------------- */
		...
		fprintf(h,"\tif ( $this->usepdo != 0 ) { $this->PdoOnModif(); }\n");
		fprintf(h,"\telse { $this->StdOnModif(); }\n");
		fprintf(h,"\t}\n");
	}
