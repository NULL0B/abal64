...
public	void	activate_StatusBar(int mode)
{
	...
	if ((Context.StatusBar.style = mode)!=0) {
		if ((Context.StatusBar.x = (Context.ToolBar.x+Context.ToolBar.w)) > 0)
			Context.StatusBar.x--;
...
public 	void	show_failure_bar_message(int position)
{
	int	boxwidth;
	int	texwidth;
	int	wipewidth;
...
public 	void	failure_message(char * mptr,int position)
{
	if ( Context.compiling )
		return;
	if ( newbar ) 
		status_bar_message(Context.StatusObject, (position | 256), mptr );
		...
		show_failure_bar_message(position);
		}
	return;
}
...
public	void	status_message_length(int position, char * message, int length )
{
	...
	if ( newbar ) 
		...
		if ( bar_message[position] != (char *) 0 )
			liberate(bar_message[position]);
		if (!(sptr = (bar_message[position] = allocate( length+8 ))))
			return;
		else	{
			for (i=0; i < length; i++ )
				*(sptr+i) = *(message+i);
