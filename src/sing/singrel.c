...
public	int	visual_frame( int atx, int aty, int nbx, int nby, int style )
{
	int	bg;
	int	vx,vy;
	...
	if ( uselimits ) {
		...
		if ( atx < 0 ) {
			if ((atx+nbx) < 0) return 0;
			else	{
				nbx += atx;
				atx = 0;
				}
			}
		else if ( atx >= xlimit ) return 0;

		else if ( (atx+nbx) >= xlimit )
			nbx = (xlimit-atx);

		if ( aty < 0 ) {
			if ((aty+nby) < 0) return 0;
			else	{
				nby += aty;
				aty = 0;
				}
			}
		else if ( aty >= ylimit ) return 0;
