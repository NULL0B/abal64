#ifndef	_singjsline_c
#define	_singjsline_c
...
public	int	_JS_include_line( FILE * sh, FILE * th, char * bptr, int  blen )
{
	int	i=0;
	int	c;
	int	status=0;
	...
	if (!( i ))
		...
		*(bptr+i) = 0;
		indent(th,1);
		status = _JS_method_line(th, bptr, i, LineParser.thisname, &feeder );
		linefeed(th);
		return(status);
		}
}

public	int	_JS_include_source( FILE * h, char * tptr )
{
	FILE *	sh;
	char *	bptr;
	int	failure=0;
	...
	if (!( tptr ))	
		...
		if ( sh != (FILE *) 0) {
			while (!(failure = _JS_include_line(sh,h,bptr,2040)));
...
private	char *	_JS_method_statement( FILE * h, char * tptr )
{
	int	c;
	int	token;
	...
	if ( string_compare( tptr, keyword_include ) ) {
		if (!( tptr = _JS_get_token(0) )) {
			_JS_syntax_error( keyword_include,1 );
			return((char *) 0);
			}
		else	{
			_JS_include_source(h,tptr);		
			return((char *) 0);
			}
		}

	else if ((mptr = is_forms_Method( tptr, LC.form )) != (struct forms_method *) 0 ) {
		if (!(c = _JS_get_punctuation() ))
			return( tptr );
		else if (c != '(' )
			return( tptr );
		...
		switch ( is_visual_member( tptr, 1 ) ) {
			...
			case	VISUAL_MEMBER_ONHIDE	:
				if (!( remove_brackets() ))
					...
					_JS_call(h,C_Config.cname, __method_hide, 0,1,(char *) 0 );
					return((char *) 0);
					}
			...
			case	VISUAL_MEMBER_ONSHOW	:
				if (!( remove_brackets() ))
					...
					_JS_call(h,C_Config.cname,__method_show, 0,1,(char *) 0 );
					return((char *) 0);
					}
			case	VISUAL_MEMBER_ONFOCUS	:
				if (!( remove_brackets() ))
					...
					_JS_call(h,C_Config.cname,__method_focus, 0,1,(char *) 0 );
					return((char *) 0);
					}
			}
		return( tptr );
...
private	char *	_JS_macro_reset( FILE * h, struct form_item * iptr )
{
	if ( iptr->Contents.events->xo_create & _EVENT_IS_FOCUS ) {
		/* Generate Position and Dimensions Variables */
#ifdef	_JS_IS_POSSIBLE
		fprintf(h,"%s.x_%s=%s;",__JS_instance_prefix,iptr->Contents.name,widget_x_position(iptr,1));
		fprintf(h,"%s.y_%s=%s;",__JS_instance_prefix,iptr->Contents.name,widget_y_position(iptr,1));
		fprintf(h,"%s.w_%s=%s;",__JS_instance_prefix,iptr->Contents.name,widget_w_dimension(iptr,1));
		fprintf(h,"%s.h_%s=%s;",__JS_instance_prefix,iptr->Contents.name,widget_h_dimension(iptr,1));
...
private	char *	_JS_macro_column( FILE * h, struct form_item * iptr, char * tptr )
{
	...
	if (!( tptr )) {
		...
		_JS_syntax_error( iptr->Contents.name,2 );
		return((char *) 0);
		}
		...
		fprintf(h,"(%s.row_%s+%u)",
			__JS_instance_prefix,iptr->Contents.name,offset);
		return((char *) 0);
		}
	else	{
		...
		)
{
	if (!( iptr->Contents.name ))
		...
		fprintf(h,"%s.row_%s=",__JS_instance_prefix,iptr->Contents.name);
		fprintf(h,"&%s.buffer_%s[%s-1][0]; } ",__JS_instance_prefix,iptr->Contents.name,tptr );
		return((char *) 0);
		}	
}

public	char *	_JS_macro_linesort( 
		FILE * h, struct form_item * iptr, char * tptr 
		)
{
int linelength,nblines;
	...
	if (!( iptr->Contents.name ))
		...
		fprintf(h,"&%s.libsort_%s[(%s-1)*%d]; } ",__JS_instance_prefix,iptr->Contents.name,tptr,linelength );
		return((char *) 0);
		}	
}
...
private	char *	_JS_column_variable(FILE * h, struct form_item * iptr )
{
	if ((iptr->Contents.style & _FRAME_TYPE) != _FORM_FRAME)
		...
		fprintf(h,"%s.column_%s",__JS_instance_prefix,iptr->Contents.name);
		return((char *) 0);
		}
}
...
private	char *	_JS_line_variable(FILE * h, struct form_item * iptr )
{
	if ((iptr->Contents.style & _FRAME_TYPE) != _FORM_FRAME)
		...
		fprintf(h,"%s.value_%s",__JS_instance_prefix,iptr->Contents.name);
		return((char *) 0);
		}
}
...
private	char *	_JS_member_linesort(FILE * h, struct form_item * iptr )
{
	int	c;
	char * tptr;
	if (!(c = _JS_get_punctuation() ))
		...
		if (!( tptr = _JS_get_token(0) ))
			...
			return( (char *) 0 );
		else 	return( _JS_macro_linesort( h, iptr, tptr ) );
		}
	else 	{
		parser_ungetch();
		return( (char *)0 );
...
public	char *	_JS_widget_overlay_method( FILE * h, struct form_item * iptr, char * tptr )
{
	int	c;
	struct	forms_method * mptr;
	...
	if (!( sptr = _JS_payload( iptr )))
		...
		for (	mptr=fptr->rootmethod;
			...
			if (!( string_compare( tptr, mptr->name )))
				...
				fprintf(h,"%s_%s(%s.%s",sptr,tptr,__JS_instance_prefix,iptr->Contents.name);
				if (method_is_valid(mptr->signature) )
					fprintf(h,",");
				break;
				}
...
public	void	invoke_c_widget_help( FILE * h, struct form_item * iptr )
{
	if ((iptr->Contents.style & _FRAME_TYPE ) != _RADIO_FRAME)
		fprintf(h,"%s_help(%s)",iptr->Contents.name,__JS_instance_prefix);
	else	fprintf(h,"%s%u_help(%s)",iptr->Contents.name,iptr->Contents.radio,__JS_instance_prefix);
	return;
}
...
private	char *	_JS_convert_token( FILE * h, char * tptr )
{
	...
	&&  ( string_compare( tptr, keyword_this ) ))
		...
		fprintf(h,"%s.keycode",__JS_instance_prefix);
		return((char *) 0);
		}
		...
		return( _JS_method_statement( h, xtptr ) );
		}
	...
	switch ( i ) {
		...
		case	VISUAL_MEMBER_VALUE	:
			switch ((iptr->Contents.style & _FRAME_TYPE)) {
				...
				case	_FORM_FRAME	:
				case	_EDIT_FRAME	:
					fprintf(h,"Conv(%s.buffer_%s)",__JS_instance_prefix,iptr->Contents.name);
					break;
				default			:
		...
		case	VISUAL_MEMBER_HINT	:
			_JS_hint_value( h, iptr );
			return((char *) 0);

		case	VISUAL_MEMBER_PAYLOAD	:
			_JS_payload_value(h,iptr);
			return((char *) 0);
		case	VISUAL_MEMBER_BUFFER	:
			fprintf(h,"%s.buffer_%s",__JS_instance_prefix,iptr->Contents.name);
			return((char *) 0);
		case	VISUAL_MEMBER_LIMIT	:
			fprintf(h,"%s.limit_%s",__JS_instance_prefix,iptr->Contents.name);
			return((char *) 0);

		case	VISUAL_MEMBER_MAX	:
			fprintf(h,"%s.max_%s",__JS_instance_prefix,iptr->Contents.name);
			return((char *) 0);
		...
		case	VISUAL_MEMBER_COLUMNSIZE	:
			if (!( tptr = _JS_dimension() ))
				...
				return( _JS_macro_columnsize( h, iptr, tptr ) );
				}
		...
		case	VISUAL_MEMBER_POSITION :
			if (!( tptr = _JS_dimension() ))
				...
				return( _JS_macro_position( h, iptr, tptr ) );
				}
		...
		case	VISUAL_MEMBER_FORMSORT	:
			fprintf(h,"%s.formsort_%s",__JS_instance_prefix,iptr->Contents.name);
			return((char *) 0);
		...
		case	VISUAL_MEMBER_TABSORT	:
			if (!( tptr = _JS_dimension() ))
				...
				fprintf(h,"sort_item_%s(%s,%s)",iptr->Contents.name,__JS_instance_prefix,tptr);
				return((char *) 0);
				}

		case	VISUAL_MEMBER_LINESORT  	:
			return( _JS_member_linesort(h, iptr ) );

		case	VISUAL_MEMBER_START_SORT	:
			if (!( tptr = _JS_dimension() ))
				break;
			else	{
				fprintf(h,"start_sort_%s(%s,%s)",iptr->Contents.name,__JS_instance_prefix,tptr);
				return((char *) 0);			
				}
		...
		case	VISUAL_MEMBER_SORT	:
			if (!( tptr = _JS_dimension() ))
				...
				fprintf(h,"sort_%s(%s,%s)",iptr->Contents.name,__JS_instance_prefix,tptr);
				return((char *) 0);				
				}
		...
		case	VISUAL_MEMBER_ONHIDE	:
			...
			if (( iptr->Contents.style & _FRAME_TYPE) == _DATA_FRAME) {
				_JS_hide_visual_widget(h,iptr,1);
				}
			return((char *) 0);
		...
		case	VISUAL_MEMBER_ONFOCUS	:
			if (!( remove_brackets() ))
				break;
			else	_JS_onfocus_widget(h,iptr,1);
			return((char *) 0);
		...
		case	VISUAL_MEMBER_ONSHOW	:
			...
			if ((iptr->Contents.style&_FRAME_TYPE) != _TAB_FRAME) {
				if (!( _JS_onshow_widget(h,iptr,1,0) ))
					fprintf(h,"on_%s_%s(%s);",iptr->Contents.name,tptr,__JS_instance_prefix);
				}
		...
		case	VISUAL_MEMBER_FILL  	:
			...
			fprintf(h,"%s.%s=%s(%s,%s,%s,%s,%u,0)",
				__JS_instance_prefix,"status",__visual_filzone,
				widget_x_position(iptr,0),
				widget_y_position(iptr,0),
				widget_w_dimension(iptr,0),
				widget_h_dimension(iptr,0),
