	...
	for (	; argi < argc; argi++ )  
	{
		...
		else if ( *(aptr++) == '-' ) 
		{
			...
			case	'L'	:
				...
				case	'j'	:
				case	'J'	:
					language = _GENERATE_ANDROID;
					continue;
				case	'r'	:
				case	'R'	:
					language = _GENERATE_REACT;
					continue;
				case	'l'	:
				case	'L'	:
...
/*	------------------------	*/
public	int	generate_configuration( char * nomfic )
{
	if (!( nomfic ))
		nomfic = "example";
	else if (!( strlen( nomfic ) ))
		nomfic = "example";
	else	
	{
		LoadConfiguration( nomfic );
		save_configuration( nomfic );
		return(0);
	}
}
...
public	int	commandline( int argc, char * argv[] )
{
	...
	for (argi=1; argi < argc; argi++ )  
	{
		...
		case	'g'	:
		case	'G'	:
			return( generate_configuration( aptr ) );
		case	'c'	:
		case	'C'	:
