...
/*	-----------------------------------------------------------------------	*/
/*			l a s e r _ o p e n _ t a b _ g r o u p 		*/
/*	-----------------------------------------------------------------------	*/
/*	opens a named tab group element or indicated visibility for use by both	*/
/*	tab buttons and by tab frames.						*/ 
/*	-----------------------------------------------------------------------	*/
void	laser_open_tab_group( FILE * handle, struct form_item * iptr, char * prefix, char * visibility, int action )
{
	struct	form_control * fptr;
	if (!( fptr = iptr->parent ))
		return;
	...
	if ( action )
	{
		fprintf(handle," onclick=%c%s%s('%s')%c",
			0x0022,_TRIGGER_PREFIX,fptr->identifier,iptr->Contents.name,0x0022);
	}
	fprintf(handle,">\n");
	mpeg_enter();
	return;
...
void	laser_tab_button( FILE * handle, struct form_item * iptr )
{
	...
	laser_comment(handle,"tab page button");

	laser_widget_group(handle,iptr);

	laser_open_tab_group(handle, iptr, _LOSE_PREFIX, 
		(iptr->Contents.page != 1 ? "inherit" : "hidden"),
		1);
	...
	laser_open_tab_group(handle, iptr, _PRESS_PREFIX, (iptr->Contents.page == 1 ? "inherit" : "hidden"),0);
...
/*	-----------------------------------------------------------------------	*/
void	laser_frame_script( FILE * handle, struct form_control * fptr, struct form_item * iptr )
{
	int	items=0;
	struct	form_item * wptr;
	laser_open_script(handle);
		...
		/* -------------------------------------- */
		/* create the current page frame  pointer */
		/* -------------------------------------- */
		laser_indent(handle);
		...
		/* -------------------------------------- */
		/* create the current page releas button  */
		/* -------------------------------------- */
		laser_indent(handle);
		fprintf(handle,"var js%s%s = document.getElementById(%c%s%s%c);\n",
		...
		/* --------------------------------------------------------------- */
		laser_indent(handle);
		fprintf(handle,"function %s%s(pagename) {\n",_TRIGGER_PREFIX,fptr->identifier);
		mpeg_enter();

			/* ------------------------------------------- */
			/* release an eventual current page controller */
			/* ------------------------------------------- */
			laser_indent(handle);
			fprintf(handle,"if ( js%s%s ) { js%s%s.setAttribute(%c%s%c,%c%s%c); }\n",
				_LOSE_PREFIX,fptr->identifier,
				_LOSE_PREFIX,fptr->identifier,
				0x0022,"visibility",0x0022,
				0x0022,"inherit",0x0022);
			...
			/* --------------------------------------- */
			/* resolve the current page frame  pointer */
			/* --------------------------------------- */
			laser_indent(handle);
			fprintf(handle,"js%s%s = document.getElementById(%c%s%c+pagename);\n",
				_FRAME_PREFIX,fptr->identifier,
				0x0022,_FRAME_PREFIX,0x0022);
			...
			/* --------------------------------------- */
			/* resolve the current page releas button  */
			/* --------------------------------------- */
			laser_indent(handle);
			fprintf(handle,"js%s%s = document.getElementById(%c%s%c+pagename);\n",
				_LOSE_PREFIX,fptr->identifier,
				0x0022,_LOSE_PREFIX,0x0022);
...
/*	-----------------------------------------------------------------------	*/
/*				l a s e r _ t a b _ f r a m e			*/
/*	-----------------------------------------------------------------------	*/
/*	generates a laser tab frame, closing an open one if needed.		*/
/*	tab frames are grouping elements that are hidden, except the first.	*/
/*	-----------------------------------------------------------------------	*/
int	laser_tab_frame( FILE * h, struct form_item * iptr, struct form_control * fptr, int started )
{
	...
	laser_open_tab_group(h, iptr, _FRAME_PREFIX, (iptr->Contents.page == 1 ? "inherit" : "hidden"),0);
		laser_relief( h, 
			iptr->Contents.x,
	...
	if ( fptr->first )
	{
		...
		if ( fptr->pages )
		{
			...
			if ( started )
			{
				...
				if ( iiptr )
				{
					laser_frame_script( h, fptr, iiptr );
				}
				started=0;
				/* --------------------------------- */
