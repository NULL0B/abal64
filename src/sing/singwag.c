...
/*	------------------------------------------------------------------------------	*/
/*	This function control the values controlling the nature of the web agent form	*/
/*	Http Form Instructions may use the POST or GET method.				*/
/*	------------------------------------------------------------------------------	*/
private	struct  web_agent_http_forms {
	char *	method;
	char *	nature;
	} AgentForm = { "GET", 0 };
...
public	void	set_agent_method( int mode, char * sptr)
{
	AgentForm.method = sptr;
	AgentForm.nature = mode;
	return;
}

public	char * get_agent_method()
{
	return( AgentForm.method );
}

public	int	get_agent_nature()
{
	return( AgentForm.nature );
}

public	void	server_interface_record(char * n, int i, int o);
...
private	int	is_widget_prefixed()
{
	if (!( current_widget_prefix ))
		return(0);
	else if (!( *current_widget_prefix ))
		return(0);
	else	return(1);
}
			...
			)
{
	fprintf(h,"function _SingApplicationWindow() {"); linefeed(h);
		indent(h,1); fprintf(h,"var __lsw=0;"); linefeed(h);
		javascript_reposition_window(h,wptr);
		indent(h,1); fprintf(h,"__lsw = window.open('#','_SingAw_%s','",fptr->identifier);
		if ( decalagex )
		...
		linefeed(h);
		indent(h,1); fprintf(h,"if ( __lsw ) { __lsw.focus(); }"); linefeed(h);
		js_close_method(h);
			...
			)
{
	fprintf(h,"function _SingNavigatorWindow() {"); linefeed(h);
		indent(h,1); fprintf(h,"var __lsw=0;"); linefeed(h);
		indent(h,1); fprintf(h,"__lsw = window.open('#','_SingNw_%s','",fptr->identifier);
...
private	void	javascript_restriction(FILE * h, struct form_item  * wptr)
{
	...
	if ( fptr->formflags & _MULTILINGUAL ) {
		fprintf(h,"function _SingBrowserLang() {");	linefeed(h);
			indent(h,1);	fprintf(h,"document.forms[0].Method.value='NatLang';"); linefeed(h);
...
{
	if ( Restriction & 2 ) {
		/* Inhibit Document Contextual Menu */
		/* -------------------------------- */
		fprintf(h,"document.oncontextmenu=new Function(%creturn false;%c);",__QUOTE,__QUOTE);
		linefeed(h);
		}
			...
			switch ( ( iptr->Contents.style & _FRAME_TYPE ) ) {
				...
				case	_FORM_FRAME	:
					if ( method_is_valid( iptr->Contents.events->on_event )) {
						...
						indent(h,1);	fprintf(h,"document.forms[0].submit();");	linefeed(h);
						/* ----------------------------------------------------------------------- */
						/* added to correct bug when using form event to load xml or other objects */
						/* otherwise the back browser button leaves this variable set to this last */
						/* value and causes problems at the server side. IJM 28/09/2008            */
						/* ----------------------------------------------------------------------- */
						indent(h,1);	fprintf(h,"document.forms[0].%sEvent.value=%c0,0%c;",prefixed_widget_name(iptr),0x0022,0x0022);	linefeed(h);
						indent(h,1);	fprintf(h,"return(false);");			linefeed(h);
						js_close_method(h);
						}
					if ( method_is_valid( iptr->Contents.events->on_lose_focus )) {
...
private	void	ClientSideMethod( FILE * h, struct form_item * iptr, char * methodname, char * methodtext )
{
	if ( methodname ) {
		linefeed(h);
		fprintf(h,"function %s_on_%s() {",prefixed_widget_name(iptr),methodname); linefeed(h);
		if ( methodtext ) {
			indent(h,1); fprintf(h,"%s",methodtext);
			}
		js_close_method(h);
		}
	return;
...
private	void	javascript_widget_inline( FILE * h, struct form_control * fptr, int page )
{
	struct	form_item * iptr;
	struct	form_item * wptr=(struct form_item *) 0;
	char		  * sptr;
	...
	for (	iptr=fptr->first;
		...
		if (( iptr->Contents.page ) && ( iptr->Contents.page != page ))
			continue;
#ifdef	INLINE_WIDGET_CREATE
		if ((!( iptr->Contents.events->xo_create & _EVENT_IS_FUNCTION ))
		&&  ( method_is_valid((sptr = iptr->Contents.events->on_create))))
			ClientSideMethod(h,iptr,"create",sptr);
#endif
#ifdef	INLINE_WIDGET_REMOVE
		if ((!( iptr->Contents.events->xo_remove & _EVENT_IS_FUNCTION ))
		&&  ( method_is_valid((sptr = iptr->Contents.events->on_remove))))
			ClientSideMethod(h,iptr,"remove",sptr);
#endif
		if ((!( iptr->Contents.events->xo_show & _EVENT_IS_FUNCTION ))
		&&  ( method_is_valid((sptr = iptr->Contents.events->on_show))))
			ClientSideMethod(h,iptr,"show",sptr);
		if ((!( iptr->Contents.events->xo_hide & _EVENT_IS_FUNCTION ))
		&&  ( method_is_valid((sptr = iptr->Contents.events->on_hide))))
			ClientSideMethod(h,iptr,"hide",sptr);
		if ((!( iptr->Contents.events->xo_get_focus & _EVENT_IS_FUNCTION ))
		&&  ( method_is_valid((sptr = iptr->Contents.events->on_get_focus))))
			ClientSideMethod(h,iptr,"get_focus",sptr);
		if ((!( iptr->Contents.events->xo_lose_focus & _EVENT_IS_FUNCTION ))
		&&  ( method_is_valid((sptr = iptr->Contents.events->on_lose_focus))))
			ClientSideMethod(h,iptr,"lose_focus",sptr);
		if ((!( iptr->Contents.events->xo_event & _EVENT_IS_FUNCTION ))
		&&  ( method_is_valid((sptr = iptr->Contents.events->on_event))))
			ClientSideMethod(h,iptr,"event",sptr);
		}
	return;
}
		...
		switch ( ( iptr->Contents.style & _FRAME_TYPE ) ) {
			...
			case	_SCROLL_FRAME	:
				...
				indent(h,1);
					...
					linefeed(h);
				indent(h,1);	
					fprintf(h,"Is%sBarDown=0;",prefixed_widget_name(iptr));	
					linefeed(h);
				indent(h,1);	
					fprintf(h,"%sBarTop=%u;",prefixed_widget_name(iptr),iy); 
					linefeed(h);
				indent(h,1);	
				...
				indent(h,1);
					...
					linefeed(h);
					if ( iptr->Contents.h > iptr->Contents.w )
						fprintf(h,"v.style.top  = %sBarTop;",prefixed_widget_name(iptr));
					else	fprintf(h,"v.style.left = %sBarTop;",prefixed_widget_name(iptr));
					linefeed(h);
					fprintf(h,"}");
					linefeed(h);
				indent(h,1);
...
private	void	prefix_javascript_widget_inline( FILE * h, struct form_control * fptr, char * prefix,int page )
{
	char * rptr = set_widget_prefix( prefix );
	javascript_widget_inline( h,fptr,page );
	set_widget_prefix( rptr );
	return;
}
...
private	void	simple_visual_script( FILE * h, struct form_control * fptr, struct form_item * wptr, int page )
{
	...
	prefix_javascript_widget_inline(h,fptr,"\0",page);
...
{
	server_interface_record(cptr,_WSI_IMAGE,0);
...
private	void	simple_styled_text_font( FILE * h, int font )
{
	...
	if (!( font ))
		...
		fprintf(h," font-size: %u%s;",html_scale_quantity(fh-(fh/4)),_metrics);
		return;
		}
}
...
private	void	simple_window_style( FILE * h, struct form_item * iptr, int wx, int wy )
{
	...
	fprintf(h,"#sing { background-color: silver; margin: 0px; padding: 0px; }");
	linefeed(h);
...
private	void	agent_window_lang_event(FILE * h, struct form_item * iptr, int fw, int fh )
{
	int	x;
	int	y;
	if (!( iptr ))	return;
	...
	html_open_tag(h,"area");
	html_field(h,"shape","rect");
	html_field(h,"title","change national language");
	simple_area_over_and_out( h, iptr );
	fprintf(h," onclick=%c_SingBrowserLang();%c ",__QUOTE,__QUOTE);
	simple_area_cursor(h);
...
private	void	simple_agent_window_events(FILE * handle, struct form_item * iptr )
{
	...
	if ( fptr->formflags & _MULTILINGUAL ) {
		agent_window_lang_event(handle, iptr,fw, fh );
		}
	return;
}
...
private	void	simple_inline_method( FILE * h, struct form_item * iptr, char * methodname )
{
	if (!( iptr ))
		return;
	else if (!( h ))
		...
		fprintf(h," onclick=%c%s_on_%s();%c",__QUOTE,iptr->Contents.name,methodname,__QUOTE);
		return;
		}
}
...
private	void	simple_image_map( FILE * h, struct form_control * fptr, int pageid )
{
	...
	int			hh;
	char			* sptr;
	char			* oldprefix=(char *) 0;
	struct	form_control 	* vfptr;
	struct	form_item 	* iptr;
	struct	form_item 	* wptr=(struct form_item *) 0;

	if (!( is_widget_prefixed() ))
		...
		switch (( iptr->Contents.style & _FRAME_TYPE )) {
			...
			case	_DATA_FRAME	:
				if (( iptr->Contents.align & 0x000F ) != _VISUAL_FRAME)
					...
					oldprefix = set_widget_prefix( iptr->Contents.name );
					simple_image_map(h,vfptr,0);
					set_widget_prefix( oldprefix );
					continue;
					}
				...
				switch ( iptr->Contents.datatype ) {
					...
					default			:
						if ( iptr->Contents.events->xo_event & _EVENT_IS_FUNCTION )
							...
							html_submit_field(h,iptr,(char *) 0,"Event");			
						else 	simple_inline_method(h,iptr,"event");
						break;
					}
				simple_area_cursor(h);
				if ( wptr ) {
...
/*	----------------------------------------------------------------------------------------------------		*/
/*			s i m p l e _ f o r m _ i m a g e _ n a m e ( nptr, page, prefix )				*/
/*	----------------------------------------------------------------------------------------------------		*/
/*	generation of a simple form image name for a tab page or standard form page.					*/
/*	----------------------------------------------------------------------------------------------------		*/
...
/*	----------------------------------------------------------------------------------------------------		*/
/*				s i m p l e _ f o r m _ i m a g e ( h, iptr )						*/
/*	----------------------------------------------------------------------------------------------------		*/
/*	generation of a simple form image widget as an html entity.							*/
/*	----------------------------------------------------------------------------------------------------		*/
...
private	int	simple_form_image(  FILE * h, struct form_control * fptr, struct form_item * wptr, char * nptr, int pageid, char * prefix )
{
	...
	if (!( iname = simple_form_image_name( nptr, pageid, prefix ) ))
		...
		fprintf(h,">");
		if (!( ActiveAgentImages )) {
			server_interface_record(iname,_WSI_IMAGE,0);
			}
		if ( iname ) {
			liberate( iname );
			}
...
/*	----------------------------------------------------------------------------------------------------		*/
/*				s i m p l e _ f o r m _ e d i t  ( h, iptr )						*/
/*	----------------------------------------------------------------------------------------------------		*/
/*	generation of a simple form edit widget as an html entity.							*/
/*	----------------------------------------------------------------------------------------------------		*/
...
private	void	simple_form_edit( FILE * h, struct form_item * iptr )
{
	...
	if ( iptr->Contents.datatype == _WIDGET_HYPERLINK )
		...
		if ( nblines > 1 ) {
			...
			/* ---------------------- */
			if ( iptr->Contents.align & _A_LINE ) {
				fprintf(h,"<input id=%c%s%c name=%s type=password",
				__QUOTE,prefixed_widget_name(iptr),__QUOTE,
...
/*	----------------------------------------------------------------------------------------------------		*/
/*			s i m p l e _ w i d g e t _ c e l l s ( h, fptr, pageid )					*/
/*	----------------------------------------------------------------------------------------------------		*/
/*	performs the generation of simple widget components as html web page entities. This may be prefixed 		*/
/*	for use by complex components.											*/
/*	----------------------------------------------------------------------------------------------------		*/
...
/*	----------------------------------------------------------------------------------------------------		*/
/*		   p r e f i x _ s i m p l e _ w i d g e t _ c e l l s (h, fptr, paheid,prefix )			*/
/*	----------------------------------------------------------------------------------------------------		*/
/*	Generates component prefixed widget cells. The preceeding component prefix is saved and restored		*/
/*	around the production of the forms items.									*/
/*	----------------------------------------------------------------------------------------------------		*/
...
/*	----------------------------------------------------------------------------------------------------		*/
/*			s i m p l e _ h t m l _ i m a g e _ f o r m ( h,fptr,wptr, nptr, page )				*/
/*	----------------------------------------------------------------------------------------------------		*/
/*	Initiates production of image base html agent pages for a non-tab window forms model.				*/
/*	A single html page is generated to represent the forms model interface.						*/
/*	----------------------------------------------------------------------------------------------------		*/
...
private	int	simple_html_image_form(  FILE * h, struct form_control * fptr, struct form_item * wptr, char * nptr, int pageid, char * prefix )
{
	...
	html_field(h,"method",get_agent_method());
...
/*	----------------------------------------------------------------------------------------------------		*/
/*			s i m p l e _ h t m l _ i m a g e _ t a b s e t ( h,fptr,wptr, nptr )				*/
/*	----------------------------------------------------------------------------------------------------		*/
/*	Initiates production of image base html agent pages for a tab window forms model.				*/
/*	Individual html pages are generated for each tab page.								*/
/*	----------------------------------------------------------------------------------------------------		*/
...
/*	----------------------------------------------------------------------------------------------------		*/
/*		e x p o r t _ h t m l _ i m a g e _ f o r m ( h,mode,auto,restrikt,para,points, meth )			*/
/*	----------------------------------------------------------------------------------------------------		*/
/*	Initiates production of the web pages required for the representation of a forms model as a web agent		*/
/*	Two different models are possible 										*/
/*		- complex page based around a collection of tab page compoents						*/
/*		- simple page without tab page comonents.								*/
/*	each of these individual cases is despatched to a different production routine.					*/
/*	----------------------------------------------------------------------------------------------------		*/

public	int	export_html_image_form(  FILE * h, int mode, int automatic, int restrikt, int parametric, int points, int hMethod )
{
	int	i;
	int	autotrad=0;
	struct	form_control 	* fptr;
