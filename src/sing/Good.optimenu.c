...
/* Target         : optimenu.c  */
/* Identification : 0.0-1094514228-2241.2240 */
/* ------------------------------------------------------------------------- */
...
private int on_Frame_create() {
	mouse_move_on();
	return(-1);
}

public	int	standard_option_menu_create()
{
...
public 	int	standard_option_menu_hide()
{
...
private int on_Frame_remove() {
	mouse_move_off();
	return(-1);
}

public	int	standard_option_menu_remove()
{

	int	result=_Context->keycode;
	(void) on_Frame_remove();
	if (_Context->p_Frame != (void *) 0)
...
public	int	standard_option_menu_show()
{
	_Context->language = visual_national_language(-1);
	visual_freeze();
	if ((_Context->p_Frame != (void *) 0)
...
private int on_Auto1607_event() {
	int result;
		standard_option_menu_hide();
;
	on_Frame_remove();
	result=general_parameters();
	on_Frame_create();
		standard_option_menu_show();
;
	return(result);
	return(-1);
}
private int on_Auto1609_event() {
	int result;
		standard_option_menu_hide();
;
	on_Frame_remove();
...
public	int	standard_option_menu_focus()
{
	int retcode=0;
...
retry:
		...
		case	0x2 :
			...
			case	0x100 :
				...
				if ((retcode=standard_option_menu_event()) == -1)
					continue;
				switch ((_Context->focus_item = retcode)) {
					case	0x1 :
						...
						if ((_Context->keycode = on_Auto1607_event()) != -1) break;
					...
					case	0x2 :
						...
						if ( visual_event(1) &  0x20 ) {
						if ((_Context->keycode = on_Auto1609_event()) != -1) break;
					...
					case	0x3 :
						...
						if ( visual_event(1) &  0x20 ) {
						if ((_Context->keycode = on_Auto1610_event()) != -1) break;
					...
					case	0x4 :
						...
						if ( visual_event(1) &  0x20 ) {
						if ((_Context->keycode = on_Auto1611_event()) != -1) break;
					...
					case	0x5 :
						...
						if ( visual_event(1) &  0x20 ) {
						if ((_Context->keycode = on_Auto1675_event()) != -1) break;
					...
					case	0x6 :
						...
						if ( visual_event(1) &  0x20 ) {
						if ((_Context->keycode = on_Auto1912_event()) != -1) break;
...
{
	int	status=0;
	status = standard_option_menu_create();
	if ( status != 0) return(status);
	status = standard_option_menu_show();
		enter_modal();
	status = standard_option_menu_focus();
		leave_modal();
	status = standard_option_menu_hide();
	status = standard_option_menu_remove();
