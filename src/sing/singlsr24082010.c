...
void	laser_pushbutton( FILE * h, struct form_item * iptr )
{
	...
	default			:
		if (method_is_valid(iptr->Contents.events->on_event) )
			started = laser_method_group( h, iptr, _EVENT_PREFIX );
		laser_button(
			h,
			iptr->Contents.x, iptr->Contents.y, 
			iptr->Contents.w, iptr->Contents.h, 
...
void	laser_radio_script( FILE * handle, struct form_item * iptr )
{
	...
	laser_open_script(handle);
		...
		mpeg_enter();
			...
			/* ------------------------------------------------ */
			laser_indent(handle);
//			fprintf(handle,"var previousnumber = item.textContent;\n");
...
void	LASER_widget( FILE * h, struct form_item * iptr )
{
	...
	case	_TEXT_FRAME	:	//	6
		...
		laser_text(
			...
			iptr->Contents.w, iptr->Contents.h, 
			abal_payload( iptr ),
			laser_palette_label, /* iptr->Contents.colour, */
			iptr->Contents.align
			);
		laser_groupend(h);
		break;
...
/*	-----------------------------------------------------------------------	*/
int	laser_unquoted_anchor( FILE * h, char * nptr )
{
	char	work[1024];
	if (!( nptr )) return(0);
...
int	laser_anchor( FILE * h, char * nptr, int nested )
{
	...
	if (!( nptr ))
		...
		return( laser_unquoted_anchor( h, (nptr+1) ) );
	...
	if ( fptr->first )
	{
			...
			iptr = iptr->next )
		{
			...
			if (method_is_valid((sptr = iptr->Contents.events->on_event)))
				started = laser_widget_method( h, iptr, _EVENT_PREFIX,sptr,started ); 
...
void	laser_standard_script( FILE * h,struct form_control * fptr)
{
	...
	if ( LC.soapInterface )
	{
		...
		mpeg_enter();
			...
			{
				laser_indent(h);
				fprintf(h,"alert('Asynch Soap Request:\\n'+workname+': '+message);\n");
			}
			/* ------------------------------- */	
			/* Ajax Server Request Preparation */
			/* ------------------------------- */			
			...
			fprintf(h,"if ( server.readyState == 4 ) {\n");
				...
				fprintf(h,"if ( server.status == 200 ) {\n");
					...
					{
						laser_indent(h);
						fprintf(h,"alert( 'Asynch Soap Response:\\n'+server.responseText );\n");
					}
					laser_js_get_cookie(h);
					laser_indent(h);
					fprintf(h,"var thisitem=workroot.firstChild;\n");
