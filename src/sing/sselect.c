		...
		int first,int visible,int align )
{
	...
	if (!(use_default_style( visual_select_framedown ) ))
		visual_element(x,y,w-fh,(visible*fh)+2,"selectframe"," ",0);			
	else	visual_element(x,y,w-fh,(visible*fh)+2,"selectframedown"," ",0);
		...
		)
{
	...
	/* ----------------------- */
	if ( w > fh ) {
		if (!(use_default_style(visual_select_frameup)))
			visual_element( x,y,(w-fh),fh+2,"selectframe"," ",0);
		else	visual_element( x,y,(w-fh),fh+2,"selectframeup"," ",0);
		}
	if ( w > fh ) {
		if (!(use_default_style(visual_select_framebtn)))
			visual_element( x+(w-fh),y,fh,fh+2,"selectframe"," ",0);
		else	visual_element( x+(w-fh),y,fh,fh+2,"selectframebtn"," ",0);
		}
	if ( w > fh )
		visual_element( x+(w-fh),y,fh,fh+2,arrow," ",0);
	...
	if ( w > fh ) {
		...
		&&  (( mptr = sptr->messages[sptr->current] ) != (char *) 0 )) {
			if ((( w - (fh+1)) ) > 0) {
				visual_element(	x, y,(w-fh),(fh+2),
					message,mptr,strlen(mptr));
				}
			}
		}
	return(-1);
