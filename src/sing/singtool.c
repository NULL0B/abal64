...
#define	_LOCK_ICON 	"lockicon.gif\0            "
...
private struct	tool_manager * AbalTools=(struct tool_manager*) 0;
private struct	tool_manager * CssTools=(struct tool_manager*) 0;
...
private	char 	IconWork[512];
private	char 	IconWorkDes[512];
...
public	void	show_sprite( char * ibuffer, int atx, int aty )
{
	...
	if (!( Sprite.buffer )) {
		...
		if (!( Sprite.buffer = allocate(((Sprite.height*Sprite.width)*pixelsize)+16) ))
			return;
		}
...
private	char *	load_icon( char * nptr, int bg )
{
	...
	if (find_image_byexa( IconWork, IconWorkDes  ))
		nptr = IconWorkDes;
	else 
	nptr = IconWork;
#else
	...
	if (!( iptr = local_image_loader( nptr )))
		return( (char *) 0 );
	else if (!( iconbuffer = allocate( iconw * iconh * pixelsize ) ))
		return( iconbuffer );
	else	{
		for (	r = 0 ;
			r < iconh;
			...
			/* -------------- */
			if ( colourmaped ) 
				memset((iconbuffer+(r*(iconw*pixelsize))),bg, iconw*pixelsize );
			else	{
				for (c=0; c < iconw; c++ )
					put_logical_pixel((iconbuffer+(c*pixelsize)),bgr,bgg,bgb);
				}
			if (( r < iptr->rows )
			&&  ((rptr = connect_pixel_row( iptr->storage, r )) != (char *) 0)){
				memcpy((iconbuffer+(r*(iconw*pixelsize))),rptr,(iptr->columns>iconw?iconw:iptr->columns)*pixelsize);
				}
			}
...
private	void	drop_icons()
{
	...
	if( LockIcon  ) { LockIcon  = liberate( LockIcon  ); }
...
private	void	load_icons()
{
	...
	if (!( LockIcon   )) LockIcon = load_icon( _LOCK_ICON, 0 );
...
public	void	SingDrawIcon( int atx, int aty, char * buffer, int style )
{
	...
	mixzone(atx+1,aty+1,Context.Tool.w,Context.fontheight,buffer);
	return;
}
...
{
	if (DataIcon)
		putzone(x,y,Context.Tool.w,Context.Tool.h,DataIcon);
...
private	void	draw_icon_tool(int position, int type, char * tptr )
{
	...
	if (( Context.Tool.h > 0 ) && ( Context.Tool.w > 0) && (!( Context.ToolBar.align))) {
		...
		if ( position == -1 ) {
			...
			y += Context.ToolBar.y;
			x = (Context.ToolBar.x + 2);
			y -= 24;
			if ( type > _DATA_FRAME ) 	
				mptr = " ";
			else if (!( mptr = get_widget_class( type ) ))
				mptr = " ";
			mlen = strlen( mptr );
#ifdef	ABAL32A
			if ( visual_element( 
				x-1,y-1,(Context.Tool.w*2)+(Context.Tool.w/2)-5,24,
				GetWidgetStyle(10),mptr,mlen) != 0)
#endif
				visual_text(x-1,y-1,(Context.Tool.w*2)+(Context.Tool.w/2)-5,24,Context.menufont,16,21,mptr,mlen,3);
#ifdef	ABAL32A
			else	styling=1;
#endif
		...
		switch ( type ) {
			...
			case	_MOVE_FRAME :
				if (MoveTool)
					mixzone(x,y,Context.Tool.w,Context.Tool.h,MoveTool);
				else	filzone(x,y,Context.Tool.w,Context.Tool.h,3);
				break;
			case	_FORM_FRAME :
				if (FormTool)
					putzone(x,y,Context.Tool.w,Context.Tool.h,FormTool);
				else	filzone(x,y,Context.Tool.w,Context.Tool.h,8);
				break;
			case	_DATA_FRAME :
				show_data_icon(x,y);
			...
			case	_PEN_FRAME :
				if (PenTool)
					mixzone(x,y,Context.Tool.w,Context.Tool.h,PenTool);
				else	filzone(x,y,Context.Tool.w,Context.Tool.h,8);
				break;
			case	_PIPE_FRAME :
				if (PipeTool)
					mixzone(x,y,Context.Tool.w,Context.Tool.h,PipeTool);
				else	filzone(x,y,Context.Tool.w,Context.Tool.h,8);
				break;
			case	_POUR_FRAME :
				if (PourTool)
					mixzone(x,y,Context.Tool.w,Context.Tool.h,PourTool);
				else	filzone(x,y,Context.Tool.w,Context.Tool.h,8);
				break;
			case	_LINE_FRAME :
			...
			case	_PROGRESS_FRAME	:
				visual_progress(x+2,y+2,Context.Tool.w-4,Context.Tool.h-4,
					Context.tinyfont,Context.foreground,Context.background,2,1,3);
				break;
			...
			case	_GRAPH_FRAME :
				...
				break;
			case	_CIRCLE_FRAME :
				drawcircle( x,y,Context.Tool.w,Context.Tool.w,Context.foreground,0);
				break;

			case	_DISC_FRAME :
				drawcircle( x,y,Context.Tool.w,Context.Tool.w,Context.foreground,1);
				break;
...
public	int	use_icon_tool(int y,int v)
{
	if ( y <= _MOVE_FRAME ) {
		...
		if ( y == _MOVE_FRAME ) { Context.TemplateWidget = (struct form_item *) 0;	}
...
public	int	select_icon_tool(int x, int y, int item)
{
	int	v=0;
	int	atx;
	int	aty;
	...
	char *	lptr;
	void *	vb;
	char *	application_template_selection(int);
	int	use_application_template_selection(int,int);
	int	status=0;
	atx = (Context.ToolBar.x+x);
	aty = (Context.ToolBar.y+y);
	if (!( nbx = (20*Context.fontwidth) ))
		return(256);
	else if (!( nby = (8*Context.fontheight) ))
		return(256);
	else if (( lptr = application_template_selection(item)) != (char*) 0) {
		if (( vb = visual_buffer(atx,aty,nbx,nby) ) != (void *) 0) {
			if ( visual_buffer_get( vb, atx,aty ) ) {
				do 	{ 
					while(visual_getch() != 256); 
					} 
				while ( visual_event(1) != _MIMO_UP );
				if ((status = visual_select(	
						atx,aty, nbx,nby,
						1,standard_black,standard_white,
						parse_selection(lptr,&v),_VISUAL_FOCUS
						)) != 27) {
					use_application_template_selection(item,v);
					}
				visual_buffer_put( vb, atx,aty );
				use_icon_tool(item,v);
				}
			visual_drop( vb );
			}
		liberate( lptr );
...
private	int	is_tool_icon_right(int x, int y)
{
	int	yy=y;
	int	xx=x;
	int	t;
...
public 	void	activate_ToolBar(int mode)	
{
	if ((Context.ToolBar.style = mode)!=0) {
		...
		Context.ToolBar.x = 0; /* was 1 */
		Context.ToolBar.y = 0; /* was 1 */
		...
		if (!( Context.ToolBar.align )) {
			...
			if (!( Context.usestyle )) {
				visual_frame(	Context.ToolBar.x,Context.ToolBar.y+Context.MenuBar.h,
					...
					Context.ToolBar.x,Context.ToolBar.y+(Context.MenuBar.h ? Context.MenuBar.h+2 : 0),
					Context.ToolBar.w+1,Context.ToolBar.h-Context.MenuBar.h,
					GetWidgetStyle(9)," ",0) != 0)
#endif
				visual_frame(	Context.ToolBar.x,Context.ToolBar.y+Context.MenuBar.h,
					Context.ToolBar.w+1,Context.ToolBar.h-Context.MenuBar.h,Context.ToolBar.style);
		...
		switch ( visual_event(1)) {
			case	_MIMO_DOWN	:
				...
				||  (!( Context.ToolManager->manager->state ))) 
					...
					return(256);
				else if ( Context.ToolBar.align )
					return(256);
				else if ( visual_event(2) == _MIMO_LEFT )
					return(256);
				else	return( (Context.ToolManager->manager->on_right)(x,y) );
...
private	int	_standard_on_right(int x, int y)
{
	return(256);
}
...
struct	tool_manager *	allocate_tool_manager()
{
	...
	tptr->on_show=0;
	tptr->on_event=0;
	tptr->on_right=_standard_on_right;
...
private	void	show_image_option(int position,int mode)
{
	char 	buffer[32];
	int	buflen=0;
	int	x;
	...
	switch ( mode ) {
		case	0 : /* point  */
			sprintf(buffer,"p%u",Context.thickness);
			break;
		case	1 : /* line   */
			sprintf(buffer,"l%u",Context.thickness);
			break;
		case	2 : /* frame  */
			sprintf(buffer,"f%u",Context.thickness);
			break;
		case	3 : /* circle */
			sprintf(buffer,"c%u",Context.thickness);
			break;
		}
	buflen = strlen( buffer );
	visual_text(x,y,Context.Tool.w,((Context.Tool.h/3)*3),Context.menufont,Context.background,Context.foreground,buffer,buflen,3);
	return;
}
...
private	void	show_image_colours(int fgposition,int bgposition)
{
	...
	if (( Context.Tool.h > 0 ) && ( Context.Tool.w > 0) && (!( Context.ToolBar.align))) {
		...
		y = (Context.ToolBar.y+
			...
			+(Context.fontheight*3));
		if (!( Context.foreground & _TRUE_COLOUR_PIXEL ))
			sprintf(message,"%u",Context.foreground);
		else	strcat(message,"XX");
		mlen = strlen( message );
		...
		redvalue = Context.fgrgb[0];
		greenvalue = Context.fgrgb[1];
		bluevalue = Context.fgrgb[2];
		...
		redvalue = Context.bgrgb[0];
		greenvalue = Context.bgrgb[1];
		bluevalue = Context.bgrgb[2];
...
private	void	show_image_tools()
{
	...
	show_image_option(20,0);
	show_image_option(21,1);
	show_image_option(22,2);
	show_image_option(23,3);
	return;
}
...
public	void	use_image_tools()
{
	if ( ImageTools ) {
		...
		use_icon_tool( ImageTools->current, -1 );
		}
	return;
}
...
private	int	is_form_tool_right(int x, int y)
{
	if (!( application_has_template() ))
		return( _standard_on_right(x,y) );
	else	return( is_tool_icon_right(x,y) );
}
...
private	void	allocate_form_tools()
{
	...
	FormTools->on_right = is_form_tool_right;
	FormTools->state    = 3;
	return;
}
...
public	void	use_form_tools()
{
	if ( FormTools ) {
		...
		use_icon_tool( FormTools->current, -1 );
		}
	return;
}
...
#ifdef	HTML_TAG_DATA
	else if (( visual_event(2) != _MIMO_RIGHT ) || ( Context.style != 17 )) {
		html_operation(HtmlTools->toolmsg[Context.style]);
		return(256);
		}
	else if ( choose_html_tag_data( &tagdata ) == 27 )
		return( 256 );
	else	{
		if ( tagdata ) {
			html_operation(tagdata);
			liberate( tagdata );
			}
		return(256);
		}
...
/*	 CSS TOOLS	*/
/*	----------	*/
private	void	liberate_css_tools()
{
	if ( CssTools )
		CssTools = liberate( CssTools );
	return;
}

private	void	show_css_tools()
{
	int	i;
	for (	i=0; i < 18; i++ ) {
		draw_icon_tool(i,_BUTTON_FRAME,CssTools->toolmsg[i]);
		CssTools->toolmap[i] = (i+1);
		}
	return;
}

private	int	is_css_tool_event(int x, int y)
{
	char	*	tagdata=(char *) 0;
	if ( is_tool_icon_event( x, y )  != 256 ) {
		return(-1);
		...
		css_operation(CssTools->toolmsg[Context.style]);
		return(256);
		}
}

private	void	allocate_css_tools()
{
	if (!( CssTools = allocate_tool_manager() ))
		return;

	CssTools->on_show  = show_css_tools;
	CssTools->on_event = is_css_tool_event;
	CssTools->font     = Context.tinyfont;
	CssTools->toolmsg[0] = "margin";
	CssTools->toolmsg[1] = "border";
	CssTools->toolmsg[2] = "padding";
	CssTools->toolmsg[3] = "content";
	CssTools->toolmsg[4] = "position";
	CssTools->toolmsg[5] = "top";
	CssTools->toolmsg[6] = "right";
	CssTools->toolmsg[7] = "bottom";
	CssTools->toolmsg[8] = "left";
	CssTools->toolmsg[9] = "colour";
	CssTools->toolmsg[10] = "style";
	CssTools->toolmsg[11] = "size";
	CssTools->toolmsg[12] = "width";
	CssTools->toolmsg[13] = "height";
	CssTools->toolmsg[14] = "font";
	CssTools->toolmsg[15] = "decoration";
	CssTools->toolmsg[16] = "align";
	CssTools->toolmsg[17] = "A";
	CssTools->state    = 2;
	return;
}

public	void	use_css_tools()
{
	if ( CssTools ) {
		use_tool_manager( CssTools );
		draw_ToolBar();
		}
	return;
}
