...
private	void	abal_normal_access(FILE * h, struct form_item * iptr, int margin )
{
	...
	if (!( iptr ))
		return;
	else	v = ((iptr->Contents.access & _WIDGET_ACCESS) >> 2 );
	indent(h,margin);
	if ((iptr->Contents.style & _FRAME_TYPE) != _RADIO_FRAME)
		fprintf(h,"a_%s=%u",iptr->Contents.name,v);
	else	fprintf(h,"a%u_%s=%u",iptr->Contents.radio,iptr->Contents.name,v);
	linefeed(h);
	return;
}
...
public  void	abal_form_body( FILE * h, struct form_control * fptr  )
{
	/*	-------		*/
	/* 	FORWARD		*/
	/*	-------		*/
