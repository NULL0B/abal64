...
public	int	TableFocusLine=0;

public	int	visual_table_focus(int fline)
{
	TableFocusLine = fline;
	return(0);
}
...
public	int	vcolumns( char * fptr, int fieldwidth[], int maxfields, int linelength )
{
	int	c=0;
	int	v=0;
	int	i=0;
...
public	int	vformat( char * fptr, int fieldwidth[], int maxfields, int linelength )
{
	...
	/* ------------------------------------------ */
	/* ensure correctly formated table descriptor */
	/* ------------------------------------------ */
	if ( (!( fptr )) ||  ( *fptr != '(' ))	{ return(0); }
	else	fptr++;

	/* ---------------------------------------------------  */
	/* detect special case where line is defined by object 	*/
	/* by the syntax : (#formname)				*/
	/* ---------------------------------------------------  */
	if ( *fptr == '#' )
	{
		fieldwidth[fields++] = linelength;
		return(1);
	}
