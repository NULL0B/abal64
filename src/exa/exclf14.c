...
EXAWORD x;
{
	...
	/* ------------------------------- */ 
	/* prpare the argument descriptors */
	/* ------------------------------- */ 
	if ( ((desc = (EXAWORD) *(fdex++)) & 1) != 0) 
		descx = (EXAWORD) *(fdex++);

	/* ---------------------------------------- */	
	/* collect the result and source parameters */
	/* ---------------------------------------- */	
	...
	case	0 :		
		...
		else	return(bcdfix(workptr2,workptr1,worklen2,worklen1,workval3,(desc & 1)));
