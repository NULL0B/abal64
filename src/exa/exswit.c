 ...
 * 09/12/1997 : Rend la main a windows lors de l'acces a une BDA (case _tc_LDF).
 ...
 * 19/05/1999 : (#AT) Bug EXA-63 fixed. Corrections bounded with assigning to null pointers in ABAL
 * 14/08/2002 : (#OK) Visibility of global variables with same names in different modules.
 *	------------ 2.2b -------------------------------------------------
 */
			...
			if ( (tcode = (EXAWORD) *(fdex++)) & 0x0080) {
				...
				/* ------------------------------ */
				if ((timersem & (TIME_ACTIF|EVENT_SWAP|EVENT_FLAG)) != 0) {
					...
					if (( timersem & TIME_ACTIF) != 0) {
						...
						if (((EXAWORD) descarto[0] & TIME_ALARM ) == 0) {
							...
							/* ----------------------- */
							if (( timersem & TIME_CALL ) != 0) {
								activate_timer_procedure( timeradr, timervalue() );
								timersem &= EVENT_KEEP;
								continue;
								}
...
case _tc_SGN 	:
  ...
  */
case _tc_LDI	:
	freg [ (e = (EXAWORD) *fdex) ] = TcodeInteger( (fdex + 1) );
...
case	_tc_GSB	:
	...
	{
		ip_hold [ (ip_next++) ] = (EXAWORD) (fdex + WORDSIZE - TcodeSegment); 
		fdex = (BPTR) (TcodeSegment + Tcodeword( fdex));
		}
#ifdef WEXADBDLL
	StepOverTrapWork(0);	/* #BG */
#endif
	continue;
...
case _tc_STI	:	/* STORE Integer to Variable */
	e = TcodeInteger(fdex); fdex += WORDSIZE;
ITOVARB:
	...
	switch ( Tcodeword( workptr2 ) ) {
		...
		case	0x0061 : /* %(x,y)	*/
			CheckAreg( 0 , workptr2 );
			workptr1 += ( ((areg [0] - 1) * ABALWORDSIZE) * (Tcodeword( (workptr2 + (2 * WORDSIZE)))));
		case	0x0049 :
		case	0x0041 : /* %(x)	*/
			CheckIreg( 0 , workptr2 );
			workptr1 += ((ireg [0] - 1) * ABALWORDSIZE);
		case	0x0009 :
		case	0x0001 : 
		...
		case	0x0065 : /* %(x,y)	*/
			CheckAreg( 0 , workptr2 );
			workptr1 += ( ((areg [0] - 1) * (2*ABALWORDSIZE)) * (Tcodeword( (workptr2 + (2 * WORDSIZE)))));
		case	0x004D :
		case	0x0045 : /* %(x)	*/
			CheckIreg( 0 , workptr2 );
			workptr1 += ((ireg [0] - 1) * (2*ABALWORDSIZE));
		case	0x000D :
		case	0x0005 : 
			balputl( e , ( workptr1 + (Tcodeword( (workptr2 + WORDSIZE) ) ) )); continue;
#endif
		case	0x006A :
		case	0x0062 : /* BCD(x,y)	*/
...
case _tc_LDV	:
	...
	switch ( Tcodeword( workptr2 ) ) {
		...
		case	VARB_INT16 | VARB_ARG | NDX1st | NDX2nd : /* x69 arg %(x) */
		case	VARB_INT16 | NDX1st | NDX2nd	    : /* x61 %(x,y) 	  */ 
			CheckAreg( e , workptr2 );
			workptr1 += ( ((areg [e] - 1) * ABALWORDSIZE) * (Tcodeword( (workptr2 + (2 * WORDSIZE)))));

		case	VARB_INT16 | VARB_ARG | NDX1st: /* x49 arg %(x) */
		case	VARB_INT16 | NDX1st		: /* x41 %(x) 	  */ 
			CheckIreg( e , workptr2 );
			workptr1 += ((ireg [e] - 1) * ABALWORDSIZE);

		case	VARB_INT16 | VARB_ARG	: /* x9 */
		case	VARB_INT16		: /* x1 */ 
			xreg [e] = IMMEDIAT;
			freg [e] = balgetw( ( workptr1 +    (Tcodeword( (workptr2 + WORDSIZE) ) ) )); 
			continue;

		/* ABAL 32 : LONG INTEGER OPERATIONS */
		/* --------------------------------- */
		case	VARB_INT32 | VARB_ARG | NDX1st | NDX2nd : /* x6D arg %(x) */
		case	VARB_INT32 | NDX1st | NDX2nd	    	: /* x65 %(x,y)	  */ 
			CheckAreg( e , workptr2 );
			workptr1 += ( ((areg [e] - 1) * (ABALWORDSIZE*2)) * (Tcodeword( (workptr2 + (2 * WORDSIZE)))));

		case	VARB_INT32 | VARB_ARG | NDX1st : /* x4D */
		case	VARB_INT32 | NDX1st		 : /* x45 */ 
			CheckIreg( e , workptr2 );
			workptr1 += ((ireg [e] - 1) * (ABALWORDSIZE*2));

		case	VARB_INT32 | VARB_ARG	: /* x0D */
		case	VARB_INT32		: /* x05 */ 
			freg [e] = balgetl( ( workptr1 + (Tcodeword( (workptr2 + WORDSIZE) ) ) )); 
			xreg [e] = IMMEDIAT;
			continue;

		/* ABAL 64 : HUGE INTEGER OPERATIONS */
		/* --------------------------------- */
		case	VARB_INT64 | VARB_ARG | NDX1st | NDX2nd : /* x6E arg %(x) */
		case	VARB_INT64 | NDX1st | NDX2nd	    	: /* x66 %(x,y)	  */ 
			CheckAreg( e , workptr2 );
			workptr1 += ( ((areg [e] - 1) * (ABALWORDSIZE*4)) * (Tcodeword( (workptr2 + (2 * WORDSIZE)))));

		case	VARB_INT64 | VARB_ARG | NDX1st 	: /* x4E */
		case	VARB_INT64 | NDX1st	 	: /* x46 */ 
			CheckIreg( e , workptr2 );
			workptr1 += ((ireg [e] - 1) * (ABALWORDSIZE*4));

		case	VARB_INT64 | VARB_ARG	: /* x0E */
		case	VARB_INT64		: /* x06 */ 
			freg [e] = balget64( ( workptr1 + (Tcodeword( (workptr2 + WORDSIZE) ) ) )); 
			xreg [e] = IMMEDIAT;
			continue;
...
VARBNDX:
	...
	switch ( (worktyp2 = (Tcodeword( workptr3 ))) ) {
		...
		case	0x0000 : /* #		*/
				...
				if ( WORDSIZE > 4 ) 	{ worktyp2 = VARB_INT64; }
				else if ( WORDSIZE > 2 ){ worktyp2 = VARB_INT32; }
				else			{ worktyp2 = VARB_INT16; }
				break;

		case	VARB_INT16 | VARB_ARG | NDX1st | NDX2nd : /* x69 arg %(x) */
		case	VARB_INT16 | NDX1st | NDX2nd	    : /* x61 %(x,y) 	  */ 
				CheckAreg( e , workptr3 );
				workptr2 += ( ((areg [e] - 1) * ABALWORDSIZE) * (Tcodeword( (workptr3 + (2 * WORDSIZE)))));

		case	VARB_INT16 | VARB_ARG | NDX1st: /* x49 arg %(x) */
		case	VARB_INT16 | NDX1st		: /* x41 %(x) 	  */ 
				CheckIreg( e , workptr3 );
				workptr2 += ((ireg [e] - 1) * ABALWORDSIZE);

		case	VARB_INT16 | VARB_ARG	: /* xD */
		case	VARB_INT16		: /* x5 */ 
		...
		case	VARB_INT32 | NDX1st | NDX2nd	    : /* x65 %(x,y) 	  */ 
				CheckAreg( e , workptr3 );
				workptr2 += ( ((areg [e] - 1) * (ABALWORDSIZE*2)) * (Tcodeword( (workptr3 + (2 * WORDSIZE)))));

		case	VARB_INT32 | VARB_ARG | NDX1st : /* x4D */
		case	VARB_INT32 | NDX1st		 : /* x45 */ 
				CheckIreg( e , workptr3 );
				workptr2 += ((ireg [e] - 1) * (ABALWORDSIZE*2));

		case	VARB_INT32 | VARB_ARG	: /* x0D */
		case	VARB_INT32		: /* x05 */ 
				workval2 = balgetl( ( workptr2 + (Tcodeword( (workptr3 + WORDSIZE) ) ) )); 
				break;
		/* ABAL 64 : HUGE INTEGER OPERATIONS */
		/* --------------------------------- */
		case	VARB_INT64 | VARB_ARG | NDX1st | NDX2nd : /* x6E arg %(x) */
		case	VARB_INT64 | NDX1st | NDX2nd	    	: /* x66 %(x,y)	  */ 
				CheckAreg( e , workptr3 );
				workptr2 += ( ((areg [e] - 1) * (ABALWORDSIZE*4)) * (Tcodeword( (workptr3 + (2 * WORDSIZE)))));

		case	VARB_INT64 | VARB_ARG | NDX1st 		: /* x4E */
		case	VARB_INT64 | NDX1st		 	: /* x46 */ 
				CheckIreg( e , workptr3 );
				workptr2 += ((ireg [e] - 1) * (ABALWORDSIZE*4));

		case	VARB_INT64 | VARB_ARG	: /* x0E */
		case	VARB_INT64		: /* x06 */ 
				workval2 = balget64( ( workptr2 + (Tcodeword( (workptr3 + WORDSIZE) ) ) )); 
				break;
		case	0x006A :
		case	0x006B :
		case	0x0063 :
...
case _tc_LXR	:
	...
	{
	case CHARONLY : 
		ireg [(e = *fdex)] = ( freg [((EXAWORD) *(fdex + 1))] & 0x00FF ); 
		fdex += (2*BYTESIZE); 
		break;
	case IMMEDIAT : 
		ireg [(e = *fdex)] = freg [((EXAWORD) *(fdex + 1))]; 
		fdex += (2*BYTESIZE);
		break;
	case KONSTANT : 
		workptr2 = (TcodeConstants + freg [((EXAWORD) *(fdex + 1))]); 
		e = *(fdex++); 
		goto KONINDEX;
	case VARIABLE : 
		worktyp3 = freg [ (e = ((EXAWORD) *(fdex+1))) ]; 
		fadj = 1; 
		goto VARBNDX;
	case PTRSTR    :
	...
	case TEMPDESC : workptr2 = (TcodeByteStack + freg [((EXAWORD) *(fdex + 1))]);
		...
		{
		case VARB_STR : 
			ireg [ (e = *fdex) ] = (EXAWORD) atoi(( workptr2 + ( 2 * WORDSIZE))  , (Tcodeword( (workptr2 + WORDSIZE) ))); 
			break;
		case VARB_BCD : 
			ireg [ (e = *fdex) ] = (EXAWORD) bcdtoi(( workptr2 + ( 2 * WORDSIZE))  , (Tcodeword( (workptr2 + WORDSIZE) ))); 
			break;
		}
		fdex += (2*BYTESIZE);
	}
...
case _tc_AXR	:
	...
	{
	case CHARONLY :
		areg [(e = *fdex)] = ( freg [((EXAWORD) *(fdex + 1))] & 0x00FF ); 
		fdex += (2*BYTESIZE);
		break;
	case IMMEDIAT : 
		areg [(e = *fdex)] = freg [((EXAWORD) *(fdex + 1))]; 
		fdex += (2*BYTESIZE);
		break;
	case KONSTANT : 
		workptr2 = (TcodeConstants + freg [((EXAWORD) *(fdex + 1))]); 
	...
	case PTRBCD    :
		...
		break;
	case TEMPDESC : 
		workptr2 = (TcodeByteStack + freg [((EXAWORD) *(fdex + 1))]);
		switch( (Tcodeword( workptr2 ) ) ) 
		{
		case VARB_STR : 
			areg [ (e = *fdex) ] = (EXAWORD) atoi(( workptr2 + ( 2 * WORDSIZE))  , (Tcodeword( (workptr2 + WORDSIZE) ))); 
			break; 
		case VARB_BCD : 
			areg [ (e = *fdex) ] = (EXAWORD) bcdtoi(( workptr2 + ( 2 * WORDSIZE))  , (Tcodeword( (workptr2 + WORDSIZE) ))); 
			break;
		}
		fdex += (2*BYTESIZE);
	}
...
//	let_windows_work
	notify_abqt = 1;
#endif
	// Appel d'une fonction d'Overlay.
	tcode = DoFuse(1); 
	break;
#endif
#else
...
case _tc_JEQI 	:
	if ( ( xreg [ (e = *fdex) ] & 0x0803 ) != IMMEDIAT ) { goto SWITSXXXX; }
	if (((BINAIRE) freg [ e ]) == (TcodeInteger( (fdex + 1) )) ) 
	{ 
	fdex = TcodeSegment + (Tcodeword( (fdex + (WORDSIZE + 1))));
#ifdef WEXADBDLL
	StepOverTrapWork(0);
#endif
	continue; 
...
case	_tc_ADI	:
	if ( (xreg [ (e = *fdex) ] & 0x0803) != IMMEDIAT ) { goto SWITSXXXX; }
 	freg [(tcode = *(fdex + (WORDSIZE + 1)))] = ((BINAIRE) freg [ e ] + (TcodeInteger( (fdex + 1) )));
...
case	_tc_DVI	:
	if ( (xreg [ (e = *fdex) ] & 0x0803) != IMMEDIAT ) { goto SWITSXXXX; }
	if ( (workval1 = (TcodeInteger( (fdex + 1) ))) == 0 )
		{ err_val = 102; tcode = 0; continue; }
	freg [(tcode = *(fdex + (WORDSIZE + 1)))] = ( (BINAIRE) freg [ e ] / workval1);
	xreg [tcode] = IMMEDIAT; fdex += (2 + WORDSIZE);
	continue;
case	_tc_ANI	:
	if ( (xreg [ (e = *fdex) ] & 0x0803) != IMMEDIAT ) { goto SWITUXXXX; }
 ...
 *	if ( (workval1 = (TcodeInteger( (fdex + 1) ))) == 0 )   { err_val = 102; tcode = 0; continue; }
 *	freg [(tcode = *(fdex + (WORDSIZE + 1)))] = ( (BINAIRE) freg [ e ] % workval1);
 ...
 */
/* ---------------------------------------- */
/* Signed operators + - / * MOD arrive here */
/* ---------------------------------------- */
SWITSXXXX:
	if ( WORDSIZE > 4 ) 	{ worktyp2 = VARB_INT64; }
	else if ( WORDSIZE > 2 ){ worktyp2 = VARB_INT32; }
	else			{ worktyp2 = VARB_INT16; }
	workval2 = (TcodeInteger(( fdex + 1 ))); 
	fadj = WORDSIZE; goto SREGEVAL;
/* ---------------------------------------- */
/* Unsigned operators AND/OR/OX arrive here */
/* ---------------------------------------- */
SWITUXXXX:
	if ( WORDSIZE > 4 ) 	{ worktyp2 = VARB_INT64; }
	else if ( WORDSIZE > 2 ){ worktyp2 = VARB_INT32; }
	else			{ worktyp2 = VARB_INT16; }
	workval2 = (BINAIRE) Tcodeword(( fdex + 1 )); 
	fadj = WORDSIZE; goto SREGEVAL;
...
VARBEVAL:
	...
	switch ( (worktyp2 = (Tcodeword( workptr3 ))) ) {
		...
		case	0x0065 : /* %(x,y)	*/
				CheckAreg( e , workptr3 );
				workptr2 += ( ((areg [e] - 1) * (2*ABALWORDSIZE)) * (Tcodeword( (workptr3 + (2 * WORDSIZE)))));
		case	0x004D :
		case	0x0045 : /* %(x)	*/
				CheckIreg( e , workptr3 );
				workptr2 += ((ireg [e] - 1) * (2*ABALWORDSIZE));
		case	0x000D :
		case	0x0005 : 
				workval2 = balgetl( ( workptr2 + (Tcodeword( (workptr3 + WORDSIZE) ) ) )); 
				goto SREGEVAL;
	...
	switch ( xreg [ e ] ) {
		...
		case VARIABLE  :
			...
			switch ( (worktyp1 = (Tcodeword( workptr3 ))) ) {
				...
				case	0x0061 : /* %(x,y)	*/
						CheckAreg( e , workptr3 );
						workptr1 += ( ((areg [e] - 1) * ABALWORDSIZE) * (Tcodeword( (workptr3 + (2 * WORDSIZE)))));
				case	0x0049 :
				case	0x0041 : /* %(x)	*/
						CheckIreg( e , workptr3 );
						workptr1 += ((ireg [e] - 1) * ABALWORDSIZE);
				...
				case	0x0065 : /* %(x,y)	*/
						CheckAreg( e , workptr3 );
						workptr1 += ( ((areg [e] - 1) * (2*ABALWORDSIZE)) * (Tcodeword( (workptr3 + (2 * WORDSIZE)))));
				case	0x004D :
				case	0x0045 : /* %(x)	*/
						CheckIreg( e , workptr3 );
						workptr1 += ((ireg [e] - 1) * (2*ABALWORDSIZE));
				case	0x000D :
				case	0x0005 : 
						workval1 = balgetl( ( workptr1 + (Tcodeword( (workptr3 + WORDSIZE) ) ) )); 
						goto DOCALC;
#endif
				case	0x006A :
				case	0x006B :
	...
	switch ( worktyp1 & VARB_ISOL ) {
		...
		case VARB_INT64	:
			...
			case VARB_BCD   :
				worklen1 = BCDMAX;
				PileCheck( worklen1 );
				(VOID) itobcd(workval1,(TcodeByteStack + worktop),worklen1);
			...
			case VARB_STR   :
				worklen1 = BCDMAX*2;
				PileCheck( worklen1 );
				(VOID) itoa(workval1,(TcodeByteStack + worktop),worklen1);
				workptr1 = (TcodeByteStack + worktop);
				worktop += worklen1;
				goto DOSTR;
			}	
		...
		case VARB_STR :
			...
			case VARB_INT16 :
				worklen2 = BCDMAX*2;
				PileCheck( worklen2 );
				(VOID) itoa(workval2,(TcodeByteStack + worktop),worklen2);
			...
			case VARB_INT32 :
				worklen2 = BCDMAX*2;
				PileCheck( worklen2 );
				(VOID) ltoa(workval2,(TcodeByteStack + worktop),worklen2);
				workptr2 = (TcodeByteStack + worktop);
				worktop += worklen2;
				goto DOSTR;
			case VARB_INT64 :
			...
			case VARB_BCD   :
				worklen2 = (BCDMAX * 2); 
				PileCheck( worklen2 );
				(VOID) bcdtoa( workptr2,worklen2,(TcodeByteStack + worktop),worklen2);
				workptr2 = (TcodeByteStack + worktop);
				worktop += worklen2;
			case VARB_STR :
DOSTR:
		...
		case VARB_BCD :
			...
			case VARB_INT16 :
				...
				workptr2 = (TcodeByteStack + worktop);
				worklen2 = BCDMAX; worktop += BCDMAX;
				goto DOBCD;
			case VARB_INT32 :
				PileCheck( BCDMAX );
				(VOID) ltobcd(workval2,(TcodeByteStack + worktop),BCDMAX);
				workptr2 = (TcodeByteStack + worktop);
				worklen2 = BCDMAX; worktop += BCDMAX;
				goto DOBCD;
			case VARB_INT64 :
				PileCheck( BCDMAX );
				(VOID) htobcd(workval2,(TcodeByteStack + worktop),BCDMAX);
				workptr2 = (TcodeByteStack + worktop);
				worklen2 = BCDMAX; worktop += BCDMAX;
				goto DOBCD;
			case VARB_STR :
...
CALCWHAT:
	...
	switch ( worktyp1 & VARB_ISOL) {
		...
		case VARB_INT32 :
			switch ( worktyp2 & VARB_ISOL) {
				case VARB_STR : workval2 = atol  (workptr2,worklen2); break;
				case VARB_BCD : workval2 = bcdtol(workptr2,worklen2);
				}
			tcode = compint((tcode & 7),workval2,workval1);
			continue;
		...
		case VARB_INT64 :
			switch ( worktyp2 & VARB_ISOL) {
				case VARB_STR : workval2 = atoh  (workptr2,worklen2); break;
				case VARB_BCD : workval2 = bcdtoh(workptr2,worklen2);
				}
			tcode = compint((tcode & 7),workval2,workval1);
			continue;
		...
		case VARB_BCD   :
			...
			case VARB_INT32 : 
				PileCheck( BCDMAX );
		 		ltobcd(workval2,(workptr2 = (TcodeByteStack + worktop)),(worklen2 = BCDMAX));
				break;
			case VARB_INT64 : 
				PileCheck( BCDMAX );
		 		htobcd(workval2,(workptr2 = (TcodeByteStack + worktop)),(worklen2 = BCDMAX));
				break;
			}
#ifndef INTELASM1
		...
		case VARB_STR   :
			...
			case VARB_INT32 :
				PileCheck( worklen1 );
				ltoa(workval2,(TcodeByteStack + worktop),worklen1);
				workptr2 = (TcodeByteStack + worktop);
				worktop += worklen1;  worktyp2 = VARB_STR; break;
			case VARB_INT64 :
				PileCheck( worklen1 );
		...
		switch ( xreg [(e = (EXAWORD) *(fdex++))] ) {
			...
			case IMMEDIAT :
				switch ( worktyp2 & VARB_ISOL ) {
					...
					case VARB_INT32 : freg [e] = workval2; continue;
					case VARB_INT64 : freg [e] = workval2; continue;
...
EXVRBCNV:
	...
	switch ( (worktyp1 << 4) | worktyp2 )	{
		...
		case 0x0013 :	balputw((EXAWORD) atoi(workptr2,worklen2),workptr1);
				break;
#ifdef	VERSION666
		case 0x0025 :	tcode = ltobcd(workval2,workptr1,worklen1); break;
		case 0x0026 :	tcode = htobcd(workval2,workptr1,worklen1); break;
#endif
		case 0x0020 :	
		case 0x0024 :	
		...
		case 0x0033 :	
			xmemcpy(0x0020); 
			if (err_val) tcode = SOFTERROR;
			break;
#ifdef	VERSION666
		case 0x0050 :
		case 0x0051 :
		case 0x0054 :
		case 0x0055 :	balputl( workval2, workptr1 ); break;
		case 0x0056 :	balputl( workval2, workptr1 ); break;
		case 0x0052 :	balputl((EXAWORD) bcdtol(workptr2,worklen2),workptr1);
				break;
		case 0x0053 :	balputl((EXAWORD) atol(workptr2,worklen2),workptr1);
				break;
		case 0x0060 :
		case 0x0061 :
		case 0x0064 :
		case 0x0065 :	balput64( workval2, workptr1 ); break;
		case 0x0066 :	balput64( workval2, workptr1 ); break;
		case 0x0062 :	balput64((EXAWORD) bcdtol(workptr2,worklen2),workptr1);
				break;
		case 0x0063 :	balput64((EXAWORD) atol(workptr2,worklen2),workptr1);
				break;
#endif
		default	    :	
			tcode = SOFTERROR; err_val = 444; continue;
		}
	break;
