...
#ifdef ABALFIND
#define	FINDFNMAX 2048
struct	findwork {
	EXALONG	pathdex;
	EXABYTE	pathbuf[FINDFNMAX];
	EXABYTE	pathsel[FINDFNMAX];
	};

struct findwork PTR pathwork;

EXABYTE	statchker[FINDFNMAX+1];
#ifdef	VERSION666X
struct stat64 StatGetZ;
#else
...
EXAWORD	mode;
{
	...
	while ( (dirptr = (struct dirent *) readdir( pathid )) != (struct dirent *) 0 ) {
...
#ifdef	VERSION666X
		if ( stat64((BPTR) statchker , &StatGetZ ) != 0 ) { continue; }
#else
		...
		if ( mode & 8 ) {
			/* Ignore Case Checking */
			/* -------------------- */
			if ( nowildcomp( dirptr->d_name , (*(pathwork)).pathsel) != 0 ) {
				continue;
				}
			}

		else if ( wildflg == MOINS_UN ) { 
			/* Simple File Check */
			/* ----------------- */
		...
		if ( ecart ) {
			...
			/* File size */
#ifdef	VERSION666
			balputl(StatGetZ.st_size,(rptr+7));
#else
...
EXAWORD	mode;
{
	...
	if (( fidperm & ASS_BUFF_VALID ) == 0 ) {
		if ( (fidbuff = (BPTR) allocate( sizeof( struct findwork ))) == (BPTR) 0) {
			err_val = ABAL_MEMORY; 
			return(MOINS_UN);
			}
	...
	pathwork = (struct findwork PTR) fidbuff;
...
/*	------------------------------------	*/
extern	EXAWORD	findfirst(BPTR , BPTR PTR);
extern	EXAWORD	findnext(EXAWORD , BPTR PTR);
...

{
	...
	if ( (mode & 1 ) == 0 ) {
		...
		*(pptr + loclen) = (EXABYTE) 0;
		...
		/* --------------------------------- */ 
		lerr = (EXAWORD) findfirst((BPTR) pptr , (BPTR PTR) & lrptr );
		...
		*(pptr + loclen) = c;
		...
		if (( fidperm & ASS_FIND_VALID) != 0 ) {
			...
			/* -------------------------- */
			lerr = (EXAWORD) findnext(fidbd , (BPTR PTR) & lrptr);	
...
int	reslen;
{
	...
	EXAWORD	lerr,l;
	char	PathBuf1[FINDFNMAX+1];
	char	WorkBuf[FINDFNMAX+1];
	char	NameStat[FINDFNMAX+1];
	EXAWORD	Level,status;

	/* Test for Nul Pointer and Allocate if needed */
	/* ------------------------------------------- */
	if ( resptr == (BPTR) 0 ) {
		if ((resptr = (BPTR) allocate( (USINT) FINDFNMAX+1)) == (BPTR) 0 ) {
			errno = 8;
			return((BPTR) 0);
			}
		}
...
BPTR	wptr;
{
	...
	wlen = (wptr) ? strlen(wptr) : 0; 
	tlen = (tptr) ? strlen(tptr) : 0;
...
#define	ignorecase(a) ( (a >= 'a') && ( a <= 'z') ? (a - ('a' - 'A')) : a )

EXAWORD	nowildcomp( tptr , wptr )
BPTR	tptr;
BPTR	wptr;
{
	EXAWORD	wlen,tlen;
	...
	while (( tlen > 0 ) && (wlen > 0)) {
		...
		switch ( *wptr ) {
			case '*' : if ( wildcomp( tptr , (wptr + 1)) == 0 ) {
				   ...
				   tptr++; tlen--;
				   continue; 
			default  : if ( (ignorecase((*tptr))) != (ignorecase((*wptr))) ) { return(MOINS_UN); }
			case '?' : wptr++; tptr++; wlen--; tlen--; continue;
			}
		}
	if (( tlen == 0 ) && ( wlen == 0 )) { return(0); }
