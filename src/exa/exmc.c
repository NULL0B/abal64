...
EXAWORD	cbRubName, RubSize, NbDec, NbRub, Type, Offset, Flag;
{
	...
	if ( Offset + (bUserLongNames ? RUB_DESC_LEN_EXT:RUB_DESC_LEN_STD) > fidblen)
	{
		if ( fidperm & ASS_BUFF_VALID )
		{
			...
			// fidbuff est une variable d'echange ABAL et ne peut pas etre étendue
			return(ABAL_BUF2SMALL);
	}
...
//	MI_TRACE((CI_TRA_F_FUNCTION,ID_KERNEL,TFN,""));
	...
	/* FCH04032005 :
	 * Suite à un plantage dans un programme de test de Laurent, je vois que rub_max vaut 0x8nnn
	 * en faisant une recherche, on voit que c'est le seul endroit où on ne fait pas de masque 0xFF
	 * sur rub_max. J'ajoute donc ce masque.
	 * Question pourquoi le flag 0x8000 est-il levé à cet endroit du code ?
	 */
						...
						)
					{
						...
						if (szTableName && cbTableName)
						{
							...
							if (cbFileName && !rubcmpi(szFileName,cbFileName,szTableName,cbTableName))
							{
								...
								/* (le return n'est plus systématique mais dépend de res)              */
								if (res) 
								{
	...
	*workptr2 = (EXABYTE) 0;
	...
	if ( fargset(3) != SOFTERROR )  
	{
		if (( worklen3 > 0 ) && ( worktyp3 == 3)) 
		{
			...
			/* ----------------------------------------------- */
			/* #BG 26/02/01: A partir de Criteria 3.6d, l'option U et S sont supportées */
