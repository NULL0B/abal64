...
static	EXAWORD	old_find_selection_compare( token, finish )
BPTR		token;
EXALONG		finish;
{
	EXAWORD	indent=0;
	EXAWORD	c;
	while ( 1 ) {
		if ((c = stream_getb(_WORK_STREAM)) == MOINS_UN)
			c = CTRL_Z;
		switch ( c ) {
...
static	EXAWORD	find_selection_compare( token, finish, mode)
BPTR		token;
EXALONG		finish;
EXAWORD		mode;
{
	...
	if (!( mode ))
		return( old_find_selection_compare( token, finish ) );

	/* For section lines */
	/* ----------------- */
	for (nblin=0; nblin < finish; nblin++ ) {
		i = 0;
		/* Collect a line */
		/* -------------- */
		while ( i < 319 ) {
...
static	EXALONG	find_finish( linfos, handle, current )
CORRESPTR	linfos;
EXAWORD		handle;
EXALONG		current;
{
	while ( linfos != (CORRESPTR) 0 ) {
		if ( linfos->file_number == handle ) {
			...
			else if ( linfos->file_offset >= current ) 
				return(( linfos->file_offset - current ));
			else	break;
			}
...
EXAWORD		parent;
{
	...
	while ( linfos != (CORRESPTR) 0) {
		...
		&&   ( stream_position( _WORK_STREAM, (BPTR) fsymb->name, (EXALONG) linfos->file_offset, (fsymb->number+1),linfos->nature ))) {
			if  ( find_selection_compare( token,
			(finish = find_finish( linfos->next, linfos->file_number,(EXALONG) linfos->file_offset )) ,
				linfos->nature) ) {
