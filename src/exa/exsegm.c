...
EXAWORD SegmentId;
{
	...
	return( ((Overlay_Map[ SegmentId ] != MOINS_UN) ? Overlay_Map[ SegmentId ] : MOINS_UN ) );
...
EXAWORD	tcp;
{
	...
	/* ----------------------------------- */
	if ((sgcp = (struct context PTR) allocate(sizeof(struct context))) == (struct context PTR) 0) {
...
VOID	LiberateSegmentContext()
{
	...
	/* ----------------------------- */
	if ( TcodeSegmentTable == (struct segmform PTR) 0 ) return;
	...
	if (((*(TcodeSegmentTable + segdt)).segm_OP & 0x0002 ) == 0) {
		...
		/* ----------------------------------------- */
		ControlLiberate((BPTR PTR) & LocalDataSegment);

		if ( ((*(TcodeSegmentTable + segdt)).segm_AK != 0 ) &&
       !( (*(TcodeSegmentTable + segdt)).segm_OP & 0x0001 )) { /* Resident segment */

			ControlLiberate((BPTR PTR) & (*(TcodeSegmentTable + segdt)).segm_KS );
			}
...
EXAWORD	rest_segm()
{
	...
	/* -------------------------------- */
	if ((sgcp = (struct context PTR) minder [--mindex]) != (struct context PTR)0) {
		...
		else	timersem	= (( sgcp->seg_EVS & TIME_RAZ ) | ( timersem & ~TIME_RAZ ));
...
EXAWORD	segdt;
{
	...
	if ( LocalDataSegment == (BPTR) 0 )  {
		...
		if (((*(TcodeSegmentTable + segdt)).segm_OP & 0x0002) != 0) { 
			...
			/* -------------------------------------------- */
			if ((LocalDataSegment = init_data((Tcodeword((LocalDataTable))),(Tcodeword((LocalDataTable + (2*WORDSIZE)))),(LocalDataTable + (3*WORDSIZE)))) == (BPTR) 0) {
		...
		/* --------------------------------------------------- */
		else if ((ldslen = (Tcodeword((LocalDataTable + (2*WORDSIZE))))) == 0)
			LocalDataSegment = (BPTR) 0;
...
/*
 *	I n i t i a l i s e S e g m e n t T a b l e ()
 *	----------------------------------------------
 *
...
EXAWORD	InitialiseSegmentTable()
{
	EXAWORD	segdex,x;
	BPTR	temp_base;
...
{
	BPTR	rptr;
	if (( rptr = allocate( (MAX_OVERLAYS * sizeof(EXAWORD)) ) ) != (BPTR) 0)
		memcpy((BPTR) rptr,(BPTR)  Overlay_Map , (MAX_OVERLAYS * sizeof(EXAWORD)));

	return( rptr );
}

VOID	UseOverlayMap(rptr)
BPTR	rptr;
{
	if ( rptr ) {
		memcpy((BPTR)  Overlay_Map,(BPTR) rptr , (MAX_OVERLAYS * sizeof(EXAWORD)));
		liberate( rptr );
		}
	return;
}
...
/*
 *	L O A D   S E G M E N T   T A B L E ( secteur , longeur )
 *	---------------------------------------------------------
 *
...
EXAWORD	LoadSegmentTable(secteur,longeur)
EXAWORD	secteur,longeur;
{
	EXAWORD x;
	BPTR	wptr=(BPTR) 0;
	BPTR	LocalSegmentTable=(BPTR)0;

	/* Reset Segment ID to Structure Entry MAP table */
	/* --------------------------------------------- */
	memset((BPTR)  Overlay_Map , MOINS_UN, MAX_OVERLAYS * sizeof(EXAWORD));
	...
	/* ----------------------------------------- */
	SegmentCount = (longeur / (12 * WORDSIZE));

	if ((TcodeSegmentTable = (struct segmform PTR) allocate((SegmentCount+1) * sizeof( struct segmform ))) == (struct segmform PTR) 0)
		return( SOFTERROR );
	else	memset( TcodeSegmentTable, 0, (SegmentCount+1) * sizeof( struct segmform ));

#ifdef	VMS_OPTIMISED
	/* Attempt to load Segment description table */
	/* ----------------------------------------- */
	if (!(LocalSegmentTable = wr_lode(secteur,longeur)))
		return(SOFTERROR); 
		
#else
	/* Attempt to load Segment description table */
	/* ----------------------------------------- */
	LocalSegmentTable = kode_lode(secteur,longeur);
	wptr = LocalSegmentTable;
	if (!( LocalSegmentTable )) return(SOFTERROR); 
	...
	for ( x = 0; x < SegmentCount; x++ ) 
	{
		...
		(*(TcodeSegmentTable + x)).segm_ID = Tcodeword((wptr + (0*WORDSIZE)));	/* Numero du segment 	*/
		(*(TcodeSegmentTable + x)).segm_AC = Tcodeword((wptr + (1*WORDSIZE)));	/* Adresse disque code 	*/
		(*(TcodeSegmentTable + x)).segm_LC = Tcodeword((wptr + (2*WORDSIZE)));	/* Longeur du Code	*/
		(*(TcodeSegmentTable + x)).segm_AD = Tcodeword((wptr + (3*WORDSIZE)));	/* Adresse disque data  */	
		(*(TcodeSegmentTable + x)).segm_LD = Tcodeword((wptr + (4*WORDSIZE)));	/* Longeur du Data	*/
		(*(TcodeSegmentTable + x)).segm_OP = Tcodeword((wptr + (5*WORDSIZE)));	/* Option Value     	*/

		if (((*(TcodeSegmentTable + x)).segm_AK = Tcodeword((wptr + (6*WORDSIZE)))) == MOINS_UN)
			(*(TcodeSegmentTable + x)).segm_AK = 0;
#ifdef	VERSION666
		else if (( (*(TcodeSegmentTable + x)).segm_AK == 0x0FFFF ) && ( WORDSIZE == 2 ))
			(*(TcodeSegmentTable + x)).segm_AK = 0;
#endif
		if (((*(TcodeSegmentTable + x)).segm_LK= Tcodeword((wptr + (7*WORDSIZE)))) == MOINS_UN )
			(*(TcodeSegmentTable + x)).segm_LK = 0;
#ifdef	VERSION666
		else if (( (*(TcodeSegmentTable + x)).segm_LK == 0x00FFFF ) && ( WORDSIZE == 2 ))
			(*(TcodeSegmentTable + x)).segm_LK = 0;
#endif
 	...
 	liberate( LocalSegmentTable );

	/* Allow further initialisations */
	/* ----------------------------- */
	return(InitialiseSegmentTable());
...
EXAWORD	secteur,longeur;
{
	...
	/* Reset Segment ID to Structure Entry MAP table */
	/* --------------------------------------------- */
	memset( (BPTR) Overlay_Map , MOINS_UN , MAX_OVERLAYS );
	...
	/* --------------------------------------------- */
	if ((TcodeSegmentTable = (struct segmform PTR) kode_wr_lode(secteur,longeur)) == (struct segmform PTR) 0)
		return(SOFTERROR); 
		
#else
	/* Attempt to allocate Segment description table */
	/* --------------------------------------------- */
	if ((TcodeSegmentTable = (struct segmform PTR) kode_lode(secteur,longeur)) == (struct segmform PTR) 0)
		return(SOFTERROR); 
...
register EXAWORD x,y;
	...
	if ((longeur % TCODEBLOCK) != 0) { y++; }

	/* Attempt to allocate Segment description table */
	/* --------------------------------------------- */
	if ( (TcodeSegmentTable = (struct segmform PTR) allocate((y * TCODEBLOCK))) == (struct segmform PTR) 0)
		return(SOFTERROR); 
	...
	/* ---------------------------------------------- */
	if ( SectRead(TcodeFileHandle,(wptr = (BPTR) TcodeSegmentTable),(y * TCODEBLOCK),secteur) == MOINS_UN )
		return(SOFTERROR); 
		

	/* Reorganise Segment Table with respect to machine characteristics */
	/* ---------------------------------------------------------------- */
	for ( x = 0 ; x < longeur ; x += WORDSIZE ) { *((WPTR) ( wptr + x )) = Tcodeword( (wptr + x) ); }
...
VOID	LiberateSegmentTable()
{
	...
	if ( TcodeSegmentTable != (struct segmform PTR) 0) {

		for (segdt = 0; segdt < SegmentCount; segdt++ ) {
			ControlLiberate((BPTR PTR) & (*(TcodeSegmentTable + segdt)).segm_CS );
			ControlLiberate((BPTR PTR) & (*(TcodeSegmentTable + segdt)).segm_DT );
			ControlLiberate((BPTR PTR) & (*(TcodeSegmentTable + segdt)).segm_DS );
			ControlLiberate((BPTR PTR) & (*(TcodeSegmentTable + segdt)).segm_KS );
			}
...
BPTR	const_buffer;
{
	...
	/* ---------------------------- */
	segstop = ((*(TcodeSegmentTable + segdt)).segm_LC - WORDSIZE);
...
EXAWORD	seg_no,tcp;
{
	...
	/* ---------------------------- */
	if (StepOverMode > 0) (StepOverMode)++;
#endif
	...
	if (Segment_Load(segdt) != SOFTERROR) { 
		...
		if ( getdtcur(cur_seg) == SOFTERROR ) {
			...
			/* ------------------------------------- */
			if ((datapnt = (Tcodeword((TcodeSegment + segstop)))) == MOINS_UN) {
...
#ifdef	VERSION666
			else if (( WORDSIZE == 2 ) && ( datapnt == 0x0FFFF )) {
				datamax = 0; datadex = 1; 
				...
				/* -------------------------------- */
				datamax = (Tcodeword( (TcodeSegment + segstop - WORDSIZE)));
				datadex = 1;
				}
...
EXAWORD ret_seg(EXAWORD	modus_operandis)
{
	...
	/* Stack managment for StepOver */
	/* ---------------------------- */
	if (StepOverMode > 1) (StepOverMode)--;
#endif
