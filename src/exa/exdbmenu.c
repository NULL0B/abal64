...
EXAWORD	QmanagerSave=1;
static	BYTE	DebugScriptName[256];

EXAWORD	GetQmanSave() { return( QmanagerSave ); }

VOID	SaveDebugConfiguration()
{
	save_break_points((BPTR) DebugScriptName );
	return;
}
...
EXAWORD	number;
{
	collect_local_variables( segproc, (segproc ? (number * (8*WORDSIZE)) : number) );
...
{
	EXABYTE	c;
	struct segmform PTR wptr;
	wptr = (struct segmform PTR) TcodeSegmentTable;
		...
		case 'D'	: cmdptr = CmdShift(cmdptr);
				  ...
				  if ( *wptr != ':' ) dptr = GlobalDataSegment;
					...
					switch ( modo ) {
						...
						case 0x0087 : dptr = (BPTR) TcodeWordStack; break;
						case 0x0047 : dptr = (BPTR) TcodePtrStack; break;
...
BPTR	cmdptr;
{
	...
	*(aptr++) = IO_NUL;
	...
	err = x_exec((BPTR) wptr,(BPTR)pptr,0);
#endif
#endif
#endif
...
#ifdef	ABAL21
		if ( trad_ver > '1' )
			db_xcprintf((BPTR) get_procedure_name((procid * (8*WORDSIZE))) );
		else
#endif
			AffProcName(( procid * (8*WORDSIZE) )); 
		db_xputch(':'); 
		}
...
VOID	prompt_version1()
{
	...
	if ( ListStatus & 0x0001 ) {
		...
		/* -------------------------- */
		if (sicico & ONSEM_PRC) {
			RetStatus = idb_window((procid * (8*WORDSIZE)),(EXAWORD) CurrentTcodeOffset(0) ,1);
			}
		else	{
...
{
	return( (sicico & ONSEM_PRC ? (procid * (8*WORDSIZE)) : cur_seg) );
}
...
/*	for the new visual exadb interface initialisation	*/
/*	-------------------------------------------------	*/
EXAWORD	get_brayk_value()	{	return( brayk );	}
...
EXAWORD	llthisvarb( x , vind , vand, flg )
...
#ifdef	ABAL21
#ifdef	VERSION666
		if (!( DebugStandardErrors( err_val ) ))
#endif
			standard_errors( err_val );
#endif
		ireg [ 0 ] = svind; 	
...
EXAWORD	x,vind,vand,flg;
{
	...
	EXAWORD	result;
	xworktop = worktop;
	result = llthisvarb(x,vind,vand,flg);
	worktop = xworktop;
	return(result);
}	
...
EXAWORD	flg;
{
	...
	do {
		...
		/* ---------------------- */
		do {	x = Tcodeword( vb);  
			if ( x == EXDB_FFFF) { newlyne(); db_xcprintf((BPTR)TalkPtr[56]); return(1); }
			vb += WORDSIZE; 
			}
		while (( x & 0x0010 ) != 0);
...
VOID	idb_backup()
{
	if ( QmanagerSave ) { SaveDebugConfiguration(); }
	return;
}
...
{
#ifdef	ABAL21
	configuration_file_name((BPTR)DebugScriptName,256);
	execute_debug_script((BPTR) DebugScriptName, 0 );
#endif	
	return;
}
...
EXAWORD	CtrlProc;
{
	...
	idb_restore();			/* Reload Eventual Break Points	*/
#ifdef	VERSION666
	WindowEditorResetFile();	/* Graphical Mode Editor Reset	*/
#endif
	return;				/* Return to EXSTOP()		*/
}

/*	-----------------------	*/
/*	save_last_tcode_adresse	*/
/*	-----------------------	*/
...
/*	execute_single_step	*/
/*	-------------------	*/
VOID	execute_single_step(int caller) /* 0: normal, 1: visual */
{
	save_last_debug_tcode();
	...
	if ( StopStatus == 0 ) {
		...
		/* ---------------------------------- */
		/* if ( exa_is_X11 < 2) { asfun_program_implicite(); } */
		...
		/* ---------------------------------- */
		/* if ( exa_is_X11  < 2 ) { asfun_monitor_implicite(); } */
...
/*	execute_till_break	*/
/*	------------------	*/
VOID	execute_till_break(int caller) /* 0: normal, 1: visual */
{
	save_last_debug_tcode();
...
/*	listmanagerfunction	*/
/*	-------------------	*/
EXAWORD	listmanagerfunction(BPTR cmdptr) 
{
	EXAWORD	c;
	c = toupper( *cmdptr ); cmdptr++;
	switch ( c ) {
		case	'A'	:
			if ( *cmdptr == '1' )
				activate_assembler_list(2);
			else	inhibit_assembler_list(2);
			break;
		}
}
#endif
...
/*	newvarmod	*/
/*	---------	*/
EXAWORD	newvarmod(BPTR  lcmdptr, EXAWORD mode)
{
	EXAWORD	vid=0;
	EXAWORD	primary=0;
	EXAWORD	secondary=0;
	if ( *lcmdptr != '/' ) {
		...
		vid = HexaTcodeId( lcmdptr );
		while ( *lcmdptr != 0 ) {
			if ( *lcmdptr == '(' ) {
				if ( primary ) return(0);
				lcmdptr++;
				while ((*lcmdptr >= '0') && ( *lcmdptr <= '9'))
					primary = ((primary*10) + ( *(lcmdptr++) - '0'));
				if (!( primary )) return(0);
				if ( *lcmdptr == ',' ) {
					lcmdptr++;
					while ((*lcmdptr >= '0') && ( *lcmdptr <= '9'))
						secondary = ((secondary*10) + ( *(lcmdptr++) - '0'));
		...
		switch ( (worktyp1 & VARB_ISOL) ) {
			...
			case	VARB_INT64	:
				...
				workval2 = 0;
				while ((*lcmdptr >= '0') && ( *lcmdptr <= '9'))
					workval2 = ((workval2*10) + ( *(lcmdptr++) - '0'));
				varconv(0);
			case	VARB_BCD	:
				return(0);
			case	VARB_STR	:
				worktyp2 = VARB_STR;
				workptr2 = lcmdptr;
				for ( worklen2=0; *(lcmdptr+worklen2) != 0; worklen2++);
				varconv(0);
			default			:
				return(0);
			}
...
/*	----------------------------	*/
/*	New Debug Command Extensions 	*/
/*	----------------------------	*/

/*	----------------	*/
/*	Qmanagerfunction	*/
/*	----------------	*/
VOID	Qmanagerfunction( lcmdptr )
BPTR	lcmdptr;
{
	switch ( *(lcmdptr++) ) {
		case	'C'	:	/* configuration functions */
			switch ( *(lcmdptr++) ) {
				case	'S'	:	/* configuration save */
					if ( *lcmdptr != '0' )
						QmanagerSave = 1;
					else	QmanagerSave = 0;
					break;
...
#ifdef	VERSION666
		case 'L' :	
			listmanagerfunction( lcmdptr );
			break;
		case 'Q' :
			Qmanagerfunction( lcmdptr );
			break;
#endif
		case 'W' :	
 ...
 *
 */
BPTR	DebugImageName(item)
EXAWORD	item;
{
	switch ( item ) {
		/* background texture image */
		/* ------------------------ */
		case	0 : return( "dbtexture.png");
		case	1 : return( "dbstep.png"   );
		case	2 : return( "dbgoto.png"   );
		case	3 : return( "dbrun.png"    );
		case	4 : return( "dbexit.png"   );
		case	5 : return( "dbbreak.png"  );
		case	6 : return( "dbclear.png"  );
		case	7 : return( "dbview.png"   );
		case	8 : return( "dbhelp.png"   );
		case	9 : return( "dbsave.png"   );
		case	10: return( "dbcursor.png" );
		case	11: return( "dbtrap.png"   );
		case	12: return( "dbstart.png"  );
		case	13: return( "dbgrid.png"   );
		case	14: return( "dbexec.png"   );
		}
	return("\0");
}
...
#ifndef	NOGIGO
	if ( exa_is_X11 > 1 )  {
		if (!( visifig = getenv("VEXADB") ))
			visifig = "vexadb";
		}
	else	visifig = "\0";
	if  ( exadb_strlen( visifig ) != 0) {
		permdb = 0;
		brayk  = 0;
		VisualDebugOpen(visifig);
		visual_debug(
				(BPTR) 0,(BPTR) 0,(BPTR) 0,(BPTR) 0, (BPTR) 0,(BPTR)0,(BPTR) 0,
				(BPTR) 0,(BPTR) 0,(BPTR) 0,(BPTR) 0, (BPTR) 0,(BPTR)0,(BPTR) 0,
				(BPTR) 0,(BPTR) 0,(BPTR) 0,(BPTR) 0, (BPTR) 0,
				(BPTR) 0,(BPTR) 0,(BPTR) 0,(BPTR) 0, (BPTR) 0,(BPTR)0,(BPTR) 0, (BPTR) 0, (BPTR) 0);
		visual_debug_detach();
		VisualDebugClose();

		/* -------------------------------------------- */
		/* Ensure Auto save of configuration is allowed */
		/* -------------------------------------------- */
		if ( QmanagerSave ) { SaveDebugConfiguration(); }
...
EXAWORD	natb;
{
	if ( exa_is_X11 < 2 ) 
	{
		...
		case 2 : ColourAtb(2); break;

		//** list line normal
		case 4 : Painter(Couleurs[14][1],Couleurs[14][0]); break;
		//** list line pointer
		case 5 : Painter(Couleurs[14][0],Couleurs[14][1]); break;
		//;** execution pointer
		case 6 : Painter(Couleurs[8][0],Couleurs[8][1]); 
		}
	}
	else	
	{
		...
		switch ( natb ) 
		{
		case 0 : Painter( 8, 15 ); break;
		case 1 : Painter( 4, 15 ); break;
		case 2 : Painter(15,  7 ); break;
		}
	}
	return;
}
