 ...
 * 16/10/2000 : (#SP)switching off prefixes in short names mode for Criteria 3.6 or higher
 *
 */
...
EXAWORD NewAnalyseSyntaxique(EXAWORD FlagCount)
{
	...
	/* Allocate memory */
	/* --------------- */
	if (!( BuffQuestion=(BPTR)allocate((EXAWORD)(worklen1 + 1)) )) 
	{
		return (EXER_MEMFULL);
	}
	...
	/* Clean the zone */
	/* -------------- */
	memset((BPTR)BuffQuestion,0,(worklen1 + 1));
	...
	/* -------------------- */
	worklen2=NewLengh;
	memcpy((BPTR)workptr2,(BPTR)NewDescriptor,(USINT)NewLengh);
...
/*      - 1 : descriptor has ben built correctly            	*/
/*--------------------------------------------------------------*/
#ifndef ABAL21
	...
	
{
	...
	else if (FlagP == 2) 
	{
		...
		/* Memory allocation for the descriptor (BUG FIX => +RUB_DESC_LEN_EXT #BG Pourquoi ?) */
		if (!(GlobStruct.DescPtr = (U8PTR) allocate ((GlobStruct.DescLength + RUB_DESC_LEN_EXT))))
			return(EXER_MEMFULL);

		memset(GlobStruct.DescPtr, 0, GlobStruct.DescLength + RUB_DESC_LEN_EXT);	
	...
	/* we analyse the input line = the quest */
...
FNT_LOC S16 NPos_Expression (Par, Err)
	...
	S16PTR Err;
 {
	...
	if (Par->Curlex == ANA_TOK_OBR) 
	{
		/* Opening bracket */
		if (Par->Flag == 1)
			/* we add length of the opening bracket token */
			Par->DescLength += 1;
...
/* local variables */
	...
	if (Par->Curlex == RUB) 
	{
		...
		/* FCH04052006 pour être conforme à l'ancien code */
		exaColLength = Par->ColLength; exaColType   = colType; 

		if (!GetColumn(fidreel,tableName,(EXAWORD)(colName-tableName-1),colName,strlen(colName),&exaColLength,&exaColType)) 
			error = 1;
		else	
		{
			colType = (EXABYTE) exaColType;
...
FNT_LOC S16 NPos_Value (Par, Err)
	...
	S16PTR Err;
 {
	...
	case CR_COL_CHAR : // old MC CHAR type has to be SQL BINARY
#ifdef	CR_COL_STRING
	case CR_COL_STRING : // new SQL CHAR type 
#endif
#ifdef	CR_COL_FLOAT
	case CR_COL_FLOAT : // new SQL FLOAT type 
#endif
#ifdef	CR_COL_TIME
	case CR_COL_TIME : // new SQL TIME type 
#endif
#ifdef	CR_COL_DATE
	case CR_COL_DATE :
#endif
...
FNT_LOC S16 NPos_Value3 (Par, Err)
	...
	S16PTR Err;
 {
	...
	/* ---------------------------------- */
	/* The column type must be alpha num  */	
	/* or a new date and time type column */
	/* ---------------------------------- */
#ifdef ABAL21
	if (( Par->ColType!=CR_COL_CHAR   )
#ifdef	CR_COL_TIME
	&&  ( Par->ColType!=CR_COL_TIME   )
#endif
#ifdef	CR_COL_DATE
	&&  ( Par->ColType!=CR_COL_DATE   )
#endif
	&&  ( Par->ColType!=CR_COL_STRING ))
#else
	...
	S16PTR Err;
{
	...
	while (1) 
	{
		switch (Par->Etat) 
		{
			/* Operator test */
			case 0 :
				//#BG 09/12/20 Eviter une boucle infinie et 100% CPU !
				if (Par->Pos == Pos0)	
				{
					Par->Etat = 27;
					break;
			...
			/* value et column test					*/
		 	/* If a value or a column is longer than 10 caracters, 	*/
			...
			case 20 :
				...
				/* ---------------------------------------------------- */
				/* aye, but first we check we dont overflow buffers con */
				/* ---------------------------------------------------- */
				if ((i = (Par->Pos - Par->Pos0 - 2)) > MAX_VALUE_SIZE)
				{
					/* ------------------------- */
					/* bah oui l'erreur poubelle */
					/* ------------------------- */
			  		*Err = EXER_MCFUNC;
					return(0);
				}
				else if (Par->EscBool == 0)
				{
			...
			case 22 :
				...
				if ((i= (Par->Pos - Par->Pos0)) > MAX_VALUE_SIZE)
				{
					/* ------------------------- */
					/* bah oui l'erreur poubelle */
					/* ------------------------- */
	...
	S16PTR Err;
{
	...
	switch (Par->ColType)
	{
		...
		case CR_COL_BIN :
			...
			if (Par->CurCompVal == ANA_OPR_EQ || Par->CurCompVal == ANA_OPR_NE)
			{
				while ((i<Par->ValBytes) && ( i < MAX_VALUE_SIZE ))
				{
					c=Par->CurVal[i];
					switch(c)
					{
		...
		case CR_COL_DECI :
			...
			switch (Par->ColLength) 
			{
				...
				case 1 :
					/* Converted Value (1 octet) */
					Val = (U16)exatoi((U8PTR)(Par->CurVal), (EXAWORD)(strlen((BPTR)(Par->CurVal))));
				...
				case 2 :
					...
					Val = (U16)exatoi((U8PTR)Par->CurVal, (EXAWORD)(strlen(Par->CurVal)));
		...
		case CR_COL_LONG :
			...
			/* Converted Value (4 octets) */
			lVal = (U32)exatol((U8PTR)Par->CurVal, (EXAWORD)strlen(Par->CurVal));
			balputl(lVal, Par->DescPtr + Par->DescPos);
			Par->DescPos += 4;
			break;
		...
		case CR_COL_HUGE :
			...
			/* Converted Value (8 octets) */
			hVal = (U64)exatol((U8PTR)Par->CurVal, (EXAWORD)strlen(Par->CurVal));
			balput64(hVal, Par->DescPtr + Par->DescPos);
			Par->DescPos += 8;
			break;
