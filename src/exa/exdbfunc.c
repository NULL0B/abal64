...
#include "exdbffff.h"		/* minus one definitions		*/
...
#include "exdbmimo.h"		/* Mouse definitions			*/
				/* -----------------------------------	*/
EXABYTE	ftc[128];		/* False Tcode matrix			*/
				/* -----------------------------------	*/
#include "exproto1.h"
...
#ifdef	ABAL21
	ftc[(tcpi++)] = 0x006E; 		/* X = 			*/
	TcodePmot( &ftc[tcpi], 0 );	tcpi += WORDSIZE;
	ftc[(tcpi++)] = 0x000E; 		/* I = 			*/
	TcodePmot( &ftc[tcpi], 0 );	tcpi += WORDSIZE;
	ftc[(tcpi++)] = 0x002E; 		/* U = 			*/
	TcodePmot( &ftc[tcpi], 0 );	tcpi += WORDSIZE;
...
#endif
	ftc[(tcpi++)] = 0x004C; 		/* ASK			*/
	TcodePmot( &ftc[tcpi], vid );	tcpi += WORDSIZE;
...
EXAWORD	reg;
{
	...
	i = 0;
	while ( *tptr != IO_NUL ) {
		*(TcodeByteStack + worktop + i + (2*WORDSIZE)) = *(tptr++); i++;
		}
	*(TcodeByteStack + worktop + i + (2*WORDSIZE)) = ' '; i++;
	TcodePmot( (TcodeByteStack + worktop) , 3 );
	TcodePmot( (TcodeByteStack + worktop + WORDSIZE) , i );
...
VOID	generate_register(EXAWORD r, BPTR wptr, EXAWORD hexa)
{
	if ( hexa )
		sprintf(wptr,"%u: %x %x (%x %x)",r,xreg[r],freg[r],ireg[r],areg[r]);
	else	sprintf(wptr,"%u: %u %u (%u %u)",r,xreg[r],freg[r],ireg[r],areg[r]);
	return;
}
...
BPTR	cmdptr;
{
	...
	EXAWORD	tcpi=0;		/* Tcode Pointer Index			*/
				/* ------------------------------------ */
	...
	/* ---------------------- */
	tcpi=0;
	ftc[tcpi++] = _NGF_ASSIGN;		/* Operation NGF ASSIGN	*/
	ftc[tcpi++] = 0x0017;		/* IMMEDIATE / REGISTRE	*/
	ftc[tcpi++] = 0x0010;		/* IMMEDIATE / **FIN**	*/
	TcodePmot( &ftc[tcpi], 1 );	tcpi += WORDSIZE;
	ftc[tcpi++] = 0x0001;		/* Filename REGISTRE 1	*/
	TcodePmot( &ftc[tcpi], 0 );	tcpi += WORDSIZE;
	...
	if ( Tcode_Err != SOFTERROR ) {
		...
		if ((mode & 2) == 0) {
			...
			/* ----------------------------------- */
			TempString( aptr , 1 );
			ftc[tcpi++] = 0x0016;		/* IMMEDIATE / REGFIN  	*/
			TcodePmot( &ftc[tcpi], 1 );	tcpi += WORDSIZE;
			ftc[tcpi++] = 0x0001;		/* Filename REGISTRE 1	*/
			}
...
EXAWORD	get_dieze_files() { return( diezfic ); }

int	load_file_table_entry( int i )
{
	int	hlfh=lfh;
	int	status;
	if (( lfh = i ) > diezfic )
		status = 116;
	else	status = loadfich();
	lfh = hlfh;
	return( status );
}
...
BPTR	build_file_table_entry()
{
	...
	else	exadb_strcpy((BPTR) askbuf, (BPTR) "unused");
...
EXAWORD	item;
{
	...
	x_bstart();
	if ((err_val = load_file_table_entry(item)) != 0) {
	...
	left_string( nc+7, nl, 57, (BPTR) bptr  );
	x_bflush();
	return;
}
...
int	sigid;
{
	...
	VerifVal = 1;
#ifdef	LINUX
	longjmp( dbsignalfailure, VerifVal );
#endif
}
...
BPTR	wptr;
{
	...
	int	(*HoldMem)();
#ifdef	LINUX
	if ((VerifVal = setjmp(dbsignalfailure))!=0) {
		goto verifptrexit;
		}
#endif
	VerifVal = 0;
