...
switch( fidperm & NGF_TYP ) {
	...
	case NGF_XML :
		fidreel = 0;
		if ((errno = AbalXmlOpen( fidname, fidperm, & fidbuff)) != 0) 
			{ 
...
/* #BG Remplace par CONNECT
#ifdef ABALSQL
	case NGF_SQL  :
		fidreel = sql_open( NormalFile, fidperm );
		break;
#endif
*/
	...
	case NGF_IM :	/* Printer file / Device opening 	*/
			/* -----------------------------	*/
#define	USEIT_CAN_PRINT_TO_FILE
#ifdef	USEIT_CAN_PRINT_TO_FILE
		ph = printer_open( fidname, (fidperm | NGF_I_C) );
		if ( ph == MOINS_UN)
			fidreel = EX_UNUSED_HANDLE;
