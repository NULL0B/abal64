...
/*----------------------------------------------------------------------*/
			...
			if ( (tcode = (EXAWORD) *(fdex++)) & 0x0080) {
				...
				/* ------------------------------ */
				if ((timersem & (TIME_ACTIF|EVENT_SWAP)) != 0) {
				   ...
				   if (( timersem & TIME_ACTIF) != 0) {
					...
					if (((EXAWORD) descarto[0] & TIME_ALARM ) == 0) {
						...
						/* Test for ON TIMER CALL */
						/* ---------------------- */
						if (( timersem & TIME_CALL ) != 0) {
							timer_call_procedure( timeradr );
							}
						else	{
...
KONINDEX:
	if ( *workptr2 == 0 ) 	{
		workptr2++; worklen2 = (EXAWORD) *(workptr2++);
		ireg [e] = (EXAWORD) bcdtoi( workptr2, worklen2 );
		}
	else	{ 
		worklen2 = (EXAWORD) *(workptr2++);
...
KONANDEX:
	if ( *workptr2 == 0 ) 	{
		workptr2++; worklen2 = (EXAWORD) *(workptr2++);
		areg [e] = (EXAWORD) bcdtoi( workptr2, worklen2 );
		}
	else	{ 
		worklen2 = (EXAWORD) *(workptr2++);
...
case _tc_PSH	:
	...
	if ( xreg [tcode] & PTRSTATUS ) { sp_hold [pushtop++] = VARIABLE; }
	else				{ sp_hold [pushtop++] = (EXAWORD) xreg [ tcode ]; }
	tcode = 1;
	if (pushtop >= diezstk) { 
...
case _tc_STC	:	/* STO CON / VAR */
	tcode = SOFTERROR;
	workptr2 = (TcodeConstants + Tcodeword( fdex )); fdex += WORDSIZE;
	if ( *workptr2 == 0 ) 	{ worktyp2 = VARB_BCD; workptr2++; }
	else			{ worktyp2 = VARB_STR; 		   }
	worklen2 = (EXAWORD) *(workptr2++);
...
case _tc_OPV	:
	...
	if ( tcode == 0 ) {
		...
		if ( opb() ) {
			fdex = TcodeSegment + Tcodeword( fdex);
			workval2 = (EXAWORD) (x_getch() & 0x00FF);
			}
...
case _tc_IOF	:	tcode = iowork();	break;
...
SREGEVAL:
	...
	switch ( xreg [ e ] ) {
		...
		case KONSTANT  :
			...
			if ( *workptr1 == 0 ) 	{ worktyp1 = VARB_BCD; workptr1++; }
			else			{ worktyp1 = VARB_STR; 		   }
			worklen1 = (EXAWORD) *(workptr1++);
			goto DOCALC;
		case PTRSTR    :
			worktyp1 = VARB_STR;
			worklen1 = lreg [e];
...
case _tc_WHN	: 
	switch ((tcode = (EXAWORD) *(fdex++))) {
		...
		case _ec_RAZ :	timersem &= EVENT_SEMOFF; continue;
