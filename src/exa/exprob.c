...
BINAIRE	x_npos()
{
	...
	if ( idflg == 0 ) {
		...
		do {
			if ((idabal = getutent()) == (struct utmp PTR) 0) {
				return(0);
				}
...
extern EXAWORD datew32(EXAWORD,EXAWORD);
	...
	if (datew32(workval1,workval2) == 0)
		{ err_val = EXER_FORBID; return(0);}
	else
	    { err_val = 0; return(1);}
#else
...
EXAWORD factor;	
{
	...
	if ( dixsec > 0 ) {
		if ( factor != 10 )
			usleep((dixsec * 100000 * factor));
		else	{
			sec = dixsec; try = 4;
			while ( (BINAIRE) sec > 0 && try > 0 ) {
				sec = sleep(sec);
				try--;
				}
			}
...
extern	int	GetNoWait(void);
	...
	while (GetNoWait() != 0) ;			/* purge message queue */
	return;
...
#ifdef	UNIX
#include "xsystem.c"
extern	iof_screen_manager();
#endif

EXAWORD	x_exec(fn,fa,fx)
BPTR	fn;
BPTR	fa;
EXAWORD	fx;

{
	...
	while ( *fptr ) { fptr++; x++; }
#endif
	if ((fptr = (BPTR) allocate(worklen2 + x + 4)) == (BPTR) 0)
		return( EXER_MEMFULL );
...
#ifdef	ABAL32A
	/* Detect Multi Tasking Load.Go */
	/* ---------------------------- */
	if ( fx & 2 ) {
		x = multitask_system( fptr );
		}
	else	{
#endif
		/* Inhibit Loadgo Refresh Loop because sharing Window */
		/* -------------------------------------------------- */
		if (!( fx & 512 )) {
	...
	*(fptr + x + worklen2) = (EXABYTE) 0;
	...
	fptr = fn; x = 0;
	while (( *fptr != ' ' ) && ( *fptr != (EXABYTE) 0)) { fptr++; x++; }
	fptr = (BPTR) allocate(worklen2 + x + 2);
...
// YT201098 : set the first char to the space ' ' char
#ifdef WIN32
	cmdptr[0] = ' ';
#endif
	...
	*(cmdptr + indent) = (EXABYTE) RETCHAR;
	...
	if ((specptr = (BPTR) getenv((BPTR) "COMSPEC")) != (BPTR) 0) {
		...
		/* Execute command via DOS FUNCTION 4B00H */
		/* ------------------------------------- */
