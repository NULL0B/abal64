 ...
 * 	F) -  	LOADUSER( number, string ) Set name of BDA/DLL
 */

EXTERN	EXAWORD	WaitFactor;
...
if (argset((((desc = ((EXAWORD) *(fdex++))) & 0x0060) >> 5),0,1) == SOFTERROR) { 
	...
	switch ( worktyp1 ) {
		...
		case	VARB_STR	:	workval1 = exatoi(workptr1,worklen1); break;
		}
	}
...
switch ( x ) {
	case 0 : /* FUNCTION 0 : WAIT */
		 ...
		 ll_wait(workval1,WaitFactor);	/* Perform SLEEP/WAIT   */
	...
	case 4 : /* FUNCTION 4 : KBF  */
		 ...
		 if ( x == 3 ) { 
			...
			if (( 1000 <= worklen3 ) && ( worklen3 < 2000 ))
				return( IpcSend( (worklen3-1000), workptr2, worklen2 ) );

			if ( 2000 <= worklen3 )
				return( tweak( (worklen3-2000), workptr2, worklen2 ) );
	...
	case	10	:	/* END.USER library_number */
		...
		release_dynamic_library( FuseBase, workval1 );
		return( GOODRESULT );
...
/*
 *	-------------------------------------------------------------
 *	F) -    LOADUSER ( lid, name)    	Set DLL Filename
 *	-------------------------------------------------------------
 *
