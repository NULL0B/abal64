...
BPTR	attachmentlist()
{
	...
	for ( 	l=0,
		attlog=AttachmentLog;
		attlog != (ATTLOGPTR) 0;
		attlog = attlog->next) {
		if ( attlog->filename != (BPTR) 0) {
			l += exadb_strlen(attlog->filename);
			l += 16;
			}
		}
	if ((!( l ))
	...
	for ( 	l=0,
		...
		if ( attlog->filename != (BPTR) 0) {
			...
			exadb_strcpy( (BPTR) wptr,(BPTR)  attlog->filename );
			wptr += exadb_strlen((BPTR) wptr );
			*(wptr++) = ' '; *(wptr++) = ':';
			exadb_itoa((BPTR) wptr, nombre );
			if ( attlog->debug != (VPTR) 0 )
				exadb_strcat((BPTR) wptr, (BPTR) " db" );
			wptr += exadb_strlen((BPTR) wptr );
			}	
		}
	return(rptr);
