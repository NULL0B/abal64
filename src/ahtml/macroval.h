#ifndef _macroval_h
#define _macroval_h

#ifndef	_MACRO_PAD
#define	_MACRO_PAD ' '
#endif
unsigned char macrovalue[256] = {

	,



#ifdef	_MACROVALDOS

	0x9B, _MACRO_PAD, 0x2C, 0x9F, 0x22, 0xF7, 0xC5, 0xD8,
	0x5E, 0x25, 0x53, 0x3C, 0x99, _MACRO_PAD, _MACRO_PAD, _MACRO_PAD,

	_MACRO_PAD, 0x60, 0x27, 0x22, 0x22, 0xF8, 0x2D, 0xC4,
	0x7E, 0x54, 0x73, 0x3E, 0x99, _MACRO_PAD, _MACRO_PAD, 0x98,

	_MACRO_PAD,     '≠','Ω','ú','œ','æ','|','ı',
		   '˘','∏','a','"','!','-','©','-',

	'¯','Ò','˝','¸','`','Ê','Ù','˙','˜','˚','o','Ø','¨','´','Û','®',
	'∑','µ','∂','«','é','è','í','Ä','‘','ê','Ô','”','ﬁ','÷','◊','ÿ',
	'—','•','„','‡','‚','Â','ô','û','ù','Î','È','Í','ö','Ì','Ë','·',
	'Ö','†','É','∆','Ñ','Ü','ë','á','ä','Ç','à','â','ç','°','å','ã',
	'–','§','ï','¢','ì','‰','î','ˆ','õ','ó','£','ñ','Å','Ï','Á','ò'
#endif
#ifdef	_MACROVALPRL
	0x9B, _MACRO_PAD, 0x2C, 0x9F, 0x22, 0xF7, 0xC5, 0xD8,
	0x5E, 0x25, 0x53, 0x3C, 0x99, _MACRO_PAD, _MACRO_PAD, _MACRO_PAD,

	_MACRO_PAD, 0x60, 0x27, 0x22, 0x22, 0xF8, 0x2D, 0xC4,
	0x7E, 0x54, 0x73, 0x3E, 0x99, _MACRO_PAD, _MACRO_PAD, 0x98,

	_MACRO_PAD, 0xAD, 0x9B, 0x9C, 0x24, 0x9D, 0x7C, 0xD2,
	0x22, 0x03, 0x46, 0xAE, 0x21, 0xF7, 0x12, 0x4D,
	0xF8, 0xF1, 0xFD, 0x33, 0x2F, 0xE6, 0xD2, 0xF9,
	0xF5, 0x31, 0x4D, 0xAF, 0xAC, 0xAB, 0xF0, 0xA8,
	0x41, 0x41, 0x41, 0x41, 0x8E, 0x8F, 0x92, 0x80,
	0x45, 0x90, 0x45, 0x45, 0x49, 0x49, 0x49, 0x49,
	0xE4, 0xA5, 0x4F, 0x4F, 0x4F, 0x4F, 0x99, 0x78,
	0xE9, 0x55, 0x55, 0x55, 0x9A, 0x59, 0xE2, 0xE1,
	0x85, 0xA0, 0x83, 0x61, 0x84, 0x86, 0x91, 0x87,
	0x8A, 0x82, 0x88, 0x89, 0x8D, 0xA1, 0x8C, 0x8B,
	0xE4, 0xA4, 0x95, 0xA2, 0x93, 0x6F, 0x94, 0xF6,
	0xED, 0x97, 0xA3, 0x96, 0x81, 0x79, 0xE2, 0x98
#endif
	};

#endif  // _macroval_h

