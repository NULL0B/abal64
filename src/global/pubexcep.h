...
#define TRY	{ jmp_buf tx;					\
		  ...
		  _ExcPushCtx((EXCEPTION_CONTEXT FAR *)&Current,\
			      (BYTE FAR *)__FILE__,__LINE__);	\
		  while(TRUE) {					\
			  _ExcPushLink((jmp_buf FAR *)&tx);	\
			  if ((Current.Value = Xsetjmp(tx)) == 0) {\
...
#define THROW(x)	_ExcThrow(x,(BYTE FAR *)__FILE__,__LINE__);

#define THROW_SAME	_ExcThrow(Current.Value,(BYTE FAR *)__FILE__,__LINE__);
...
#define AssViol(x,y)	(_ExcSetAndThrow((BYTE FAR *)x,(BYTE FAR *)#y,(BYTE FAR *)__FILE__,(WORD)__LINE__))
#else
#define AssViol(x,y)	(_ExcSetAndThrow(x,(BYTE FAR *)__FILE__,(WORD)__LINE__))
#endif
