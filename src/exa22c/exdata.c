...
EXAWORD getdata()
{
	...
	if ( *datawork == IO_NUL ) { datawork++; xtyp = VARB_BCD; }
	else { xtyp = VARB_STR; }
	xlen = (EXAWORD) *(datawork++);
	if (( xtyp == VARB_STR ) && (worktyp1 != VARB_STR)) { err_val = ABAL_BCDFORM; return(SOFTERROR); }
	c = IO_ESP;
	switch ( worktyp1 ) {
...
/* printf("\nSetting data pointer : Seg %d , Item %d",cur_seg,x); */
...
if ( x <= datamax ) {	
	...
	while ( y < x ) {
		...
		datawork += l; y++; }
	return(1); }
else {  if (( datapnt == 0 ) || ( datapnt == 0xFFFF )) {
		err_val = 0; return(1);	  
		}
	else {	err_val = ABAL_TCODCODE; return(SOFTERROR); }
	}
}
