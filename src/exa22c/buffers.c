...
#else
#include "stdio.h"		/* Modified IO handling routines	*/
#endif
...
static	EXAWORD	GarbageReady=0;
static	EXAWORD	MessageReady=0;
...
static	EXAWORD	local_garbage()
{
	return( 0 );
}

static	EXAWORD	local_message()
{
	return( 0 );
}

VOID	set_swap_garbage( fptr )
VPTR	fptr;
{
	if ((BufferControl.garbage = fptr ) == (VPTR) 0)
		BufferControl.garbage	= local_garbage;
	return;
}		

VOID	set_swap_message( fptr )
VPTR	fptr;
{
	if ((BufferControl.message = fptr ) == (VPTR) 0)
		BufferControl.message	= local_message;
	return;
}		


EXAWORD	initialise_buffer_control( filename, buffer_size, max_buffers, min_buffers )
BPTR	filename;
EXAWORD	buffer_size;
EXAWORD	max_buffers;
EXAWORD	min_buffers;
{
	...
	BufferControl.swaphandle= EX_UNUSED_HANDLE;
...
void	liberate_buffer_controller()
{
	...
	if ( BufferControl.swaphandle != EX_UNUSED_HANDLE ) {
...
EXAWORD	free_swap_node( adresse )
long	adresse;
{
#ifdef	OPTIMISED_SWAP
...
EXAWORD	swap_buffer_text_out( bcnode )
BCNODEPTR	bcnode;
{
	long	swap_adresse;
	EXAWORD	result;
	...
	if ( BufferControl.swaphandle == EX_UNUSED_HANDLE ) {
		...
		if (( BufferControl.swaphandle = open( BufferControl.swapname,(O_RDWR | O_TRUNC | O_CREAT | O_BINARY),0666)) == -1)
			return( 0 );
#else
		if (( BufferControl.swaphandle = open( (BPTR) BufferControl.swapname, 2, 0)) != EX_FAILURE ) {
			close(  BufferControl.swaphandle  );
			unlink( BufferControl.swapname );
			}
		if (( BufferControl.swaphandle = creat( (BPTR) BufferControl.swapname, 2)) == EX_FAILURE ) {
			BufferControl.swaphandle = EX_UNUSED_HANDLE;
			return( 0 );
			}
#endif
...
EXAWORD	swap_buffer_text_in(handle, bcnode )
BUFFERHANDLE	handle;
BCNODEPTR	bcnode;
{
	EXAWORD	result;
...
EXAWORD	remove_buffer_control_node()
{
