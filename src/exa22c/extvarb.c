...
ULONG	VmemOfs;	/* Buffer  offset    	*/
EXAWORD	VmemDif;	/* = 1;			*/
EXAWORD	VmemExt;	/* = VmemAdj;		*/
ULONG	VmemWot;	/* = adr_phy_fich;	*/
...
LPTR	retlong;
{
	...
	switch ( worktyp & VARB_ISOL ) {
		...
		case VARB_INT16 : indleng = 2; break;
	...
	if ( worktyp & NDX1st) {
		...
		temp3 += ( indleng * ( temp - 1 ) );
		if ( worktyp & NDX2nd) {
			temp = areg [reg]; temp2 = Tcodeword( descptr); descptr += WORDSIZE;
			IndexCheck( temp , descptr );
			temp2 *= indleng; temp3 += ( temp2 * ( temp - 1 ) );
			}
		}
	*retlong += temp3;	
	return(indleng);
...
{
	ULONG	dbyte;		/* File byte offset of starting block	*/
	...
	/* ------------------------------------------ */
	dbyte = ((ULONG) Tcodeword((descptr + WORDSIZE)) ) << 16;
	dbyte += (ULONG) Tcodeword(descptr); descptr += (2 * WORDSIZE);

	/* Adjust byte starting point for Array variables */
	/* ---------------------------------------------- */
	if ((extvleng = indent(worktyp,descptr,reg,(LPTR) &dbyte)) == SOFTERROR) {
		reset_file_device();
		return(SOFTERROR); 
		}
...
BPTR	vp;			/* Pointer to the Workspace Value	*/
{
	...
	/* ------------------------------------------ */
	if ((extvleng = indent( worktyp , descptr , reg ,(LPTR) &adr_phy_fich)) == SOFTERROR) {
		reset_file_device();
		return(Semaphoff(SOFTERROR)); 
		}
...
#ifdef	ABAL14
/*
 *	L TO B C D ( bcdptr , bcdlen , LONG_value )
 *	-------------------------------------------
...
EXAWORD	ltobcd( bcdptr , bcdlen , longvalue )
BPTR	bcdptr;
EXAWORD	bcdlen;
EXALONG	longvalue;
{
...
#define	EMEMPTR	struct etdform SPTR
struct	etdform	{
	EXAWORD	nbpage;
	PAGEPTR	pageptr[2];
};
...
#define	allocate_extended_page(n)  (PAGEPTR) malloc(ETDPAGE * 16)
#define liberate_extended_page( p ) free( p );
...
EXAWORD	nbparas;
{
	...
	/* --------------------------- */
	if ((ExtendedMemory = (EMEMPTR) malloc( ((x+1)  * sizeof(PAGEPTR)) + WORDSIZE )) == (EMEMPTR) 0)
		return(EXER_MEMFULL);
...
EXAWORD	workset,reg;
{
	...
	if ( ProcSwap == 0 ) {
		if ((bigvlen = indent(worktyp,descptr,reg,(LPTR) &bigvofs)) == SOFTERROR) {
			return(SOFTERROR); 
			}
		}
	...
	if ( worktyp & (ISPOINTER | ISDYNAMIC | ISMEMBER) ) {
		...
		switch ( worktyp ) {
			...
			case VARB_INT16 : awval = (BINAIRE) balgetw(bigbase);break;
			default 	: awval = 0; break;
			}
