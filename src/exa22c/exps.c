	...
	switch ( tcode ) {
		...
		case _esc_POPDS : if ( mindex == 0 ) { err_val = 105; return(0); }
				   switch ( e ) {
					...
					case 6 : IP_Hold   = (UBINAIR *) minder [--mindex]; break;
					case 7 : Minder	   = (EXABYTE **) minder [--mindex]; break;
					default: break;
					}
				  break;
		case _esc_GENDS : EsReg = (EXBPTR) calloc( freg [e] , 1 ); 
				  ...
				  break;			
		case _esc_FREDS : free(EsReg); break;
		...
		case _esc_REGVF : if ( freg [e] >= diezreg ) { err_val = 1; return(0); }
				  break;
