...
VOID	posepiege( xa,xb,xc,AB_mode )
EXAWORD	xa,xb,xc,AB_mode;
{
	...
	if ((ltrap = TrapExist(xa,xb,xc)) != (TRAPTR) 0)
		...
		if ((ltrap = (TRAPTR) allocate( sizeof( struct exadb_trap ))) != (TRAPTR) 0) {
			...
			ltrap->trap_owner  = program_owner();
...
#ifdef	ABAL21
			if ( trad_ver > '1' ) {
				x_cprintf( (BPTR) ((mptr = get_procedure_name((ltrap->trap_block * 16 )))));
	...
	while ( ltrap != (TRAPTR) 0 ) {
		if ( verify_trap_owner( ltrap ) ) {
			...
			if ((ltrap->trap_state & DBTRAP_PROC) == 0) {
				...
				stream_putb(vh,'P');
				stream_puts(vh,(BPTR) get_procedure_name((ltrap->trap_block * 16)));
				}
			stream_putb(vh,':');
...
TRAPTR	ltrap;
{
	...
	/* --------------------------------- */
	if ((ltrap->trap_state & DBTRAP_PROC) == DBTRAP_PROC)
		(void) tcode_correspondance( 1, (ltrap->trap_block * 16),ltrap->trap_offset, 3 );
...
BPTR	cmdptr;
{
	...
	/* Establish the Trap */
	/* ------------------ */
	posepiege(csval,ipval,mode,absel);
	return;
