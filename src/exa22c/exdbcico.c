...
struct  pushpop {
	...
	struct pushpop  SPTR fwdptr;    /* Pointer to Next structure    */
	};
struct  pushpop SPTR PopBase;           /* Base of Push / Pop Stack     */
#endif
...
struct Fenetre SPTR Window;             /* window Manager               */
...
EXTERN  EXAWORD    prnleng;
EXTERN  EXAWORD    prndex;
EXTERN  EXAWORD    prnfid;
EXTERN  BPTR    prnptr;
...
#ifdef  WIN1632
void    __fastcall      ExadbCreeFenetre(void);
void    __fastcall      ExadbSupprimeFenetre(void);
void    __fastcall      ExadbSelecteFenetre(void);
#endif
...
EXAWORD    ZoneBack;
{
	...
	if ((lzptr = (struct ZoneKeep SPTR) allocate((USINT)  sizeof(struct ZoneKeep) )) == (struct ZoneKeep SPTR) 0 ) {
		return(ABAL_MEMORY);
		}
	if (( PZflag & 4 ) == 0 ) {
...
EXAWORD    PopZone()
{
	...
	lzptr = (struct ZoneKeep SPTR) ZoneStack [ --ZoneDex ];
...
EXAWORD    n;
{
	...
	if ( ZoneDex == 0 ) { return( 105 ); }
	lzptr = (struct ZoneKeep SPTR) ZoneStack [ (ZoneDex-1) ];
	lzbuf = (*(lzptr)).Z_buf;       
	lno   = (*(lzptr)).Z_lno;       cno = (*(lzptr)).Z_cno;
	nbl   = (*(lzptr)).Z_nbl;       nbc = (*(lzptr)).Z_nbc;
	Zlen = (nbl * nbc);
...
EXAWORD    db_initerm()
{
	...
	ExadbCreeFenetre();
#endif
	Window = (struct Fenetre SPTR) allocate( (USINT) (sizeof( struct Fenetre )  * 2) );
	if ( Window == (struct Fenetre SPTR) 0 ) { return(ABAL_MEMORY); }
	PhysLin   = (INITSLIN);
...
#ifdef  PUSHPOP
		while ( PopBase != (struct pushpop SPTR) 0) {
...
VOID    PushScreen()
{
	...
	/* ------------------------------------ */
	tptr = (struct pushpop SPTR ) allocate( sizeof( struct pushpop ) );
	if ( tptr == ( struct pushpop SPTR ) 0 ) {
		return;
		}
...
{
	struct pushpop SPTR tptr;
	if ( PopBase != (struct pushpop SPTR) 0 ) {
