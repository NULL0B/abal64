...
switch ( worktyp2 ) {
	case VARB_INT8  : if ( ExFlgSgn != 0 )	{ workval2 = (EXAWORD) *((BPTR) workptr2); }
			  ...
			  break;
	case VARB_INT16 : workval2 = (BINAIRE) balgetw(workptr2);
			  break;
...
switch (worktyp1) {
	case VARB_INT8  : workval1 = (BINAIRE) ReadSchar( workptr1 ); /* *((EXCPTR) workptr1); */ break;
	case VARB_INT16 : workval1 = (BINAIRE) balgetw(workptr1); break;
	default		: if (workval1 > 0) {
				preg [workval1] = workptr1;
				lreg [workval1] = worklen1;
				xreg [workval1] = (worktyp1 | PTRSTATUS); 
