...
struct GestProc SPTR PrcNbase; /* Procedure Names Table Pointer 	*/
...
EXABYTE	PrcGetc()
{
	...
	/* -------------------- */
	if ( Pindent >= BLOCK ) {
		if ( SectRead(TcodeFileHandle,sectptr,BLOCK,(adr_npr + sectdex)) == MOINS_UN) {
			return(' ');
			}
		sectdex++; Pindent = 0;
		}
...
EXAWORD	InitNomProc()
{
	...
	/* ---------------------------- */
	if ( (sectptr = (BPTR) allocate(BLOCK)) == (BPTR) 0) {
		err_val = ABAL_MEMORY; return(MOINS_UN);
		}
	
	/* Read in First Sector of Table of Names */
	/* -------------------------------------- */
	if ( SectRead(TcodeFileHandle,sectptr,BLOCK,adr_npr) == MOINS_UN ) {
	...
	if ( ( PrcNbase = (struct GestProc SPTR) allocate( ( PrcNfois * sizeof( struct GestProc ) ) )) == (struct GestProc SPTR) 0) {
