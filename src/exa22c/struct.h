...
struct	aplus_expression {
	EXAWORD		type;	/* Type of usage 		*/
	VALUEPTR	value;	/* Value ptr	 		*/
	EXPRESSPTR	next;	/* next in list of expressions	*/
	};

struct	class_const {
	EXAWORD		type;	/* # % $ or Bcd type		*/
	EXAWORD		value;	/* Value for integer constants	*/
...
struct	aplus_struct_member {
	...
	EXAWORD		hash;	/* name hash value			*/
	TYPEPTR		dcltype; /* Used by variable management		*/
	EXPRESSPTR	expression;
	USERPTR		userlist;
/*	--------------------------------------------------------------- */
/*	EXAWORD		argument;  Indicates Command Reception Member	*/
/* 	--------------------------------------------------------------- */
	EXAWORD		condition;
	EXAWORD		length;	/* Memory size of member (LEN)		*/
	EXAWORD		type;	/* Data Type of member #,%,N,$		*/
	EXAWORD		number; /* Variable number in class		*/
	EXAWORD		dim1;	/* 1st Dimension			*/
	EXAWORD		dim2;	/* Second dimension			*/
	EXAWORD		adjust;	/* Number of members in this member	*/
	EXAWORD		members; /* Static Structure Member count	*/
/* 	--------------------------------------------------------------- */
/*	EXAWORD		access;    PRIVATE , PROTECT , PUBLIC           */
/* 	--------------------------------------------------------------- */
	EXAWORD		common;	/* Indicates Common variable (VRAI)	*/
/* 	--------------------------------------------------------------- */
/*	EXAWORD		cmnlock;   Locks auto methods of common class	*/
/* 	--------------------------------------------------------------- */
	EXAWORD		function; /* Indicates Not InLine (VRAI)	*/
	EXAWORD		overlay; /* Indicates an overlay segment method */
	TYPEPTR		rettype; /* Procedural return type (ABAL 1.5)	*/
	EXAWORD		routine;/* Indicates a Gosub Inline		*/
/* 	--------------------------------------------------------------- */
/*	EXAWORD		flag;		End of Union Flag		*/
/* 	--------------------------------------------------------------- */
	EXAWORD		istatic; /* Static linkage number		*/
	EXAWORD		special; /* Identifies constructors / destruct  */
/* 	--------------------------------------------------------------- */
/*	EXAWORD		crossref;  Indicates an external Data member  	*/
...
/*	--------------------------------------------------------------- */
	...
	EXAWORD		fileline; /* Line in source file		*/
	EXALONG		filebyte; /* For Debug APLUS Source following	*/
/* 	--------------------------------------------------------------- */
#endif

	EXAWORD		users;	/* Counts usage of local members	*/
...
struct	aplus_struct_class {
	...
	EXAWORD		size;	/* Size of complete structure		*/
	EXAWORD		count;	/* Number of members in this CLASS	*/
	EXAWORD		cumul;	/* Cumulate Members in class		*/
	EXAWORD		type;	/* Class or Structure header type	*/
	EXAWORD		limit;	/* Explicite Class Size 		*/
	EXAWORD		number;	/* Unique Class Number			*/
	EXAWORD		status; /* Inhibited class functions flag	*/
#ifdef	USING_DLL
	EXAWORD		structure;
#endif
	EXAWORD		functions; /* Number of functions		*/
...
struct	aplus_structure {
	...
	EXAWORD		number;	/* identification number allocated 	*/
	EXAWORD		type;	/* Normally 0 : Global : 1 : Local	*/
	EXAWORD		status; /* Extern, Common etc			*/
	EXAWORD		field;	/* Current Memory Field			*/
...
EXAWORD		start_struct( EXAWORD );
EXAWORD		end_struct( VOID );
EXAWORD		create_class_member( BPTR ,EXAWORD, EXAWORD );
EXAWORD		declare_class_structure( CLASSPTR, EXAWORD );
EXAWORD		inherit_class_structure( CLASSPTR, EXAWORD );
EXAWORD		generate_structure_member( OBJECTPTR );
CCONSTPTR	is_class_constant( BPTR );
MEMBERPTR	resolve_member_linkage( MEMBERPTR, MEMBERPTR XPTR );
#else
EXAWORD		start_struct();
EXAWORD		end_struct();
EXAWORD		create_class_member();
EXAWORD		declare_class_structure();
EXAWORD		inherit_class_structure();
EXAWORD		generate_structure_member();
