...
/* On n'imprime les couleurs que sous windows */
	...
	if ( prndest < 2 ) {
		...
		if ( fy == 8 ) 
#ifdef	WIN1632
			iof_backgr((workval1 & 0x000F));
#else
			iof_backgr((workval1 & 0x0007));
#endif
		else	iof_foregr((workval1 & 0x000F));
#ifndef WIN1632	
		}
#endif
	return(GOODRESULT);
...
EXAWORD fx;
{
	...
	if (( fx & 0x00E0) == 0x0060) {
		if ( argset1( (fx & 0x00E0) >> 5 , 0 ) == SOFTERROR )
			return(SOFTERROR);
		}
	else 
		if ( argsetw( (fx & 0x00E0) >> 5 , 0 ) == SOFTERROR ) 
			return(SOFTERROR);
	...
	switch ( worktyp1 ) {
		...
		case VARB_STR   :
			...
			if ( extwrite == 1 ) { 
				if (( extv_base = (struct extvform SPTR) allocate( sizeof(struct extvform))) == (struct extvform SPTR) 0 ) { 
					err_val = 114; 
					return(SOFTERROR); 
					}
				sicico |= ONSEM_XVRB; 
	...
	if ( asklen > 0 ) {
		...
		switch ( worktyp1 ) {
			...
			case VARB_STR   : 
				...
				/* --------------------------------- */
				while ( worklen1 > 0 ) { 
					if ( *d != IO_NUL) { *(workptr1++) = *(d++); worklen1--; }
					else 		   { break; }
					}
...
EXAWORD	sf_print(EXAWORD fx)
{
	...
	switch ( worktyp1 ) {
		...
		case	VARB_INT16 :
				...
				return( iof_print_int( workval1, worktyp1 ));

		case	VARB_BCD   :	

				return( iof_print_bcd( workptr1, worklen1 ));

		case	VARB_STR : 	

				return( iof_print_str( workptr1, worklen1 ));
...
do {
	...
	ret = sighold(SIGALRM);
		...
		case 14 : tcode = iof_special_hotkey(tcode,_GOTO_HOTKEY);
			  tcode = 1; 
			  break;
