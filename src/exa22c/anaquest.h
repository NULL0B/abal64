...
typedef struct PARAM2{
    ...
    U16 Flag; 			/* if first time we pass in syntaxical analysis Flag = 1
		  			 	if second time Flag = 2 */
#ifdef ABAL21	
	U16 FlagCount;		/* = 0 if anaquest called for a COUNT
						   = 1 if called for a POSIT */
#else
	...
	U8 QuestType;		/* Quest Type : 	1 -> WHERE CLAUSE
											3 -> WHERE + ORDER BY CLAUSES */
	...
	U8 ColName[17];		/* name of the column */
	...
	U8 EscBool;			/* 0 -> no ESCAPE in the LIKE clause 
						   1 -> ESCAPE in  the LIKE clause */
	U8 EscVal[256];		/* Escape value */
