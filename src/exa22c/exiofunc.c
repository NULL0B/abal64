...
EXALONG	prnseek;		/* Print file seek value		*/
#endif
#endif
...
#ifdef	UNIX
SCOPE	WORD	cico_use_abqt=1;
#endif

SCOPE	EX_PRINTER_HANDLE prnfid;	/* File handle OS print device	*/
SCOPE	EXAWORD	prndest;		/* Destination to print 	*/
SCOPE	BPTR  	prnptr;			/* Variable adresse		*/
SCOPE	EXAWORD prnleng;		/*				*/
SCOPE	EXAWORD	prndex;			/* Length of the variable 	*/
SCOPE	EXAWORD	err_val;		/* Global Abal Error code value	*/

EXAWORD	prncol;			/*					*/
EXAWORD	prnlin;			/* Printer Column and line current	*/
EXAWORD	tabflg;			/*					*/
 ...
 *	-------------------
 *
 * 	Affichage sur l'Ecran avec controle du mode SECRET (Mask 64)
	...
	if ( x & 0x0040 ) {	/* If .NOT. Character filter element UZAN etc */
		...
		switch ( x ) {
			...
			case	FMT_V	:	/* Virgule interne	 */
					fbcdv = (BINAIRE) ReadSchar( fmtptr ); fmtptr++; /* *((CPTR) fmtptr++); */
					break;
...
BINAIRE	workval1;
{
	...
	if (( workval1 & 0x04000 ) != 0 ) {
		...
		switch ( workval1 ) {
			...
			case 26: AtbContext &= 0x7FEF; break;
			case 27: AtbContext |= 0x0200; break;
			case 28: AtbContext &= 0x7FDF; break;
...
if ( worktyp1 == 2 ) {
	...
	if ( (abufdex+1-yy) > ( worklen1 * 2 ) ) {
		...
		/* ----------------------------		*/
		x_bell(); bufwipe(); return(0);
		}
	}
askbuf[abufdex] = IO_NUL; cleol(); abufdex++; 
#ifdef	BUF_CICO
		...
		/* Type of format underway FILTER or ACTION */
{
	...
	if ( ap == 1 ) {
		...
		if ( worktyp1 < VARB_STR ) {
			...
			fmthld[fmtdex] = FMT_N;
			fmtgod[fmtdex] = 0;
			if ( worktyp1 >  VARB_INT16 ) 
				fmtcpt[fmtdex] = 13;
			else	fmtcpt[fmtdex] = ((worktyp1 + 1) * 2);
			fmtdex++;
			fmthld[fmtdex] = FMT_Z;
			fmtgod[fmtdex] = 0;
			fmtcpt[fmtdex] = 1;
	...
	if ( (fmtflg & 0x007F) == 0 ) 
		...
		do 	{
			...
			switch ( x ) {
				...
				case	FMT_V	: fbcdv = (BINAIRE) ReadSchar( fmtptr ); fmtptr++;  /* *((CPTR) fmtptr++); */
						  continue;
...
EXAWORD	seekfin()
{
	...
	pos(askl,askc);
	for ( y = 0; y < x; y++ )
		if ( ( chkfmt(askbuf[y]) ==  0 ) 
		||   ((z = bufgest(askbuf[y])) != 0 ) )
			break;
	if ( x > 0 ) 
		x--;
	for ( ; y < x; y++ ) 
...
EXAWORD	bufmax;			/* Default Length of Input zone	*/
{
	...
	EXAWORD	x,y,z;		/* Working variables		*/
...
do {
   ...
   &&   (( askmask & MASK1024    ) == 0) ) {
	while ((!opb()) && (( (EXAWORD) descarto[0] & TIME_ALARM ) != 0))
#ifdef DOS
	(VOID) dos_arto();
#else
; 
   ...
   if ( AskContext.Flags & 2)
	ask_curser_on();
#endif
   c = x_getch();		/* Read Keyboard character 		*/ 
#ifdef	ABAL21
    if ( AskContext.Flags & 2)
	ask_curser_off();
...
#endif
   if ( (abufdex == 0) || ((askmask & MASK512) != 0) )  {
	if ( is_hotkey( (EXAWORD) c ) == 0 ) { 

		/* Check for Keep Data Input */
		/* ------------------------- */
		if ((askmask & (MASK4096 | MASK512)) != (MASK4096 | MASK512) )
		...
		/* ---------------------------------------------- */
#ifdef	ABAL14
		if (( askmask & (MASK4096 | MASK512 | MASK8192)) == (MASK4096 | MASK512 | MASK8192))
...
/*		else	c = askbuf[abufdex];	*/
	...
	/* ----------------------------------------------------------  */
	case IO_RC	: z = askout(z);	continue;

	/* ASK Editor KEY : FIN/END : position at end of current input */
	/* ----------------------------------------------------------- */
	case IO_FIN	: if ( askmask & MASK256) { 
				abufbig = lenuze((BPTR) askbuf, fmtlen);
				z = seekfin();
				}
			  else 	{
	...
	/* -------------------------------------------------------- */
	case IO_INS	: if (( z == 0 ) && (askmask & MASK256)) {
				abufbig = lenuze((BPTR) askbuf, fmtlen);
				if ( insflg == 0 ) 	{ insflg = 1; }
				else 			{ insflg = 0; }
				SignalStatus(insflg);
				}
			   continue;
	...
	/* ---------------------------------------------------- */ 
	case IO_DEL	:  if (( z==0) && ( askmask & MASK256)) {
	...
	default		: /* If awaiting RC then filter all other keys */
			  /* ----------------------------------------- */
		if ( z == 2 ) { 
			switch ( maskwork() ) {
				case 0 :	return(SOFTERROR);
   ...
   /*
    *	Valid character so display (MASK 64 controled) and
...
VOID	iof_reset()
{
	...
	askmask   = 3;			/* ASK MASK default value	*/
...
{
	EXAWORD	n;
	if ((n = CicoContextItem( (BPTR SPTR) & xptr )) != 0)
		iof_tabl(n);
	else	iof_tabl(1);
	if ((n = CicoContextItem( (BPTR SPTR) & xptr )) != 0)
		iof_tabc(n);
...
EXAWORD	get_op_event() { return( cico_use_abqt ); }
...
/*	Establish INT 24 Critical Error Trap	*/
/*	------------------------------------	*/
	Trap24();
...
#ifdef	ABALX11
#ifndef	DEBUGGER
	set_desktop_name("Abal Application");
	set_desktop_icon("Exa");
 ...
 *	-------------------------------------------------
 *	Insertion d'un regle de debranchement STANDARD dans la table
