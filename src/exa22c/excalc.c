...
VOID	standard_image_quality()	{ return; }
...
EXAWORD	how;
{
	...
	switch ( (worktyp1 << 2) | worktyp2 )	{
		...
		case 0x0003 :	*workptr1 = (EXABYTE) atoi(workptr2,worklen2);
				break;
		case 0x0004 :	
		case 0x0005 :	balputw((EXAWORD) workval2,workptr1); break;
		case 0x0006 :   flg = (EXAWORD) bcdtoi(workptr2,worklen2);
				balputw( flg , workptr1 );
				break;
		case 0x0007 :   flg = (EXAWORD) atoi(workptr2,worklen2);
				balputw( flg , workptr1 );
				break;
		case 0x0008 :	
		case 0x0009 :	return(itobcd(workval2,workptr1,worklen1));
		case 0x000A :	
		...
		case 0x000B :	return(atobcd(workptr2,workptr1,worklen1,worklen2));
		case 0x000C :	
		case 0x000D : if ( how == 1 ) {
				StackCheck( 12 );
		...
		case 0x000F :	xmemcpy(0x0020); break;
 ...
 */
#ifdef WIN32
EXAWORD lenuzend(cp,lenmax)
BPTR	cp;
EXAWORD	lenmax;
{ EXAWORD i;

	for (i = 0 ; i < lenmax ; i++) {
		if (cp[i] == IO_NUL)	break;
	}
	while (i != 0) {
		if (cp[i-1] == IO_ESP)	i--;
		else			break;
...
/* ----------------------------------- */
while (( *workptr1 == IO_ESP ) && ( worklen1 > 0)) { workptr1++; worklen1--; }
while (( *workptr2 == IO_ESP ) && ( worklen2 > 0)) { workptr2++; worklen2--; }
