...
EXTERN	struct fichdt SPTR AssignTable;	/* Assign Table Base Pointer	*/
...
struct idblabel SPTR	GestEtt; /* LABEL MANAGER base pointer	*/
...
struct	QwikFind SPTR idb_QFP;	/* Quick find Proc Table	*/
struct	QwikFind SPTR idb_QFS;	/* Quick Find Segm Table	*/
...
EXAWORD	idb_MapRead()
{
	...
	do	{
		VerifTcode();
		if (( flg = SectRead( TcodeFileHandle , wptr ,BLOCK , blokid )) != MOINS_UN) {
			blokid++; i = 0;
			while (( i < BLOCK) && ( flg != MOINS_UN )) {
...
EXAWORD	lid;
{
	...
	while ( 1 ) {
		...
		/* ---------------------------------------- */
		if ( SectRead(TcodeFileHandle,(BPTR) lfblk, BLOCK ,lfsect) != BLOCK ) {
			PopInfo(); return(0);
			}
...
EXAWORD	idb_flopen( )
{
	...
	GestEtt = (struct idblabel SPTR) 0; LibCtrl = 0;
	...
	/* ------------------------------------- */
	if (SectRead(TcodeFileHandle,(BPTR) lfnom , BLOCK , LstSect) != BLOCK ) {
		return(0);
		}
...
VOID idb_flclose()
{
	...
	/* --------------------- */
	if ( GestEtt != (struct idblabel SPTR) 0 ) {
		for ( x = 0; x < LibCtrl; x++ ) {
			if ( (*(GestEtt + x)).Flag != 0 ) {
		...
		LibCtrl = 0;	
		liberate( GestEtt );
		GestEtt = (struct idblabel SPTR) 0;
		}

	/* Liberate Quick find table pointers */
	/* ---------------------------------- */
	if ( idb_QFP != (struct QwikFind SPTR) 0 ) { 
		liberate( idb_QFP ); 
		idb_QFP = (struct QwikFind SPTR) 0;
		}
	if ( idb_QFS != (struct QwikFind SPTR) 0 ) { 
		liberate( idb_QFS ); 
		idb_QFS = (struct QwikFind SPTR) 0;		
		}
	return;
}
...
EXAWORD	PushInfo()
{
	struct FileGard SPTR lfptr;

	/* Attempt to allocate new structure */
	/* --------------------------------- */
	if ( lfSP >= STACKSIZE )
		return(105);
	if ( (lfptr = (struct FileGard SPTR) allocate((USINT) sizeof( struct FileGard ))) == (struct FileGard SPTR) 0)
		return(ABAL_MEMORY);
...
EXAWORD	i;
{
	...
	lfptr = (struct FileGard SPTR) lfstack[lfSP+i];
	lfptr->Fichier = filegard.Fichier;
	lfptr->Handle  = filegard.Handle;
	lfptr->ByteLim = filegard.ByteLim;
	lfptr->ByteGod = filegard.ByteGod;
...
EXAWORD	PopInfo()
{
	struct FileGard SPTR lfptr;
	if (lfSP == 0 ) { return(105); }
	lfptr = (struct FileGard SPTR) lfstack[--lfSP];
...
while ( l > 0 );
lfPrcId = prcname; xcodip = 0; lfSegId = 0xFFFF;
baklignes(1); LibInfo();
return(1);
}
...
lfSegId = segid; xcodip = 0; lfPrcId = 0xFFFF;
baklignes(1); LibInfo();
return(1);
}
...
if ( idb_flopen() != 0 )  { 
	...
	if ( idb_seekseg( 0 ) == 0 )  { 
		...
		/* ----------------------------- */
		lfSegId = 0; xcodip = 0; lfPrcId = 0xFFFF;
...
EXAWORD	codip;				/* Offset to be saught	*/
{
	...
	do 	{
		...
		if ( lword[0] != '(' ) { 
			...
			thisip = hextoi( (BPTR) &lword[1] , 4); 
			}

		/* Loose last State if less or Equal */
		/* --------------------------------- */
		if ( thisip <= codip ) { LibInfo(); }
		}
	while (thisip <= codip);			
PopInfo();
return;
...
EXAWORD	idb_InitList()
{
	...
	/* ------------------------------- */
	if (( MapSect == 0 ) || ( MapSect == MOINS_UN )) { return(40); }

	/* Allocate Quick Find Table For Procedures */
	/* ---------------------------------------- */
	if ( adr_tpr != 0 ) {
		if ( (idb_QFP = (struct QwikFind SPTR) allocate((USINT) ((procmaxid + 1) * sizeof(struct QwikFind)))) == (struct QwikFind SPTR) 0 )
			return(27); 
		}
	else	{
		idb_QFP = (struct QwikFind SPTR) 0;
	...
	/* -------------------------------------- */
	if ( (idb_QFS = (struct QwikFind SPTR) allocate((USINT) ((SegmentCount + 1) * sizeof(struct QwikFind)))) == (struct QwikFind SPTR) 0 )
		return(27); 
...
EXAWORD	codip;
{
	...
	do	{
		...
		if ( x > 0 ) { 
			baklignes(x+1); 
			if ((IdbFlag & IDB_ASM) == 0 ) {
				if (idb_fgetmot((BPTR) lmots) == 0 ) { return(0); }
				}
			if ((l = idb_fgetmot((BPTR) lmots) ) == 0) { return(0); }
			lmots[l] = (EXABYTE) 0;
			if ( lmots[0] == '(' ) {
...

{
	...
	if ( csmode == 0 ) {	/* Alors Recherche Segment */
		...
		if ( csval != lfSegId ) {
			...
			lfSegId = csval; xcodip = 0; lfPrcId = 0xFFFF;
			}
		else	{
			if ( codip < xcodip ) {
		...
		if ( csval != lfPrcId ) {
			...
			lfPrcId = csval; xcodip = 0; lfSegId = 0xFFFF;
			}
		else	{	
...
BPTR	lptr;
{
	...
	BPTR	wptr;
	if ( GestEtt == (struct idblabel SPTR) 0 ) {
		if ( (GestEtt = (struct idblabel SPTR) allocate((USINT) (sizeof( struct idblabel ) * LIBELMAX ))) == (struct idblabel SPTR) 0) {
			Debordement();
			return;
			}
		}
