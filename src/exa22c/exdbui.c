...
#else
EXAWORD display_correspondances();
...
EXAWORD	mode;
{
	...
	for ( ctrlcode = 0; ctrlcode < 26; ctrlcode++ ) {
		...
		switch ( (ctrlcode+1) ) {
			...
			case CTRL_D : indent = 0; id =116; break;
			...
			case CTRL_M : indent = 0; id =116; break;
			...
			case CTRL_Z : indent = 9; id = 32; break;
			default     : continue;
			}
...
SYMBOLPTR	xsymb;
{
	...
	if ( xsymb != (SYMBOLPTR) 0 ) {
		switch ( xsymb->type ) {
			case PNAME_SYMBOL	:
				return(checkpiege((xsymb->number / 16) ,0, 2 ));
...
EXAWORD	identity;
{
	...
	if (( identity == 0 ) || ( identity == 0xFFFF )) {
		Verbose((BPTR) interface_message(9) );
		return(0);
		}

	if (((secteur = ( DebugSymbols->ldt_sector != 0  ? DebugSymbols->ldt_sector : TcGetWord((BPTR) & DebugSymbols->AtHeader[0x001A]))) != 0)
	&&  (secteur != MOINS_UN)
	&&  ( stream_seek( _TCODE_STREAM, (secteur+((identity & 0x7F00) >> 8)), (identity & 0x00FF) )) ) {
...
#ifndef	STD_C_CALLS
	if ( type )
		posepiege( blockid, linfos->tcode_offset, BlockType, (type & 1 ? 1 : 0));
	else	wipepiege( blockid, linfos->tcode_offset, (BlockType ? 0x0002 : 0));
#endif
	return( VRAI );
}
...
EXAWORD	verbose;
{
	...
	if ( stream_position(_TEMP_STREAM , (BPTR) nomfic, (EXALONG) 0L, MOINS_UN, 0 ) != 0) {
		...
		l = 0;
		while (1) {
			switch ((c = stream_getc(_TEMP_STREAM))) {
				case 0x000D : 
					continue;
				case 0x000A : 
					if ( l == 0 )
...
EXAWORD	mode;
{
	...
	*(nptr+l) = 0;
	...
	return(standard_ask( (BPTR) nptr, 64, (BPTR) mptr,l));
}
...
/*	if ( check_tcode_location( BlockType, BlockIdentity, BlockOffset ) == 0 ) { */
...
SYMBOLPTR	lsymb;
{
	...
	switch ((choice = display_menu( 22,17, 2, 
...
EXAWORD	type;
{
	...
	switch ( lsymb->type ) {
		...
		case SNAME_SYMBOL :
			if ( type )
				posepiege( lsymb->number, 0, 0, (type & 1 ? 1 : 0));
			else	wipepiege( lsymb->number, 0, 0);
			break;

		case PNAME_SYMBOL :
			if ( type )
				posepiege( (lsymb->number / 16), 0, 1, (type & 1 ? 1 : 0));
			else	wipepiege( (lsymb->number / 16), 0, 2);
			break;
		}
	return;
...
SYMBOLPTR	lsymb;
{
	...
	mouse_title_box( (topcol-1), toplin, 20, maxsline,(BPTR)(tptr = liste_titre(lsymb)) );
	draw_liste( topcol+2, toplin+1, 16, maxsline,(hsymb = xsymb=lsymb) );
	while ( 1 ) {
		symbol_atribut(2,xsymb);
		display_symbol( topcol+2,toplin+item,16,xsymb);
		symbol_atribut(0,xsymb);
		c = mouse_or_keyboard();
		display_symbol( topcol+2,toplin+item,16,xsymb);
		switch ( c ) {
		...
		draw_liste( topcol+2, toplin+1, 16, maxsline,lsymb );
...
VOID	symbols_browser()
{
	...
	while ( 1 ) {
		switch ((choice = display_menu( 2, 6, 7, 
