...
/*	---------------------------	*/
struct	param_type {
	EXAWORD		typecheck;	/* Explicite type of data	*/
	EXAWORD		length;		/* Length of string data	*/
	EXAWORD		first;		/* First dimension		*/
	EXAWORD		second;		/* Second dimension		*/
	EXAWORD		status;		/* Allows protection of FunArgs	*/
...
struct macro_param {
	...
	EXAWORD		convert;	/* VRAI : Activates param value	*/
	EXAWORD		type;		/* Static or Dynamic		*/
	EXAWORD		lock;		/* Parameter under expansion	*/
	EXAWORD		retval;		/* Explicite Return Value	*/
...
struct aplus_macro {
	...
	EXAWORD		usage;		/* Count of usage for labels	*/
	EXAWORD		number;		/* Macro identification number  */
	EXAWORD		lock;		/* Recursivity indicator	*/
	EXAWORD		type;
	EXAWORD		users;		/* Indicates number of users	*/
	};

struct	aplus_define {
	BPTR		name;		/* Name of token defined	*/
	EXAWORD		status;		/* Definition status		*/
	EXAWORD		hash;		/* Hash value for quick search	*/
...
#ifndef	UNIX
#ifndef	VMS
#define FARBPTR EXABYTE far XPTR
#endif
#endif
...
#else
EXAWORD	fn_parser( FARBPTR , FARBPTR , EXAWORD );
EXAWORD	systeme( FARBPTR );
#endif
#endif
