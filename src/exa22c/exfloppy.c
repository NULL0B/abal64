...
#define UNDERWINNT	 	1
#define UNDERWIN95	 	2
extern EXAWORD Win95orWinNT(void);
extern EXAWORD floppy_io_winNT(EXAWORD, EXAWORD, EXABYTE *, EXAWORD, EXAWORD);
#endif
...
EXALONG	logiq;		/* Logical Starting Sector	0 -> MaxSect	*/
{
	...
	if (Win95orWinNT() == UNDERWINNT) {
		errno=0;
		err_val=floppy_io_winNT(did,RegAx,bptr,blen,(EXAWORD)logiq);
		if(err_val!=0){
			errno=err_val;
			return(MOINS_UN);
			}
