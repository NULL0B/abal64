				...
				/* 	Type de data desiree	*/
{
	...
	switch ( ltyp ) {
		case 0 :
		case 1 :	hugeint = ((EXALONG) lval * BLOCK);       break;
...
/* ------------------------------ */
	msr = (struct talkzone SPTR ) malloc((USINT) worklen1 + (LONGSIZE * 2) + WORDSIZE);
...
EXAWORD	sf_chain()
{
	...
	if ( fargset(2) != SOFTERROR ) {
		...
		*(workptr1 + worklen2) = (EXABYTE) 0x000D;
		...
		fidperm |= ASS_BUFF_VALID;	/* Indicate buffer needs free	*/
