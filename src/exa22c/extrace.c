...
//	EXTRACE.C	has been adapted from the file described below
//			in an appropriate manner to be used from an ABAL
//			application program, notably string variables are
//			length controlled rather than zero terminated.
//			TraTrace and TraDebug have been modified to use 
//			lengths for all string parameters.
//	IJM 24/09/2002
...
#ifdef	_extrace_c
   char            pri_Buf[320];    // To make the trace line
#else
...
#ifdef	_extrace_c
unsigned short TraTrace(unsigned short Level,unsigned short Id, 
		const char * tfn, unsigned short tfnl, 
		const char * mes, unsigned short mesl);
unsigned short TraDump(unsigned short Level,unsigned short Id, 
		const char * tfn, unsigned short tfnl, 
		const char * mes, unsigned short mesl, 
		const char * zon, unsigned short zonl);
#else
unsigned short TraTrace(unsigned short Level,unsigned short Id, 
		const char * tfn, const char * mes, ...);
unsigned short TraDump(unsigned short Level,unsigned short Id, 
		const char * tfn, unsigned short tfnl, 
		const char * mes, unsigned short mesl, 
		const char * zon, unsigned short zonl, ...);
...
if ((Handle=fopen(pCtx->pri_File,"a"))==NULL)
   return;  
...
// *
// ****************************************************************************
static void TraCtxDump( const TI_TRA_CTX* pCtx, FILE* Handle, unsigned char * pBuf, const char * mes, unsigned int mesl, const unsigned char * Zone, const unsigned short lgZone)
{
...
if (Level & CI_TRA_F_DUMP)
   ...
   if (pCtx->FilterLevel & CI_TRA_F_DUMP)
      ...
      va_end( marker);
     TraCtxDump(pCtx, Handle, pBuf, mes, Zone, lgZone );   
      }
   }
else
   {
...
// * Id        IN  : Module ID 
// * tfn       IN  : origin of the message (typically, name of the calling function)
// * tfnl      IN  : length of the above to be used	
// * mes       IN  : string message (cf printf)
// * mesl      IN  : length of the above to be used	
// * ...       IN  : optional arguments (cf printf)
// *
// * VarGlob:
// *  all      IN / OUT (at initialisation)
...
// * mes       IN  : string message (cf printf)
// * mesl      IN  : length of the above to be used	
// * zon       IN  : hex byte zone to be dumped 
// * zonl      IN  : length of the above to be used	
// *
// * VarGlob:
// *  all      IN / OUT (at initialisation)
// *
...
// --- Message
// ---------------------------------------------
	TraCtxDump( pCtx, Handle, pBuf, mes, mesl, zon, zonl );
