...
#define	use_permissions( p, e, c ) { \
	e=((p & 0x0002) ? (WORD)1 : (WORD)0);\
	c=((p & 0x0004) ? (WORD)1 : (WORD)0);\
	c |= ((p & 0x0001)==0 ? (WORD)2 : (WORD)0);\
	}

#ifdef	VERSION666
#define FFread    FFread64
#define FFwrite   FFwrite64
#endif
...
/*AD************* FIND *************************************************/
/************************************************************************ 
; WORD EeEad_find(hfic, newname)					;
; EX_FILE_HANDLE hfic	- handle du fichier				;
; BPTR Name   		- nouveau nom					;
; BPTR pInfo  		- infos de retour				;
; 	-->> code erreur						;
*************************************************************************/

WORD NOTFASTCALL EeEad_find(EX_FILE_HANDLE  hfic,BPTR  Name,BPTR pInfo)
{
	if ((Name) && (!(hfic))) {
		while ((*Name==' ') || (*Name=='\t')) { 
			Name++; 
...
#ifdef	_USING_FFFIND
	return((WORD)FFfind((EHANDLE)hfic, (LPSTR)Name,pInfo));
#else
	return(56);
#endif
}
...
WORD NOTFASTCALL EeEad_extend(hfic, nbs)
	...
	WORD nbs;
{
	unsigned long bytes = nbs*256;
	return(FFextend((EHANDLE)hfic, (unsigned long) bytes ));
}
...
int len;
	...
	*perr=0;
	...
	use_permissions( perm, ex,cp ); cp &= 1;
...
/*AD************ CONSULT ***********************************************/
/************************************************************************
; WORD EeEad_consult(name, option, length, result, legnth )		;
; BPTR server name to be consulted					;
; BPTR option value required       					;
; WORD length of option string     					;
; BPTR result buffer for return of value				;
; WORD length result buffer     					;
; 	-->> code erreur						;
*************************************************************************/

WORD	EeEad_consult( servername, option, olength, result, rlength )
BPTR	servername;
BPTR	option;
WORD	olength;
BPTR	result;
