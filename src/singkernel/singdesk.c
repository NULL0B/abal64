...
public	void	reactivate_DeskTop(int mode)	
{
	if ((Context.DeskTop.style = mode)!=0) {
		if ( Context.DeskTop.style != _SCROLL_FRAME ) {
			...
			Context.DeskTop.w = inner_width(0);
...
public	void	show_DeskTop()
{
	...
	filzone	(Context.DeskTop.x,Context.DeskTop.y,
		Context.DeskTop.w,Context.DeskTop.h,_DEFAULT_GUI_NEUTRAL);
...
public	int	treeview_Message( int atx, int aty,char * message, int mlen, int iconid , int state, int indent )
{
	...
	filzone( Context.TreeView.x+atx, Context.TreeView.y+aty,
		(zw = (nby-atx)),Context.treefontheight, bg);
	...
	switch ( iconid ) {
		...
		case	TREE_ROOT	:
			if ( RootIcon ) {
				...
				mixzone8(Context.TreeView.x+atx,Context.TreeView.y+aty+padding,Context.Tool.w,Context.fontheight,RootIcon);
				adjust = 20;
				}
			break;
		case	TREE_PROJECT	:
			if ( ProjectIcon ) {
				filzone( Context.TreeView.x+atx, Context.TreeView.y+aty+padding,
					20,Context.fontheight,  bg /* standard_backwash */);
				mixzone8(Context.TreeView.x+atx,Context.TreeView.y+aty+padding,Context.Tool.w,Context.fontheight,ProjectIcon);
				adjust = 20;
				}
			break;
		...
		case	TREE_TEXT	:
			if ( TextIcon ) {
				filzone( Context.TreeView.x+atx, Context.TreeView.y+aty+padding,
					20,Context.fontheight,  bg /* standard_backwash */);
				mixzone8(Context.TreeView.x+atx,Context.TreeView.y+aty+padding,Context.Tool.w,Context.fontheight,TextIcon);
				adjust = 20;
				}
			break;
		...
		case	TREE_IMAGE	:
			if ( ImageIcon ) {
				filzone( Context.TreeView.x+atx, Context.TreeView.y+aty+padding,
					20,Context.fontheight,  bg /* standard_backwash */);
				mixzone8(Context.TreeView.x+atx,Context.TreeView.y+aty+padding,Context.Tool.w,Context.fontheight,ImageIcon);
				adjust = 20;
				}
			break;
		...
		case	TREE_FORM   	:
			if ( TreeIcon ) {
				filzone( Context.TreeView.x+atx, Context.TreeView.y+aty+padding,
					20,Context.fontheight,  bg /* standard_backwash */);
				mixzone8(Context.TreeView.x+atx,Context.TreeView.y+aty+padding,Context.Tool.w,Context.fontheight,TreeIcon);
				adjust = 20;
				}
			break;
	...
	while (( gmlen = textsizelen( Context.treefont, message, mlen )) > (zw - adjust )) {
		mmlen = ( mlen-1 );
...
#ifdef	WASWAS
	if ((mlen * Context.fontwidth) > (zw-adjust)) {
		mmlen = ((zw-adjust) / Context.fontwidth);
		if ( iconid != TREE_WIDGET )
			message += (mlen - mmlen);
		mlen = mmlen;
		}
...
public 	void	image_save( char * nptr, struct standard_image * iptr, int mode )
{
	...
	x_event(35);
	image_write( tempname, iptr,Context.imageformat, Context.transparent );
	x_event(34);
	tempname = liberate( tempname );
	return;
