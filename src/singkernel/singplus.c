...
public	void	instance_aplus(FILE * handle, struct form_item * iptr )
{
	...
	switch (((s=iptr->Contents.style)&_FRAME_TYPE)) {
		...
		case	_WINDOW_FRAME	:
			...
			aplus_font_paramter(handle,iptr);
			aplus_string_and_length(handle,iptr);
			fprintf(handle,";%u;3)",iptr->Contents.align);
			linefeed(handle);
			break;
	...
	for (	focus_item=0,iptr=Context.Form->first; 
		...
		switch (( iptr->Contents.style & _FRAME_TYPE )) {
			...
			case	_TAB_FRAME	:	/* TODO */
				...
				indent(h,2);
				fprintf(h,"if ((visual_event..parameter(1) and /0%x) <> 0)",iptr->Contents.events.mimo);
				linefeed(h);
...
public	void	aplus_visual_apl( FILE * h )
{
	...
	fprintf(h,"%s visual",keyword_use);	  linefeed(h);
}
...
private	void	aplus_overlay_loader(FILE * h)
{
	...
	redirected_method( h,"on_hide");
	redirected_method( h,"on_show");
	redirected_method( h,"on_event");
	redirected_method( h,"on_get_focus");
	redirected_method( h,"on_exec");
	redirected_method( h,"on_attach");
	redirected_method( h,"on_detach");
