...
private	void	configure_editor_window()
{
	...
	||  (!( aed.h ))) {
		...
		sprintf(workbuffer,"%u:%u:%u:%u:0:%u:0:%u",aed.x,aed.y,aed.w,aed.h,SingConf->aed.font,(aed.fontwidth*2));
		}
...
private	void	AbalEditorOnUpdate()
{
	char	*	nptr=(char *) 0;
...
public	int	open_window_editor( int atx, int aty, int nbx, int nby, char * nomfic )
{
	...
	if ( setjmp( Ed_Exit ) != 0 ) {
		...
		sprintf(workbuffer,"%u:%u:%u:%u:0:%u:0:%u",atx+1,aty+1,nbx-2,nby-2,SingConf->aed.font,(aed.fontwidth*2));
		editor_window_connection( workbuffer );
