...
char	rowbuffer[1280];	/* optimisation row buffer */
	...
	if ( image ) {
		...
		for ( 	r=0; r < nr; r++ ) {
			...
			if ((nc = ((fc-ic)+1)) > 0) {
				...
				if (!( dath ))
					mixzone8( ic,r+ir,nc,1,rowbuffer);
				else	putzone8( ic,r+ir,nc,1,rowbuffer);
				}
			}
	...
	if ( image ) {
		...
		for ( 	c=0; c < nc; c++ ) {
			...
			if ((nr = ((fr-ir)+1)) > 0) {
				...
				if (!( dath ))
					mixzone8( c+ic,ir,1,nr,rowbuffer);
				else	putzone8( c+ic,ir,1,nr,rowbuffer);
				}
			}
