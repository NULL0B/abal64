...
#include <string.h>		/* WIN32 */

//FCH #include "stdtypes.h"
#include "fnparser.h"	/* WIN32 */
...
{
	while ( ri > 0 ) {
		*rptr = (BYTE) Majescule( *rptr );
		rptr++; ri--;
		}
	return;
}

WORD	fn_fields(sptr , rptr , mode )
BPTR	sptr;	/* Source file name string				*/
BPTR	rptr;	/* Pointer to Result Buffer 				*/
		/* ---------------------------------------------------- */
WORD	mode;	/* Extraction type Bit definitions			*/
		/* ---------------------------------------------------- */
{
	WORD	ri=0;
		...
		/* ---------------------------------------------------- */
{
   ...
   WORD     di;
...
#if defined(AIX_VERS) || defined(WIN32)
	...
	/* -------------------------------- */
	*termptr = (BYTE)termine; *(rptr + ri) = 0; 
