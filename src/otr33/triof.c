...
WORD	translate_atb()
{
	RESULTPTR	xptr;
	if (( is_left_brace()  )
	&&  ((xptr = evaluate_typed_expression(1)) != (RESULTPTR) 0)
	&&  ( is_right_brace() ))
...
WORD	modeabal;
{
	...
	RESULTPTR	cptr;
	WORD		status;
	if ((cptr = evaluate_typed_expression(1)) == (RESULTPTR) 0)
	...
	if (!(modeabal))
		status = VRAI;
	if ( status ) {
		if (((lptr = evaluate_typed_expression(1)) == (RESULTPTR) 0)
...
WORD	translate_paint()
{
	...
	WORD		status;
	if ( is_left_brace() ) {
		if (((xptr = evaluate_typed_expression(1)) == (RESULTPTR) 0)
...
RESULTPTR	device;
{
	...
	&&  ( is_right_brace() ))
		return( flush_print( device, VRAI /* was FAUX, why ? */ ) );
