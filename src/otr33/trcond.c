		...
		WORD nesting		)
{
	...
	if (!( hptr->head )) {
		...
		switch ( parental_logic( hptr ) /* masterlogic */ ) {
			case	_LOGICAL_END	:
		...
		switch ( opcode ) {
			...
			case	_LOGICAL_AND	:
				...
				for ( 	lptr=hptr->head; lptr != (LEVELPTR) 0; lptr = lptr->next) {
					...
					if (!( lptr->head )) {
						if ( lptr->combine != _LOGICAL_END ) {
							if ((result = generate_and_conditional( lptr, ( parental_logic( hptr ) /* masterlogic */ != _LOGICAL_END ? &thislabel :  truelabel),falselabel,nesting)) !=0)
								status = result;
							}
						else 	{
							if ( parental_logic( hptr ) /* masterlogic */ == _LOGICAL_OR ) {
								if ((result = generate_or_conditional( lptr,truelabel,( parental_logic( hptr ) /* masterlogic */ != _LOGICAL_END ? &thislabel : falselabel),nesting)) !=0)
									status = result;
								}
							...
							else if ((result = generate_and_conditional( lptr, ( parental_logic( hptr ) /* masterlogic */ != _LOGICAL_END ? &thislabel :  truelabel),falselabel,nesting)) !=0)
									status = result;
			...
			case	_LOGICAL_END	:
				...
				for ( 	lptr=hptr->head; lptr != (LEVELPTR) 0; lptr = lptr->next) {
					...
					if (!( lptr->head )) {
						if ((result = generate_end_conditional( lptr, ( parental_logic( hptr ) /* masterlogic */ != _LOGICAL_END ? &thislabel :  truelabel),falselabel,nesting)) !=0)
							status = result;
						}
					else 	{
						/* --------------------------------------------- */
						/* Recursive call for generation of conditionals */
						/* --------------------------------------------- */
						if ((result = generate_new_conditionals(lptr->combine, lptr,( parental_logic( hptr ) /* masterlogic */ != _LOGICAL_END ? &thislabel : truelabel),falselabel,(nesting+1))) != 0)
							status = result;
		...
		/* added */			else if ( parental_logic( lptr->parent ) == _LOGICAL_OR ) {
							if ((result = generate_or_conditional( lptr, truelabel ,( parental_logic( hptr ) /* masterlogic */ != _LOGICAL_END ? &thislabel :  falselabel),nesting)) !=0)
								status = result;
							}
						else 	{
						...
						if ( lptr->combine == _LOGICAL_END ) {
							...
							if ((result = generate_new_conditionals(lptr->combine, lptr,truelabel ,&templabel,(nesting+1))) != 0)
								status = result;
