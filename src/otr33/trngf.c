...
static	VOID	reset_ngf()
{
	...
	memset(	NGF.ErrorLabel, 0, 32 );
	memset(	NGF.Descriptor, 0, 32 );
	return;
}
		...
		do	{
			if ( is_mnemonic("WR") )
				...
				if ( is_left_brace() ) {
					result &= ~_NGF_MC;
					result |= _NGF_DBF;
					NGF.DataBase = evaluate_integer_expression(0);
					if (!( is_right_brace()))
						return( (RESULTPTR) 0 );
					}
				}
...
static	WORD	sf_options()
{
	...
	trace("sf_options()\n");
	if ( is_comma() )
		options = evaluate_integer_expression(0);
...
static WORD	translate_index_size()
{
	...
	if ( remove_white_space() ) {
		...
		if ( is_equals() )
			rptr = evaluate_integer_expression(0);
...
static RESULTPTR	translate_data_size()
{
	...
	if ( remove_white_space() ) {
		if ( is_mnemonic("VD") ) {
			...
			else if (!( rptr = evaluate_integer_expression(0) )) {
				syntax_error( 8116 );
				return((RESULTPTR) 0 );
				}
			else	return( rptr );
...
WORD	l;
{
	...
	trace("ngf_length()\n");
	if ( is_comma() ) {
		if ((length = evaluate_integer_expression(0)) == (RESULTPTR) 0)
...
{
	if (!(is_comma())) 
		return( store_parameter( integer_result( 3 ) ) );
	else if ( is_mnemonic( "A" ) )
		return( store_parameter( integer_result( 3 ) ) );
	else if ( is_mnemonic( "B" ) )
		return( store_parameter( integer_result( 2 ) ) );
	else if ( is_mnemonic( "N" ) )
		return( store_parameter( integer_result( 1 ) ) );
	else if ( is_mnemonic( "L" ) )
		return( store_parameter( integer_result( 5 ) ) );
	else	return( store_parameter( evaluate_integer_expression(0) ) );
}
...
{
#ifdef	ABALSQL
	return( translate_sql( _NGF_SELECT ) );
#else
	return( MOINS_UN );
#endif
}
