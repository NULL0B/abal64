...
TCTYPPTR	tptr;
{
	...
	if ((context = (PRCPTR) allocate( sizeof( struct otr_callproc ) )) != (PRCPTR) 0) {
		...
		memset(context->ErrorLabel, 0, 32 );
		}	
	return( context );
}
	...
	&&  (!(is_right_brace()))) {
		...
		do	{
			if ( pptr != (TCPRCPTR) 0) {
				...
				else	datatype = vptr->type->type;
				}
			else	datatype = MOINS_UN;
...
WORD	mode;	/* 0 : Tcode .AT or .O  , 1 : Debug .DBG or .DBO */
{
	...
	if ( mode != MOINS_UN ) {
		...
		if (!( type & _CONTEXT_NOCLR ))
			i = (0x0040 | (OtrOption.Symbolic & 1));
		else	i = (0x0042 | (OtrOption.Symbolic & 1));
		...
		tcode_integer( 0 );		/* Adr Image GDS    x4C	*/
		tcode_integer( 0 );		/* Lng Image GDS    x4E	*/

		/* END OF VERSION 1.4 COMPATIBLE ZONE			*/
		/* ---------------------------------------------------- */
		/* START OF VERSION 2.1 COMPATIBLE ZONE			*/

		tcode_integer( pragma_page_size() ); /* Etd Page Size    x50	*/
		tcode_integer( pragma_swap_size() ); /* Max Swap Buffer  x52	*/
		tcode_integer( 0 );		/* Max Optimisation x54	*/
		...
		if (( OtrOption.Symbolic )
		&&  ( mode ))	{		/* --------------------- */
			tcode_integer(Context->sector_XFT);/* VarExt x5A */
			tcode_integer(Context->sector_XMT);/* NomMod x5C */
			tcode_integer(0);	/* Adr Tab Csp Vgb   x5E */
			tcode_integer(0);	/* Adr Tab Csp Cnst  x60 */
			tcode_integer(0);	/* Lng Tab Csp Cnst  x62 */
						/* --------------------- */
			}
		else	{
			for ( i = 0; i < 5; i++ )
		...
		if ( Target->OutputHandle != 0 ) {
			...
			if ( GlobalError )
				unlink( Target->OutputName );
			     ...
			     &&  ( Context->sector_XFT == 0 )
			     &&  (!( context_is_module     ))) {
				/* Not a Module (Program, Overlay, APL...) */
				...
				if (OtrOption.TargetType == 2) {
					/* Module was waited, check only for programs */
...
static	WORD	run_linker(BPTR args)
{
	...
	if ((cptr = (BPTR)allocate( strlen(args) + 5 )) != (BPTR)0) {
...
static	VOID	perform_autolink(BPTR filename)
{
	...
	if (OtrOption.AutoLink != (BPTR)0) {
		...
		if ((args = (BPTR) allocate(build_linker_default_options((BPTR)0) + strlen(filename) + 1)) != (BPTR)0) {
...
VOID	terminate_output()
{
	...
	if (context_is_program && (OtrOption.NoAutoLink == FAUX))
		if (Context->length_LMT != 0)	/* Something to be linked */
