...
EXAWORD	open_tcode( MODULE * mptr )
{
	...
	else	
	{
		...
		mptr->headersize = HEADERSIZE;
		if (!(mptr->athandle = fopen( mptr->atname, "rb" ))) 
			return( 40 );
		else if (!( read_tcode( mptr->athandle, 0, mptr->atheader,  mptr->headersize ) )) 
			return( 53 );
		else	return( 0  );
	}
}
...
EXAWORD	open_debug( MODULE * mptr )
{
	...
	else	
	{
		...
		if (!(mptr->dbhandle = fopen( mptr->dbname, "rb" ))) 
			return( 40 );
		else if (!( read_tcode( mptr->dbhandle, 0, mptr->dbheader,   mptr->headersize ) )) 
			return( 53 );
		else	return( 0  );
	}
}
...
EXAWORD	build_debug( MODULE * mptr )
{
	...
	status = write_tcode( mptr->dbhandle, 0, mptr->dbheader,  mptr->headersize);
#ifdef ABAL32DBG
	if (!status)
		status = write_tcode( mptr->dbhandle, 1, mptr->dbheaderex,  mptr->sectorsize );
#endif
	return status;
}
...
EXAWORD	build_tcode( MODULE * mptr )
{
	...
	build_header( mptr, mptr->atheader, 1 );	
#ifdef ABAL32DBG
	build_headerex( mptr, mptr->atheaderex, 0 );
#endif

	status = write_tcode( mptr->athandle, 0, mptr->atheader,  mptr->headersize );
#ifdef ABAL32DBG_MESS
	if (!status)
		status = write_tcode( mptr->athandle, 1, mptr->atheaderex,  mptr->sectorsize );
#endif
	return status;
}
...
VOID	flush_tcode( MODULE * mptr ) 
{
	...
	lock_headers( mptr );
	(void) write_tcode( mptr->dbhandle, 0, mptr->dbheader,  mptr->headersize );
	(void) write_tcode( mptr->athandle, 0, mptr->atheader,  mptr->headersize );
#ifdef ABAL32DBG_MESS
	(void) write_tcode( mptr->dbhandle, 1, mptr->dbheaderex,  mptr->sectorsize );
	(void) write_tcode( mptr->athandle, 1, mptr->atheaderex,  mptr->sectorsize );
#endif
	return;
}
