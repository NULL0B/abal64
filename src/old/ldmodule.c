...
MODULE *	allocate_module( BPTR nptr )
{
	...
	if ((mptr = (MODULE *) allocate( sizeof( MODULE ))) != (MODULE*)0) {
		...
		reset_location(&mptr->newvarbnames);
#endif
		reset_location(&mptr->varbhosttable);
...
#ifdef ABAL32DBG
	/* transfer modname to output */
	if ( Linker.debug ) {
		if (!(nptr = allocate_name())) 
			return( report_error(158, mptr->rootname) );
		nptr->name = allocate_string(mptr->progname);
		status = add_modname(rptr, nptr);
		liberate_name(nptr);
		if (status !=0)
			return( report_error( 150, mptr->progname  ) );
		}
#endif
