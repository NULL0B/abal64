...
private	int load_procedure( MODULE * mptr, PROCEDURE * pptr )
{
	...
	{
		sprintf(ErrorBuffer, "proc %s(/%llx) : tc(%llu,%llu) lv(%llu,%llu) lc(%llu,%llu) o(%llu) xr(%llu), cr(%llu,%llu)",
...
private	int load_procedures( MODULE * mptr )
{
	...
	&&  ((mptr->proctable.length = tclength( &mptr->atheader[(TCPREFIX + (10*mptr->wordsize)+2)],mptr->wordsize )) != 0 )) 
	{
		...
		mptr->exportable.sector = tcsector( &mptr->atheader[(TCPREFIX + (35*mptr->wordsize)+2)],mptr->wordsize );
		mptr->exportable.length = tcsector( &mptr->atheader[(TCPREFIX + (36*mptr->wordsize)+2)],mptr->wordsize );
		...
		if ( Linker.sourcetrace ) 
		{
			sprintf(ErrorBuffer, "gpt       : sector =%llu, length =%llu, procedures : %llu",mptr->proctable.sector,mptr->proctable.length,mptr->nbproc);
			issue_message(4, ErrorBuffer);
		}
...
private	int load_external_procedures( MODULE * mptr )
{
	...
	if ((sector = tcsector( &mptr->dbheader[(TCPREFIX + (14*mptr->wordsize)+2)],mptr->wordsize )) != 0 ) 
	{
		...
		for (p=0; p < nbp; p++ ) 
		{
			...
			if ( Linker.sourcetrace ) 
			{
				sprintf(ErrorBuffer, "extern proc %s in module %llu", pptr->name,pptr->module );
				issue_message(4, ErrorBuffer);
			}	
		}
...
EXAWORD	close_procedure_table( MODULE * mptr ) 
{
	...
	{
		identity = 0;
		reset_location(&mptr->exportable);
		next_free_location(&mptr->exportable);
		buffer_start( mptr, mptr->athandle, &mptr->exportable );
		buffer_putw(mptr,exported);
		buffer_putw(mptr,0);
		for (	pptr=mptr->procedures.root;
			pptr != (PROCEDURE*) 0;
			pptr = pptr->next ) 
...
#ifdef ABAL32DBG
EXAWORD	flush_procedure_host_table( MODULE * mptr ) 
{
	PROCEDURE *	pptr;
	EXAWORD		module_id;
	...
	reset_location( &mptr->prochosttable );
	next_debug_location(&mptr->prochosttable);
	if( Linker.targettrace )
		issue_message(5, "Procedure Host Table");

	buffer_start( mptr, mptr->dbhandle, &mptr->prochosttable );
	...
	for (	pptr = mptr->procedures.root;
		...
		pptr = pptr->next ) 
	{
		module_id = get_module_id(mptr, pptr->original);
		buffer_putw(mptr, module_id);
	}
