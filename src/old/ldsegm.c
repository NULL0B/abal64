...
private	int load_segment( MODULE * mptr, SEGMENT * sptr )
{
	...
	if ( Linker.sourcetrace ) {
		sprintf(ErrorBuffer, "segm %s(/%llx) : tc(%llu,%llu) lv(%llu,%llu) lc(%llu,%llu) o(%llu) xr(%llu), cr(%llu,%llu)",
...
private	int load_segments( MODULE * mptr )
{
	...
	&&  ((mptr->segmtable.length = tclength( &mptr->atheader[(TCPREFIX + (8*mptr->wordsize)+2)],mptr->wordsize )) != 0 )) {
		...
		mptr->SdtAdjust= 0;
		if ( Linker.sourcetrace ) {
			sprintf(ErrorBuffer, "gst       : sector =%llu, length =%llu, segments : %llu",mptr->segmtable.sector,mptr->segmtable.length,mptr->nbsegm);
			issue_message(4, ErrorBuffer);
			}
...
private	int load_external_segments( MODULE * mptr )
{
	...
	if ((sector = tcsector( &mptr->dbheader[(TCPREFIX + (34*mptr->wordsize)+2)],mptr->wordsize )) != 0 ) {
		...
		for (s=0; s < nbs; s++ ) {
			...
			if ( Linker.sourcetrace ) {
				sprintf(ErrorBuffer, "extern segm %s as %llu in module %llu", sptr->name, sptr->number, sptr->module );
				issue_message(4, ErrorBuffer);
				}	
			}
...
EXAWORD	add_segment( MODULE * mptr, SEGMENT * sptr )
{
	...
	if ( Linker.sourcetrace ) {
		sprintf(ErrorBuffer, "add_segment(%llu)",nptr->identity);
		issue_message(4, ErrorBuffer);
		}
	if (!( sptr->host ))
		return( 0 );
...
EXAWORD	add_external_segment( MODULE * mptr, XSEGMENT * sptr )
{
	...
	if ( Linker.sourcetrace ) 
	{
		sprintf(ErrorBuffer, "add_external_segment(%llu)", nptr->number);
		issue_message(4, ErrorBuffer);
	}
	return(0);
}
...
#ifdef ABAL32DBG
EXAWORD	flush_segment_host_table( MODULE * mptr ) 
{
	SEGMENT *	sptr;
	EXAWORD		module_id;
	...
	reset_location( &mptr->segmhosttable );
	next_debug_location(&mptr->segmhosttable);
	if( Linker.targettrace )
		issue_message(5, "Segment Host Table");

	buffer_start( mptr, mptr->dbhandle, &mptr->segmhosttable );
	...
	for (	sptr = mptr->segments.root;
		sptr != (SEGMENT*) 0;
		sptr = sptr->next ) {
		module_id = get_module_id(mptr, sptr->original);
