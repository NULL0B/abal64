...
*/
typedef void (*FRAME_PROC) (MPEG_HEADER* h, unsigned char* mpeg, unsigned char* pcm);
...
void L1decode_frame(MPEG_HEADER* h, unsigned char* mpeg, unsigned char* pcm);
void L2decode_frame(MPEG_HEADER* h, unsigned char* mpeg, unsigned char* pcm);
void L3decode_frame(MPEG_HEADER* h, unsigned char* mpeg, unsigned char* pcm);
...
int mp3GetHeader(unsigned char* buf, MPEG_HEADER* h)
{
...
int mp3FindSync(unsigned char* buf, int size, int* sync)
{
	int i;
...
static int extractInt4(unsigned char* buf)
{// big endian extract
	return (buf[3] | (buf[2] << 8) | (buf[1] << 16) | (buf[0] << 24));
}

int mp3GetDecodeInfo(unsigned char* mpeg, int size, MPEG_DECODE_INFO* info, int decFlag)
{
	MPEG_HEADER* h = &info->header;
	unsigned char* p = mpeg;
	int vbr;
	unsigned int minBitRate, maxBitRate;
...
/*
 * -----------------------------------------------
 * what the fuck has this been commented out for ?
 * -----------------------------------------------
		//•W€VBR‘Î‰ž
...
int mp3DecodeStart(unsigned char* mpeg, int size)
{
...
void null_frame_proc(MPEG_HEADER* h, unsigned char* mpeg, unsigned char* pcm) {}
void L3decode_reset();
