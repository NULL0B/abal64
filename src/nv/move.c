...
#define _MODULE_TITLE	"File Move Utility"
...
#define	_MODULE_USAGE	move_syntax
void	move_syntax()
{
	printf("\n   Syntax : \n\tmove [ -s -r -f ] <sourcefiles> <target>");
	printf("\n   Options: ");
	printf("\n\t-s\tinhibit messages ");
	printf("\n\t-r\trecursive move   ");
	printf("\n\t-f\tno confirmation  \n");
...
void	mv( char *sptr, char * rptr )
{
	...
	sprintf(cmd_buffer,"mv %s %s",sptr,rptr);
#ifdef	UNIX
	system( cmd_buffer );
#else
	printf("%s\n",cmd_buffer);
