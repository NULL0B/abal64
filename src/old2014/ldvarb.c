...
private	void	collect_variable_type( VARIABLE * vptr, BPTR dptr, EXAWORD ws )
{
	...
	/* collect length */
	switch ((vptr->type & 7)) {
		case	0	:	vptr->length = 1; break;
		case	1	:	vptr->length = 2; break;
		case	5	:	vptr->length = 4; break;
		case	2	:
		case	3	:	vptr->length = tcgetword(dptr,ws);
					dptr += ws;
		}
...
private	int load_data_labels ( MODULE * mptr, EXAWORD sector )
{
	...
	if ( Linker.sourcetrace ) {
		sprintf(ErrorBuffer, "glt : sector %u", sector);
	...
	if ((status = load_buffer( mptr, mptr->dbhandle, sector )) != 0)
		...
		return(0);
	if ( Linker.sourcetrace ) {
		sprintf(ErrorBuffer, "label count %u", number);
		issue_message(4, ErrorBuffer);
		}
	...
	for ( l=0; l < number; l++ ) {
		...
		if ( Linker.sourcetrace ) {
			sprintf(ErrorBuffer, "label %u) %s=%c at %x:%x",l,lptr->name,lptr->type,lptr->extra,lptr->value);
			issue_message(4, ErrorBuffer);
			}
		}
	return( 0 );
...
private	int load_variables ( MODULE * mptr )
{
	...
	&&  ((mptr->varbtable.length = tclength( &mptr->atheader[(TCPREFIX + (4*mptr->wordsize)+2)],mptr->wordsize )) != 0 )) {
		if ((mptr->gvt = load_tcode( mptr->athandle, mptr->varbtable.sector, mptr->varbtable.length )) != (BPTR) 0) {
			...
			if ( Linker.sourcetrace ) {
				sprintf(ErrorBuffer, "gvt       : sector =%u, length =%u, variables : %u, memory : %u",
			...
			for ( v=0; v < mptr->nbvarb; v++ ) {
				...
				if ( Linker.sourcetrace ) {
					sprintf(ErrorBuffer, "dcl %s:%x %x,%u,%u,%u",
						vptr->name,vptr->identity,
						vptr->type,vptr->length,vptr->first,vptr->second);
					issue_message(4, ErrorBuffer);
					}
				}
...
private	int load_external_variables( MODULE * mptr )
{
	...
	&&  ((mptr->xvarbtable.length = tclength( &mptr->dbheader[(TCPREFIX + (43*mptr->wordsize)+2)],mptr->wordsize )) != 0 )) {
		if ((mptr->xvt = load_tcode( mptr->dbhandle, mptr->xvarbtable.sector, mptr->xvarbtable.length )) != (BPTR) 0) {
			...
			if ( Linker.sourcetrace ) {
				sprintf(ErrorBuffer, "xvt       : sector =%u, length =%u, variables : %u",
			...
			for ( v=0; v < mptr->xnbvarb; v++ ) {
				...
				if ( Linker.sourcetrace ) {
					sprintf(ErrorBuffer, "extern dcl %s:%x %x,%u,%u,%u",
						vptr->name,vptr->identity,
						vptr->type,vptr->length,vptr->first,vptr->second);
					issue_message(4, ErrorBuffer);
					}
...
public	int	relocate_extern_field( FIELD * fptr )
{
	...
	||  (!(cptr = fptr->client    )))
		...
		for ( 	sptr = fptr->solution.root,
			...
			if ( Linker.verbose & 2 ) {
				sprintf(ErrorBuffer, "reloc member : %s::%s (%x + %x) = %x",
					optr->rootname,
...
private	int load_external_fields( MODULE * mptr )
{
	...
	if ((mptr->fieldtable.sector = tcsector( &mptr->dbheader[(TCPREFIX + (35*mptr->wordsize)+2)],mptr->wordsize )) != 0 ) {
		...
		if (!( mptr->nbfields = buffer_getw( mptr ) ))
			return( 0 );
		if ( Linker.verbose & 8 ) {
			sprintf(ErrorBuffer, "extern fields of %s : %u", mptr->rootname, mptr->nbfields);
			issue_message(0, ErrorBuffer);
			}
		for ( f=0; f < mptr->nbfields; f++) {
			...
			if ( Linker.verbose & 8 ) {
				sprintf(ErrorBuffer, "extern field,%s(%x) in %u at %x,%x sector %u",
		...
		for (	fptr=mptr->fields.root; 
			...
			for ( v=0; v < nbv; v++ ) {
				...
				if ( Linker.verbose & 8 ) {
					sprintf(ErrorBuffer, "extern member: %s(%x)", vptr->name,vptr->identity);
					issue_message(0, ErrorBuffer);
					}
				vptr->relocate = collect_relocation( mptr,8 );
				collect_variable_type( vptr, (mptr->xvt + vptr->identity), mptr->wordsize );
				if ( Linker.verbose & 8 ) {
					sprintf(ErrorBuffer, "type : 0x%x=%u(%u,%u)", vptr->type, vptr->length, vptr->first, vptr->second);
					issue_message(0, ErrorBuffer);
					}
				}
			}	
...
EXAWORD	output_variables( MODULE * rptr, MODULE * sptr )
{
	...
	if (!( sptr->GdtAdjust = rptr->GdtLimit ))  {
		...
		sptr->GdsAdjust = 0;
		if ( Linker.targettrace ) {
			sprintf(ErrorBuffer, "Data Table : nb=%u, ns=%u, mem=%u", rptr->GlobalVariables, 0, rptr->GlobalAllocation );
...
/* Use start_variable_names_table() !
	reset_location( &pptr->varbnames );
	next_debug_location(&pptr->varbnames);
*/
	if( Linker.targettrace )
...
/* Duplicated and completed variable names table. Not used. */
EXAWORD	flush_new_variable_names_table( MODULE * pptr ) 
{
	COMPONENT 	* cptr;
	MODULE		* mptr;	
	VARIABLE 	* vptr;
	...
	reset_location( &pptr->newvarbnames );
	next_debug_location(&pptr->newvarbnames);
	if( Linker.targettrace )
		issue_message(5, "New Variable Names Table");

	buffer_start( pptr, pptr->dbhandle, &pptr->newvarbnames );
	...
	for (	cptr = pptr->components.root;
		...
		for (	vptr = mptr->variables.root;
			...
			buffer_puts(pptr,vptr->name);
			buffer_putw(pptr,vptr->identity+mptr->GdtAdjust);
			module_id = get_module_id(pptr, vptr->host); /* vptr->host != mptr ? */
