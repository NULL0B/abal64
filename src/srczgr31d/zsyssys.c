...
MI_TRACE_BEGIN();
  ...
  if (memcmp(version,"LCLH",4) == 0) {
	...
	// FRC23042001 config of graphic mode by zini
	...
	if      (strcmp(GrMode,"0")==0) RequestedMode = CI_GR_320X200; 
...
#ifdef	UNIX
	else if ((eptr = getenv("SYS_ZGRMODE")) != (char *) 0) {
		...
		else	RequestedMode = CI_GR_800X600; 
		}
#endif
	else	RequestedMode = CI_GR_800X600; 

	g_VesaMode = initialisepixel(RequestedMode|256);

	if (g_VesaMode == -1) {
		MI_TRACE((CI_TRA_F_ERROR,CI_MODULE_ID,TFN,"MfSysSetGrMode %d",g_VesaMode));
      		MI_TRACE_RETURN_MSG(CE_ERR_INCOHERENT,"MfSysSetGrMode");
      		}
...
MI_TRACE_BEGIN();
  ...
  if (IsBeforeLoadgo)
    ...
    if ((NewMode==-1) || (NewMode!=g_VesaMode))
      {
      MI_TRACE((CI_TRA_F_ERROR,CI_MODULE_ID,TFN,"MfSysSetGrMode %d",NewMode));
      zzend();
      MI_TRACE_RETURN_MSG(CE_ERR_INCOHERENT,"MfSysSetGrMode");
      }
    else
...
MI_TRACE_BEGIN();
  MI_TRACE((CI_TRA_F_DEBUG,CI_MODULE_ID,TFN,"(%d,%d)",screenlins,screencols));
