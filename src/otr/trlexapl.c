...
TCSTRUCTPTR 	class;
{
	...
	lptr->class = class;
	lptr->next = (struct library_extension *) 0;
	if (!( LexFoot ))
		LexRoot = lptr;
...
static	WORD	instance_library_extension()
{
	...
	/* ----------------------------- */
	/* Essential information to keep */
	/* ----------------------------- */
	pDLT = Context->DLT;
	pXDLT = Context->XDLT;
	count_DLT = Context->count_DLT;
