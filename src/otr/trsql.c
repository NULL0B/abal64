...
WORD	translate_sql_begin( WORD operation ) 		
{
	...
	{
		if ( (is_comma()) )
			if ((status = store_parameter( evaluate_expression(0) ) ))	/* Optionnal parameter! */
				return( status );
...
WORD	translate_sql_getcolname( WORD operation ) 		
{
	...
	&&  ((status = ngf_operand())		== 0 )
	&&  ((status = ngf_error_trap())	== 0 )
/* #BG ngf_buffer() est ici approprie car prend en compte le parametre optionnel indiquant la longueur du buffer! */
	&&  ((status = ngf_buffer())		== 0 ))
		return( flush_ngf() );
	else	return( status );
	return(0);
...
WORD	translate_sql_fetch( WORD operation ) 		
{
	...
	&&  ((status = ngf_error_trap())	== 0 ))
	{
/* #BG ngf_buffer() n'est ici pas approprie car genere un parametre supplementaire
   indiquant la longueur de la variable.
   De plus le parametre attendu peut etre numerique!
*/
		/* n optionnal parameters */
		while ( is_comma() )
			if ((status = store_parameter( evaluate_expression(0) ) ))
				return( status );
...
WORD	translate_sql_getfield( WORD operation ) 		
{
	...
	&&  ((status = ngf_error_trap())	== 0 ))
	{
		...
		if (count > 3)
			return( syntax_error(4001) );	/* Incorrect parameter count */
...
/*	----------------------------------------------------------	*/
/*	      t r a n s l a t e _ s q l _ b i n d p a r a m		*/
/*	----------------------------------------------------------	*/
WORD	translate_sql_bindparam( WORD operation ) 		
{
	WORD	status;
	WORD	count = 0;
	...
	&&  ((status = ngf_error_trap()) == 0 ))
	{
		/* 1 parameter and 2 optionnal parameter */
		while ( is_comma() )
		{
			count++;
			if ((status = store_parameter( evaluate_expression(0) ) ))
...
/*	----------------------------------------------------------	*/
/*	 	t r a n s l a t e _ s q l _ b i n d c o l		*/
/*	----------------------------------------------------------	*/
WORD	translate_sql_bindcol( WORD operation ) 		
{
	WORD	status;
	WORD	count = 0;
