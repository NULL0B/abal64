...
TCPRCPTR	allocate_for_procedure()
{
	...
	if ((pptr = (TCPRCPTR) allocate( sizeof( struct tcode_procedure ))) != (TCPRCPTR) 0) 
	{
		...
		pptr->ErrorTrapping = OtrOption.ErrorCatcher;
		pptr->ErrorTrap = (TCERRPTR) 0;
		if (( OtrOption.ExportAll ) && (!( Context->IsExport )))
			Context->ExpliciteExport++;
		pptr->isexport = (Context->IsExport | OtrOption.ExportAll);
		Context->IsExport = 0;
	}
	return( pptr );
}
...
/*	------------------------------	*/
/*	ensure_correct_parameter_types	*/
/*	------------------------------	*/
...
/*	------------------------------	*/
/*	ensure_correct_parameter_names	*/
/*	------------------------------	*/
