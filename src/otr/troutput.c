...
{
	if (!( Target )) return;
	Target->OutputSector += SECTORSPERBLOCK;
...
VOID	reset_output()
{
	...
	memset( Target->OutputBuffer, BYTEMASK, Target->OutputLength );
	return;
}
...
VOID	write_target()
{
	WORD	length=SECTORSIZE;	
	if (( OtrOption.Generate )
	&&  (!( GlobalError ))) 
	{
		if (!( Target->OutputPosition ))
			length = HEADERSIZE;
		...
		&&  ( Target->OutputPosition != 0L )) 
		{
			(void) abal_crypt( Target->CryptCode,Target->OutputBuffer, length,  ENCODE );
		}
		if ( OtrOption.ListTcode & 2 ) { tcode_show_write(Target->OutputPosition,length); }
		if ( write( Target->OutputHandle, Target->OutputBuffer, length) != length)
			syntax_error( 1 );
	}
	return;
}
...
#ifdef	otr64
		if (!( h ))
			sysprintf(" %016x ",Target->OutputBytes);
		else	fprintf(h," %016x ",Target->OutputBytes);
#else
#ifdef	otr32
		if (!( h ))
			sysprintf(" %08x ",Target->OutputBytes);
		else	fprintf(h," %08x ",Target->OutputBytes);
#else
		if (!( h ))
			sysprintf(" %04x ",Target->OutputBytes);
		else	fprintf(h," %04x ",Target->OutputBytes);
...
VOID	store_sector()
{
	...
	if (!( Target )) 
		return;
	else if (!(sptr = allocate_for_sector()))
		return;
	else
	{
		memcpy(sptr->buffer,Target->OutputBuffer,Target->OutputLength);
...
/*	tcputbyte	*/
/*	---------	*/
VOID	tcputbyte( WORD v )
{
	if (!( Target )) return;
	Target->OutputBytes++;
	Target->OutputBuffer[Target->OutputIndex++] = (BYTE) v;
	if ( Target->OutputIndex >= Target->OutputLength )
		flush_to_target();
	return;
}	
...
/*	tcputword	*/
/*	---------	*/
VOID	tcputword( WORD v )
{
	tcputbyte( v & BYTEMASK );
	tcputbyte( (v >> 8) & BYTEMASK );

#ifdef	otr64
	if ( OtrOption.WordSize > _SIZE_INT16 ) 
	{
		tcputbyte( (v>>16) & BYTEMASK );
		tcputbyte( (v>>24) & BYTEMASK );
		if ( OtrOption.WordSize > _SIZE_INT32 ) 
		{
			tcputbyte( (v>>32) & BYTEMASK );
			tcputbyte( (v>>40) & BYTEMASK );
			tcputbyte( (v>>48) & BYTEMASK );
			tcputbyte( (v>>56) & BYTEMASK );
		}
	}
#else
#ifdef	otr32
...
/*	---------------------	*/
WORD	generate_nurone_entry(NURONETPTR nptr, NURONEPTR uptr )
{
	WORD		status;
	TCVARPTR	vptr;
	WORD	i;
...
/*	-------------------------	*/
WORD	generate_network_header( NURONETPTR nptr )
{
	TCVARPTR	vptr;
...
/*	---------------------	*/
VOID	symbolic_module_entry( BPTR fptr, WORD n )
{
...
/*	symbolic_file_entry	*/
/*	------------------	*/
VOID	symbolic_file_entry( BPTR fptr, WORD n )
{
	if (( OtrOption.Symbolic )
	&&  (DebugTarget != (OUTPUTPTR) 0)) 
	{
...
WORD	tcode_instruction( WORD n )
{
	...
	if (OtrOption.ListTcode & 1)
	{
		tcode_show_instruction(n);
	}
...
/*	tcode_label	*/
/*	-----------	*/
WORD	tcode_label( WORD n )
{
	if (OtrOption.ListTcode & 1) { tcode_show_word(n,"&"); }
	tcputword( n );
	return(0);
}
...
/*	tcode_constant	*/
/*	--------------	*/
WORD	tcode_constant( TCCONPTR cptr )
{
	WORD	status;
	TCPRCPTR pptr;
	if (!( Target )) return(0);
	if ( context_is_module )
	{
		...
		{
			/* --------------------------------- */
			/* perform variable relocation trace */
			/* --------------------------------- */
			if ( OtrOption.SourceTrace & 4 )
			{
				if (!( pptr = Context->PRC))
					printf("relocate constant %llx at %s:0%llx [s=%llu, o=%llx] )\n",
						cptr->identity,"segment",Target->OutputBytes,Target->OutputSector, Target->OutputIndex);
				else	printf("relocate constant %llx at %s:0%llx [s=%llu, o=%llx] )\n",
						cptr->identity,pptr->name,Target->OutputBytes,Target->OutputSector, Target->OutputIndex);
			}
			if ((status = add_constant_rellocation( cptr,Target->OutputSector, Target->OutputIndex  )) != 0)
				return( status );
		}
	}
	if (OtrOption.ListTcode & 1) { tcode_show_word(cptr->identity,"C"); }
...
/*	tcode_variable	*/
/*	--------------	*/
WORD		tcode_variable( TCVARPTR vptr )
{
	TCPRCPTR pptr;
	WORD	status;
	...
	||  ( context_is_module ))
	{
		...
		{
			if (!( pptr = Context->PRC))
				printf("relocate variable(%llx) %s at %s:0%llx [s=%llu, x=%llx])\n",
					vptr->descriptor,vptr->name,"segment",Target->OutputBytes,Target->OutputSector, Target->OutputIndex);
			else	printf("relocate variable(%llx) %s at %s:0%llx [s=%llu, x=%llx]))\n",
					vptr->descriptor,vptr->name,pptr->name,Target->OutputBytes,Target->OutputSector, Target->OutputIndex);
		}
		if ((status = add_variable_rellocation( vptr,Target->OutputSector, Target->OutputIndex  )) != 0)
			return( status );
	}

	if (OtrOption.ListTcode & 1) { tcode_show_word(vptr->descriptor,"V"); }
...
/*	---------------------	*/
/*	tcode_variable_offset	*/
/*	---------------------	*/
WORD	tcode_variable_offset( TCVARPTR vptr )
{
	WORD		status;
	TCPRCPTR	pptr;
	if (!( Target )) return(0);
	...
	||  ( context_is_module ))
	{
		...
		{
			if (!( pptr = Context->PRC))
				printf("relocate vmoffset(%llx) %s at %s:0%llx )\n",vptr->offset,vptr->name,"segment",Target->OutputBytes);
			else	printf("relocate vmoffset(%llx) %s at %s:0%llx )\n",vptr->offset,vptr->name,pptr->name,Target->OutputBytes);
		}
		/* ---------------------------------------------------------------------------- */
		/* was 0xFF00 but this will be a problem with 64but code and 512 sector buffers */
		/* ---------------------------------------------------------------------------- */
		if ((status = add_variable_rellocation( vptr,Target->OutputSector, (Target->OutputIndex | 0xFE00)  )) != 0)
			return( status );
	}
	if (OtrOption.ListTcode & 1) { tcode_show_word(vptr->offset,"X"); }
...
/*	tcode_register	*/
/*	--------------	*/
WORD	tcode_register(WORD n )
{
	
	if (OtrOption.ListTcode & 1) { tcode_show_byte(n,"R"); }
	tcputbyte( n );
	return(0);
}
...
/*	generate_byte	*/
/*	-------------	*/
WORD	generate_byte( WORD b )
{
	if (OtrOption.ListTcode & 1) { tcode_show_byte(b," "); }
	tcputbyte( b );
	return( 0 );
}
...
/*	generate_constant	*/
/*	-----------------	*/
WORD	generate_constant( TCCONPTR cptr )
{
	BPTR	vptr;
	WORD	i;
	WORD	hlc;
...
/*	generate_data_item	*/
/*	------------------	*/
VOID	generate_data_item( WORD t, WORD l, BPTR vptr )
{
	WORD	i;
	if ( t == _TYPE_STRING ) 
	{
...
/*	------------------------	*/
/*	output_symbolic_variable	*/
/*	------------------------	*/
VOID	output_symbolic_variable( TCVARPTR vptr )
{
...
/*	generate_descriptor	*/
/*	-------------------	*/
WORD	generate_descriptor( TCVARPTR vptr, WORD mode )
{
	WORD	t;
...
#ifdef	otr64
	tcputword(((WORD) (vptr->offset & 0xFFFFFFFFFFFFFFFF)));
	if ((!( t & 0x8000 )) &&  ( t & 0x7F00 ))
		tcputword( (WORD)(vptr->offset >> 64) );
#else
#ifdef	otr32
...
/*	generate_data_table	*/
/*	-------------------	*/
WORD	generate_data_table( TCVDTPTR tptr, FIELDPTR fptr, WORD mode )
{
	WORD	globsize=0;
	if ( fptr != (FIELDPTR) 0)
...
/*	-------------------------	*/
VOID	generate_function_entry( TCDLBPTR lptr )
{
	WORD	i;
	WORD	n;
	if ( lptr != (TCDLBPTR) 0 ) 
...
/*	----------------------	*/
/*	symbolic_segment_entry	*/
/*	----------------------	*/
VOID	symbolic_segment_entry( TCSEGPTR sptr )
{
	if (( sptr != (TCSEGPTR) 0 )
	&&  ( OtrOption.Symbolic )
	&&  (DebugTarget != (OUTPUTPTR) 0)) 
...
/*	----------------------------	*/
/*	generate_procedure_paratypes	*/
/*	----------------------------	*/
VOID	generate_procedure_paratypes( TCPRCPTR pptr )
{
	TCVARPTR	vptr;
	WORD		nombre=0;
	WORD		args=0;
	...
	case	_INT16_PROC	: /* 0x0200 */
		...
		break;
	case	_INT32_PROC	: /* 0x0500 */
		if ( pptr->options & _PTR_PROC )
			nombre = 0xD800;
		else	nombre = 0x5800;
		break;
	case	_INT64_PROC	: /* 0x0600 */
		if ( pptr->options & _PTR_PROC )
			nombre = 0xE800;
		else	nombre = 0x6800;
		break;
...
/*	-------------------------	*/
WORD	generate_cross_references( XREFPTR xptr )
{
	WORD	secteur;
	if ( xptr == (XREFPTR) 0 )
		return( 0 );
...
/*	---------------------------	*/
/*	generate_sector_rellocation	*/
/*	---------------------------	*/
VOID	generate_sector_rellocation(WORD sector, WORD length)
{
	if ( length == Target->OutputLength )
	{
		printf("generate_sector_rellocation(%u) : length(%u) == output(%u) \n", sector, length, Target->OutputLength);
	}
	while ( length >= 256 )
	{
		sector++;
	...
	tcode_integer( sector );
	tcode_integer( length );
	return;
}

/*	-------------------------	*/
/*	generate_code_rellocation	*/
/*	-------------------------	*/
VOID		generate_code_rellocation( RELOCPTR rptr )
{
	if (rptr != (RELOCPTR) 0) 
	{
		do	
		{
			generate_sector_rellocation( rptr->sector, rptr->length );
		}
...
/*	------------------------	*/
WORD	generate_external_member( XTVARPTR xptr )
{
	TCVARPTR	vptr;
	if (( xptr != (XTVARPTR) 0 )
...
/*	-------------------------	*/
VOID	generate_external_local( XTLOCALPTR fptr )
{
	if ( fptr != (XTLOCALPTR) 0) 
	{
		trace("generate_external_local()\n");
		tcode_integer( fptr->number );
		generate_sector_rellocation( fptr->sector, fptr->length );
	}
	return;
}
...
/*	-------------------------	*/
VOID	generate_external_field( XTFIELDPTR fptr )
{
...
/*	---------------------------------	*/
/*	 generate_external_segment_entry	*/
/*	---------------------------------	*/
VOID	generate_external_segment_entry( XTSEGPTR sptr )
{
	if ( sptr != (XTSEGPTR) 0 ) 
	{
...
/*	------------------------	*/
/*	symbolic_procedure_entry	*/
/*	------------------------	*/
VOID	symbolic_procedure_entry( TCPRCPTR pptr )
{
	WORD		args=0;
	WORD		nombre=0;
...
/*	-------------------------	*/
/*	 tcode_procedure_options	*/
/*	-------------------------	*/
static	WORD	tcode_procedure_options( WORD options )
{
	WORD	result=0;
...
/*	tcode_operand	*/
/*	-------------	*/
VOID	tcode_operand( RESULTPTR xptr )
{
	switch ( xptr->nature ) 
	{
	...
	case _RESULT_ALEXPR  :
	case _RESULT_EXPR    : 
		tcode_register( xptr->value );
		break;

	case _RESULT_ALIAS :
	case _RESULT_VPTR  :
	case _RESULT_VARB  : 
		tcode_variable( xptr->contents ); 
		break;

	case _RESULT_CONST : 
		tcode_constant( xptr->contents ); 
		break;

	case _RESULT_INT   : 
		tcode_integer ( xptr->value ); 
		break;
	}
	return;
}
...
/*	------------------------	*/
/*	instruction_requires_LDV	*/
/*	------------------------	*/
WORD	instruction_requires_LDV(WORD mode) 
{ 
...
/*	generate_on_timer	*/
/*	-----------------	*/
WORD	generate_on_timer(RESULTPTR xptr, WORD type )
{
	if (( xptr != (RESULTPTR) 0)
...
/*	generate_error	*/
/*	--------------	*/
WORD	generate_error( RESULTPTR rptr )
{
	if ( OtrOption.ListFlag & 1 )
		echo_error( rptr );
	tcode_instruction( _tc_ESC );
...
/*	----------------------	*/
/*	generate_on_error_call	*/
/*	----------------------	*/
static	WORD	generate_on_error_call( TCVARPTR vptr, WORD type )
{
	tcode_instruction( _tc_ESC   );
	if ( type == MOINS_UN ) 
	{
		if ( OtrOption.ListFlag & 1 )
			echo_instruction( "ON ERROR CALL" );
		tcputbyte( _esc_GEC );
	}
	else
	{
		if ( OtrOption.ListFlag & 1 )
			echo_instruction( "ON LOCAL ERROR CALL" );
		tcputbyte( _esc_LEC );
	}
	tcode_variable( vptr );
	return(0);
...
/*	generate_on_error	*/
/*	-----------------	*/
static	WORD	generate_on_error( TCVARPTR vptr, WORD type )
{
	if ( type == MOINS_UN ) 
	{
		if ( OtrOption.ListFlag & 1 )
...
/*	-------------------------	*/
/*	 explicite_on_error_call	*/
/*	-------------------------	*/
WORD	explicite_on_error_call( TCVARPTR vptr, WORD type, TCPRCPTR pptr)
{
	WORD	status;
	if ((status = generate_on_error_call( vptr, type )) != 0)
		return( status );
	else if (!(  pptr  ))
		return( 666 );
	else
...
/*	generate_event_type	*/
/*	-------------------	*/
WORD	generate_event_type( WORD type )
{
	switch ( type ) 
	{
	case	1 : /* THEN */
...
/*	---------------------	*/
WORD	generate_extern_event( WORD type, WORD mode )
{
	tcode_instruction( _tc_WHN );
	switch ( type ) 
	{
...
/*	----------------------	*/
/*	tcode_library_identity	*/
/*	----------------------	*/
WORD	tcode_library_identity(TCDLBPTR lptr, WORD mode)
{
	WORD	status;
	if (!( Target )) 
		return(0);
	else if ( context_is_module )
...
/*	------------------------	*/
/*	tcode_procedure_identity	*/
/*	------------------------	*/
WORD	tcode_procedure_identity( WORD n )
{
...
/*	 tcode_procedure	*/
/*	-----------------	*/
WORD	tcode_procedure( TCPRCPTR pptr )
{
...
/*	generate_event_call	*/
/*	-------------------	*/
WORD	generate_event_call( TCPRCPTR pptr, TCVARPTR vptr, WORD mode )
{
	WORD	status;
	if ( pptr != (TCPRCPTR) 0 ) 
	{
...
/*	---------------------	*/
VOID	generate_clp_identity( RESULTPTR xptr )
{
...
/*	---------------------	*/
/*	flush_clp_descriptors	*/
/*	---------------------	*/
static	WORD	flush_clp_descriptors(PRCPTR xptr)
{
	WORD		i=0;
	WORD		phase;
	WORD		descriptor;
...
/*	flush_clp_parameter	*/
/*	-------------------	*/
static	WORD	flush_clp_parameter(RESULTPTR lparam)
{
...
/*	--------------------	*/
/*	flush_clp_parameters	*/
/*	--------------------	*/
static	WORD	flush_clp_parameters(PRCPTR xptr)
{
	ARGPTR		cptr;
	WORD		status=0;
...
/*	echo_call_procedure	*/
/*	-------------------	*/
static	VOID	echo_call_procedure( PRCPTR xptr )
{
	WORD	nombre=0;
	ARGPTR	cptr;
	WORD	mode=FAUX;
...
/*	store_clp	*/
/*	---------	*/
WORD		store_clp(PRCPTR xptr, RESULTPTR rptr, WORD mode )
{
	ARGPTR		cptr;
	TCVARPTR	vptr;
	if (!( mode ))
...
/*	start_prc	*/
/*	---------	*/
PRCPTR		start_prc( TCPRCPTR pptr, TCTYPPTR tptr)
{
...
/*	---------------------	*/
/*	ll_activate_procedure	*/
/*	---------------------	*/
WORD	ll_activate_procedure(TCPRCPTR pptr, RESULTPTR rptr, TCPRCPTR sigptr )
{
	WORD		option=0;
	WORD		datatype;
	WORD		status;
...
/*	generate_clf	*/
/*	------------	*/
WORD	generate_clf( WORD code, WORD group )
{
	trace("generate_clf()\n");
...
/*	---------------------	*/
/*	generate_format_label	*/
/*	---------------------	*/
VOID	generate_format_label( BPTR lptr )
{
	tcputbyte( _iof_FMT );
...
/*	------------------------	*/
/*	generate_iof_instruction	*/
/*	------------------------	*/
WORD		generate_iof_instruction( WORD opcode, RESULTPTR rptr )
{
...
/*	generate_iof_format	*/
/*	-------------------	*/
WORD	generate_iof_format( WORD opcode, BPTR fptr )
{
	tcputbyte( opcode );
...
/*	generate_iof_hotkey	*/
/*	-------------------	*/
WORD	generate_iof_hotkey( HOTPTR hptr )
{
	WORD	status;
	BPTR	lptr;
...
/*	resolve_dimensions	*/
/*	------------------	*/
RESULTPTR	resolve_dimensions( RESULTPTR xptr )
{
	TCVARPTR	vptr;
...
/*	------------------------	*/
/*	generate_first_dimension	*/
/*	------------------------	*/
WORD	generate_first_dimension( WORD r, RESULTPTR rptr )
{
...
/*	-------------------------	*/
WORD	generate_next_dimension( WORD r, RESULTPTR rptr )
{
	WORD	HoldLDV=ForceLDV;
	ForceLDV = VRAI;
	if (( rptr != (RESULTPTR) 0)
...
/*	generate_dimensions	*/
/*	-------------------	*/
WORD	generate_dimensions( WORD nbreg, TCVARPTR vptr )
{
	WORD	status;
...
/*	 negate_variable	*/
/*	-----------------	*/
WORD	negate_variable( RESULTPTR rptr )
{
	WORD	status;
	if (( rptr != (RESULTPTR) 0)
...
/*	generate_op	*/
/*	-----------	*/
WORD	generate_op( RESULTPTR rptr, BPTR label )
{
	WORD	status;
	WORD	target;
	WORD	nextlabel=0;
...
/*	-------------------------	*/
/*	 optimised_variable_load	*/
/*	-------------------------	*/
static WORD	optimised_variable_load( RESULTPTR rptr, RESULTPTR sptr )
{
	TCVARPTR	vptr;
	WORD		operand=0;
	WORD		status;
	...
	&&  ( vptr->type != (TCTYPPTR) 0)) 
	{
		...
		else	
		{
			if ((status = generate_dimensions( rptr->value, sptr->contents )) != 0)
				return( status );
			else if (((vptr->type->type & _TYPE_BASIC) > _TYPE_INT16)
...
/*	----------------------	*/
WORD	generate_copy_register( WORD treg, WORD sreg )
{
...
/*	----------------------	*/
/*	generate_register_load	*/
/*	----------------------	*/
RESULTPTR	generate_register_load( RESULTPTR rptr, RESULTPTR sptr, WORD mode )
{
	WORD	status;
	TCVARPTR vptr;
...
/*	generate_label	*/
/*	--------------	*/
WORD	generate_label( WORD label )
{
	tcode_label( label );
	return( 0 );
}
...
/*	tc_arithmetic	*/
/*	-------------	*/
WORD	tc_arithmetic( WORD opcode )
{
	switch ( opcode ) 
	{
	case	'+' : return( _tc_ADI );
...
/*	generate_compare	*/
/*	----------------	*/
WORD		generate_compare( WORD operator, RESULTPTR aterm, RESULTPTR bterm, WORD logic, WORD label) 
{
...
WORD		value;
{
	...
	if ( OtrOption.Optimise & _OPTIMISE_STORE ) 
	{
		...
		{
#if	otr32 || otr64
			if (( vptr->type->type & _TYPE_BASIC) == _TYPE_INT32) 
			{
				tcode_instruction( _tc_ESC );	
				tcode_instruction( _esc_SLL );	
			}
			else if (( vptr->type->type & _TYPE_BASIC) == _TYPE_INT64) 
			{
				tcode_instruction( _tc_ESC );	
				tcode_instruction( _esc_SLH );	
			}
			else	
#endif
				tcputbyte( _tc_LWS );	
		...
		else	
		{
			...
			else if (( vptr->type->type & _TYPE_BASIC) == _TYPE_INT32) 
			{
				tcode_instruction( _tc_ESC );	
				tcputbyte( _esc_SGH );	
			}
...
VOID	commit_code_block()
{
	...
	while ((sptr = Target->CodeSectors[(sector/SECTORSPERBLOCK)]) != (SECTORPTR) 0) 
	{
		memcpy(Target->OutputBuffer,sptr->buffer,Target->OutputLength);
		flush_to_target();
		sptr->next = SectorHeap;
		SectorHeap = sptr;
		Target->CodeSectors[(sector/SECTORSPERBLOCK)]= (SECTORPTR) 0;
		sector += SECTORSPERBLOCK;
...
WORD	atadr;
{
	SECTORPTR	sptr;
	if (!( Target )) return(0);
	if ((sptr = Target->CodeSectors[(((atadr >> 8)/SECTORSPERBLOCK) & MAXSECTORMASK)]) == (SECTORPTR) 0)
...
WORD	get_sector_word( WORD atadr )
{
	...
	if ( OtrOption.WordSize > _SIZE_INT16 ) 
	{
		...
		if ( OtrOption.WordSize > _SIZE_INT32 ) 
		{
			result |=((get_sector_byte(atadr+4) & BYTEMASK) << 32);
			result |=((get_sector_byte(atadr+5) & BYTEMASK) << 40);
			result |=((get_sector_byte(atadr+6) & BYTEMASK) << 48);
			result |=((get_sector_byte(atadr+7) & BYTEMASK) << 56);
		}
	}
#endif
#ifdef	otr32 
...
WORD 	withadr;
{
	...
	if ( OtrOption.WordSize > _SIZE_INT16 ) 
	{
		...
		if ( OtrOption.WordSize > _SIZE_INT32 ) 
		{
			put_sector_byte( (atadr+4),((withadr >> 32) & BYTEMASK));
			put_sector_byte( (atadr+5), ((withadr >> 40) & BYTEMASK));
			put_sector_byte( (atadr+6),((withadr >> 48) & BYTEMASK));
			put_sector_byte( (atadr+7), ((withadr >> 56) & BYTEMASK));
		}
	}
#endif
#ifdef	otr32 
...
WORD	withadr;
{
	...
	if ( atadr != MOINS_UN ) 
	{
		...
		{
#ifdef	otr64
			sysprintf("backpatch( at 0x%16llX ( > 0x%16llX ) with 0x%16llX\n",atadr,Target->OutputBytes,withadr);
#else
#ifdef	otr32
			sysprintf("backpatch( at 0x%8lX ( > 0x%81lX ) with 0x%8lX\n",atadr,Target->OutputBytes,withadr);
#else
			sysprintf("backpatch( at 0x%4X ( > 0x%4X ) with 0x%4X\n",atadr,Target->OutputBytes,withadr);
#endif
#endif
...
/*	abal_crypt	*/
/*	----------	*/
BPTR	abal_crypt( WORD16 pkey, BPTR sptr, WORD16 slen, WORD16	mode)
{
	BPTR	rptr;
	BYTE	exkm[16][16];
	short int kdex,kdey;
...
WORD	mode;	/* 0 : Tcode .AT or .O  , 1 : Debug .DBG or .DBO */
{
	...
	WORD	i;
	if (!( Target )) return;
	Target->OutputLength = HEADERSIZE;
	first_sector();
	...
	flush_sector();
	Target->OutputLength = SECTORSIZE;
	return;
}
...
BPTR		extension3;
{
	...
	if ((Target = (OUTPUTPTR) allocate( sizeof( struct otr_output ) )) != (OUTPUTPTR) 0) 
	{
		...
		for ( l = 0; l < MAXOTRSECTORS; l++ )
			Target->CodeSectors[(l/SECTORSPERBLOCK)] = (SECTORPTR) 0;
		reset_byte_count();
		reset_output();
		first_sector();
