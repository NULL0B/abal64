...
WORD            tusage;
{
	if ( tptr != (TCVDTPTR) 0 ) 
	{
		...
		if (( tptr->nombre == 0 ) &&  ( tptr->descriptor == 0)) 
		{
		...
		if ((!( OtrOption.Optimise & _OPTIMISE_TABLES )) &&  ( tusage ))
			tptr->nombre++;
...
/*	--------------------	*/
/*	allocate_for_segment	*/
/*	--------------------	*/
TCSEGPTR        allocate_for_segment(mode) 
WORD		mode;
{
	TCSEGPTR        sptr;
	...
	if ((sptr = (TCSEGPTR) allocate( sizeof( struct tcode_segment ))) != (TCSEGPTR) 0) 
	{
		...
		sptr->ErrorTrapping = OtrOption.ErrorCatcher;
		sptr->ErrorTrap = (TCERRPTR) 0;
...
TCCONPTR        cptr;
{
	...
	||  (( cptr->type->type & _TYPE_BASIC ) == _TYPE_INT64))
...
BPTR    valptr;
{
	...
	{
	case _TYPE_INT64        :
		cptr->type->size = _SIZE_INT64;
		cptr->identity = convert_numeric_token( valptr );
		break;
	case _TYPE_INT32        :
		cptr->type->size = _SIZE_INT32;
		cptr->identity = convert_numeric_token( valptr );
		break;
	case _TYPE_INT8         :
	case _TYPE_INT16        :
...
TCVARPTR        vptr;
{
	...
	if ( vptr->type->size ) 
	{
		...
		&&  (( vptr->type->type & _TYPE_BASIC) != _TYPE_INT64))
...
TCVARPTR        vptr;
{
	...
	case 0  : if ( vmem == _KW1_PTR )
			Context->ThisField->SmallField += OtrOption.PointerSize;
...
/*	--------------------------	*/
/*	trace_constant_rellocation	*/
/*	--------------------------	*/
WORD	trace_constant_rellocation(WORD id,  WORD number, RELOCPTR rptr)
{

	for (	;
		rptr != (RELOCPTR) 0;
		rptr = rptr->next)
		printf("constant relocation record(%llu) id=%llx [s=%llu, x=%llx]\n",number++,id,rptr->sector, rptr->length);
	return(number);
}


/*	--------------------------	*/
/*	flush_constant_rellocation	*/
/*	--------------------------	*/
WORD    flush_constant_rellocation()
{
	...
	if ((cptr = Context->GCT) != (TCCONPTR) 0) 
	{
		do      
		{
			...
			{
				/* ---------------------------------------- */
				/* generate the constant relocation records */
				/* ---------------------------------------- */
				tcode_integer( cptr->identity );
				generate_code_rellocation( cptr->relocation );

				/* -------------------------------------------- */
				/* dump the constant relocation list if tracing */
				/* -------------------------------------------- */
				if ( OtrOption.TargetTrace )
					number = trace_constant_rellocation( cptr->identity, number, cptr->relocation );
			}
		}
...
WORD            symbol;
{
	...
	if ( symbol ) 
	{
		set_symbol_module(pptr->module);	/*#BG*/
	...
	if (( mode != MOINS_UN ) && ((lptr = Context->PDT) != (TCPRCPTR) 0)) 
	{
		do      
		{
...
/*	build_segment	*/
/*	-------------	*/
TCSEGPTR        build_segment( identity, nptr, check, isprototype )
WORD    identity;
BPTR    nptr;
WORD    check;
WORD	isprototype;	/* 1 : called from LDGO, 0: called from SEGMENT */
{
	...
	&&  ((sptr = locate_internal_segment( nptr )) != (TCSEGPTR) 0))
		return( sptr );
	else if ((sptr = allocate_for_segment(isprototype)) == (TCSEGPTR) 0) 
	{
		(void) allocation_failure();
		return( sptr );
	}
...
/*	is_known_segment	*/
/*	----------------	*/
WORD	is_known_segment( WORD number, BPTR nptr )
{
	TCSEGPTR	sptr;
	BYTE		snom[256];
	sprintf(snom,"%u",number);
	for (	sptr = Context->SDT;
		sptr != (TCSEGPTR) 0;
		sptr = sptr->next ) 
	{
		if ( nptr ) 
		{
			if (!( sptr->name ))
				continue;
			else if (!( compare_tokens( nptr, sptr->name ) ))
				continue;
			else if ( sptr->prototype )
				return(0);
			else	return(1);
		}
		else 	
		{
			if ( sptr->name ) 
			{
				if (!( compare_tokens( snom,sptr->name )))
					continue;
				else if ( sptr->prototype )
					return(0);
				else	return(1);
			}
			else if ( sptr->identity != number )
				continue;
			else if ( sptr->prototype )
				return(0);
			else	return(1);
...
WORD    mode;
{
	...
	if ((sptr = build_segment( identity, nptr, 1, 0 )) == (TCSEGPTR) 0)
		return( syntax_error(672) );
	else    return( start_segment( sptr, options, mode ) );
}

/*	----------------------	*/
/*	implicite_segment_zero	*/
/*	----------------------	*/
...
/*	-------------------------	*/
/*	 explicite_named_segment	*/
/*	-------------------------	*/
/*      Called from LDGO.SEG to 	*/
/*	find segment number of a	*/
/*	named segment.			*/
/*	-------------------------	*/
...
BPTR    token;
{
	...
	if ((sptr = build_segment( MOINS_UN, token, 1, 1)) != (TCSEGPTR) 0)
...
WORD    flush_procedures()
{
	...
	{
		Context->sector_PXT = start_export_procedures();
		for (	pptr = Context->PDT;
			pptr != (TCPRCPTR) 0;
			pptr = pptr->next ) 
		{
			if ( pptr->isexport ) 
			{
				export_procedure_record(pptr);
			}
		}
		Context->length_PXT = terminate_procedure_table();
	}
...
BPTR    identity;
{
	...
	Context->IsExport       = 0;
	Context->ExpliciteExport= 0;
	Context->sector_PXT 	= 0;
	Context->length_PXT 	= 0;
...
BPTR    token;
{
	...
	else if ((sptr = build_segment( MOINS_UN, token, 1,1)) != (TCSEGPTR) 0)
...
WORD            mode;
{
	...
	||  ( vptr->type->size != OtrOption.PointerSize ))
