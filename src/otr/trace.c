...
#elif defined (UNIX)
  #pragma message("PROLOGUE MSG: Trace ON for UNIX")
#else
...
#define CI_TRA_NAME                  "/tmp/prltrace.txt"
#define CI_TRA_ININAME_DEFAULT_DIR   "/etc/prltrace.ini"
...
// ----------------------------------------------------------------------------
// - flags pour les niveaux de trace
// ----------------------------------------------------------------------------
...
#define CI_TRA_F_WARNING    0x0002 // Info très importante devant être tracée quelque soit l'ID
#define CI_TRA_F_ERROR      0x0001 // Info très importante devant être tracée quelque soit l'ID

#define CI_TRA_F_ERROR_OR_WARNING 0x0003

// ----------------------------------------------------------------------------
// - Etat d'activation de la trace
// ----------------------------------------------------------------------------

#define CI_TRA_ON_OK           0x0001
#define CI_TRA_ON_FIRST_TIME   0x0002
#define CI_TRA_ON_TIMERSET		 0x0010
#define CI_TRA_ON_TIMEROK	 	 0x0020
...
TI_TRA_CTX  g_TraCtx = 
   ...
   CI_TRA_ON_FIRST_TIME , // On
   0xFFFF               , // FilterLevel
   0xFFFF               , // FilterId
   // other values not need an initial values
   };

static char g_TraIniFile[512];   // Nom du fichier ini
...
// ----------------------------------------------------------------------------
// - Sortie des infos (accès fichier ou fenêtre de debug)
// ----------------------------------------------------------------------------
...
// --- Process
// ---------------------------------------------
pBuf+= sprintf(pBuf,"%05lu W >>> %s <<< %s Key='%s' MASK_L=%04X MASK_ID=%04X (%04X,%04X) ",
                   pCtx->pri_Process         , 
                   pCtx->pri_Prg             ,
...
if (pCtx->On & CI_TRA_ON_TIMERSET)
	pBuf+=sprintf(pBuf,"timer=%d %lus ", (pCtx->On & CI_TRA_ON_TIMEROK) ? 1 : 0 , pCtx->Timer);
...
// ------------------------------------------------
#if defined(WIN32)
if ( SHGetFolderPath(NULL, CSIDL_PERSONAL, NULL, 0, g_TraIniFile) == S_OK) 
{
	if (g_TraIniFile[strlen(g_TraIniFile)-1] != '\\')
		strcat(g_TraIniFile,"\\");
	strcat(g_TraIniFile,CI_TRA_ININAME_WORKING_DIR);
}
#endif
...
// FCH28032007 (begin) Ajout de l'activation par timer 
// --- Réactivation par timer
//     0 = pas de trace, mais relecture selon la tempo
//     1 = trace activée et relecture selon la tempo
if (TraGetKey(buf, pSetMasterKey, "TIMERON", 5, CI_TRA_FALSE))
	{
   pCtx->On |= CI_TRA_ON_TIMERSET;
	if (buf[0]=='1')
		pCtx->On |= CI_TRA_ON_TIMEROK;
	}

// --- Temporisation de la relecture de la config
if (TraGetKey(buf, pSetMasterKey, "TIMERDELAY", 5, CI_TRA_FALSE))
   sscanf(buf,"%hd",&pCtx->Timer);
// FCH28032007 (end)
...
// --- Test de cohérence
if (pCtx->Timer < 1)
	pCtx->Timer = 5*60; //5mn par défaut

if (pCtx->On)
	{
	// Si rien n'est tracé, pas de trace
	if (!pCtx->FilterLevel || !pCtx->FilterId)
		pCtx->On = 0;
	}
...
if (pCtx->On & CI_TRA_ON_TIMERSET) 
	{
	time( (time_t*)&pCtx->pri_TimerIni );
	pCtx->pri_TimerCur = pCtx->pri_TimerIni;
	}
// FCH28032007 (end)
...
if (pCtx->FilterLevel & CI_TRA_F_TIME_MILLI)
   pCtx->p += sprintf(pCtx->p,"%02d.%03d ",Tm->tm_sec,Timeb.millitm);
else
   pCtx->p += sprintf(pCtx->p,"%02d/%02d/%02d %02d:%02d:%02d.%03d ",Tm->tm_mday,Tm->tm_mon+1,Tm->tm_year-100,Tm->tm_hour,Tm->tm_min,Tm->tm_sec,Timeb.millitm);
...
// Level requested
if ( !( (Level&pCtx->FilterLevel)  &&  ((Level&CI_TRA_F_ERROR_OR_WARNING&pCtx->FilterLevel) || (Id&pCtx->FilterId)) ) )
   return 1;
...
if (pCtx->FilterLevel & CI_TRA_F_THREAD)
	pCtx->p += sprintf(pCtx->p, "%04X ", (unsigned short)GetCurrentThreadId());
#endif
...
pCtx_des->pri_TimerIni= pCtx_ori->pri_TimerIni;
pCtx_des->pri_TimerCur= pCtx_ori->pri_TimerCur;
...
// ****************************************************************************
// * TraCtxTrace
// * Trace with context (idem TraTrace)
// *
// * pCtx    IN/OUT: Pointer on a trace context
// * Level     IN  : level
// * Id        IN  : Module ID 
...
if (pCtx->On & CI_TRA_ON_TIMERSET)
	{
	if ( pCtx->pri_TimerCur - pCtx->pri_TimerIni > pCtx->Timer )
		pCtx->On |= CI_TRA_ON_FIRST_TIME;
	else
		time( (time_t*)&pCtx->pri_TimerCur );

	if ( !(pCtx->On & CI_TRA_ON_TIMEROK) )
		return 1;
	}
// FCH28032007 (end)
...
if (Level & CI_TRA_F_DUMP)
   ...
   {
/* NOK en 64b
   pCtx->p += vsprintf(pCtx->p, mes, (char*)(&mes+1));
*/
   va_list args;

   va_start(args, mes);
   pCtx->p += vsprintf(pCtx->p, mes, args);
   va_end(args);
...
// ****************************************************************************
// * TraTrace
// * Trace with implicit g_TraCtx global variable (idem TraCtxTrace)
// *
// * Level     IN  : level
// * Id        IN  : Module ID 
// * tfn       IN  : origin of the message (typically, name of the calling function)
