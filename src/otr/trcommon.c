...
TCSTRUCTPTR	sptr;
{
	...
	&&  ( dptr->structname != (BPTR) 0 )) 
	{
		...
		if ((status = declare_structure( dptr->structure, member->name, nature ,
				(member->access & _MEMBER_PRIVATE),0,dptr->construction, dptr->field )) != 0)
			return( status );
	}
	else 	
	{
		member->lock |= _MEMBER_LOCKED;
		status = declare_member( member->class, member, 
				(member->access & _MEMBER_PRIVATE),0,
				( member->class->access & _MEMBER_OVERLAY ? 10 : structure_occupation( member->class ) ));
		member->lock &= ~_MEMBER_LOCKED;
		if ( status )
			return( status );
	}
