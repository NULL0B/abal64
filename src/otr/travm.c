	...
	BPTR	vptr;
{
	...
	if ( OtrOption.Special ) {
		sprintf(RegBuffer,"R[%llu] = %s",number,AtocRegister[number]);	
	...
	while ( cptr != (TCCONPTR) 0) {
		...
		if ( cptr->identity == v ) {
			if ( cptr->name != (BPTR) 0 ) {
				...
				if (( nptr = allocate_for_tnode( 16 )) != (BPTR) 0)
					sprintf(nptr,"$%s%llu",pptr,cptr->identity);
				break;
				}
			}
		else	cptr = cptr->next;
		...
		case	_tc_GWS :
			if ((sptr = avm_integer( GetTcodeWord(offset) )) == (BPTR) 0)
				...
				if ((tptr = allocate_for_tnode( 16 ))== (BPTR) 0) 
					...
					sprintf(tptr,"G%llu",GetTcodeWord(offset));
					offset += 2;
					break;
					}
				}
		...
		case	_tc_LWS :
			if ((sptr = avm_integer( GetTcodeWord(offset) )) == (BPTR) 0)
				...
				if ((tptr = allocate_for_tnode( 16 ))== (BPTR) 0) 
					...
					sprintf(tptr,"L%llu",GetTcodeWord(offset));
					offset += 2;
					break;
					}
				}
