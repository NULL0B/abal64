					...
					/* 2/3/4  : Variable if Possible	*/
{
	...
	{
		if ((tptr = next_free_register()) != (RESULTPTR) 0)
			(void) set_shadow_type( tptr->contents, _TYPE_STRING  );
		return( tptr );
	}
	else if ( mode < 2 ) 
	{
...
CLFPTR	xptr;
{
	...
	case 0x00CD     :	/* SEGM		*/
	case 0x00DD     :	/* USER		*/
		return( make_target_variable(_TYPE_STRING) );
	...
	case 0x007D	:	/* CHR$ 	*/
		return( make_target_variable(_TYPE_STRING) );
	case 0x000E	:	/* FIX		*/
		return( make_target_variable(_TYPE_NUMERIC) );
	...
	default		:
		if ((rptr = next_free_register()) != (RESULTPTR) 0) 
		{
			...
			case	15 :
			case 	5  : 
				set_shadow_type( rptr->contents, _TYPE_INT  );
				break;
			...
			case	14 :
				...
				{ 	
					/* STRN */
					set_shadow_type( rptr->contents, _TYPE_STRING );
					break;
				}
				else if ( xptr->code == 8 ) 
				{ 	
...
CLFPTR	xptr;
{
	RESULTPTR	rptr;
	WORD		ht;
	ht = set_clf_parameter_type(_TYPE_STRING);
...
CLFPTR	xptr;
{
	RESULTPTR	rptr;
	WORD		ht;
	ht = set_clf_parameter_type(_TYPE_INT);
	if ((rptr = evaluate_expression(0)) != (RESULTPTR) 0) 
	{
...
CLFPTR	xptr;
{
	RESULTPTR	rptr;
	WORD		ht;
	ht = set_clf_parameter_type(_TYPE_NUMERIC);
	if ((rptr = evaluate_expression(0)) != (RESULTPTR) 0) 
	{
...
/*	---------------------------------	*/
/*	optional_parameters_without_brace	*/
/*	---------------------------------	*/
...
CLFPTR	xptr;
{
	...
	if ( is_left_brace() ) 
	{
		if ((status = optional_parameters_without_brace(xptr)) != 0)
			return status;
...
/*	 translate_label	*/
/*	-----------------	*/
/*	LABEL(c,l,nc,nl,fs,a,m)	*/
/*	-----------------	*/

static	WORD	translate_label()
{
	WORD		status;
	CLFPTR		xptr=(CLFPTR) 0;

	if (((xptr = start_clf(_CLF3_LABEL,3))  != (CLFPTR) 0)
	&&  ((status = optional_parameters(xptr)) == 0))
		return( release_context(xptr,flush_void_clf(xptr)));
	else	return( release_context(xptr,(RESULTPTR) 0));
}
...
static	WORD	translate_upload()
{
	WORD		status;
	CLFPTR		xptr=(CLFPTR) 0;

	if (((xptr = start_clf(_CLF3_UPLOAD,3))  != (CLFPTR) 0)
	&&  ((status = optional_parameters(xptr)) == 0))
		return( release_context(xptr,flush_void_clf(xptr)));
	else	return( release_context(xptr,(RESULTPTR) 0));
}
...
static	WORD	translate_color()
{
	WORD		status;
	CLFPTR		xptr=(CLFPTR) 0;

	if (((xptr = start_clf(_CLF3_COLOR,3))  != (CLFPTR) 0)
	&&  ((status = optional_parameters(xptr)) == 0))
		return( release_context(xptr,flush_void_clf(xptr)));
	else	return( release_context(xptr,(RESULTPTR) 0));
}
...
static	WORD	translate_image()
{
	WORD		status;
	CLFPTR		xptr=(CLFPTR) 0;

	if (((xptr = start_clf(_CLF3_IMAGE,3))  != (CLFPTR) 0)
	&&  ((status = optional_parameters(xptr)) == 0))
		return( release_context(xptr,flush_void_clf(xptr)));
	else	return( release_context(xptr,(RESULTPTR) 0));
}
...
static	WORD	translate_video()
{
	WORD		status;
	CLFPTR		xptr=(CLFPTR) 0;

	if (((xptr = start_clf(_CLF3_VIDEO,3))  != (CLFPTR) 0)
	&&  ((status = optional_parameters(xptr)) == 0))
		return( release_context(xptr,flush_void_clf(xptr)));
	else	return( release_context(xptr,(RESULTPTR) 0));
}
...
static	WORD	translate_popup()
{
	WORD		status;
	CLFPTR		xptr=(CLFPTR) 0;

	if (((xptr = start_clf(_CLF3_POPUP,3))  != (CLFPTR) 0)
	&&  ((status = optional_parameters(xptr)) == 0))
		return( release_context(xptr,flush_void_clf(xptr)));
	else	return( release_context(xptr,(RESULTPTR) 0));
}
...
static	WORD	translate_size()
{
	WORD		status;
	CLFPTR		xptr=(CLFPTR) 0;

	if (((xptr = start_clf(_CLF3_SIZE,3))  != (CLFPTR) 0)
	&&  ((status = optional_parameters(xptr)) == 0))
		return( release_context(xptr,flush_void_clf(xptr)));
	else	return( release_context(xptr,(RESULTPTR) 0));
}
...
static	WORD	translate_font()
{
	WORD		status;
	CLFPTR		xptr=(CLFPTR) 0;

	if (((xptr = start_clf(_CLF3_FONT,3))  != (CLFPTR) 0)
	&&  ((status = optional_parameters(xptr)) == 0))
		return( release_context(xptr,flush_void_clf(xptr)));
	else	return( release_context(xptr,(RESULTPTR) 0));
}
...
static	WORD	translate_rgb_fg()
{
	WORD		status;
	CLFPTR		xptr=(CLFPTR) 0;

	if (((xptr = start_clf(_CLF3_FOREGROUND,3))  != (CLFPTR) 0)
	&&  ((status = optional_parameters(xptr)) == 0))
		return( release_context(xptr,flush_void_clf(xptr)));
	else	return( release_context(xptr,(RESULTPTR) 0));
}
...
static	WORD	translate_rgb_bg()
{
	WORD		status;
	CLFPTR		xptr=(CLFPTR) 0;

	if (((xptr = start_clf(_CLF3_BACKGROUND,3))  != (CLFPTR) 0)
	&&  ((status = optional_parameters(xptr)) == 0))
		return( release_context(xptr,flush_void_clf(xptr)));
	else	return( release_context(xptr,(RESULTPTR) 0));
}
...
/* -------------------------------------------------------------- */
static	WORD	translate_clf3( WORD	type )
{
	switch ( type ) 
	{
#ifdef	ABAL32A
	...
	case	_CLF3_FONT	:
		return( translate_font() );
	case	_CLF3_FOREGROUND	:
		return( translate_rgb_fg() );
	case	_CLF3_BACKGROUND	:
		return( translate_rgb_bg() );
	case	_CLF3_COLOR	:
		return( translate_color() );
	case	_CLF3_UPLOAD	:
		return( translate_upload() );
#endif
	}
...
/*	-------------------------------------------------------------------	*/
/*			CLF POINTER FUNCTION SYNTAX OVERVIEW			*/
/*	-------------------------------------------------------------------	*/
...
WORD	type;
{
	...
	case 	_CLF_CONV 	:
		...
		else 	
		{
			...
			if ( type == _TYPE_STRING )
				Context->OperandType= _TYPE_NUMERIC;
		}
	...
	case	_CLF_CONV3	:
		...
		||  ( type == _CLF_CONV  )) 
		{
			...
			&&  ( Context->TargetVariable->type != (TCTYPPTR) 0)) 
			{
				...
				/* it is essentiel for integer to string types 	 	*/
				/* --------------------------------------------------	*/
				if ((temptype = (Context->TargetVariable->type->type & _TYPE_BASIC)) == _TYPE_STRING ) 
				{
					if ( type != _CLF_CONV )
						Context->OperandType=_TYPE_NUMERIC;
					type = 666;
	...
	case	_CLF_CONV5	:
	case	_CLF_CONV6	:
		return( translate_conv( type & _TYPE_BASIC ) );
	...
	case	_CLF_SEGMNAME :
		return( translate_segment_name() );
	case	_CLF_PROCNAME :
		return( translate_procedure_name() );
	case	_CLF_PROGNAME :
		return( translate_program_name() );
	case	_CLF_TRTRUE :
		return( integer_result(1) );
	case	_CLF_TRFALSE :
		return( integer_result(0) );
	case	_CLF_WORDSIZE: 
		return( integer_result(OtrOption.WordSize) );
	case	_CLF_PTRSIZE:
		return( integer_result(OtrOption.PointerSize) );
	}
