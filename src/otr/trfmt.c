...

{
	...
	*(wptr++) = (BYTE) FMT_DEB; xtrl = 0; nlen--;
	...
	do 	{
		...
		case	'$'	: 
			/* -------------------------------- */ 
			/* Check for DISPLAY till CHARACTER */
			/* -------------------------------- */ 
			if ( *fptr == '(' ) 
			{
				*(wptr++) = FMT_DCH;
			  	fptr++; flen--; /* remove the brace */
			  	*(wptr++) = *(fptr++); flen--;
			  	fptr++; flen--; /* remove teh brace */
				nlen -= 2;
			}
			else 	
			{
		...
		case	' '	: 
		case	'\t'	: 
			while ((flen > 0 ) && ((*fptr == '\t') || (*fptr == ' '))) { fptr++; flen--; }
			continue;

		/* --------------------------------- */
		/* Power displacement +- power of 10 */
		/* --------------------------------- */
		case	'V'	: fbcdc |= 1; continue;

		/* ---------------------------- */
		/* Display Constant text string */
		/* ---------------------------- */
		case	'"'	: 
			*(wptr++) = (BYTE) FMT_PR; nlen--;
			hptr = wptr; wptr++; x = 0; nlen--;
			while (( *fptr != '"' ) && ( flen > 0 ) && (nlen > 0)) 
			{
				*(wptr++) = *(fptr++); nlen--;
				x++; flen--;
			}
		...
		case	'S'	: 
			...
			/* ------------------------------------- */
			/* Check for EXPLICTE count or separator */
			/* ------------------------------------- */
			if ( *fptr == '(' ) 
			{
				/* ---------------------- */
