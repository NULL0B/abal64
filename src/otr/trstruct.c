...
TCSTRUCTPTR	hostclass;
{
	...
	status = dcl_variable(
				...
				3,			/* Data type	 */
				OtrOption.PointerSize,	/* Data length	 */
				1,			/* 1st dimension */
				1,			/* 2nd dimension */
				_KW1_DCL,		/* DCL type	 */
				0,			/* Already typed */
	...
	if (( status == 0 ) && ( member->access & (_MEMBER_OVERLOAD | _MEMBER_VIRTUAL) )) 
	{
		...
		{ 	
			if ((variable = local_extern_global()) != (TCVARPTR) 0) 
			{
...
TCMEMBERPTR	mptr;
{
	...
	if ( dptr->structname != (BPTR) 0 ) 
	{
		...
		||  ( mptr->nature == _KW1_OVL ))
			return((mptr->size = OtrOption.PointerSize ));
...
WORD		master;
{
	...
	if ((status = dcl_variable( 
				...
				(functions * OtrOption.PointerSize * 2),/* Table Size     	*/
				1,		/* 1st dimension	*/
				1,		/* 2nd dimension 	*/
...
WORD		master;
{
	...
	if ((status = dcl_filler( OtrOption.PointerSize * 2 )) == 0 )
		/* -------------------------------- */
...
WORD		master;
{
	...
	if ((status = dcl_variable( 
				...
				OtrOption.PointerSize,		/* EIMT size 		*/
...
WORD		master;
{
	...
	if ((status = dcl_variable( 
				...
				OtrOption.PointerSize,		/* Size of Proc Ptr	*/
...
WORD		level;
{
	...
	if ((bptr = local_extern_global()) != (TCVARPTR) 0)
	{
...
WORD		field;
{
	...
	if ( nature != _KW1_OVL ) 
	{
		...
		do	
		{
			...
			if (!( mptr->nature )) 
			{
				if ((status = declare_member(sptr,mptr,(level+adjust),master,(
					 sptr->access & _MEMBER_OVERLAY ? OtrOption.PointerSize * 2 : sptr->size ))) != 0)
					return( status );
				else	continue;
			}
			else if ( mptr->nature == _KW1_CONST )
			...
			/* ----------------------------------- */
			else if ((status = declare_member(sptr,mptr,(level+adjust),master,( sptr->access & _MEMBER_OVERLAY ? OtrOption.PointerSize*2 : sptr->size ))) != 0)
				return( status );
	...
	if ( sptr != (TCSTRUCTPTR) 0) 
	{
		if ((mptr = sptr->first) != (TCMEMBERPTR) 0) 
		{
			do	
			{
				...
				if (!( mptr->nature )) 
				{
					if ((status = declare_member(sptr,mptr,0,0,( sptr->access & _MEMBER_OVERLAY ? OtrOption.PointerSize*2 : sptr->size ))) != 0)
						return( status );
					else	continue;
				}
				else if ( mptr->nature == _KW1_CONST )
				...
				/* Declaration of Terminal Data Member */
				/* ----------------------------------- */
				else if ((status = declare_member(sptr,mptr,0,0,( sptr->access & _MEMBER_OVERLAY ? OtrOption.PointerSize*2 : sptr->size ))) != 0)
					return( status );
...
/*	------------------------------	*/
/*	translate_structured_parameter	*/
/*	------------------------------	*/
...
TCSTRUCTPTR	sptr;
{
	...
	if ((vptr->type = allocate_for_type( 
				(vtype | (_TYPE_STRING | _TYPE_DEFINED)) , 
				(sptr->access & _MEMBER_OVERLAY ? OtrOption.PointerSize*2 : sptr->size), 
				1, 1, sptr )) == (TCTYPPTR) 0)
		return( allocation_failure() );
	else	return( 0 );
}
...
WORD		level;		/* Initial Entry componant may be private */
{
	...
	if ((member = class->first) != (TCMEMBERPTR) 0) 
	{
		...
		do	
		{
			...
			{
				if ((wptr = locate_member( class, nptr )) != (TCMEMBERPTR) 0) 
				{
					*optr = componant;
					return( wptr );
				}
...
/*	----------------------	*/
RESULTPTR	translate_program_name()
{
	BYTE	name[32];
	int	i;
	RESULTPTR	rptr;
...
/*	----------------------	*/
/*	translate_segment_name	*/
/*	----------------------	*/
RESULTPTR	translate_segment_name()
{
	RESULTPTR	rptr;
	TCSEGPTR	sptr;
	BYTE		name[1024];
	if (!( sptr = Context->CurrentSegment))
		strcpy(name, " ");
	else
	{
		if (!( sptr->name ))
			sprintf(name,"%u",sptr->identity);
		else	sprintf(name,"%s",sptr->name);
	}
	trace( name );
	if ((rptr = allocate_for_result()) != (RESULTPTR) 0)
		return( build_string_constant( rptr, name ) );
...
/*	------------------------	*/
/*	translate_procedure_name	*/
/*	------------------------	*/
RESULTPTR	translate_procedure_name()
{
	BPTR		name=" ";
	RESULTPTR	rptr;
	TCPRCPTR	pptr;
...
/*	---------------------	*/
RESULTPTR	translate_method_name()
{
	TCMBMETHPTR 	method;
	TCSTRUCTPTR	class;
	BYTE		name[2048];
	...
	trace("translate_method_name()\n");
	if (!(method = (TCMBMETHPTR)input_method()))
		strcpy(name," ");
	...
	else
	{
		...
		strcpy(root, method->member->name);
		...
		case	_CLASS_INVARIANT	:
			/* resolve the hosting method */
			if (!( method = (TCMBMETHPTR) input_parent() ))
				strcpy(name,root);
			else if (!( method->member ))
				strcpy(name,root);
			else	sprintf(name,"%s.%s",method->member->name,root);
			break;
		default	:
			strcpy(name,root);
		}
