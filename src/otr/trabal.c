...
WORD	nature;		/* DCL, PROC, SEGM, USER, CODE 	*/
{
	...
	if ( copy_token( vname, _MAX_OTR_TOKEN ) == 0 )
		return( 30 );
	...
	if ( type == _KW1_CONST ) 
	{
		...
		switch ( xtype ) 
		{
		case '#' : vsize = _SIZE_INT8;
			   vtype = _TYPE_INT8;
			   sprintf(TokenBuffer,"%u",(integer_expression() & 0x0000FF));
			   break;

		case '%' : vsize = _SIZE_INT16;  
			   vtype = _TYPE_INT16;
			   sprintf(TokenBuffer,"%u",(integer_expression() & 0x00FFFF));
			   break;
#ifdef	otr64
		case '&' : vsize = _SIZE_INT64;  
			   vtype = _TYPE_INT64;
			   sprintf(TokenBuffer,"%llu",integer_expression());
			   break;
#ifdef	otr32 
		case ':' : vsize = _SIZE_INT32;  
			   vtype = _TYPE_INT32;
	...
	/* --------------------------- */
	if (( vtype == _TYPE_NUMERIC ) && ( vsize > _MAX_NUMERIC ))
...
WORD	translate_endif()
{
	...
	status = leave_iteration();
	return(status);
}

/*	--------------------------	*/
/*	old_conditional_expression	*/
/*	--------------------------	*/
...
TCTYPPTR	tptr;
{
	...
	if ((tptr->type & _TYPE_POINTER) != _TYPE_POINTER) 
	{
		...
		case	_TYPE_STRING	:
			...
			/* ----------------------------------------------------- */
			if (!(rptr->contents))	/*#BG ???*/
				if (!( rptr = convert_to_type( rptr, (WORD)(tptr->type & _TYPE_BASIC) )))
	...
	if  ( get_token() ) 
	{
		...
		case _CTRL_CALL		:
			...
			else
			{
				...
				if (( is_comma() )
				&&  ((rptr = evaluate_expression(0)) != (RESULTPTR) 0))
					return( explicite_on_error_call( rptr->contents, mode, pptr) );
			}
			break;
...
#ifdef	_KW1_REMOTE
	case _KW1_REMOTE    : return( translate_remote()  );
#endif
...
WORD	segmental_translation()
{
	...
	if ( remove_white_space() ) 
	{
		...
		{
			if (((status = translate_control_keyword()) == MOINS_UN)
			&&  ((status = translate_statement())       == MOINS_UN)
			&&  ((status = translate_clf())             == MOINS_UN)
			&&  ((status = translate_iof())             == MOINS_UN)
			&&  ((status = translate_ngf())             == MOINS_UN)
			&&  ((status = translate_level_one()) 	    == MOINS_UN)
			&&  ((status = translate_level_two()) 	    == MOINS_UN))
