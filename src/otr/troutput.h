...
#define SECTORSPERBLOCK (Target->OutputLength/256)
...
#ifdef	otr64
#define	MAXOTRSECTORS 	0x00100000
#define	BYTEMASK	(SECTORSIZE-1)
#else
#ifdef	otr32
...
WORD	instruction_requires_LDV( WORD ) ;
WORD	implicite_on_error( TCVARPTR, WORD, WORD );
WORD	explicite_on_error( TCVARPTR, WORD, BPTR );
WORD	explicite_on_error_call( TCVARPTR, WORD, TCPRCPTR );
...
#ifdef	WITH_ATOC
WORD	generate_op( BPTR, TCVARPTR );
WORD	store_clp( PRCPTR, BPTR, WORD );
WORD	generate_resume( WORD, BPTR );
WORD	generate_gosub( BPTR );
WORD	generate_ldgoseg( BPTR, WORD, WORD );
WORD	generate_iof_instruction( WORD, BPTR );
#else
