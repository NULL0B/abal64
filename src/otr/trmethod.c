...
WORD		field;
{
	...
	if ((overlay = allocate_overlay_method()) != (TCMBOVLPTR) 0) 
	{
		...
		else if ((overlay->segment = build_segment( MOINS_UN, member->name, 0, 1)) == (TCSEGPTR) 0) 
		{
			liberate( overlay );
			(void) allocation_failure();
			return((TCMBOVLPTR) 0);
...
TCSTRUCTPTR	class;
{
	...
	if ((object->type = allocate_for_type(
				((class->access & _MEMBER_OVERLAY) ? 
				(_TYPE_OVERLAY | 0x901B) : 0x901B),
				object_occupation(class),1,1,class)) == (TCTYPPTR) 0)
		return( allocation_failure() );
	else	
	{
...
/*	---------------------------------	*/
/*	 collect_construction_parameters	*/
/*	---------------------------------	*/
...
WORD		mode;
{
	...
	/* --------------------------------- */
	if ((status = establish_macro_input( 	object, method, method->value->buffer, parameters, action )) != 0 )
		return( status );
...
TCMBMETHPTR	method;
{
	...
	{
		sprintf(FunctionName,"_F%u_%s.%s",NextFunction++ ,member->class->name,member->name);
...
TCMBMETHPTR	method;
{
	...
	field_equals_s();  
	return( field_equals_vbm( bptr, 0L, member ) );
}

/*	---------------------	*/
/*	locate_method_pointer	*/
/*	---------------------	*/
	...
	case	_CLASS_OVERLAY	:
		if ( method->options & _METHOD_INDIRECT ) 
		{
			...
			&&  ( method->nature != _CLASS_USER ))
				...
				return( OtrOption.PointerSize * 2 );
			else if ( member_enhanced_indirect( member ) )
				return( OtrOption.PointerSize );
		}
	default			:
		return( 0 );
	}
...
/*	---------------------------	*/
/*	ll_verify_strict_parameters	*/
/*	---------------------------	*/
WORD		ll_verify_strict_parameters( vptr, pptr )
TCVARPTR	vptr;
TCMBPARAPTR	pptr;
{
...
TCMBPARAPTR	pptr;
{
	...
	Context->VerifyStrict = 0; /* tolerate bcd/integer as numeric */
	status = ll_verify_strict_parameters( vptr, pptr );
	Context->VerifyStrict = holder; /* restore previous situation */
	return( status );	 
}
...
TCMBMETHPTR	nptr;
{
	...
	return( compare_parameter_list(	
			explicite_method_parameters(nptr), 
			explicite_method_parameters(optr) ) );
}

/*	-------------------------	*/
/*	locate_alternative_method	*/
/*	-------------------------	*/
