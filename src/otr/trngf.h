...
#define	_NGF_KEY_F	4	// FLOAT / DOUBLE
#define	_NGF_KEY_L	5	// LONG
#define	_NGF_KEY_H	6	// HUGE
...
struct	ngf_control	{
	...
	BPTR		Parameter[16];
#endif
	BYTE		ErrorLabel[MAX_LABEL_SIZE];
	BYTE		Descriptor[MAX_LABEL_SIZE];
	};
...
WORD	translate_ngf();
#ifdef ABALSQL
WORD	translate_sql_connect( WORD );
WORD	translate_sql_begin( WORD );
WORD	translate_sql_commit( WORD );
WORD	translate_sql_rollback( WORD );
WORD	translate_sql_prepare( WORD );
WORD	translate_sql_exec( WORD );
WORD	translate_sql_getrowcol( WORD );
WORD	translate_sql_getcolname( WORD );
WORD	translate_sql_fetch( WORD );
WORD	translate_sql_getfield( WORD );
WORD	translate_sql_drop( WORD );
WORD	translate_sql_getsqlerror( WORD );
WORD	translate_sql_getserviceinfo( WORD );
WORD	translate_sql_bindparam( WORD );
WORD	translate_sql_bindcol( WORD );
#endif
