...
static	unsigned char InputCharSet[256];
...
{
	WORD	c;
	sysprintf("%c",(c=memory_source()));
	return( c );
}

#ifndef USEICONV
/*	----------------------	*/
/*	iconv_source_character	*/
/*	----------------------	*/
/*	ICONV is not available	*/
/*	----------------------	*/
WORD	iconv_source_character( SOURCEPTR s)
{
	int	c;
	return( fgetc( s->Source.Handle ) );
}
#else
...
/*	using  iconv  library.	*/
/*	----------------------	*/
WORD	iconv_source_character( SOURCEPTR sptr)
{
	int	c;
	char	sbytes[8];
	...
	int	tn=8;
	int	i=0;
	if ((c = fgetc(sptr->Source.Handle)) == -1)
		return( c );
	else if (!( c & 0x0080))
		return( c & 0x007F );
	else if (!( c  & 0x0020))
	...
	while ( i < nb ) 
	{
		sbytes[i++] = (fgetc(sptr->Source.Handle) & 0x00FF);
	}

	if (!( sptr->encflag )) 
	{
		sptr->enchandle = (VPTR) iconv_open( "ISO-8859-1", sptr->encoding );
		sptr->encflag = 1;
	}
	iconv( sptr->enchandle, &iptr, &sn, &optr, &tn );
...
WORD	file_source()
{
	WORD	c;
	BYTE	cc;
	switch ( Input->encmode ) 
	{
	case	__ICNV_ENCODING	:	/* utf8 and others */
		c = iconv_source_character( Input );
		break;

	case	__ABAL_ENCODING	:
	case	__ANSI_ENCODING	:
	case	__FILE_ENCODING	:
		if ( fread( & cc, 1,1 , Input->Source.Handle ) == 1 ) {
			c = (InputCharSet[(cc & 0x00FF)]& 0x00FF);
			}
		else	c = 0x001A; 
	}
	return( c );
...
SOURCEPTR	allocate_for_source()
{
	...
	/* --------------------------------- */
	/* Allows auto Pop of Input Contexts */
	/* --------------------------------- */
	sptr->Status = 1;	
	sptr->Nature = _INPUT_FILE;	
	sptr->Level  = 0;
	sptr->BalComment = 1;
	sptr->Lines  = 0L;
	...
	sptr->encflag	  = 0;
	sptr->enchandle   = (VPTR) 0;
	sptr->encoding	  = _ABAL_ENCODING;
	sptr->encmode	  = __ABAL_ENCODING;
...
VALUEPTR	vptr;
{
	...
	/* ------------------------------- */
	/* Inhibits Auto Pop Input Context */
	/* ------------------------------- */
	sptr->Status       = 0;	
...
/*	set_method_infos	*/
/*	----------------	*/
VOID	set_method_infos( WORD file, WORD line, WORD offset )
{
	Input->Number       = file;
	Input->Lines	    = (line-1);
...
WORD		mode;
{
	...
	{
		/* ----------------------------------------- */
		/* Inline Method : Optimised Parameters Only */
		/* ----------------------------------------- */
...
WORD*	pcol;
{
	if (( Input != (SOURCEPTR) 0 ) && ( Input->Name != (BPTR) 0)) 
	{
		...
		else	sprintf( bptr, "%s(%lu)", Input->Name, Input->Lines );
	}
	else 
	{ 
		*bptr = '\0'; 
	...
	&&  ( Input->Name != (BPTR) 0)) 
	{
		if ( Eof.Number != -1 ) 
		{
			*bptr = 0;
			...
			sprintf( (bptr+strlen(bptr)), " %lu 1", Input->Lines );
		}
		else	
		{
			sprintf( bptr, "%s %lu 1", Input->Name, Input->Lines );
		}
...
/*	---------------------	*/
/*	drop_extended_charset	*/
/*	---------------------	*/
WORD	drop_extended_charset(SOURCEPTR sptr)
{
	if ( sptr ) 
	{
		switch ( sptr->encmode ) 
		...
		case	__FILE_ENCODING	:
			break;
		case	__ICNV_ENCODING	:
			if ( sptr->encoding ) 
			{
				if ( sptr->encflag ) 
				{
					iconv_close( sptr->enchandle );
					sptr->encflag = 0;
					sptr->enchandle = (VPTR) 0;
				}
				liberate( sptr->encoding );
			}
		}
	}
	return(0);
...
/*	--------------------	*/
/*	use_extended_charset	*/
/*	--------------------	*/
WORD	use_extended_charset(BPTR nptr)
{
	if ( Input != (SOURCEPTR) 0) 
	{
		drop_extended_charset(Input);
		Input->encoding	  = allocate_for_string( nptr );
		Input->encmode	  = __ICNV_ENCODING;
	}
	return(0);
}
...
/*	use_abal_charset	*/
/*	----------------	*/
WORD	use_abal_charset()
{
	WORD	i;
	if ( Input != (SOURCEPTR) 0) 
	{
		drop_extended_charset(Input);
		Input->encoding	  = _ABAL_ENCODING;
		Input->encmode	  = __ABAL_ENCODING;
	}
	for (i=0; i < 256; i++ ) { InputCharSet[i] = i; }
	return(0);
}
...
/*	use_ansi_charset	*/
/*	----------------	*/
WORD	use_ansi_charset()
{
	WORD	i;
	if ( Input != (SOURCEPTR) 0) 
	{
		drop_extended_charset(Input);
		Input->encoding	  = _ANSI_ENCODING;
		Input->encmode	  = __ANSI_ENCODING;
	}
	for (i=0; i < 256; i++ ) { InputCharSet[i] = Ansi2Twin[i]; }
	return(0);
}
...
/*	load_input_charset	*/
/*	------------------	*/
WORD	load_input_charset( BPTR filename )
{
	FILE *	h;
	WORD	status;
	if (!( filename ))
		return(30);
	else if (!( h = fopen( filename, "rb" )))
		return(40);
	else	
	{
		if ( Input != (SOURCEPTR) 0) 
		{
			drop_extended_charset(Input);
			if (!( Input->encoding = allocate_for_string( filename ) )) 
			{
				fclose(h);
				return(27);
			}
			Input->encmode = __FILE_ENCODING;
		}
		if ( fread(InputCharSet,256,1,h) != 1 ) 
			status = 48;
		else	status = 0;
		fclose(h);
		return(status);
