...
/*	compare_abal_types	*/
/*	------------------	*/
WORD	compare_abal_types( WORD ltype, WORD rtype )
{
	switch ( ltype & _TYPE_BASIC ) 
	{
	case	_TYPE_INT8	:
	case	_TYPE_INT16	:
	case	_TYPE_INT32	:
	...
	case	_TYPE_NUMERIC	:
		...
		/* numeric must match */
		/* ------------------ */
		if ((rtype & _TYPE_BASIC) != _TYPE_NUMERIC )
			return( FAUX );
		else	return( VRAI );
	case	_TYPE_STRING	:
		/* ----------------------- */
		/* alpha string must match */
		/* ----------------------- */
		if ((rtype & _TYPE_BASIC) != _TYPE_STRING )
			return( FAUX );
		else	return( VRAI );
	default			:
		return(FAUX);
...
/*	--------------------	*/
/*	compare_strict_types	*/
/*	--------------------	*/
WORD	compare_strict_types( WORD ltype, WORD rtype )
{
	switch ( ltype ) 
	{
	case	_TYPE_INT8	:
...
/*	------------------------	*/
/*	compare_compatible_types	*/
/*	------------------------	*/
WORD    compare_compatible_types( TCTYPPTR lptr, TCTYPPTR rptr )
{
	switch ( lptr->type & _TYPE_BASIC )
	{
	case	_TYPE_INT8	:
	case	_TYPE_INT16	:
	case	_TYPE_INT32	:
	case	_TYPE_INT64	:
		switch ( rptr->type & _TYPE_BASIC )
		{
		case	_TYPE_INT8	:
		case	_TYPE_INT16	:
		case	_TYPE_INT32	:
...
/*	compare_types	*/
/*	-------------	*/
WORD    compare_types( TCTYPPTR lptr, TCTYPPTR rptr )
{
	trace("compare_types()\n");
	...
	if (!( rptr->type & _TYPE_DEFINED )) 
	{
		if (( rptr->type & _TYPE_BASIC ) == ( lptr->type & _TYPE_BASIC ))
			return( VRAI );
		else if (!(Context->VerifyStrict ))
			return( compare_compatible_types(lptr,rptr));
		else    return( compare_strict_types(
				( rptr->type & _TYPE_BASIC ),	
				( lptr->type & _TYPE_BASIC )));
	}
	else    
	{
...
TCCONPTR        cptr;
{
	...
	else	
	{
		...
		||  (( cptr->type->type & _TYPE_BASIC ) > _TYPE_STRING ))
		{
...
RESULTPTR	rptr;
{
	...
	if (!(cptr = (TCCONPTR) rptr->contents))
		...
		return( rptr );
	else if (!( wptr = allocate( strlen( TokenBuffer ) + 64 ) ))
		return( rptr );
	else	
	{
...
RESULTPTR       rptr;
{
	...
	{
#ifdef	otr64
	case _RESULT_INT   : return( _TYPE_INT64 );
#else
#ifdef	otr32
	case _RESULT_INT   : return( _TYPE_INT32 );
#else
...
RESULTPTR	numeric_term()
{
	...
	if ((c = is_numeric_token( TokenBuffer )) != 0) 
	{
		...
		else    
		{
			...
			else if ( is_integer(TokenBuffer) ) 
			{
				tptr->nature = _RESULT_INT;
				tptr->value  = convert_numeric_token( TokenBuffer );
			}
...
/*	------------------------------	*/
/*	evaluate_extern_call_procedure	*/
/*	------------------------------	*/
...
RESULTPTR       xterm;
{
	...
	&&  ((vptr = (TCVARPTR) xterm->contents) != (TCVARPTR) 0)
	&&  (vptr->type != (TCTYPPTR) 0)
	&&  ((vptr->type->type & _TYPE_BASIC) == _TYPE_STRING))
		return( VRAI );
	else    return( FAUX );
}
...
RESULTPTR       xterm;
{
	if ( result_type( xterm ) == _TYPE_NUMERIC )
		return( VRAI );
	else    return( FAUX );
}
...
WORD            type;
{
	...
	{
#ifdef	otr64
		sprintf(buffer,"%lld",xterm->value);
#else
#ifdef	otr32 
...
RESULTPTR       rterm;
{
	...
	switch ((lterm->nature & _RESULT_BASIC)) {
		case _RESULT_INT   :
#if	otr32 || otr64
			sprintf(TokenBuffer,"%ld",(signed long int)lterm->value);
#else
			sprintf(TokenBuffer,"%d",(signed short int)lterm->value);
#endif
...
#endif
		/* ---------------------------------------------- */ 
		/* Avoid problems with integer division losing FP */
		/* ---------------------------------------------- */ 
		if (( operation == '/' ) && ( Context->ConvertType == _TYPE_NUMERIC ))
			return( MOINS_UN );
...
FACTORPTR       root;
{
	...
	/* --------------------------------------------------------------------------------- */
	/* Triple pass evaluation for priority operators  ( * or / ) ( AND / OX ) ( others ) */
	/* --------------------------------------------------------------------------------- */
	for ( pass=0; pass < 3; pass++  ) 
	{
		...
		while ( fptr ) 
		{
			...
			/* --------------------------------- */
			/* watch out for no previous operand */
			/* --------------------------------- */
...
/*	-------------------------	*/
RESULTPTR	evaluate_typed_expression(WORD type)
{
	WORD	rt;
	WORD		ht;
	WORD		ct;
	...
	ct = Context->ConvertType;
	Context->ConvertType = (type & _TYPE_BASIC);
	if ( type != MOINS_UN ) { Context->OperandType = (type & _TYPE_BASIC); }
	AllowAutoConv(0);
	rptr = evaluate_expression(0);
	autoconv = IsAutoConv();
	AllowAutoConv(0);
	Context->ConvertType = ct;
