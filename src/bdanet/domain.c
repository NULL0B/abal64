...
PRIVATE	DNSCONTEXTPTR	DnsManager=(DNSCONTEXTPTR) 0;
...
PRIVATE	VOID	dns_server_banner()
{
	printf("   domain: Domain Name Server  Version %s \n",__BASIC_VERSION);
...
#ifdef	_TRACE_H
	if ( trace_active ) {
		trace_puts("DNS FAILURE ");
		trace_puts(mptr); trace_putw(errno);
		trace_line();
		terminate_trace();
		}
#endif
	printf("DNS FAILURE %s (%d) \n",mptr,errno);
	if ( errno )
		return( errno );
	else	return( 1 );
}
...
PRIVATE	VOID	report_allocation_failure()
{
...
PRIVATE	DNSMSGPTR	locate_by_name_server( sptr, qname, qtype, qclass )
	DNSRRPTR	sptr;
	BPTR		qname;
	INT		qtype;
	INT		qclass;
...
PRIVATE	DNSNODEPTR	locate_record( BPTR, INT );
...
PRIVATE	DNSNODEPTR	drop_hashed_node( cptr, nptr, qname )
	DNSCACHEPTR	cptr;
	DNSNODEPTR	nptr;
	BPTR		qname;
{
...
PRIVATE	DNSNODEPTR	locate_hashed_record( cptr, qname, qclass )
	DNSCACHEPTR	cptr;
	BPTR		qname;
	INT		qclass;
{
...
PRIVATE	DNSNODEPTR	locate_authority_record( qname, qclass )
	BPTR		qname;
	INT		qclass;
{
...
PRIVATE	DNSNODEPTR	locate_record( qname, qclass )
	BPTR		qname;
	INT		qclass;
{
...
PRIVATE	INT		store_record_data( nptr, rptr )
	DNSNODEPTR	nptr;
	DNSRRPTR	rptr;
{
	DNSDATAPTR	dptr;
...
PRIVATE	DNSNODEPTR	build_hashed_node( cptr, name, class )
	DNSCACHEPTR	cptr;
	BPTR		name;
	INT		class;
{
...
PRIVATE	INT		cache_records( rptr )
	DNSRRPTR	rptr;
{
	INT	status=0;
...
PRIVATE	INT		cache_message( mptr )
	DNSMSGPTR	mptr;
{
	INT	status=0;
	if ( mptr != (DNSMSGPTR) 0 )
...
PRIVATE	INT	validate_authority()
{
 ...
 *	--------------------------------------------------------------------
 */
PRIVATE	INT 		authoritive_record( xptr )
	DNSRRPTR	xptr;
{
	DNSNODEPTR	rptr;
	INT		status;
...
PRIVATE	DNSDATAPTR	locate_record_data( dptr, dtype )
	DNSDATAPTR	dptr;
	INT		dtype;
{
...
PRIVATE	DNSNODEPTR	resolve_alias( response, rtype, qname, qclass )
	DNSMSGPTR	response;
	INT		rtype;
	BPTR		qname;
	INT		qclass;
...
PRIVATE	DNSNODEPTR	resolve_server( qname, qclass )
	BPTR		qname;
	INT		qclass;
{
	DNSNODEPTR	nptr;
...
PRIVATE	DNSRRPTR	resolve_address( response, rtype, qname, qclass )
	DNSMSGPTR	response;
	INT		rtype;
	BPTR		qname;
	INT		qclass;
...
PRIVATE	DNSMSGPTR	remote_request( aptr, qname, qclass, qtype )
...
PRIVATE	INT		resolve_by_server( response, qname, qtype, qclass, nptr, dptr )
	DNSMSGPTR	response;
	BPTR		qname;
	INT		qtype;
	INT		qclass;
...
PRIVATE	INT		resolve_partial( response, qname, qtype, qclass )
	DNSMSGPTR	response;
	BPTR		qname;
	INT		qtype;
	INT		qclass;
...
PRIVATE	INT 		resolve_response( response, qname, qtype, qclass )
	DNSMSGPTR	response;
	BPTR		qname;
	INT		qtype;
INT		qclass;
 ...
 while ( 1 ) {
	...
	if ((nptr = locate_record( qname, qclass )) == (DNSNODEPTR) 0) {
		...
		switch ( resolve_partial( response, qname, qclass, qtype ) ) {
			...
			case	0 :	
				/* No Partial Response : Wild or  Failure */
				/* -------------------------------------- */
#ifdef	____DNS_WILD
				return( resolve_wild( response, qclass, qtype ) );
#else
				return(0);
#endif
...
PRIVATE	INT		resolve_question( qptr, response )
	DNSQUESTPTR	qptr;
	DNSMSGPTR	response;
{
	DNSRRPTR	rptr;
...
PRIVATE	DNSRCVPTR	extract_response( rcvptr )
	DNSRCVPTR	rcvptr;
{
...
PRIVATE	INT		locate_response( qname, qclass, qtype )
	BPTR		qname;
	INT		qclass;
	INT		qtype;
{
...
PRIVATE	DNSRCVPTR	integrate_response( rcvptr )
	DNSRCVPTR	rcvptr;
{
	DNSMSGPTR	response;
	CONNECTIONPTR	connection;
	...
	INT		qtype;
{
	...
	/* ------------------------------------------------- */
	reason->identity = new_dns_message();
...
PRIVATE	DNSRCVPTR	recursive_response( rcvptr )
	DNSRCVPTR	rcvptr;
{
	DNSMSGPTR	response; /* Output or Sent    Response Message */
	DNSMSGPTR	query;    /* Output or Sent    Response Message */
...
PRIVATE	INT 	dns_initialisation( master_file )
	BPTR	master_file;
{
	INT	status=0;
	...
	if ((DnsManager = allocate_for_context(master_file)) == (DNSCONTEXTPTR) 0)
		return( 27 );
	else if (((status = initialise_url_services()) != 0 )
...
PRIVATE	INT	dns_liberation( status )
	INT	status;
{
	DNSFWDPTR	fptr;
...
PRIVATE	INT	dns_server_operation( config_name )
	BPTR  	config_name;
{
	...
	/* --------------------------------- */
	if ((status = dns_initialisation(config_name)) != 0)
	...
	/* ----------------------------------------------- */
	if ((SocketHandle = open_connection(	
					ServerUrl->transport,
					( ServerUrl->port ? ServerUrl->port : ServerUrl->base ),
					ServerUrl )) != (CONNECTIONPTR) 0) {
...
PRIVATE	VOID	dns_server_usage( nptr )
	BPTR 	nptr;
{
	printf("\n Usage %s \n",nptr);
	show_global_usage();
	show_server_usage();
	printf("         [ -r[a | d | ]       ] : Set DNS Recursivity Type      \n");
	printf("         [ -u[v | d | r]      ] : Set DNS Transport Type        \n");
	printf("         <configuration>        : Server Master File            \n");
	return;
}
		...
		for ( argi=1; argi < argc; argi++) {
			...
			if ( *sptr == '-' ) {
				switch ( *(sptr+1) ) {
					...
					/* ----------- */
					case 'r' :
					case 'R' : dns_set_recursive( (sptr + 2) ); 
						   break;

					case 'U' :
					case 'u' : dns_set_transport( (sptr + 2) );
						   break;
			...
			else	return( dns_server_operation( sptr ) );
			}
		dns_server_banner();
