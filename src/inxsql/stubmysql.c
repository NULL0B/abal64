...
int	mysql_options( void * v, int t, void * p )
{
		return( 0 );
}
...
MYSQL *		mysql_real_connect( MYSQL * handle, char * host, char * user, char * pass, char * base, int a, int b, int c )
{
	...
	else 
	{
		...
		if (!( h->hostname = inxsql_allocate_string( host ) ))
		{
			mysql_close( h );
			return((MYSQL *) 0);
		}
		if (!( h->database = inxsql_allocate_string( base ) ))
		{
			mysql_close( h );
			return((MYSQL *) 0);
		}
		if (!( h->username = inxsql_allocate_string( user ) ))
		{
			mysql_close( h );
			return((MYSQL *) 0);
		}
		if (!( h->password = inxsql_allocate_string( pass ) ))
		{
			mysql_close( h );
			return((MYSQL *) 0);
		}
