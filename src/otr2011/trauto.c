...
/* -------------------------------------------------------------------------
 * This Symbol may be defined in order to include memory allocation
 * qualification code which notifies allocation/liberation discrepancies
 * ------------------------------------------------------------------------- */
/* #define	_QUALIFY_AUTO_ALLOCATION */
...
/*
 *	Called from TABLES.initialise_tcode_context()
 *	---------------------------------------------
 *	to prepare for construction/destruction operation collection
...
{
#ifdef	_QUALIFY_AUTO_ALLOCATION
	sysprintf("initialise_auto_context()\n");
#endif
...
WORD		level;
{
	...
	&&  (( AutoContext->Constructors ) ||  ( mode ))) {	
		...
		if ((nptr = allocate_for_string( vname )) == (BPTR) 0)
			...
			if  ((rptr = variable_result(AutoContext->Catcher)) != (RESULTPTR) 0)
				return( explicite_on_error( rptr->contents, _CTRL_LOCAL, AutoContext->Catcher->name ) );
