	...
	if ( is_left_brace() ) {
		do	{
			if ((paratype = get_dll_type()) == MOINS_UN)
				break;
			switch ( paratype ) {
				case	_TYPE_INT8  	: parasize = 1; break;
				case 	_TYPE_INT16 	: parasize = 2; break;
				case	_TYPE_INT32 	: parasize = 4; break;
				case	_TYPE_NUMERIC   : parasize = 8; break;
				case	_TYPE_BASIC 	: 
				case	_TYPE_STRING	: parasize = 15; break;
		...
		do	{
			if ( compare_tokens( lptr->name, nptr ) ) {
				if ( lptr->version != MOINS_UN ) {
					...
					break;
					/* was like this but problem web services */
...
RESULTPTR	iptr;
{
	...
	i = 0;
	if ( xptr->func != (TCDLFPTR) 0 ) {
		if ((status = generate_ldf( xptr->func->library /* ->identity */ )) != 0)
			return( status );
		}
