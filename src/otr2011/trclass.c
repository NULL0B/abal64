...
/*      else if ( ListMemberCounter )
		return( syntax_error(812) ); */
...
WORD    t;
{
	...
	switch ((ClassOptions.Type = t)) {
		case    1 : ClassOptions.Size =  2; break;
		case    2 : ClassOptions.Size = 12; break;
		case    3 : ClassOptions.Size = 15; break;
		}
	return( 0 );
}
...
static  WORD    translate_method_pointer()
{
	...
	if ( is_mnemonic("POINTER") )
		type |= _TYPE_POINTER;
	if (( ClassOptions.ReturnType = allocate_for_type( type, 5, 1, 1, ClassOptions.ReturnType )) == (TCTYPPTR) 0)
...
{
	if (( ClassOptions.Strict ) &&  ( ClassOptions.Strict == _CLASS_STRICT ))
...
WORD    translate_class_keyword()
{
	...
	switch ( type ) {
		...
		case    _CLASS_INTEGER          :
			return( basic_return_type( 1 ) );

		case    _CLASS_NUMERIC          :
			remove_useless_length();
			return( basic_return_type( 2 ) );
		
		case    _CLASS_STRING           :
			remove_useless_length();
			return( basic_return_type( 3 ) );
...
WORD    nature;         /* UNION     */
{
	...
	if (!( class_definition_active() )) {
		...
		else    /* Structure/Union Definition Expression */
			/* ------------------------------------- */
...
TCMEMBERPTR     rptr;
{
	...
	switch ( nature ) {
		...
		case _KW1_CONST :
			return( VRAI );
/*                      return( compare_const_members( lptr->special, rptr->special ) ); */
		default         : 
