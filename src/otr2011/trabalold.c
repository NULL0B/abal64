...
WORD	type;
{
	...
	if (( get_token() ) &&  ( is_control_keyword() == _CTRL_SEG )) {
		...
		if ( type == _CTRL_RET ) {
			...
			if ((rptr = evaluate_integer_expression(0)) != (RESULTPTR) 0)
...
WORD	translate_for()
{
	...
	else if ((vptr = for_initialisation()) == (TCVARPTR) 0)
