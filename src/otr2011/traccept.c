...
RESULTPTR	device;
{
	...
	&&  ( is_mnemonic("CRT") ))
		...
		for ( dval = 1; dval <= 3; dval++ ) {
			if ((operand = convert_to_type(clf_date( dval ),1)) == (RESULTPTR) 0)
				return( allocation_failure() );
			else if ((status = store_iof_prn( operand )) != 0)
				return( status );
			}
		...
		if ((status = store_iof_fmt( "Z2,Z3" )) != 0)
			return( status );
		else if ((operand = convert_to_type(clf_date( 1 ),1)) == (RESULTPTR) 0)
			return( allocation_failure() );
		else if ((status = store_iof_prn( operand )) != 0)
			return( status );
		if ((operand = convert_to_type(clf_date( 4 ),1)) == (RESULTPTR) 0)
			return( allocation_failure() );
		else if ((status = store_iof_prn( operand )) != 0)
			return( status );
		}
		...
		if ((status = store_iof_fmt( "Z" )) != 0)
			return( status );
		else if ((operand = convert_to_type(clf_date( 9 ),1)) == (RESULTPTR) 0)
			return( allocation_failure() );
		else if ((status = store_iof_prn( operand )) != 0)
			return( status );
		}
