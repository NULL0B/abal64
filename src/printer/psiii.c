...
int	DriverInitialise(char * palette, int dpi, int grnbcol, int grnbrow, int options )
{
	...
	if (!( colourmapping ))
		set_white_limit( pixelsize );
	RasterReady   = PrinterReady = 0;
	PaletteBuffer = palette;
	PageOptions   = options;
	set_paper_dimensions( 75, ((PageOptions & _PAPER_TYPE) >> 8));
	RowCount      = grnbrow;
	ColCount      = grnbcol;
	TopMargin     = 0;
	LeftMargin    = 0;
	...
	switch ( PageOptions & _PAGE_ALIGNMENT ) {
		case	_ALIGN_RIGHT : /* Left Top Align */
			if ( ColCount < PaperWidth )
				LeftMargin = (PaperWidth - ColCount);
			break;

		case	_ALIGN_LEFT : /* Right and Bottom Align */
		...
		case	_ALIGN_CENTER  : /* Center */
			if ( ColCount < PaperWidth )
				LeftMargin = ((PaperWidth - ColCount)/2);
			if ( RowCount < PaperHeight )
				TopMargin = ((PaperHeight - RowCount)/2);
			break;

		}

/*	printf("pw=%u,cc=%u,lm=%u\r\nph=%u,rc=%u,tm=%u\r\n",PaperWidth,ColCount,LeftMargin,PaperHeight,RowCount,TopMargin);	*/
