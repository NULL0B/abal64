...
private	int	accept_form_load_create(
	char * pFileName)
{
...
private	int	accept_form_load_hide()
{
...
private	int	accept_form_load_remove(
	char * pFileName)
{
...
private	int	accept_form_load_show()
{
	if ((_Context->pushbuffer != (void *) 0)
	&& (!(_Context->ctrlbuffer))) {
	...
	visual_image(104,232,592,154,"skin.gif",4);
	visual_text(120,250,560,16,2,34,0,"Name of Form to Load",20,258);
	visual_frame(120,270,560+2,16+2,5);
	visual_text(120+1,270+1,560,16,1,34,0,_Context->buffer_FileName,70,0);
	visual_button(120,340,110,30,2,34,0,"&Accept",7,0);
	visual_button(460,340,100,30,2,27,24,"&Select",7,0);
	visual_button(570,340,110,30,2,34,0,"&Cancel",7,0);
	visual_image(326,322,50,50,"formicon.gif",0);
...
private	int	accept_form_load_event()
{
	int	mx,my,mt,mb;
...
private	int	accept_form_load_focus()
{
	int retcode=0;
	...
	while(1) {
		switch (_Context->focus_item) {
			...
			case	0x3 :
				/* Accept */
				visual_button(120,340,110,30,2,34,0,"&Accept",7,2);
				_Context->keycode = visual_getch();
				visual_button(120,340,110,30,2,34,0,"&Accept",7,0);
				break;
			case	0x4 :
				/* Select */
				visual_button(460,340,100,30,2,27,24,"&Select",7,2);
				_Context->keycode = visual_getch();
				visual_button(460,340,100,30,2,27,24,"&Select",7,0);
				break;
			case	0x5 :
				/* Cancel */
				visual_button(570,340,110,30,2,34,0,"&Cancel",7,2);
				_Context->keycode = visual_getch();
				visual_button(570,340,110,30,2,34,0,"&Cancel",7,0);
				break;
...
retry:
		...
		case	0x2 :
			...
			case	0x100 :
				...
				if ((retcode=accept_form_load_event()) == -1)
					continue;
				switch ((_Context->focus_item = retcode)) {
					...
					case	0x3 :
						...
						if (visual_event(1) & _MIMO_DOWN) {
							visual_button(120,340,110,30,2,34,0,"&Accept",7,8);
							}
					...
					case	0x4 :
						/* Select */
						if (visual_event(1) & _MIMO_DOWN) {
							visual_button(460,340,100,30,2,27,24,"&Select",7,8);
							}
						else if (visual_event(1) & _MIMO_UP) {
							visual_button(460,340,100,30,2,27,24,"&Select",7,0);
							}
					...
					case	0x5 :
						/* Cancel */
						if (visual_event(1) & _MIMO_DOWN) {
							visual_button(570,340,110,30,2,34,0,"&Cancel",7,8);
							}
						else if (visual_event(1) & _MIMO_UP) {
							visual_button(570,340,110,30,2,34,0,"&Cancel",7,0);
							}
	...
	char * pFileName)
{
	...
	status = accept_form_load_show();
		enter_modal();
	status = accept_form_load_focus();
		leave_modal();
	status = accept_form_load_hide();
